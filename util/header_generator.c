...
void OutputCFunction(Entry *entry) {
	...
	FilePrintFormat(output, "#ifdef LT_FORWARD\n#ifndef __cplusplus\nLT_EXTERN_C ");
	...
	FilePrintFormat(output, ");\n#ifndef __cplusplus\n#define %s ((__typeof_%s) LT_API_BASE[%d])\n#else\n", 
	...
	if (anyDefaultArguments) {
		...
		FilePrintFormat(output, "#define %s ((__typeof_%s) LT_API_BASE[%d])", 
				functionName, functionName, entry->function.apiArrayIndex);
	}
...
void OutputC(Entry *root) {
	...
	for (int i = 0; i < arrlen(root->children); i++) {
		...
		if (entry->isPrivate) {
			FilePrintFormat(output, "#if defined(LT_PRIVATE_APIS)\n");
		}
			...
			for (int pass = 0; pass < 2; pass++) {
				...
				for (int i = 0; i < arrlen(entry->children); i++) {
					...
					if (foundType) {
						...
						if (!foundType && 0 == strcmp(argument.variable.type, "LT_INSTANCE_TYPE")) {
...
int AnalysisResolve(Entry *root, Entry e, Entry **unresolved, Entry *resolved, Entry *annotations) {
	...
	if (e.variable.pointer == 1 && !e.variable.isConst && (0 == strcmp(e.variable.type, "LT_INSTANCE_TYPE"))) {
		return 1;
	}
