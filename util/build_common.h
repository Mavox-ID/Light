...
#define SYSTEM_FOLDER_NAME "Light"
...
void DependenciesListRead() {
	EsINIState s = { .buffer = (char *) LoadFile(DEPENDENCILT_FILE, &s.bytes) };
	char *start = s.buffer;
	if (!start) return;
...
void DependenciesListWrite() {
	...
	// TODO.
#else
	FILE *f = fopen(DEPENDENCILT_FILE, "wb");
...
Option options[] = {
	...
	{ "Driver.LightFS", OPTION_TYPE_BOOL, { .b = true } },
	...
	{ "Driver.NTFS", OPTION_TYPE_BOOL, { .b = true }, .warning = "The NTFS driver has not been updated to work with the new FS layer." },
	...
	{ "Flag.ENABLE_POSIX_SUBSYSTEM", OPTION_TYPE_BOOL, { .b = true } },
	...
	{ "Emulator.Audio", OPTION_TYPE_BOOL, { .b = true } },
	{ "Emulator.MemoryMB", OPTION_TYPE_STRING, { .s = "2048" } },
	...
	{ "General.wallpaper", OPTION_TYPE_STRING, { .s = "0:/Light/bg/Desert.jpg" } },
	{ "General.window_color", OPTION_TYPE_STRING, { .s = "3" } },
