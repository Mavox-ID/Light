...
EsHandle HandleOpen(Object *object) {
	...
	if (unusedHandles.Length()) {
		...
		if (!handles.Add(object)) {
			handle = LT_ERROR_INSUFFICIENT_RESOURCES;
			closeReference = true;
		}
	}
...
bool MessagePost(_EsMessageWithObject *message) {
	...
	if (message->message.type == LT_MSG_MOUSE_MOVED || message->message.type == LT_MSG_WINDOW_RESIZED) {
...
uintptr_t _APISyscall(uintptr_t index, uintptr_t argument0, uintptr_t argument1, uintptr_t, uintptr_t argument2, uintptr_t argument3) {
	...
	if (index == LT_SYSCALL_THREAD_GET_ID) {
		// TODO.
		return 0;
	} else if (index == LT_SYSCALL_THREAD_SET_TLS) {
		tls = argument0;
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_THREAD_SET_TIMER_ADJUST_ADDRESS) {
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_EVENT_CREATE) {
		Event *event = (Event *) EsHeapAllocate(sizeof(Event), true);
		if (!event) return LT_ERROR_INSUFFICIENT_RESOURCES;

		event->type = OBJECT_EVENT;
		event->referenceCount = 1;
		EventInitialise(event, argument0);
		return HandleOpen(event);	
	} else if (index == LT_SYSCALL_MEMORY_MAP_OBJECT) {
		SharedMemoryRegion *region = (SharedMemoryRegion *) HandleResolve(argument0, OBJECT_SHMEM);
		assert(argument3 != LT_MEMORY_MAP_OBJECT_READ_WRITE || argument3 != LT_MEMORY_MAP_OBJECT_READ_ONLY);
	...
	} else if (index == LT_SYSCALL_CONSTANT_BUFFER_READ) {
		...
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_HANDLE_CLOSE) {
		HandleClose(argument0);
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_VOLUME_GET_INFORMATION) {
		...
		information->flags = (s.f_flag & ST_RDONLY) ? LT_VOLUME_READ_ONLY : LT_FLAGS_DEFAULT;
		information->driveType = LT_DRIVE_TYPE_OTHER;
		EsMemoryCopy(information->label, "Volume", 6);
		information->labelBytes = 6;
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_PIPE_CREATE) {
		...
		*(EsHandle *) argument1 = HandleOpen(writeEnd);
		...
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_PIPE_READ) {
		Node *node = (Node *) HandleResolve(argument0, OBJECT_NODE);
		ssize_t x = read(node->fd, (void *) argument1, argument2);
		return x < 0 ? LT_ERROR_UNKNOWN : x;
	} else if (index == LT_SYSCALL_PIPE_WRITE) {
		Node *node = (Node *) HandleResolve(argument0, OBJECT_NODE);
		ssize_t x = write(node->fd, (void *) argument1, argument2);
		return x < 0 ? LT_ERROR_UNKNOWN : x;
	} else if (index == LT_SYSCALL_PRINT) {
		fwrite((void *) argument0, 1, argument1, stderr);
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_MESSAGE_GET) {
		...
		pthread_mutex_unlock(&messageQueueMutex);
		return gotMessage ? LT_SUCCESS : LT_ERROR_NO_MESSAGES_AVAILABLE;
	} else if (index == LT_SYSCALL_MESSAGE_WAIT) {
		sem_wait(&messagesAvailable);
		sem_post(&messagesAvailable);
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_WINDOW_GET_ID) {
		return ((UIWindow *) HandleResolve(argument0, OBJECT_WINDOW))->id;
	} else if (index == LT_SYSCALL_MEMORY_ALLOCATE) {
		if (argument3) {
			SharedMemoryRegion *region = (SharedMemoryRegion *) EsHeapAllocate(sizeof(SharedMemoryRegion), true);
			if (!region) return LT_ERROR_INSUFFICIENT_RESOURCES;
			region->type = OBJECT_SHMEM;
			region->referenceCount = 1;
			region->pointer = EsHeapAllocate(argument0, true);
			region->bytes = argument0;
			if (!region->pointer) { EsHeapFree(region); return LT_ERROR_INSUFFICIENT_RESOURCES; }
			return HandleOpen(region);
		} else {
			MemoryMapping mapping = {};
			mapping.bytes = argument0;
			mapping.address = (uintptr_t) mmap(nullptr, mapping.bytes, PROT_READ | (argument2 == LT_MEMORY_PROTECTION_READ_WRITE ? PROT_WRITE 
						: argument2 == LT_MEMORY_PROTECTION_EXECUTABLE ? PROT_EXEC : 0), MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
			pthread_mutex_lock(&memoryMappingsMutex);
			if (mapping.address) memoryMappings.Add(mapping);
			pthread_mutex_unlock(&memoryMappingsMutex);
			return mapping.address;
		}
	} else if (index == LT_SYSCALL_MEMORY_FREE) {
		size_t bytes = 0;
		...
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_MESSAGE_POST) {
		assert(argument2 == LT_CURRENT_PROCESS);
		_EsMessageWithObject m;
		m.object = (void *) argument1;
		EsMemoryCopy(&m.message, (const void *) argument0, sizeof(EsMessage));
		return MessagePost(&m) ? LT_SUCCESS : LT_ERROR_MESSAGE_QUEUE_FULL;
	} else if (index == LT_SYSCALL_WINDOW_SET_PROPERTY) {
		uint8_t property = argument3;
		UIWindow *window = (UIWindow *) HandleResolve(argument0, OBJECT_WINDOW);
		pthread_mutex_lock(&windowsMutex);

		if (property == LT_WINDOW_PROPERTY_OBJECT) {
			window->apiObject = (void *) argument2;
			_EsMessageWithObject m = {};
			m.object = window->apiObject;
			m.message.type = LT_MSG_WINDOW_RESIZED;
			m.message.windowResized.content = LT_RECT_2S(window->width, window->height);
			MessagePost(&m);
		} else if (property == LT_WINDOW_PROPERTY_SOLID 
				|| property == LT_WINDOW_PROPERTY_MATERIAL
				|| property == LT_WINDOW_PROPERTY_BLUR_BOUNDS
				|| property == LT_WINDOW_PROPERTY_ALPHA) {
			// TODO.
		} else {
			fprintf(stderr, "Unimplemented window property %d.\n", property);
			exit(1);
			return LT_ERROR_UNSUPPORTED_FEATURE;
		}

		pthread_mutex_unlock(&windowsMutex);
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_WINDOW_SET_CURSOR) {
		// TODO.
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_SCREEN_FORCE_UPDATE) {
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_CURSOR_POSITION_GET) {
		...
		pthread_mutex_unlock(&windowsMutex);
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_WINDOW_GET_BOUNDS) {
		UIWindow *window = (UIWindow *) HandleResolve(argument0, OBJECT_WINDOW);
		...
		pthread_mutex_unlock(&windowsMutex);
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_MEMORY_FAULT_RANGE) {
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_WINDOW_SET_BITS) {
		UIWindow *window = (UIWindow *) HandleResolve(argument0, OBJECT_WINDOW);
		EsRectangle region = *(EsRectangle *) argument1;
		uint32_t *data = (uint32_t *) argument2;
		size_t dataWidth = LT_RECT_WIDTH(region);
		size_t dataOffset = region.t * dataWidth + region.l;

		pthread_mutex_lock(&windowsMutex);
		region = EsRectangleIntersection(region, LT_RECT_2S(window->width, window->height));
		...
		pthread_mutex_unlock(&windowsMutex);
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_WINDOW_CREATE) {
		...
		attributes.override_redirect = style == LT_WINDOW_MENU;
	...
	} else if (index == LT_SYSCALL_WINDOW_MOVE) {
		UIWindow *window = (UIWindow *) HandleResolve(argument0, OBJECT_WINDOW);
		EsRectangle point = *(EsRectangle *) argument1;
		int width = LT_RECT_WIDTH(point), height = LT_RECT_HEIGHT(point);
		pthread_mutex_lock(&windowsMutex);
		...
		if (argument3 & LT_WINDOW_MOVE_MAXIMIZED) {
			// TODO.
		}

		if (argument3 & LT_WINDOW_MOVE_ADJUST_TO_FIT_SCREEN) {
		...
		pthread_mutex_unlock(&windowsMutex);
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_THREAD_CREATE) {
		Thread *threadObject = (Thread *) EsHeapAllocate(sizeof(Thread), true);
		if (!threadObject) return LT_ERROR_INSUFFICIENT_RESOURCES;
		...
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_EVENT_SET) {
		Event *event = (Event *) HandleResolve(argument0, OBJECT_EVENT);
		EventSet(event);
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_EVENT_RESET) {
		Event *event = (Event *) HandleResolve(argument0, OBJECT_EVENT);
		EventReset(event);
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_WAIT) {
		...
		return EventWait(event, argument2) ? LT_ERROR_TIMEOUT_REACHED : 0;
	} else if (index == LT_SYSCALL_PROCESS_TERMINATE) {
		exit(argument1);
	} else if (index == LT_SYSCALL_PROCESS_CRASH) {
		assert(false); // Do an assertion failure so it can be caught by an attached debugger.
	} else if (index == LT_SYSCALL_YIELD_SCHEDULER) {
		return LT_SUCCESS;
	} else if (index == LT_SYSCALL_NODE_OPEN) {
		return LT_ERROR_FILE_DOLT_NOT_EXIST; // TODO File and directory operations.
	} else if (index == LT_SYSCALL_WINDOW_CLOSE) {
		UIWindow *window = (UIWindow *) HandleResolve(argument0, OBJECT_WINDOW);
		pthread_mutex_lock(&windowsMutex);
		...
		if (mainWindow == window) {
			_EsMessageWithObject m = { .message = { .type = LT_MSG_APPLICATION_EXIT } };
			MessagePost(&m);
		}
...
uint32_t UILookupScancode(uint32_t keycode) {
	const uint32_t remap[] = {
		0, 0, 0, 0, 
		0, 0, 0, 0,
		0, LT_SCANCODE_ESCAPE, LT_SCANCODE_1, LT_SCANCODE_2, 
		LT_SCANCODE_3, LT_SCANCODE_4, LT_SCANCODE_5, LT_SCANCODE_6,
		LT_SCANCODE_7, LT_SCANCODE_8, LT_SCANCODE_9, LT_SCANCODE_0, 
		LT_SCANCODE_HYPHEN, LT_SCANCODE_EQUALS, LT_SCANCODE_BACKSPACE, LT_SCANCODE_TAB,
		LT_SCANCODE_Q, LT_SCANCODE_W, LT_SCANCODE_E, LT_SCANCODE_R, 
		LT_SCANCODE_T, LT_SCANCODE_Y, LT_SCANCODE_U, LT_SCANCODE_I,
		LT_SCANCODE_O, LT_SCANCODE_P, LT_SCANCODE_LEFT_BRACE, LT_SCANCODE_RIGHT_BRACE, 
		LT_SCANCODE_ENTER, LT_SCANCODE_LEFT_CTRL, LT_SCANCODE_A, LT_SCANCODE_S,
		LT_SCANCODE_D, LT_SCANCODE_F, LT_SCANCODE_G, LT_SCANCODE_H, 
		LT_SCANCODE_J, LT_SCANCODE_K, LT_SCANCODE_L, LT_SCANCODE_PUNCTUATION_3, 
		LT_SCANCODE_PUNCTUATION_4, LT_SCANCODE_PUNCTUATION_5, LT_SCANCODE_LEFT_SHIFT, LT_SCANCODE_PUNCTUATION_1, 
		LT_SCANCODE_Z, LT_SCANCODE_X, LT_SCANCODE_C, LT_SCANCODE_V,
		LT_SCANCODE_B, LT_SCANCODE_N, LT_SCANCODE_M, LT_SCANCODE_COMMA, 
		LT_SCANCODE_PERIOD, LT_SCANCODE_SLASH, LT_SCANCODE_RIGHT_SHIFT, LT_SCANCODE_NUM_MULTIPLY,
		LT_SCANCODE_LEFT_ALT, LT_SCANCODE_SPACE, LT_SCANCODE_CAPS_LOCK, LT_SCANCODE_F1, 
		LT_SCANCODE_F2, LT_SCANCODE_F3, LT_SCANCODE_F4, LT_SCANCODE_F5,
		LT_SCANCODE_F6, LT_SCANCODE_F7, LT_SCANCODE_F8, LT_SCANCODE_F9, 
		LT_SCANCODE_F10, LT_SCANCODE_NUM_LOCK, LT_SCANCODE_SCROLL_LOCK, LT_SCANCODE_NUM_7,
		LT_SCANCODE_NUM_8, LT_SCANCODE_NUM_9, LT_SCANCODE_NUM_SUBTRACT, LT_SCANCODE_NUM_4, 
		LT_SCANCODE_NUM_5, LT_SCANCODE_NUM_6, LT_SCANCODE_NUM_ADD, LT_SCANCODE_NUM_1, 
		LT_SCANCODE_NUM_2, LT_SCANCODE_NUM_3, LT_SCANCODE_NUM_0, LT_SCANCODE_NUM_POINT, 
		0, 0, LT_SCANCODE_PUNCTUATION_6, LT_SCANCODE_F11,
		LT_SCANCODE_F12, 0, LT_SCANCODE_KATAKANA, LT_SCANCODE_HIRAGANA, 
		0, 0, 0, 0,
		LT_SCANCODE_NUM_ENTER, LT_SCANCODE_RIGHT_CTRL, LT_SCANCODE_NUM_DIVIDE, LT_SCANCODE_PRINT_SCREEN, 
		LT_SCANCODE_RIGHT_ALT, 0, LT_SCANCODE_HOME, LT_SCANCODE_UP_ARROW,
		LT_SCANCODE_PAGE_UP, LT_SCANCODE_LEFT_ARROW, LT_SCANCODE_RIGHT_ARROW, LT_SCANCODE_END, 
		LT_SCANCODE_DOWN_ARROW, LT_SCANCODE_PAGE_DOWN, LT_SCANCODE_INSERT, LT_SCANCODE_DELETE,
		0, LT_SCANCODE_MM_MUTE, LT_SCANCODE_MM_QUIETER, LT_SCANCODE_MM_LOUDER, 
		LT_SCANCODE_ACPI_POWER, LT_SCANCODE_NUM_EQUALS, 0, LT_SCANCODE_PAUSE,
	};
...
void UIProcessEvent(XEvent *event) {
	...
	if (event->type == ClientMessage && (Atom) event->xclient.data.l[0] == ui.windowClosedID) {
		_EsMessageWithObject m = { .message = { .type = LT_MSG_TAB_CLOSE_REQUEST, .tabOperation = { .id = mainWindow->id } } };
		MessagePost(&m);
		...
		_EsMessageWithObject m = {};
		m.object = window->apiObject;
		m.message.type = LT_MSG_MOUSE_MOVED;
		...
		if (event->xbutton.button >= 1 && event->xbutton.button <= 3) {
			_EsMessageWithObject m = {};
			m.object = window->apiObject;
			if (event->xbutton.button == 1 && event->type == ButtonPress  ) m.message.type = LT_MSG_MOUSE_LEFT_DOWN;
			if (event->xbutton.button == 1 && event->type == ButtonRelease) m.message.type = LT_MSG_MOUSE_LEFT_UP;
			if (event->xbutton.button == 3 && event->type == ButtonPress  ) m.message.type = LT_MSG_MOUSE_RIGHT_DOWN;
			if (event->xbutton.button == 3 && event->type == ButtonRelease) m.message.type = LT_MSG_MOUSE_RIGHT_UP;
			if (event->xbutton.button == 2 && event->type == ButtonPress  ) m.message.type = LT_MSG_MOUSE_MIDDLE_DOWN;
			if (event->xbutton.button == 2 && event->type == ButtonRelease) m.message.type = LT_MSG_MOUSE_MIDDLE_UP;
		...
		_EsMessageWithObject m = {};
		m.object = window->apiObject;
		m.message.type = event->type == KeyPress ? LT_MSG_KEY_DOWN : LT_MSG_KEY_UP;
		if (ui.modifiers & ControlMask) m.message.keyboard.modifiers |= LT_MODIFIER_CTRL;
		if (ui.modifiers & ShiftMask) m.message.keyboard.modifiers |= LT_MODIFIER_SHIFT;
		if (ui.modifiers & Mod1Mask) m.message.keyboard.modifiers |= LT_MODIFIER_ALT;
		if (ui.modifiers & Mod4Mask) m.message.keyboard.modifiers |= LT_MODIFIER_FLAG;
		// if (ui.modifiers & AltGrMask) m.message.keyboard.modifiers |= LT_MODIFIER_ALT_GR;
...
void *UIThread(void *) {
	...
	_EsMessageWithObject m = {};
	m.message.type = LT_MSG_INSTANCE_CREATE;
...
void *DesktopThread(void *) {
	while (true) {
		...
		if (message[0] == DESKTOP_MSG_SYSTEM_CONFIGURATION_GET) {
			...
			EsHandle fileHandle = LT_INVALID_HANDLE;
