...
const int UI_KEYCODE_A = LT_SCANCODE_A;
const int UI_KEYCODE_0 = LT_SCANCODE_0;
const int UI_KEYCODE_BACKSPACE = LT_SCANCODE_BACKSPACE;
const int UI_KEYCODE_DELETE = LT_SCANCODE_DELETE;
const int UI_KEYCODE_DOWN = LT_SCANCODE_DOWN_ARROW;
const int UI_KEYCODE_END = LT_SCANCODE_END;
const int UI_KEYCODE_ENTER = LT_SCANCODE_ENTER;
const int UI_KEYCODE_ESCAPE = LT_SCANCODE_ESCAPE;
const int UI_KEYCODE_F1 = LT_SCANCODE_F1;
const int UI_KEYCODE_HOME = LT_SCANCODE_HOME;
const int UI_KEYCODE_LEFT = LT_SCANCODE_LEFT_ARROW;
const int UI_KEYCODE_RIGHT = LT_SCANCODE_RIGHT_ARROW;
const int UI_KEYCODE_SPACE = LT_SCANCODE_SPACE;
const int UI_KEYCODE_TAB = LT_SCANCODE_TAB;
const int UI_KEYCODE_UP = LT_SCANCODE_UP_ARROW;
const int UI_KEYCODE_INSERT = LT_SCANCODE_INSERT;
...
UIMenu *UIMenuCreate(UIElement *parent, uint32_t flags) {
	ui.menuIndex = 0;
	return EsMenuCreate(parent->window->window, LT_MENU_AT_CURSOR);
}
...
void UIMenuAddItem(UIMenu *menu, uint32_t flags, const char *label, ptrdiff_t labelBytes, void (*invoke)(void *cp), void *cp) {
	...
	EsMenuAddItem(menu, (flags & UI_BUTTON_CHECKED) ? LT_MENU_ITEM_CHECKED : LT_FLAGS_DEFAULT, 
...
int _UIWindowCanvasMessage(EsElement *element, EsMessage *message) {
	...
	if (!window) {
		...
		EsRectangle bounds = LT_RECT_4PD(message->painter->offsetX, message->painter->offsetY, window->width, window->height);
	...
	} else if (message->type == LT_MSG_MOUSE_MOVED || message->type == LT_MSG_HOVERED_END
			|| message->type == LT_MSG_MOUSE_LEFT_DRAG || message->type == LT_MSG_MOUSE_RIGHT_DRAG || message->type == LT_MSG_MOUSE_MIDDLE_DRAG) {
		...
		return _UIWindowInputEvent(window, UI_MSG_KEY_TYPED, 0, &m) ? LT_HANDLED : 0;
	} else if (message->type == LT_MSG_MOUSE_LEFT_CLICK) {
		_UIInspectorSetFocusedWindow(window);
	} else if (message->type == LT_MSG_USER_START) {
		...
		if (window->cursor == UI_CURSOR_TEXT)              message->cursorStyle = LT_CURSOR_TEXT;
		if (window->cursor == UI_CURSOR_SPLIT_V)           message->cursorStyle = LT_CURSOR_SPLIT_VERTICAL;
		if (window->cursor == UI_CURSOR_SPLIT_H)           message->cursorStyle = LT_CURSOR_SPLIT_HORIZONTAL;
		if (window->cursor == UI_CURSOR_FLIPPED_ARROW)     message->cursorStyle = LT_CURSOR_SELECT_LINES;
		if (window->cursor == UI_CURSOR_CROSS_HAIR)        message->cursorStyle = LT_CURSOR_CROSS_HAIR_PICK;
		if (window->cursor == UI_CURSOR_HAND)              message->cursorStyle = LT_CURSOR_HAND_HOVER;
		if (window->cursor == UI_CURSOR_RESIZE_UP)         message->cursorStyle = LT_CURSOR_RESIZE_VERTICAL;
		if (window->cursor == UI_CURSOR_RESIZE_LEFT)       message->cursorStyle = LT_CURSOR_RESIZE_HORIZONTAL;
		if (window->cursor == UI_CURSOR_RESIZE_UP_RIGHT)   message->cursorStyle = LT_CURSOR_RESIZE_DIAGONAL_1;
		if (window->cursor == UI_CURSOR_RESIZE_UP_LEFT)    message->cursorStyle = LT_CURSOR_RESIZE_DIAGONAL_2;
		if (window->cursor == UI_CURSOR_RESIZE_DOWN)       message->cursorStyle = LT_CURSOR_RESIZE_VERTICAL;
		if (window->cursor == UI_CURSOR_RESIZE_RIGHT)      message->cursorStyle = LT_CURSOR_RESIZE_HORIZONTAL;
		if (window->cursor == UI_CURSOR_RESIZE_DOWN_RIGHT) message->cursorStyle = LT_CURSOR_RESIZE_DIAGONAL_1;
		if (window->cursor == UI_CURSOR_RESIZE_DOWN_LEFT)  message->cursorStyle = LT_CURSOR_RESIZE_DIAGONAL_2;
	}
	
	else if (message->type == LT_MSG_MOUSE_LEFT_DOWN)   _UIWindowInputEvent(window, UI_MSG_LEFT_DOWN, 0, 0);
	else if (message->type == LT_MSG_MOUSE_LEFT_UP)     _UIWindowInputEvent(window, UI_MSG_LEFT_UP, 0, 0);
	else if (message->type == LT_MSG_MOUSE_MIDDLE_DOWN) _UIWindowInputEvent(window, UI_MSG_MIDDLE_DOWN, 0, 0);
	else if (message->type == LT_MSG_MOUSE_MIDDLE_UP)   _UIWindowInputEvent(window, UI_MSG_MIDDLE_UP, 0, 0);
	else if (message->type == LT_MSG_MOUSE_RIGHT_DOWN)  _UIWindowInputEvent(window, UI_MSG_RIGHT_DOWN, 0, 0);
	else if (message->type == LT_MSG_MOUSE_RIGHT_UP)    _UIWindowInputEvent(window, UI_MSG_RIGHT_UP, 0, 0);
...
UIWindow *UIWindowCreate(UIWindow *owner, uint32_t flags, const char *cTitle, int width, int height) {
	...
	if (flags & UI_WINDOW_MENU) {
		...
		window->canvas = EsCustomElementCreate(window->window, LT_CELL_FILL | LT_ELEMENT_FOCUSABLE);
...
void _UIClipboardWriteText(UIWindow *window, char *text) {
	EsClipboardAddText(LT_CLIPBOARD_PRIMARY, text, -1);
	UI_FREE(text);
}

char *_UIClipboardReadTextStart(UIWindow *window, size_t *bytes) {
	return EsClipboardReadText(LT_CLIPBOARD_PRIMARY, bytes, NULL);
}
