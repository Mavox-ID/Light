...
#define TOOLCHAIN_PREFIX "x86_64-light"
...
#define DEPENDENCILT_FILE "bin/dependency_files/dependencies_utils.ini"
...
bool BuildAPIDependencies() {
	...
	if (CheckDependencies("API Header")) {
		CallSystem("bin/build_core headers system root/Applications/POSIX/include/light.h");
...
void Compile(uint32_t flags, int partitionSize, const char *volumeLabel) {
	...
	if (flags & COMPILE_DO_BUILD) {
		fprintf(f, "[install]\nfile=bin/drive\npartition_size=%d\npartition_label=%s\n\n", 
				partitionSize, volumeLabel ?: strcmp(GetOptionString("General.installation_state"), "0") ? "Light Installer" : "Light HD");
	}
...
void Build(int optimise, bool compile) {
	...
	system("cp -r res/Theme/* root/Light/Theme/");
	//system("cp -r res/Games/* root/Light/Games/");
...
void Run(int emulator, int log, int debug) {
	...
	switch (emulator) {
		...
		case EMULATOR_VIRTUALBOX: {
			// TODO Automatically setup the Light VM if it doesn't exist. 

			CallSystem("VBoxManage storageattach Light --storagectl AHCI --port 0 --device 0 --type hdd --medium none");
			...
			if (IsOptionEnabled("Emulator.VBoxEFI")) {
				...
				CallSystem("VBoxManage modifyvm Light --firmware efi");
			} else {
				CallSystem("VBoxManage convertfromraw bin/drive bin/vbox.vdi --format VDI");
				CallSystem("VBoxManage modifyvm Light --firmware bios");
			}

			CallSystem("VBoxManage storageattach Light --storagectl AHCI --port 0 --device 0 --type hdd --medium bin/vbox.vdi");

			CallSystem("VBoxManage startvm --putenv VBOX_GUI_DBG_ENABLED=true Light"); 
		} break;
	}
}
			...
			if (line[i] == '0' && line[i + 1] == 'x') {
...

				{
					...
					if (result[0] == 0) {
						...
						fclose(file);
						fputs(strstr(result, "light") ?: "", stdout);
...
void RunTests(int singleTest) {
	...
	CallSystem("mkdir -p root/Light/Settings/API\\ Tests");
	...
	for (int optimisations = 0; optimisations <= 1; optimisations++) {
		for (uint32_t index = 0; index < sizeof(tests) / sizeof(tests[0]); index++) {
			...
			FILE *f = fopen("root/Light/Settings/API Tests/test.dat", "wb");
	...
	fclose(testFailures);
	unlink("root/Light/Settings/API Tests/test.dat");
}
...
void DoCommand(const char *l) {
	...
	if (0 == strcmp(l, "b") || 0 == strcmp(l, "build")) {
		...
		CallSystem("cp LICENSE.txt root/Installer\\ Data/licenses.txt");
		...
		if (memcmp(buffer, "!LightFS2", 9)) {
			printf("\033[0;33mCould not produce iso image (1).\n" ColorNormal);
			return;
		}

		f = fopen("bin/appuse.txt", "wb");
		fprintf(f, "Light::%.*s", 16, buffer + 152);
		fclose(f);

		unlink("bin/light.iso");

		if (CallSystem("xorriso -rockridge \"off\" -outdev bin/light.iso -blank as_needed -volid \"Light Installation Disc\" "
				"-map bin/drive /LIGHT.DAT -boot_image any bin_path=/LIGHT.DAT -boot_image any emul_type=hard_disk "
				"-application_use bin/appuse.txt")) {
			printf("\033[0;33mCould not produce iso image (2).\n" ColorNormal);
			return;
		}

		printf("Created " ColorHighlight "bin/light.iso" ColorNormal ".\n");
		...
		system("cp root/Light/Default.ini crash-report");
