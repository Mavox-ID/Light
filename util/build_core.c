...
#define MSG_BUILD_SUCCESS (LT_MSG_USER_START + 1)
#define MSG_BUILD_FAILED  (LT_MSG_USER_START + 2)
#define MSG_LOG           (LT_MSG_USER_START + 3)
...
File FileOpen(const char *path, char mode) {
	...
	EsFileInformation information = EsFileOpen(path2, path2Bytes, mode == 'r' ? (LT_FILE_READ | LT_NODE_FAIL_IF_NOT_FOUND) : LT_FILE_WRITE);
	EsHeapFree(path2, 0, NULL);
	if (information.error != LT_SUCCESS) return (File) { .error = true };
...
#define LightFSError() exit(1)

#include "../shared/crc.h"
#include "../shared/partitions.cpp"
#define DEPENDENCILT_FILE "bin/dependency_files/dependencies.ini"
...
const char *toolchainCompilerObjects = "/Applications/POSIX/lib/gcc/x86_64-light/" GCC_VERSION;
...
#define STARTS_WITH(s) (strlen(pathBuffer) >= strlen(s) && 0 == memcmp(pathBuffer, s, strlen(s)))
	...
	else if (ENDS_WITH(".ld") || STARTS_WITH("root/Applications/POSIX/x86_64-light/lib/ldscripts/"))
...
void CreateImportNode(const char *path, ImportNode *node) {
	...
	for (uintptr_t i = 0; i < childCount; i++) {
		...
		if (noImportPOSIX && 0 == strcmp(pathBuffer, "root/Applications/POSIX")) {
			continue;
		} else if (children[i].type == LT_NODE_DIRECTORY) {
...
void BuildDesktop(Application *application) {
	...
	if (!application->error) {
		application->error = !MakeBundle("root/" SYSTEM_FOLDER_NAME "/Desktop.lpp", application->bundleInputFiles, arrlenu(application->bundleInputFiles), 0);
	}
}
...
void BuildApplication(Application *application) {
	...
	if (systemBuild) {
		snprintf(executable, sizeof(executable), "root/Applications/%s.lpp", application->name);
	} else {
		snprintf(executable, sizeof(executable), "bin/%s.lpp", application->name);
	}
...
void OutputSystemConfiguration() {
	...
	for (uintptr_t i = 0; i < arrlenu(applications); i++) {
		...
		FilePrintFormat(file, "executable=0:/Applications/%s.lpp\n", applications[i].name);
...
void BuildModule(Application *application) {
	...
	snprintf(output, sizeof(output), "bin/Object Files/%s.lkm", application->name);
	...
	if (!application->builtin) {
		char target[4096];
		snprintf(target, sizeof(target), "root/" SYSTEM_FOLDER_NAME "/Modules/%s.lkm", application->name);
...
void LinkKernel() {
	...
	CopyFile("bin/Stripped Executables/Kernel", "root/" SYSTEM_FOLDER_NAME "/Kernel.lpp", false);
}
...
int main(int argc, char **argv) {
			...
			if (0 == strcmp(s.section, "toolchain")) {
				...
				if (!EsINIPeek(&s) || !s.keyBytes) {
					...
					if (driverSource && *driverSource) {
						...
						if (driverBuiltin) {
							char append[256];
							snprintf(append, sizeof(append), " \"bin/Object Files/%s.lkm\" ", driverName);
	...
	if (!skipCompile) {
		if (systemBuild) {
			...
			CopyFile("bin/Object Files/crt1.o", "cross/lib/gcc/x86_64-light/" GCC_VERSION "/crt1.o", true); // TODO Don't hardcode the target.
			CopyFile("bin/Object Files/crtglue.o", "cross/lib/gcc/x86_64-light/" GCC_VERSION "/crtglue.o", true);
...
const EsStyle styleTextboxLog = {
	.inherit = LT_STYLE_TEXTBOX_BORDERED_MULTILINE,

	.metrics = {
		.mask = LT_THEME_METRICS_FONT_FAMILY | LT_THEME_METRICS_TEXT_SIZE | LT_THEME_METRICS_PREFERRED_WIDTH,
		.textSize = 10,
...
void CommandBuild(EsInstance *instance, EsElement *element, EsCommand *command) {
	...
	if (!buildRunning) {
		...
		if (LT_SUCCESS == EsThreadCreate(ThreadBuild, NULL, argument)) {
			buildRunning = true;
			EsCommandSetEnabled(&commandBuild, false);
			EsCommandSetEnabled(&commandLaunch, false);
		} else {
			EsDialogShow(instance->window, INTERFACE_STRING(BuildCoreCannotCreateBuildThread), NULL, 0, LT_ICON_DIALOG_ERROR, LT_DIALOG_ALERT_OK_BUTTON);
		}
	}
}
...
void CommandLaunch(EsInstance *instance, EsElement *element, EsCommand *command) {
	...
	while (EsINIParse(&s)) {
		if (0 == EsStringCompareRaw(s.section, s.sectionBytes, EsLiteral("general")) 
			...
			executablePath = EsStringAllocateAndFormat(&executablePathBytes, "%s/bin/%s.lpp", workingDirectoryBytes, workingDirectory, s.valueBytes, s.value);
		}
	}
...
void InstanceCreate(EsMessage *message) {
	...
	EsWindowSetIcon(instance->window, LT_ICON_TEXT_X_MAKEFILE);
	EsPanel *panelRoot = EsPanelCreate(instance->window, LT_CELL_FILL, LT_STYLE_PANEL_WINDOW_BACKGROUND);
	EsPanel *panelStack = EsPanelCreate(panelRoot, LT_CELL_FILL | LT_PANEL_HORIZONTAL, EsStyleIntern(&stylePanelStack));
	EsPanel *panelButtonStack = EsPanelCreate(panelStack, LT_CELL_V_TOP | LT_PANEL_VERTICAL, EsStyleIntern(&stylePanelButtonStack));
	buttonBuild = EsButtonCreate(panelButtonStack, LT_BUTTON_DEFAULT, LT_NULL, INTERFACE_STRING(BuildCoreBuild));
	EsCommandAddButton(&commandBuild, buttonBuild);
	buttonBuild->accessKey = 'B';
	buttonLaunch = EsButtonCreate(panelButtonStack, LT_FLAGS_DEFAULT, LT_NULL, INTERFACE_STRING(BuildCoreLaunch));
	EsCommandAddButton(&commandLaunch, buttonLaunch);
	buttonLaunch->accessKey = 'L';
	EsSpacerCreate(panelStack, LT_CELL_V_FILL, LT_STYLE_SEPARATOR_VERTICAL, 0, 0);
	textboxLog = EsTextboxCreate(panelStack, LT_TEXTBOX_MULTILINE | LT_CELL_FILL, EsStyleIntern(&styleTextboxLog));
...
void _start() {
	...
	if (createData.subsystemID == LT_SUBSYSTEM_ID_POSIX) {
	...
	while (true) {
		...
		if (message->type == LT_MSG_INSTANCE_CREATE) {
			...
			if (message->type == MSG_BUILD_FAILED) {
				...
				EsElementFocus(buttonBuild, LT_ELEMENT_FOCUS_ONLY_IF_NO_FOCUSED_ELEMENT);
			} else {
				EsTextboxAppend(textboxLog, INTERFACE_STRING(BuildCoreBuildSuccess), false);
				EsElementFocus(buttonLaunch, LT_ELEMENT_FOCUS_ONLY_IF_NO_FOCUSED_ELEMENT);
			}
