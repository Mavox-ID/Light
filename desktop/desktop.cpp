...
#define MSG_SETUP_DESKTOP_UI ((EsMessageType) (LT_MSG_USER_START + 1))
...
const EsStyle styleSmallParagraph = {
	.inherit = LT_STYLE_TEXT_PARAGRAPH,

	.metrics = {
		.mask = LT_THEME_METRICS_TEXT_SIZE,
		.textSize = 8,
	},
};
...
const EsStyle styleNewTabContent = {
	.metrics = {
		...
		.insets = LT_RECT_4(50, 50, 50, 50),
		.gapMajor = 25,
	},
};

const EsStyle styleButtonGroupContainer = {
	.inherit = LT_STYLE_BUTTON_GROUP_CONTAINER,
...
const EsStyle styleClockButton = {
	.inherit = LT_STYLE_TASK_BAR_EXTRA,

	.metrics = {
		.mask = LT_THEME_METRICS_TEXT_FIGURES,
		.textFigures = LT_TEXT_FIGURE_TABULAR,
	},
};
...
void ReorderItemDragged(ReorderItem *item, int mouseX) {
	...
	if (draggedIndex != currentIndex) {
		...
		EsMessage m = { .type = LT_MSG_REORDER_ITEM_MOVED, .child = item };
...
int ReorderListMessage(EsElement *_list, EsMessage *message) {
	...
	if (message->type == LT_MSG_LAYOUT) {
		...
		EsMessage m = { LT_MSG_REORDER_ITEM_TEST };

		if (LT_HANDLED == EsMessageSend(message->child, &m)) {
...
void DesktopInspectorThread(EsGeneric) {
	...
	EsWindow *window = EsWindowCreate(nullptr, LT_WINDOW_PLAIN);
	EsRectangle screen;
	EsSyscall(LT_SYSCALL_SCREEN_WORK_AREA_GET, 0, (uintptr_t) &screen, 0, 0);
	screen.l = screen.r - 400;
	EsSyscall(LT_SYSCALL_WINDOW_MOVE, window->handle, (uintptr_t) &screen, 0, LT_WINDOW_MOVE_ALWAYS_ON_TOP);
	EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, LT_FLAGS_DEFAULT, 0, LT_WINDOW_PROPERTY_SOLID);
	EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, 180, 0, LT_WINDOW_PROPERTY_ALPHA);
	EsPanel *panel = EsPanelCreate(window, LT_CELL_FILL, LT_STYLE_PANEL_FILLED);
	...
	while (true) {
		...
		EsTextDisplayCreate(panel, LT_CELL_H_FILL, LT_STYLE_TEXT_HEADING1, "Desktop Inspector");
		EsSpacerCreate(panel, LT_CELL_H_FILL, 0, 0, 5);
		...
		for (uintptr_t i = 0; i < desktop.allApplicationInstances.Length(); i++) {
			...
			EsTextDisplayCreate(panel, LT_CELL_H_FILL, EsStyleIntern(&styleSmallParagraph), buffer, bytes);
		}
...
void WindowTabActivate(WindowTab *tab, bool force = false) {
	...
	if (tab->container->active != tab || force) {
		...
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, tab->window->handle, handle, 0, LT_WINDOW_PROPERTY_EMBED);
	}
}
...
void WindowTabDestroy(WindowTab *tab) {
	...
	if (container->openTabs.Length() == 1) {
		...
		if (container->taskBarButton) {
			...
			EsElementStartTransition(container->taskBarButton, LT_TRANSITION_FADE_OUT, LT_ELEMENT_TRANSITION_HIDE_AFTER_COMPLETE);
		...
		tab->exiting = true;
		tab->SetStyle(LT_STYLE_WINDOW_TAB_INACTIVE);
		EsElementStartTransition(tab, LT_TRANSITION_FADE_OUT, LT_ELEMENT_TRANSITION_HIDE_AFTER_COMPLETE);
...
WindowTab *WindowTabMoveToNewContainer(WindowTab *tab, ContainerWindow *container, int32_t width, int32_t height) {
	...
	if (container) EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, newTab->window->handle, 0, 0, LT_WINDOW_PROPERTY_FOCUSED);
...
int ProcessGlobalKeyboardShortcuts(EsElement *, EsMessage *message) {
	...
	if (message->type == LT_MSG_KEY_DOWN) {
		bool ctrlOnly = message->keyboard.modifiers == LT_MODIFIER_CTRL;
		uint32_t scancode = ScancodeMapToLabel(message->keyboard.scancode);

		if (ctrlOnly && scancode == LT_SCANCODE_N && !message->keyboard.repeat && !desktop.installationState) {
		...
		} else if (message->keyboard.modifiers == (LT_MODIFIER_CTRL | LT_MODIFIER_FLAG) && scancode == LT_SCANCODE_D) {
		...
		} else if (message->keyboard.modifiers == (LT_MODIFIER_CTRL | LT_MODIFIER_FLAG) && scancode == LT_SCANCODE_DELETE) {
			for (uintptr_t i = 0; i < desktop.installedApplications.Length(); i++) {
		...
		} else if (scancode == LT_SCANCODE_ACPI_POWER) {
			ShutdownModalCreate();
		} else {
			return 0;
		}
		...
		return LT_HANDLED;
	} else if (message->type == LT_MSG_KEY_UP) {
		if (message->keyboard.scancode == LT_SCANCODE_LEFT_CTRL && message->keyboard.single) {
			if (EsSystemConfigurationReadInteger(EsLiteral("general"), EsLiteral("locate_cursor_on_ctrl"))) {
				...
				EsElement *wrapper = EsCustomElementCreate(window, LT_CELL_FILL, LT_STYLE_CLEAR_BACKGROUND);
				wrapper->messageUser = CursorLocatorMessage;
				window->announcementBase = position;
				EsElement *element = EsCustomElementCreate(wrapper, LT_CELL_FILL, LT_STYLE_CURSOR_LOCATOR);
				int width = element->GetWidth(0), height = element->GetHeight(0);
				EsRectangle bounds = LT_RECT_4PD(position.x - width / 2, position.y - height / 2, width, height);
				EsSyscall(LT_SYSCALL_WINDOW_MOVE, window->handle, (uintptr_t) &bounds, 0, LT_WINDOW_MOVE_ALWAYS_ON_TOP);
...
int ContainerWindowMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_WINDOW_ACTIVATED) {
		if (container->taskBarButton) {
		...
		if (((message->keyboard.modifiers & ~LT_MODIFIER_SHIFT) == LT_MODIFIER_CTRL) && message->keyboard.scancode == LT_SCANCODE_TAB) {
			int tab = -1;
			...
			EsAssert(tab != -1);
			tab += ((message->keyboard.modifiers & LT_MODIFIER_SHIFT) ? -1 : 1);
		...
		} else if (ctrlOnly && scancode == LT_SCANCODE_T && !message->keyboard.repeat) {
			_EsApplicationStartupInformation startupInformation = { .id = APPLICATION_ID_DESKTOP_BLANK_TAB };
			ApplicationInstanceCreate(&startupInformation, container);
		} else if (ctrlOnly && scancode == LT_SCANCODE_W && !message->keyboard.repeat) {
			WindowTabClose(container->active);
		} else if (message->keyboard.modifiers == LT_MODIFIER_FLAG && scancode == LT_SCANCODE_UP_ARROW) {
			WindowSnap(container->window, false, false, SNAP_EDGE_MAXIMIZE);
		} else if (message->keyboard.modifiers == LT_MODIFIER_FLAG && scancode == LT_SCANCODE_DOWN_ARROW) {
			if (container->window->isMaximised) {
				WindowRestore(container->window);
			} else {
				EsSyscall(LT_SYSCALL_WINDOW_MOVE, container->window->handle, 0, 0, LT_WINDOW_MOVE_HIDDEN);
			}
		} else if (message->keyboard.modifiers == LT_MODIFIER_FLAG && scancode == LT_SCANCODE_LEFT_ARROW) {
			if (container->window->restoreOnNextMove) {
				WindowRestore(container->window);
			} else {
				WindowSnap(container->window, false, false, SNAP_EDGE_LEFT);
			}
		} else if (message->keyboard.modifiers == LT_MODIFIER_FLAG && scancode == LT_SCANCODE_RIGHT_ARROW) {
			if (container->window->restoreOnNextMove) {
				WindowRestore(container->window);
			} else {
			...
			for (uintptr_t i = 0; i < 9; i++) {
				if (ctrlOnly && scancode == (int) (LT_SCANCODE_1 + i) && container->openTabs.Length() > i) {
...
int WindowTabMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_DESTROY) {
		...
		EsDrawContent(message->painter, element, LT_RECT_2S(message->painter->width, message->painter->height), 
		...
		if (band->container->openTabs.Length() == 1) {
			...
			EsSyscall(LT_SYSCALL_WINDOW_FIND_BY_POINT, (uintptr_t) &hoverWindowID, mousePositionOnScreen.x, mousePositionOnScreen.y, tab->window->id);
			EsWindow *hoverWindow = WindowFromWindowID(hoverWindowID);
			bool dragInto = false;

			if (hoverWindow && hoverWindow->windowStyle == LT_WINDOW_CONTAINER) {
			...
			if (!dragInto) {
				...
				EsSyscall(LT_SYSCALL_WINDOW_TRANSFER_PRESS, tab->window->handle, hoverWindow->handle, 0, 0);
				EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, tab->window->handle, 0, 0, LT_WINDOW_PROPERTY_EMBED);
			...
			if (EsRectangleContains(EsRectangleAdd(band->GetWindowBounds(), LT_RECT_1I(-dragOffThreshold)), mousePosition.x, mousePosition.y)) {
				...
				EsMessage m = { .type = LT_MSG_MOUSE_LEFT_UP };
				...
				if (newTab) {
					...
					EsSyscall(LT_SYSCALL_WINDOW_TRANSFER_PRESS, band->window->handle, newTab->window->handle, 0, 0);
		...
		EsElementSetDisabled(band->GetChild(0), false);
	} else if (message->type == LT_MSG_MOUSE_RIGHT_CLICK && tab->applicationInstance) {
		EsMenu *menu = EsMenuCreate(tab, LT_FLAGS_DEFAULT);
		uint64_t disableIfOnlyTab = tab->container->openTabs.Length() == 1 ? LT_ELEMENT_DISABLED : LT_FLAGS_DEFAULT;

		EsMenuAddItem(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(DesktopCloseTab), [] (EsMenu *, EsGeneric context) {
		...
		if (EsKeyboardIsShiftHeld()) {
			...
			EsMenuAddItem(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(DesktopInspectUI), [] (EsMenu *, EsGeneric context) {
				WindowTab *tab = (WindowTab *) context.p;
				ApplicationInstance *instance = tab->applicationInstance;
				if (!instance) return;
				EsMessage m = { LT_MSG_TAB_INSPECT_UI };
		...
		EsMenuShow(menu);
	} else if (message->type == LT_MSG_MOUSE_MIDDLE_UP && element->window->hovered == element) {
		if (EsButtonGetCheck(tab->closeButton) == LT_CHECK_CHECKED) {
		...
		WindowTabClose(tab);
	} else if (message->type == LT_MSG_REORDER_ITEM_TEST) {
	} else {
		return 0;
	}
...
WindowTab *WindowTabCreate(ContainerWindow *container) {
	...
	tab->Initialise(container->tabBand, LT_CELL_H_FILL | LT_CELL_V_BOTTOM, WindowTabMessage, 0);
	tab->cName = "window tab";
	container->openTabs.Add(tab);

	tab->closeButton = EsButtonCreate(tab, LT_FLAGS_DEFAULT, LT_STYLE_WINDOW_TAB_CLOSE_BUTTON);
...
int WindowTabBandMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_LAYOUT) {
		for (uint16_t i = 0; i < band->container->openTabs.Length(); i++) {
			...
			tab->SetStyle(tab == tab->container->active ? LT_STYLE_WINDOW_TAB_ACTIVE : LT_STYLE_WINDOW_TAB_INACTIVE);
		...
		EsMenu *menu = EsMenuCreate(band, LT_MENU_AT_CURSOR);
		...
		EsMenuAddItem(menu, LT_FLAGS_DEFAULT, string, -1, [] (EsMenu *, EsGeneric context) {
		...
		EsMenuAddItem(menu, band->window->isMaximised ? LT_ELEMENT_DISABLED : LT_FLAGS_DEFAULT,
		...
		EsMenuAddItem(menu, band->window->isMaximised ? LT_ELEMENT_DISABLED : LT_FLAGS_DEFAULT,
			...
			EsSyscall(LT_SYSCALL_WINDOW_MOVE, ((EsWindow *) context.p)->handle, 0, 0, LT_WINDOW_MOVE_HIDDEN);
		}, band->window);
		...
		EsMenuAddItem(menu, band->window->isMaximised ? LT_ELEMENT_DISABLED : LT_FLAGS_DEFAULT, 
			...
			EsSyscall(LT_SYSCALL_SCREEN_WORK_AREA_GET, 0, (uintptr_t) &workArea, 0, 0);
			...
			EsSyscall(LT_SYSCALL_WINDOW_MOVE, band->window->handle, (uintptr_t) &newBounds, 0, LT_FLAGS_DEFAULT);
		}, band);

		EsMenuAddItem(menu, LT_FLAGS_DEFAULT,
		...
		EsMenuShow(menu);
	} else if (message->type == LT_MSG_REORDER_ITEM_MOVED) {
...
void ContainerWindowShow(ContainerWindow *container, int32_t width, int32_t height) {
	...
	EsRectangle bounds = LT_RECT_4(cascadeX, cascadeX + window->windowWidth, cascadeY, cascadeY + window->windowHeight);
	...
	EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, 0, 0, LT_WINDOW_PROPERTY_FOCUSED);
}

ContainerWindow *ContainerWindowCreate() {
	ContainerWindow *container = (ContainerWindow *) EsHeapAllocate(sizeof(ContainerWindow), true);
	EsWindow *window = EsWindowCreate(nullptr, LT_WINDOW_CONTAINER);
	desktop.allContainerWindows.Add(container);

	window->messageUser = ContainerWindowMessage;
	window->userData = container;
	
	window->mainPanel = EsPanelCreate(window, LT_ELEMENT_NON_CLIENT | LT_CELL_FILL, LT_STYLE_PANEL_CONTAINER_WINDOW_ROOT);
	window->SetStyle(LT_STYLE_CONTAINER_WINDOW);

	EsMessage m = { .type = LT_MSG_UI_SCALE_CHANGED };
	...
	container->tabBand->Initialise(container->window, LT_CELL_FILL, WindowTabBandMessage, LT_STYLE_WINDOW_TAB_BAND);
	container->tabBand->cName = "window tab band";

	EsButton *newTabButton = EsButtonCreate(container->tabBand, LT_FLAGS_DEFAULT, LT_STYLE_WINDOW_TAB_BAND_NEW);
	...
	if (!desktop.installationState) {
		...
		container->taskBarButton->Initialise(&desktop.taskBar.taskList, LT_CELL_FILL, 
				TaskBarButtonMessage, LT_STYLE_TASK_BAR_BUTTON);
...
int TaskBarButtonMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_PAINT && button->containerWindow) {
	...
	} else if (message->type == LT_MSG_MOUSE_LEFT_CLICK && button->containerWindow) {
		if (button->customStyleState & THEME_STATE_SELECTED) {
			EsSyscall(LT_SYSCALL_WINDOW_MOVE, button->containerWindow->window->handle, 0, 0, LT_WINDOW_MOVE_HIDDEN);
		} else {
			EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, button->containerWindow->window->handle, 0, 0, LT_WINDOW_PROPERTY_FOCUSED);
		}
	} else if (message->type == LT_MSG_ANIMATE) {
...
int TaskBarWindowMessage(EsElement *, EsMessage *message) {
	if (message->type == LT_MSG_ANIMATE) {
		...
		EsSyscall(LT_SYSCALL_WINDOW_MOVE, desktop.taskBar.window->handle, (uintptr_t) &bounds, 0, LT_WINDOW_MOVE_UPDATE_SCREEN);
	}
...
void TaskBarTasksButtonUpdate() {
	if (desktop.allOngoingUserTasks.Length()) {
		if (EsElementIsHidden(desktop.tasksButton)) {
			...
			EsElementStartTransition(desktop.tasksButton, LT_TRANSITION_FADE_IN, LT_FLAGS_DEFAULT, 1.5f);
		...
		if (!EsElementIsHidden(desktop.tasksButton)) {
			EsPanelStartMovementAnimation((EsPanel *) EsElementGetLayoutParent(desktop.tasksButton), 1.5f /* duration scale */);
			EsElementStartTransition(desktop.tasksButton, LT_TRANSITION_FADE_OUT, LT_ELEMENT_TRANSITION_HIDE_AFTER_COMPLETE, 1.5f);
		}
	}
...
int TaskBarTasksButtonMessage(EsElement *element, EsMessage *message) {
	if (message->type == LT_MSG_GET_WIDTH) {
		...
		EsDrawContent(message->painter, element, LT_RECT_2S(message->painter->width, message->painter->height), title, titleBytes);
		...
		destination = EsRectangleFit(destination, LT_RECT_2S(1, 1), true); // Center with a 1:1 aspect ratio.
		...
		if (RastSurfaceInitialise(&surface, painter->target->width, painter->target->height, true)) {
...

			{
				...
				RastPath path = {};
				RastPathAppendArc(&path, center, Width(destination) * 0.45f, LT_PI * 2.0f, 0.0f);
...

			{
				...
				RastPath path = {};
				RastPathAppendArc(&path, center, Width(destination) * 0.45f, LT_PI * 1.5f + progress * LT_PI * 2.0f, LT_PI * 1.5f);
				RastShape shape = RastShapeCreateContour(&path, style, true);
				RastSurfaceFill(surface, shape, paint, false);
				RastPathDestroy(&path);
			}
...
void TaskBarClockUpdateThread(EsGeneric _clock) {
	...
	while (true) {
		...
		int tzOffset = EsSystemConfigurationReadInteger(EsLiteral("general"), EsLiteral("timezone_offset"));
		bool use12Hour = EsSystemConfigurationReadInteger(EsLiteral("general"), EsLiteral("clock_12_hour"));
		bool showSeconds = EsSystemConfigurationReadInteger(EsLiteral("general"), EsLiteral("clock_show_seconds"));

		int displayHour = (currentTime.hour + tzOffset) % 24;
		...
		if (showSeconds) {
			labelBytes = EsStringFormat(label, sizeof(label), "%d%d:%d%d:%d%d%s", 
					h / 10, h % 10, currentTime.minute / 10, currentTime.minute % 10, currentTime.second / 10, currentTime.second % 10, amPmBytes, amPmStr);
		} else {
			labelBytes = EsStringFormat(label, sizeof(label), "%d%d:%d%d%s", 
					h / 10, h % 10, currentTime.minute / 10, currentTime.minute % 10, amPmBytes, amPmStr);
		}
...
void Shutdown(uintptr_t action) {
	...
	if (!desktop.allApplicationProcesses.Length()) {
		...
		EsSyscall(LT_SYSCALL_SHUTDOWN, action, 0, 0, 0);
	}
	...
	EsThreadCreate([] (EsGeneric action) {
		...
		EsSyscall(LT_SYSCALL_SHUTDOWN, action.u, 0, 0, 0);
	}, nullptr, action);
}
...
void ShutdownModalCreate() {
	...
	EsSyscall(LT_SYSCALL_WINDOW_MOVE, window->handle, (uintptr_t) &screen, 0, LT_WINDOW_MOVE_ALWAYS_ON_TOP);
	EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, (uintptr_t) &screen, 0, LT_WINDOW_PROPERTY_BLUR_BOUNDS);
	EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, LT_WINDOW_SOLID_TRUE, 0, LT_WINDOW_PROPERTY_SOLID);
	EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, 0, 0, LT_WINDOW_PROPERTY_FOCUSED);
	// EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, BLEND_WINDOW_MATERIAL_LIGHT_BLUR, 0, LT_WINDOW_PROPERTY_MATERIAL);

	// Setup the UI.

	EsPanel *stack = EsPanelCreate(window, LT_CELL_FILL | LT_PANEL_Z_STACK);
	stack->cName = "window stack";
	EsPanelCreate(stack, LT_CELL_FILL, LT_STYLE_PANEL_SHUTDOWN_OVERLAY)->cName = "modal overlay";
	EsPanel *dialog = EsPanelCreate(stack, LT_PANEL_VERTICAL | LT_CELL_CENTER | LT_ELEMENT_AUTO_GROUP, LT_STYLE_DIALOG_SHADOW);
	dialog->cName = "dialog";
	EsPanel *heading = EsPanelCreate(dialog, LT_PANEL_HORIZONTAL | LT_CELL_H_FILL, LT_STYLE_DIALOG_HEADING);
	EsIconDisplayCreate(heading, LT_FLAGS_DEFAULT, {}, LT_ICON_SYSTEM_SHUTDOWN);
	EsTextDisplayCreate(heading, LT_CELL_H_FILL | LT_CELL_V_CENTER, LT_STYLE_TEXT_HEADING2, 
			INTERFACE_STRING(DesktopShutdownTitle))->cName = "dialog heading";
	EsTextDisplayCreate(EsPanelCreate(dialog, LT_PANEL_VERTICAL | LT_CELL_H_FILL, LT_STYLE_DIALOG_CONTENT), 
			LT_CELL_H_FILL, LT_STYLE_TEXT_PARAGRAPH, INTERFACE_STRING(DesktopConfirmShutdown))->cName = "dialog contents";
	EsPanel *buttonArea = EsPanelCreate(dialog, LT_PANEL_HORIZONTAL | LT_PANEL_REVERSE | LT_CELL_H_FILL, LT_STYLE_DIALOG_BUTTON_AREA);
	EsButton *cancelButton = EsButtonCreate(buttonArea, LT_BUTTON_DEFAULT | LT_BUTTON_CANCEL, 0, INTERFACE_STRING(CommonCancel));
	EsButton *restartButton = EsButtonCreate(buttonArea, LT_FLAGS_DEFAULT, LT_STYLE_PUSH_BUTTON_DANGEROUS, INTERFACE_STRING(DesktopRestartAction));
	EsButton *shutdownButton = EsButtonCreate(buttonArea, LT_FLAGS_DEFAULT, LT_STYLE_PUSH_BUTTON_DANGEROUS, INTERFACE_STRING(DesktopShutdownAction));
	...
	EsButtonOnCommand(shutdownButton, [] (EsInstance *, EsElement *, EsCommand *) {
		Shutdown(LT_SHUTDOWN_ACTION_POWER_OFF);
	});

	EsButtonOnCommand(restartButton, [] (EsInstance *, EsElement *, EsCommand *) {
		Shutdown(LT_SHUTDOWN_ACTION_RESTART);
	});
...
void InstanceCrashedTabCreate(EsMessage *message) {
	...
	if (reason != CRASHED_TAB_NOT_RESPONDING) EsWindowSetIcon(instance->window, LT_ICON_DIALOG_ERROR);
	EsElement *toolbar = EsWindowGetToolbar(instance->window);
	EsPanel *panel = EsPanelCreate(toolbar, LT_CELL_V_CENTER | LT_CELL_V_PUSH | LT_CELL_H_SHRINK | LT_CELL_H_PUSH | LT_PANEL_VERTICAL, LT_STYLE_PANEL_CRASH_INFO);

	if (reason == CRASHED_TAB_FATAL_ERROR) {
		EsTextDisplayCreate(panel, LT_CELL_H_FILL, LT_STYLE_TEXT_PARAGRAPH, INTERFACE_STRING(DesktopCrashedApplication));
	} else if (reason == CRASHED_TAB_PROGRAM_NOT_FOUND) {
		EsTextDisplayCreate(panel, LT_CELL_H_FILL, LT_STYLE_TEXT_PARAGRAPH, INTERFACE_STRING(DesktopNoSuchApplication));
		EsWindowSetTitle(instance->window, INTERFACE_STRING(CommonErrorTitle));
	} else if (reason == CRASHED_TAB_INVALID_EXECUTABLE) {
		EsTextDisplayCreate(panel, LT_CELL_H_FILL, LT_STYLE_TEXT_PARAGRAPH, INTERFACE_STRING(DesktopApplicationStartupError));
		EsWindowSetTitle(instance->window, INTERFACE_STRING(CommonErrorTitle));
	} else if (reason == CRASHED_TAB_NOT_RESPONDING) {
		EsTextDisplayCreate(panel, LT_CELL_H_FILL, LT_STYLE_TEXT_PARAGRAPH, INTERFACE_STRING(DesktopNotResponding));
		EsButton *button = EsButtonCreate(panel, LT_CELL_H_RIGHT, LT_STYLE_PUSH_BUTTON_DANGEROUS, INTERFACE_STRING(DesktopForceQuit));
...
void InstanceBlankTabCreate(EsMessage *message) {
	...
	EsPanel *content = EsPanelCreate(windowBackground, LT_CELL_FILL | LT_PANEL_V_SCROLL_AUTO, EsStyleIntern(&styleNewTabContent));
	EsPanel *buttonGroup;

	// Installed applications list.

	buttonGroup = EsPanelCreate(content, LT_PANEL_VERTICAL | LT_CELL_H_SHRINK, EsStyleIntern(&styleButtonGroupContainer));
	buttonGroup->separatorStylePart = LT_STYLE_BUTTON_GROUP_SEPARATOR;
	buttonGroup->separatorFlags = LT_CELL_H_FILL;

	for (uintptr_t i = 0; i < desktop.installedApplications.Length(); i++) {
		InstalledApplication *application = desktop.installedApplications[i];
		if (application->hidden) continue;

		EsButton *button = EsButtonCreate(buttonGroup, LT_CELL_H_FILL | LT_ELEMENT_NO_FOCUS_ON_CLICK, LT_STYLE_BUTTON_GROUP_ITEM, application->cName);
		button->userData = application;

		if (application->iconID) {
			EsButtonSetIcon(button, (EsStandardIcon) application->iconID);
		} else {
			EsButtonSetIcon(button, LT_ICON_APPLICATION_DEFAULT_ICON);
			...
			size_t fileBytes;
			void *file = EsFileMap(application->cExecutable, -1, &fileBytes, LT_MEMORY_MAP_OBJECT_READ_ONLY);
...
void ApplicationInstanceClose(ApplicationInstance *instance) {
	...
	EsMessage m = { LT_MSG_TAB_CLOSE_REQUEST };
	m.tabOperation.id = instance->embeddedWindowID;
	EsMessagePostRemote(instance->process->handle, &m);
}
...
void ApplicationInstanceCleanup(ApplicationInstance *instance) {
	...
	if (instance->process) {
		...
		if (!instance->process->instanceCount) {
			EsMessage m = { LT_MSG_APPLICATION_EXIT };
			EsMessagePostRemote(instance->process->handle, &m);
		}
	}
...
ApplicationProcess *DesktopGetApplicationProcessForDesktop() {
	EsObjectID desktopProcessID = EsProcessGetID(LT_CURRENT_PROCESS);
...
bool ApplicationInstanceStart(_EsApplicationStartupInformation *startupInformation, ApplicationInstance *instance) {
	...
	if (application && application->useSingleInstance && application->singleInstance) {
		...
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, application->singleInstance->tab->window->handle, 0, 0, LT_WINDOW_PROPERTY_FOCUSED);
		return false;
	}
	...
	if (instance->tab) {
		EsButtonSetCheck(instance->tab->closeButton, LT_CHECK_UNCHECKED, false);
	}
	...
	if (application->createInstance) {
		...
		if (!process) {
			...
			process->handle = EsSyscall(LT_SYSCALL_HANDLE_SHARE, LT_CURRENT_PROCESS, LT_CURRENT_PROCESS, 0, 0);
			process->id = EsProcessGetID(LT_CURRENT_PROCESS);
		...
		EsError error = NodeOpen(application->cExecutable, EsCStringLength(application->cExecutable), 
				LT_FILE_READ | LT_NODE_FAIL_IF_NOT_FOUND, &executableNode);

		if (LT_CHECK_ERROR(error)) {
			ApplicationTemporaryDestroy(application);
			_EsApplicationStartupInformation s = {};
			s.id = APPLICATION_ID_DESKTOP_CRASHED;
		...
		arguments.permissions = LT_PERMISSION_WINDOW_MANAGER;
		...
		if (application->permissions & APPLICATION_PERMISSION_MANAGE_PROCESSES) {
			arguments.permissions |= LT_PERMISSION_TAKE_SYSTEM_SNAPSHOT;
			arguments.permissions |= LT_PERMISSION_PROCESS_CREATE;
			arguments.permissions |= LT_PERMISSION_PROCESS_OPEN;
		}

		if (application->permissions & APPLICATION_PERMISSION_POSIX_SUBSYSTEM) {
			arguments.permissions |= LT_PERMISSION_PROCESS_OPEN;
			arguments.permissions |= LT_PERMISSION_POSIX_SUBSYSTEM;
		...
		if (application->permissions & APPLICATION_PERMISSION_SHUTDOWN) {
			arguments.permissions |= LT_PERMISSION_SHUTDOWN;
		}

		if (application->permissions & APPLICATION_PERMISSION_ALL_FILES) {
			arguments.permissions |= LT_PERMISSION_GET_VOLUME_INFORMATION;
		} 
		...
		for (uintptr_t i = 0; i < api.connectedDevices.Length(); i++) {
			if ((application->permissions & APPLICATION_PERMISSION_ALL_DEVICES)
					|| ((application->permissions & APPLICATION_PERMISSION_ALL_FILES) && api.connectedDevices[i].type == LT_DEVICE_FILE_SYSTEM)) {
...

		{
			error = NodeOpen(application->settingsPath, application->settingsPathBytes, 
					LT_NODE_DIRECTORY | LT_NODE_CREATE_DIRECTORIES | LT__NODE_DIRECTORY_WRITE, &settingsNode);
			...
			if (error == LT_SUCCESS) {
				...
				settingsNode.handle = LT_INVALID_HANDLE;
			}
		}
		...
		arguments.data.systemData = EsConstantBufferCreate(buffer.out, buffer.position, LT_CURRENT_PROCESS);
	...
	EsMessage m = { LT_MSG_INSTANCE_CREATE };
	...
	if (startupInformation->readHandle) {
		startupInformation->readHandle = EsSyscall(LT_SYSCALL_HANDLE_SHARE, startupInformation->readHandle, process->handle, 0, 0);
	}
	...
	EsHandle handle = EsSyscall(LT_SYSCALL_WINDOW_CREATE, LT_WINDOW_NORMAL, 0, 0, 0);
	instance->embeddedWindowHandle = handle;
	instance->embeddedWindowID = EsSyscall(LT_SYSCALL_WINDOW_GET_ID, handle, 0, 0, 0);
	m.createInstance.window = EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, handle, process->handle, 0, LT_WINDOW_PROPERTY_EMBED_OWNER);
...
void ApplicationInstanceCrashed(EsMessage *message) {
	...
	const char *fatalErrorString = state.crashReason.errorCode >= LT_FATAL_ERROR_COUNT ? "[unknown]" 
...
void OpenDocumentListUpdated() {
	if (desktop.fileManager && desktop.fileManager->singleProcess) {
		EsMessage m = { .type = LT_MSG_FILE_MANAGER_DOCUMENT_UPDATE };
...
void OpenDocumentWithApplication(EsApplicationStartupRequest *startupRequest, ContainerWindow *container) {
	...
	if (!foundDocument) {
		EsFileInformation file = EsFileOpen(startupInformation.filePath, startupInformation.filePathBytes, 
				LT_FILE_READ_SHARED | LT_NODE_FAIL_IF_NOT_FOUND);
...
EsError TemporaryFileCreate(EsHandle *handle, char **path, size_t *pathBytes, uint32_t additionalFlags) {
	...
	size_t temporaryFilePathBytes = EsStringFormat(temporaryFilePath, LT_STRING_FORMAT_ENOUGH_SPACE, "%s/%s", 
	...
	EsFileInformation file = EsFileOpen(temporaryFilePath, temporaryFilePathBytes, 
			LT_NODE_FAIL_IF_FOUND | LT_NODE_CREATE_DIRECTORIES | additionalFlags);
...
void ApplicationInstanceRequestSave(ApplicationInstance *instance, const char *newName, size_t newNameBytes, bool failIfAlreadyExists) {
	...
	EsMessage m = {};
	m.type = LT_MSG_INSTANCE_SAVE_RESPONSE;
	m.tabOperation.id = instance->embeddedWindowID;
	...
	if (!instance->documentID) {
		...
		if (!nameBytes || (failIfAlreadyExists && nameBytes != folderBytes + newNameBytes)) {
			EsHeapFree(name);
			m.tabOperation.error = nameBytes ? LT_ERROR_ALREADY_EXISTS : LT_ERROR_TOO_MANY_FILLT_WITH_NAME;
			EsMessagePostRemote(instance->process->handle, &m);
			return;
		}

		EsFileInformation file = EsFileOpen(name, nameBytes, LT_FILE_READ_SHARED | LT_NODE_FAIL_IF_FOUND);
		...
		*desktop.openDocuments.Put(&document.id) = document;
...

		{
			...
			EsMessage m = { LT_MSG_INSTANCE_DOCUMENT_RENAMED };
	...
	if (document->currentWriter) {
		m.tabOperation.error = LT_ERROR_OPERATION_BLOCKED;
	} else {
		EsHeapFree(document->temporarySavePath);
		document->temporarySavePath = nullptr;

		if (LT_SUCCESS != EsFileControl(document->readHandle, LT_FILE_CONTROL_GET_CONTENT_TYPE, 
					&document->temporarySavePreviousContentType, sizeof(EsUniqueIdentifier))) {
			document->temporarySavePreviousContentType = {};
		}

		EsHandle fileHandle;
		m.tabOperation.error = TemporaryFileCreate(&fileHandle, &document->temporarySavePath, &document->temporarySavePathBytes, LT_FILE_WRITE);

		if (m.tabOperation.error == LT_SUCCESS) {
			document->currentWriter = instance->embeddedWindowID;
			m.tabOperation.handle = EsSyscall(LT_SYSCALL_HANDLE_SHARE, fileHandle, instance->process->handle, 0, 0);
...
void InstanceAnnouncePathMoved(InstalledApplication *fromApplication, const char *oldPath, size_t oldPathBytes, const char *newPath, size_t newPathBytes) {
	...
	if (fromApplication != desktop.fileManager && desktop.fileManager && desktop.fileManager->singleProcess) {
		...
		EsMessage m = {};
		m.type = LT_MSG_FILE_MANAGER_PATH_MOVED;
...
void ApplicationInstanceCompleteSave(ApplicationInstance *fromInstance) {
	...
	EsPathMove(document->temporarySavePath, document->temporarySavePathBytes, document->path, document->pathBytes, LT_PATH_MOVE_ALLOW_COPY_AND_DELETE);

	// Re-open the read handle.

	EsFileInformation file = EsFileOpen(document->path, document->pathBytes, LT_FILE_READ_SHARED | LT_NODE_FAIL_IF_NOT_FOUND);
	...
	if (LT_SUCCESS == EsFileControl(document->readHandle, LT_FILE_CONTROL_GET_CONTENT_TYPE, &newContentType, sizeof(EsUniqueIdentifier))) {
		if (0 == EsMemoryCompare(&newContentType, &zeroContentType, sizeof(EsUniqueIdentifier))
			...
			EsFileInformation write = EsFileOpen(document->path, document->pathBytes, LT_FILE_WRITE_SHARED | LT_NODE_FAIL_IF_NOT_FOUND);

			if (write.error == LT_SUCCESS) {
				EsFileControl(write.handle, LT_FILE_CONTROL_SET_CONTENT_TYPE, 
						&document->temporarySavePreviousContentType, sizeof(EsUniqueIdentifier));
	...
	if (desktop.fileManager && desktop.fileManager->singleProcess) {
		EsMessage m = {};
		m.type = LT_MSG_FILE_MANAGER_FILE_MODIFIED;
	...
	for (uintptr_t i = 0; i < desktop.allApplicationInstances.Length(); i++) {
		...
		EsMessage m = { LT_MSG_INSTANCE_DOCUMENT_UPDATED };
		m.tabOperation.isSource = instance == fromInstance;
		m.tabOperation.id = instance->embeddedWindowID;
		m.tabOperation.handle = EsSyscall(LT_SYSCALL_HANDLE_SHARE, document->readHandle, instance->process->handle, 0, 0);
		EsMessagePostRemote(instance->process->handle, &m);
	}
}
...
void ConfigurationLoadApplications() {
...

	{
		...
		application->iconID = LT_ICON_PREFERENCES_DESKTOP;
...
#define READ_PERMISSION(x, y) if (SystemConfigurationGroupReadInteger(group, EsLiteral(x), 0)) application->permissions |= y
		...
		if (SystemConfigurationGroupReadInteger(group, EsLiteral("background_service"))) {
			...
			startupInformation.flags = LT_APPLICATION_STARTUP_BACKGROUND_SERVICE;
			ApplicationInstanceCreate(&startupInformation, nullptr, true /* hidden */);
		}
	}
...
void ConfigurationWriteToFile() {
	...
	if (!buffer.error) {
		if (LT_SUCCESS == EsFileWriteAll(EsLiteral(K_SYSTEM_CONFIGURATION "_"), buffer.out, buffer.position)) {
			// TODO Atomic delete and move.
			if (LT_SUCCESS == EsPathDelete(EsLiteral(K_SYSTEM_CONFIGURATION))) {
				if (LT_SUCCESS == EsPathMove(EsLiteral(K_SYSTEM_CONFIGURATION "_"), EsLiteral(K_SYSTEM_CONFIGURATION))) {
...
void WallpaperLoad(EsGeneric) {
	...
	EsRectangle region = LT_RECT_2S(desktop.wallpaperWindow->windowWidth, desktop.wallpaperWindow->windowHeight);
	EsSyscall(LT_SYSCALL_WINDOW_SET_BITS, desktop.wallpaperWindow->handle, (uintptr_t) &region, (uintptr_t) buffer, 0);
...
void CheckForegroundWindowResponding(EsGeneric) {
	...
	if (state.flags & LT_PROCESS_STATE__PINGED) {
		if (tab->notRespondingInstance) {
...
void DesktopSetup() {
	...
	{
		if (!desktop.wallpaperWindow) desktop.wallpaperWindow = EsWindowCreate(nullptr, LT_WINDOW_PLAIN);
		EsRectangle screen;
		EsSyscall(LT_SYSCALL_SCREEN_BOUNDS_GET, 0, (uintptr_t) &screen, 0, 0);
		EsSyscall(LT_SYSCALL_WINDOW_MOVE, desktop.wallpaperWindow->handle, (uintptr_t) &screen, 0, LT_WINDOW_MOVE_AT_BOTTOM);
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, desktop.wallpaperWindow->handle, (uintptr_t) &screen, 0, LT_WINDOW_PROPERTY_OPAQUE_BOUNDS);
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, desktop.wallpaperWindow->handle, 
				LT_WINDOW_SOLID_TRUE | LT_WINDOW_SOLID_NO_BRING_TO_FRONT, 0, LT_WINDOW_PROPERTY_SOLID);
...

	{
		EsRectangle screen;
		EsSyscall(LT_SYSCALL_SCREEN_BOUNDS_GET, 0, (uintptr_t) &screen, 0, 0);
		EsSyscall(LT_SYSCALL_SCREEN_WORK_AREA_SET, 0, (uintptr_t) &screen, 0, 0);
	}

	if (desktop.installationState == INSTALLATION_STATE_NONE) {
		// Create the taskbar.

		EsWindow *window = desktop.setupDesktopUIComplete ? desktop.taskBar.window : EsWindowCreate(nullptr, LT_WINDOW_PLAIN);
		window->messageUser = TaskBarWindowMessage;
		window->appearActivated = true;
		window->StartAnimating();
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, LT_WINDOW_SOLID_TRUE | LT_WINDOW_SOLID_NO_ACTIVATE, 0, LT_WINDOW_PROPERTY_SOLID);
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, BLEND_WINDOW_MATERIAL_GLASS, 0, LT_WINDOW_PROPERTY_MATERIAL);

		if (!desktop.setupDesktopUIComplete) desktop.taskBar.Initialise(window, LT_CELL_FILL, TaskBarMessage, LT_STYLE_TASK_BAR_BAR);
		...
		bounds.l = bounds.t = 0;
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, (uintptr_t) &bounds, 0, LT_WINDOW_PROPERTY_BLUR_BOUNDS);

		if (!desktop.setupDesktopUIComplete) {
			EsPanel *panel = EsPanelCreate(&desktop.taskBar, LT_PANEL_HORIZONTAL | LT_CELL_FILL, {});

			EsButton *newWindowButton = EsButtonCreate(panel, LT_FLAGS_DEFAULT, LT_STYLE_TASK_BAR_NEW_WINDOW);
			...
			desktop.taskBar.taskList.Initialise(panel, LT_CELL_FILL, ReorderListMessage, LT_STYLE_TASK_BAR_LIST);
			desktop.taskBar.taskList.cName = "task list";

			desktop.tasksButton = EsButtonCreate(panel, LT_ELEMENT_HIDDEN, LT_STYLE_TASK_BAR_BUTTON);
			desktop.tasksButton->messageUser = TaskBarTasksButtonMessage;

			EsButton *clockButton = EsButtonCreate(panel, LT_BUTTON_COMPACT, EsStyleIntern(&styleClockButton));
			...
			EsButton *shutdownButton = EsButtonCreate(panel, LT_FLAGS_DEFAULT, LT_STYLE_TASK_BAR_EXTRA);
			EsButtonSetIcon(shutdownButton, LT_ICON_SYSTEM_SHUTDOWN_SYMBOLIC);
...
bool /* returns false on fatal error */ DesktopSyscall(EsObjectID windowID, ApplicationProcess *process, uint8_t *buffer, size_t bytes, EsBuffer *pipe) {
	...
	if (buffer[0] == DESKTOP_MSG_START_APPLICATION) {
		...
		if (!b.error) {
			...
			if ((request.flags & LT_APPLICATION_STARTUP_IN_SAME_CONTAINER) && instance && instance->tab) {
				container = instance->tab->container;
			}

			if (request.flags & LT_APPLICATION_STARTUP_NO_DOCUMENT) {
				_EsApplicationStartupInformation startupInformation = {};
		...
		char *path;
		size_t pathBytes;
		EsError error = TemporaryFileCreate(&handle, &path, &pathBytes, LT_FILE_WRITE);
		...
		if (error == LT_SUCCESS) {
			...
			handle = EsSyscall(LT_SYSCALL_HANDLE_SHARE, handle, process->handle, 0, 0);

			EsHeapFree(path);
		} else {
			handle = LT_INVALID_HANDLE;
		}
		...
		if (information->error == LT_SUCCESS) {
			...
			if (foreground && foreground->process) {
				EsMessage m = { LT_MSG_PRIMARY_CLIPBOARD_UPDATED };
		...
		desktop.nextClipboardFile = LT_INVALID_HANDLE;
		desktop.nextClipboardProcessID = 0;
	} else if (buffer[0] == DESKTOP_MSG_CLIPBOARD_GET && pipe) {
		EsHandle fileHandle = desktop.clipboardFile 
			? EsSyscall(LT_SYSCALL_HANDLE_SHARE, desktop.clipboardFile, process->handle, 1 /* LT_FILE_READ_SHARED */, 0) : LT_INVALID_HANDLE;
		...
		EsHandle windowHandle = EsSyscall(LT_SYSCALL_WINDOW_CREATE, LT_WINDOW_NORMAL, 0, 0, 0);
		ApplicationInstance *instance = (ApplicationInstance *) EsHeapAllocate(sizeof(ApplicationInstance), true);
		bool added = false;
		...
		if (!windowHandle || !instance || !added) {
			...
			EsHandle invalid = LT_INVALID_HANDLE;
		...
		instance->embeddedWindowID = EsSyscall(LT_SYSCALL_WINDOW_GET_ID, windowHandle, 0, 0, 0);
		instance->process = process;
		instance->process->instanceCount++;
		instance->application = application;

		EsHandle targetWindowHandle = EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, windowHandle, process->handle, 0, LT_WINDOW_PROPERTY_EMBED_OWNER);
		...
		EsHandle handle;
		EsError error = TemporaryFileCreate(&handle, &temporaryApplication->settingsPath, &temporaryApplication->settingsPathBytes, LT_NODE_DIRECTORY);
		if (error == LT_SUCCESS) EsHandleClose(handle);
		...
		if (instance->tab) {
			EsButtonSetCheck(instance->tab->closeButton, buffer[1] ? LT_CHECK_CHECKED : LT_CHECK_UNCHECKED, false);
		}
		...
		if (!instance->documentID) {
			...
			if (oldPathBytes == newPathBytes && 0 == EsMemoryCompare(oldPath, newPath, oldPathBytes)) {
				...
				EsMessage m = {};
				m.type = LT_MSG_INSTANCE_RENAME_RESPONSE;
				m.tabOperation.id = instance->embeddedWindowID;
...
void DesktopSendMessage(EsMessage *message) {
	...
	if (message->type == LT_MSG_EMBEDDED_WINDOW_DESTROYED) {
		EmbeddedWindowDestroyed(message->embeddedWindowDestroyedID);
	} else if (message->type == LT_MSG_APPLICATION_CRASH) {
		ApplicationInstanceCrashed(message);
	} else if (message->type == LT_MSG_DEVICE_CONNECTED) {
		EsHandle handle = message->device.handle;

		EsPrint("Desktop received LT_MSG_DEVICE_CONNECTED with ID %d and type %z.\n", 
		...
		for (uintptr_t i = 0; i < desktop.allApplicationProcesses.Length(); i++) {
			...
			if ((process->application->permissions & APPLICATION_PERMISSION_ALL_DEVICES)
					|| ((process->application->permissions & APPLICATION_PERMISSION_ALL_FILES) && message->device.type == LT_DEVICE_FILE_SYSTEM)) {
				message->device.handle = EsSyscall(LT_SYSCALL_HANDLE_SHARE, handle, process->handle, 0, 0);
		...
		if (message->device.type == LT_DEVICE_CLOCK) {
			EsDateComponents reading;
			uint64_t linear;

			if (LT_SUCCESS == EsDeviceControl(handle, LT_DEVICE_CONTROL_CLOCK_READ, &reading, &linear)) {
		...
		} else if (message->device.type == LT_DEVICE_GRAPHICS_TARGET) {
			if (desktop.setupDesktopUIComplete) {
				DesktopSetup(); // Refresh desktop UI.
			} else {
				// The screen resolution will be correctly queried in DesktopSetup.
			}
		} else if (message->device.type == LT_DEVICE_FILE_SYSTEM) {
			if (EsDeviceControl(message->device.handle, LT_DEVICE_CONTROL_FS_IS_BOOT, 0, 0) == 1) {
	...
	} else if (message->type == MSG_SETUP_DESKTOP_UI || message->type == LT_MSG_UI_SCALE_CHANGED) {
		DesktopSetup();
	}
}
