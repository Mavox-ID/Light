...
const char *KeyboardLayoutLookup(uint32_t scancode, bool isShiftHeld, bool isAltGrHeld, bool enableTabs, bool enableNewline) {
	...
	if (scancode == LT_SCANCODE_ENTER || scancode == LT_SCANCODE_NUM_ENTER) return enableNewline ? "\n" : nullptr;
	if (scancode == LT_SCANCODE_TAB) return enableTabs ? "\t" : nullptr;
	if (scancode == LT_SCANCODE_BACKSPACE || scancode == LT_SCANCODE_DELETE) return nullptr;
...
uint32_t ScancodeMapToLabel(uint32_t scancode) {
	...
	if (string && string[0] && !string[1]) {
		char c = string[0];
		if (c >= 'a' && c <= 'z') return LT_SCANCODE_A + c - 'a';
		if (c >= 'A' && c <= 'Z') return LT_SCANCODE_A + c - 'A';
		if (c >= '0' && c <= '9') return LT_SCANCODE_0 + c - '0';
		if (c == '/')  return LT_SCANCODE_SLASH;
		if (c == '[')  return LT_SCANCODE_LEFT_BRACE;
		if (c == ']')  return LT_SCANCODE_RIGHT_BRACE;
		if (c == '=')  return LT_SCANCODE_EQUALS;
		if (c == '-')  return LT_SCANCODE_HYPHEN;
		if (c == ',')  return LT_SCANCODE_COMMA;
		if (c == '.')  return LT_SCANCODE_PERIOD;
		if (c == '\\') return LT_SCANCODE_PUNCTUATION_1;
		if (c == ';')  return LT_SCANCODE_PUNCTUATION_3;
		if (c == '\'') return LT_SCANCODE_PUNCTUATION_4;
		if (c == '`')  return LT_SCANCODE_PUNCTUATION_5;
	}
...
bool ScancodeIsNonTypeable(uint32_t scancode) {
	switch (scancode) {
		case LT_SCANCODE_CAPS_LOCK:
		case LT_SCANCODE_SCROLL_LOCK:
		case LT_SCANCODE_NUM_LOCK:
		case LT_SCANCODE_LEFT_SHIFT:
		case LT_SCANCODE_LEFT_CTRL:
		case LT_SCANCODE_LEFT_ALT:
		case LT_SCANCODE_LEFT_FLAG:
		case LT_SCANCODE_RIGHT_SHIFT:
		case LT_SCANCODE_RIGHT_CTRL:
		case LT_SCANCODE_RIGHT_ALT:
		case LT_SCANCODE_PAUSE:
		case LT_SCANCODE_CONTEXT_MENU:
		case LT_SCANCODE_PRINT_SCREEN:
		case LT_SCANCODE_F1:
		case LT_SCANCODE_F2:
		case LT_SCANCODE_F3:
		case LT_SCANCODE_F4:
		...
		case LT_SCANCODE_F8:
		case LT_SCANCODE_F9:
		case LT_SCANCODE_F10:
		case LT_SCANCODE_F11:
		case LT_SCANCODE_F12:
		case LT_SCANCODE_ACPI_POWER:
		case LT_SCANCODE_ACPI_SLEEP:
		case LT_SCANCODE_ACPI_WAKE:
		case LT_SCANCODE_MM_NEXT:
		case LT_SCANCODE_MM_PREVIOUS:
		case LT_SCANCODE_MM_STOP:
		case LT_SCANCODE_MM_PAUSE:
		case LT_SCANCODE_MM_MUTE:
		case LT_SCANCODE_MM_QUIETER:
		case LT_SCANCODE_MM_LOUDER:
		case LT_SCANCODE_MM_SELECT:
		case LT_SCANCODE_MM_EMAIL:
		case LT_SCANCODE_MM_CALC:
		case LT_SCANCODE_MM_FILES:
		case LT_SCANCODE_WWW_SEARCH:
		case LT_SCANCODE_WWW_HOME:
		case LT_SCANCODE_WWW_BACK:
		case LT_SCANCODE_WWW_FORWARD:
		case LT_SCANCODE_WWW_STOP:
		case LT_SCANCODE_WWW_REFRESH:
		case LT_SCANCODE_WWW_STARRED:
			return true;
...
void EsTextboxStartEdit(EsTextbox *textbox) {
	...
	if ((textbox->flags & LT_TEXTBOX_EDIT_BASED) && !textbox->editing) {
		EsMessage m = { LT_MSG_TEXTBOX_EDIT_START };
...
void TextboxEndEdit(EsTextbox *textbox, bool reject) {
	if ((textbox->flags & LT_TEXTBOX_EDIT_BASED) && textbox->editing) {
		...
		EsMessage m = { LT_MSG_TEXTBOX_EDIT_END };
		...
		if (reject || LT_REJECTED == EsMessageSend(textbox, &m)) {
			EsTextboxSelectAll(textbox);
...
void TextboxUpdateCommands(EsTextbox *textbox, bool noClipboard) {
	...
	command = EsCommandByID(textbox->instance, LT_COMMAND_DELETE);
	...
	command = EsCommandByID(textbox->instance, LT_COMMAND_COPY);
	command->data = textbox;
	...
	EsCommandSetCallback(command, [] (EsInstance *, EsElement *, EsCommand *command) {
		EsTextbox *textbox = (EsTextbox *) command->data.p;
		size_t textBytes;
		char *text = EsTextboxGetContents(textbox, &textBytes, textbox->editing ? LT_TEXTBOX_GET_CONTENTS_SELECTED_ONLY : LT_FLAGS_DEFAULT);
		EsError error = EsClipboardAddText(LT_CLIPBOARD_PRIMARY, text, textBytes);
		EsHeapFree(text);
		...
		if (error == LT_SUCCESS) {
			EsAnnouncementShow(textbox->window, LT_FLAGS_DEFAULT, x, y, INTERFACE_STRING(CommonAnnouncementTextCopied));
		} else if (error == LT_ERROR_INSUFFICIENT_RESOURCES || error == LT_ERROR_DRIVE_FULL) {
			EsAnnouncementShow(textbox->window, LT_FLAGS_DEFAULT, x, y, INTERFACE_STRING(CommonAnnouncementCopyErrorResources));
		} else {
			EsAnnouncementShow(textbox->window, LT_FLAGS_DEFAULT, x, y, INTERFACE_STRING(CommonAnnouncementCopyErrorOther));
		}
	});

	command = EsCommandByID(textbox->instance, LT_COMMAND_CUT);
	command->data = textbox;
	EsCommandSetDisabled(command, selectionEmpty || textbox->readOnly);
	...
	EsCommandSetCallback(command, [] (EsInstance *, EsElement *, EsCommand *command) {
		...
		size_t textBytes;
		char *text = EsTextboxGetContents(textbox, &textBytes, textbox->editing ? LT_TEXTBOX_GET_CONTENTS_SELECTED_ONLY : LT_FLAGS_DEFAULT);
		EsClipboardAddText(LT_CLIPBOARD_PRIMARY, text, textBytes);
		EsHeapFree(text);
	...
	command = EsCommandByID(textbox->instance, LT_COMMAND_SELECT_ALL);
	command->data = textbox;
	...
	if (!noClipboard) {
		command = EsCommandByID(textbox->instance, LT_COMMAND_PASTE);
		command->data = textbox;
		EsCommandSetDisabled(command, textbox->readOnly || !EsClipboardHasText(LT_CLIPBOARD_PRIMARY));

		EsCommandSetCallback(command, [] (EsInstance *, EsElement *, EsCommand *command) {
			EsTextbox *textbox = (EsTextbox *) command->data.p;

			size_t textBytes = 0;
			char *text = EsClipboardReadText(LT_CLIPBOARD_PRIMARY, &textBytes);
...
void TextboxEnsureCaretVisibleActionCallback(EsElement *element, EsGeneric context) {
	...
	for (uintptr_t i = 0; i < 3; i++) {
		// ScrollPane::SetY causes LT_MSG_SCROLL_Y to get sent to the textbox.
...
void EsTextboxMoveCaretRelative(EsTextbox *textbox, uint32_t flags) {
	if (~flags & LT_TEXTBOX_MOVE_CARET_SECOND_ONLY) {
		TextboxMoveCaret(textbox, &textbox->carets[0], ~flags & LT_TEXTBOX_MOVE_CARET_BACKWARDS, 
				flags & 0xFF, flags & LT_TEXTBOX_MOVE_CARET_STRONG_WHITESPACE);
	}

	if (~flags & LT_TEXTBOX_MOVE_CARET_FIRST_ONLY) {
		TextboxMoveCaret(textbox, &textbox->carets[1], ~flags & LT_TEXTBOX_MOVE_CARET_BACKWARDS, 
				flags & 0xFF, flags & LT_TEXTBOX_MOVE_CARET_STRONG_WHITESPACE);
	}
}

void TextboxRepaintLine(EsTextbox *textbox, int line) {
	if (line == -1 || (~textbox->flags & LT_TEXTBOX_MULTILINE)) {
		textbox->Repaint(true);
	} else {
		...
		if (visibleLine) {
			EsRectangle bounds = textbox->GetBounds();
			EsRectangle lineBounds = LT_RECT_4(bounds.l + borders.l, bounds.r - borders.r, 
...
void TextboxUndoItemCallback(const void *item, EsUndoManager *manager, EsMessage *message) {
	if (message->type == LT_MSG_UNDO_INVOKE) {
		...
		EsTextboxEnsureCaretVisible(textbox);
	} else if (message->type == LT_MSG_UNDO_CANCEL) {
		// Nothing to do.
	}
}
...
void EsTextboxInsert(EsTextbox *textbox, const char *string, ptrdiff_t stringBytes, bool sendUpdatedMessage) {
...

	{
		...
		if (!newlines || (~textbox->flags & LT_TEXTBOX_MULTILINE)) {
	...
	if (sendUpdatedMessage) {
		EsMessage m = { LT_MSG_TEXTBOX_UPDATED };
...
void EsTextboxAppend(EsTextbox *textbox, const char *string, ptrdiff_t stringBytes, bool sendUpdatedMessage) {
	...
	EsTextboxMoveCaretRelative(textbox, LT_TEXTBOX_MOVE_CARET_ALL);
...
char *EsTextboxGetContents(EsTextbox *textbox, size_t *_bytes, uint32_t flags) {
	...
	bool includeNewline = textbox->flags & LT_TEXTBOX_MULTILINE;
	...
	if (flags & LT_TEXTBOX_GET_CONTENTS_SELECTED_ONLY) {
...
bool EsTextboxFind(EsTextbox *textbox, const char *needle, intptr_t _needleBytes, int32_t *_line, int32_t *_byte, uint32_t flags) {
	...
	while (true) {
		...
		if (flags & LT_TEXTBOX_FIND_BACKWARDS) {
...
bool TextboxFindCaret(EsTextbox *textbox, int positionX, int positionY, bool secondCaret, int clickChainCount) {
	...
	if (clickChainCount >= 4) {
		...
		for (uintptr_t i = 0; i < textbox->visibleLines.Length(); i++) {
			...
			EsRectangle lineBounds = LT_RECT_4(textbox->insets.l, bounds.r, 
...
int ProcessTextboxMarginMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_PAINT) {
		...
		for (int32_t i = 0; i < (int32_t) textbox->visibleLines.Length(); i++) {
			...
			properties.flags = LT_TEXT_V_CENTER | LT_TEXT_H_RIGHT | LT_TEXT_ELLIPSIS | LT_TEXT_PLAN_SINGLE_USE;
...
void TextboxStyleChanged(EsTextbox *textbox) {
	...
	if (textbox->flags & LT_TEXTBOX_MARGIN) {
...
void TextboxAddSmartContextMenu(EsTextbox *textbox, EsMenu *menu) {
	if ((~textbox->flags & LT_TEXTBOX_NO_SMART_CONTEXT_MENUS) && textbox->carets[0].line == textbox->carets[1].line) {
...
void TextboxUnregisterCommands(EsTextbox *textbox) {
	EsCommandSetCallback(EsCommandByID(textbox->instance, LT_COMMAND_SELECT_ALL), nullptr);
	EsCommandSetCallback(EsCommandByID(textbox->instance, LT_COMMAND_DELETE), nullptr);
	EsCommandSetCallback(EsCommandByID(textbox->instance, LT_COMMAND_COPY), nullptr);
	EsCommandSetCallback(EsCommandByID(textbox->instance, LT_COMMAND_CUT), nullptr);
	EsCommandSetCallback(EsCommandByID(textbox->instance, LT_COMMAND_PASTE), nullptr);
}
...
int ProcessTextboxMessage(EsElement *element, EsMessage *message) {
	...
	if (!textbox->editing && textbox->overlayCallback) {
		...
		if (response != 0 && message->type != LT_MSG_DESTROY) return response;
	}
	...
	if (message->type == LT_MSG_PAINT) {
		...
		selectionProperties.hideCaret = (~textbox->state & UI_STATE_FOCUSED) || (textbox->flags & LT_ELEMENT_DISABLED) || !textbox->editing;
		...
		EsRectangle clip;
		EsRectangleClip(painter->clip, LT_RECT_4(painter->offsetX + textbox->borders.l, 
		...
		for (int32_t i = 0; i < (int32_t) textbox->visibleLines.Length(); i++) {
			...
			EsRectangle lineBounds = LT_RECT_4(painter->offsetX + textbox->insets.l, 
			...
			if (~textbox->flags & LT_TEXTBOX_MULTILINE) {
			...
			if (textbox->carets[1].line == i + textbox->firstVisibleLine && textbox->syntaxHighlightingLanguage) {
				EsRectangle line = LT_RECT_4(painter->offsetX, painter->offsetX + painter->width, lineBounds.t, lineBounds.b);
			...
			EsTextPlanProperties properties = {};
			properties.flags = LT_TEXT_V_CENTER | LT_TEXT_H_LEFT | LT_TEXT_PLAN_SINGLE_USE;
		...
		EsHeapFree(textbox->editStartContent);
	} else if (message->type == LT_MSG_KEY_TYPED && !ScancodeIsNonTypeable(message->keyboard.scancode)) {
		bool verticalMotion = false;
		bool ctrl = message->keyboard.modifiers & LT_MODIFIER_CTRL;

		if (message->keyboard.modifiers & ~(LT_MODIFIER_CTRL | LT_MODIFIER_ALT | LT_MODIFIER_SHIFT | LT_MODIFIER_ALT_GR)) {
			// Unused modifier.
			return 0;
		}

		if (message->keyboard.scancode == LT_SCANCODE_LEFT_ARROW || message->keyboard.scancode == LT_SCANCODE_RIGHT_ARROW
				|| message->keyboard.scancode == LT_SCANCODE_HOME || message->keyboard.scancode == LT_SCANCODE_END
				|| message->keyboard.scancode == LT_SCANCODE_UP_ARROW || message->keyboard.scancode == LT_SCANCODE_DOWN_ARROW) {
			bool direction = (message->keyboard.scancode == LT_SCANCODE_LEFT_ARROW || message->keyboard.scancode == LT_SCANCODE_HOME 
					|| message->keyboard.scancode == LT_SCANCODE_UP_ARROW)
				? MOVE_CARET_BACKWARDS : MOVE_CARET_FORWARDS;
			int moveType = (message->keyboard.scancode == LT_SCANCODE_HOME || message->keyboard.scancode == LT_SCANCODE_END) 
			...
			if (message->keyboard.modifiers & LT_MODIFIER_SHIFT) {
		...
		} else if (message->keyboard.scancode == LT_SCANCODE_PAGE_UP || message->keyboard.scancode == LT_SCANCODE_PAGE_DOWN) {
			for (uintptr_t i = 0; i < 10; i++) {
				TextboxMoveCaret(textbox, textbox->carets + 1, 
						message->keyboard.scancode == LT_SCANCODE_PAGE_UP ? MOVE_CARET_BACKWARDS : MOVE_CARET_FORWARDS, 
						MOVE_CARET_VERTICAL);
			}
			...
			textbox->Repaint(true);
			verticalMotion = true;
		} else if ((message->keyboard.scancode == LT_SCANCODE_BACKSPACE || message->keyboard.scancode == LT_SCANCODE_DELETE) && !textbox->readOnly) {
			if (!textbox->editing) {
				EsTextboxStartEdit(textbox);
			}

			if (!TextboxCompareCarets(textbox->carets + 0, textbox->carets + 1)) {
				TextboxMoveCaret(textbox, textbox->carets + 1, message->keyboard.scancode == LT_SCANCODE_BACKSPACE ? MOVE_CARET_BACKWARDS : MOVE_CARET_FORWARDS, 
						ctrl ? MOVE_CARET_WORD : MOVE_CARET_SINGLE);
			}

			EsTextboxInsert(textbox, EsLiteral(""));
		} else if (message->keyboard.scancode == LT_SCANCODE_ENTER && (textbox->flags & LT_TEXTBOX_EDIT_BASED)) {
			if (textbox->editing) {
				TextboxEndEdit(textbox, false);
			} else {
				EsTextboxStartEdit(textbox);
			}
		} else if (message->keyboard.scancode == LT_SCANCODE_ESCAPE && (textbox->flags & LT_TEXTBOX_EDIT_BASED)) {
			TextboxEndEdit(textbox, true);
		} else if (message->keyboard.scancode == LT_SCANCODE_TAB && (~textbox->flags & LT_TEXTBOX_ALLOW_TABS)) {
			response = 0;
			...
			const char *inputString = KeyboardLayoutLookup(message->keyboard.scancode, 
					message->keyboard.modifiers & LT_MODIFIER_SHIFT, message->keyboard.modifiers & LT_MODIFIER_ALT_GR, 
					true, textbox->flags & LT_TEXTBOX_MULTILINE);

			if (inputString && (message->keyboard.modifiers & ~(LT_MODIFIER_SHIFT | LT_MODIFIER_ALT_GR)) == 0) {
		...
		TextboxMoveCaretToCursor(textbox, message->mouseDown.positionX, message->mouseDown.positionY, message->type == LT_MSG_MOUSE_RIGHT_DOWN);
	} else if (message->type == LT_MSG_MOUSE_LEFT_CLICK) {
		EsTextboxStartEdit(textbox);
	} else if (message->type == LT_MSG_FOCUSED_START || message->type == LT_MSG_PRIMARY_CLIPBOARD_UPDATED) {
		...
		textbox->Repaint(true);
	} else if (message->type == LT_MSG_STRONG_FOCUS_END) {
		TextboxEndEdit(textbox, textbox->flags & LT_TEXTBOX_REJECT_EDIT_IF_LOST_FOCUS);
	} else if (message->type == LT_MSG_MOUSE_LEFT_DRAG || message->type == LT_MSG_MOUSE_RIGHT_DRAG || (message->type == LT_MSG_ANIMATE && textbox->scroll.dragScrolling)) {
		int32_t lineFrom = textbox->carets[1].line;

		if (gui.lastClickButton == LT_MSG_MOUSE_RIGHT_DOWN && !textbox->inRightClickDrag) {
		...
		if (lineFrom > lineTo) { int32_t t = lineTo; lineTo = lineFrom; lineFrom = t; }
		for (int32_t i = lineFrom; i <= lineTo; i++) TextboxRepaintLine(textbox, i);
	} else if (message->type == LT_MSG_GET_CURSOR) {
		if (!textbox->editing || (textbox->flags & LT_ELEMENT_DISABLED)) {
		...
		EsMenu *menu = EsMenuCreate(textbox, LT_MENU_AT_CURSOR);
		if (!menu) return LT_HANDLED;

		// TODO User customisation of menus.

		if (textbox->readOnly) {
			EsMenuAddCommand(menu, 0, INTERFACE_STRING(CommonClipboardCopy), EsCommandByID(textbox->instance, LT_COMMAND_COPY));
			EsMenuAddCommand(menu, 0, INTERFACE_STRING(CommonSelectionSelectAll), EsCommandByID(textbox->instance, LT_COMMAND_SELECT_ALL));
		} else {
			if (textbox->editing) {
				EsMenuAddCommand(menu, 0, INTERFACE_STRING(CommonUndo), EsCommandByID(textbox->instance, LT_COMMAND_UNDO));
				EsMenuAddSeparator(menu);
			}

			EsMenuAddCommand(menu, 0, INTERFACE_STRING(CommonClipboardCut), EsCommandByID(textbox->instance, LT_COMMAND_CUT));
			EsMenuAddCommand(menu, 0, INTERFACE_STRING(CommonClipboardCopy), EsCommandByID(textbox->instance, LT_COMMAND_COPY));
			EsMenuAddCommand(menu, 0, INTERFACE_STRING(CommonClipboardPaste), EsCommandByID(textbox->instance, LT_COMMAND_PASTE));

			if (textbox->editing) {
				EsMenuAddSeparator(menu);
				EsMenuAddCommand(menu, 0, INTERFACE_STRING(CommonSelectionSelectAll), EsCommandByID(textbox->instance, LT_COMMAND_SELECT_ALL));
				EsMenuAddCommand(menu, 0, INTERFACE_STRING(CommonSelectionDelete), EsCommandByID(textbox->instance, LT_COMMAND_DELETE));
		...
		EsMenuShow(menu);
	} else if (message->type == LT_MSG_COLOR_CHANGED) {
		EsAssert(~textbox->flags & LT_TEXTBOX_NO_SMART_CONTEXT_MENUS); // Textbox sent color changed message, but it cannot have smart context menus?
	...
	} else if (message->type == LT_MSG_SCROLL_X) {
		TextboxSetHorizontalScroll(textbox, message->scroll.scroll);
	} else if (message->type == LT_MSG_SCROLL_Y) {
...
EsTextbox *EsTextboxCreate(EsElement *parent, uint64_t flags, EsStyleID style) {
	...
	if (!style) {
		if (flags & LT_TEXTBOX_MULTILINE) {
			style = LT_STYLE_TEXTBOX_BORDERED_MULTILINE;
		} else {
			style = LT_STYLE_TEXTBOX_BORDERED_SINGLE;
		}
	}

	textbox->Initialise(parent, LT_ELEMENT_FOCUSABLE | flags, ProcessTextboxMessage, style);
	textbox->cName = "textbox";

	textbox->scroll.Setup(textbox, 
			(flags & LT_TEXTBOX_MULTILINE) ? LT_SCROLL_MODE_AUTO : LT_SCROLL_MODE_HIDDEN, 
			(flags & LT_TEXTBOX_MULTILINE) ? LT_SCROLL_MODE_AUTO : LT_SCROLL_MODE_NONE,
			LT_SCROLL_X_DRAG | LT_SCROLL_Y_DRAG);
	...
	if (~flags & LT_TEXTBOX_EDIT_BASED) {
		textbox->editing = true;
	}

	if (textbox->flags & LT_TEXTBOX_MARGIN) {
		textbox->margin = EsCustomElementCreate(textbox, LT_CELL_FILL, LT_STYLE_TEXTBOX_MARGIN);
...
void EsTextboxUseNumberOverlay(EsTextbox *textbox, bool defaultBehaviour) {
	...
	EsAssert(textbox->flags & LT_TEXTBOX_EDIT_BASED); // Using textbox overlay without edit based mode.
	EsAssert(~textbox->flags & LT_TEXTBOX_MULTILINE); // Using number overlay with multiline mode.
	...
	textbox->overlayCallback = [] (EsElement *element, EsMessage *message) {
		...
		if (message->type == LT_MSG_MOUSE_LEFT_DRAG) {
		...
		} else if (message->type == LT_MSG_KEY_TYPED && message->keyboard.scancode == LT_SCANCODE_UP_ARROW) {
			EsMessage m = { LT_MSG_TEXTBOX_NUMBER_DRAG_DELTA };
			m.numberDragDelta.delta = 1;
			m.numberDragDelta.fast = EsKeyboardIsShiftHeld();
			m.numberDragDelta.hoverCharacter = 0;
			EsMessageSend(textbox, &m);
		} else if (message->type == LT_MSG_KEY_TYPED && message->keyboard.scancode == LT_SCANCODE_DOWN_ARROW) {
			EsMessage m = { LT_MSG_TEXTBOX_NUMBER_DRAG_DELTA };
			m.numberDragDelta.delta = -1;
			m.numberDragDelta.fast = EsKeyboardIsShiftHeld();
			m.numberDragDelta.hoverCharacter = 0;
			...
			if (gui.draggingStarted) {
				EsMessage m = { LT_MSG_TEXTBOX_NUMBER_DRAG_END };
				EsMessageSend(textbox, &m);
			}
		} else if (message->type == LT_MSG_GET_CURSOR) {
			if (EsMouseIsLeftHeld()) {
				message->cursorStyle = LT_CURSOR_BLANK;
			} else if (~textbox->flags & LT_ELEMENT_DISABLED) {
				message->cursorStyle = LT_CURSOR_RESIZE_VERTICAL;
			} else {
				message->cursorStyle = LT_CURSOR_NORMAL;
			}
		} else if (message->type == LT_MSG_TEXTBOX_EDIT_END && defaultBehaviour) {
			...
			if (value.error) {
				...
				EsMessage m = { LT_MSG_TEXTBOX_NUMBER_UPDATED };
		...
		} else if (message->type == LT_MSG_TEXTBOX_NUMBER_DRAG_DELTA && defaultBehaviour) {
			TextboxSetActiveLine(textbox, -1);
...
void TextboxBreadcrumbOverlayRecreate(EsTextbox *textbox) {
	...
	EsPanel *panel = EsPanelCreate(textbox, LT_PANEL_HORIZONTAL | LT_CELL_FILL | LT_ELEMENT_NO_HOVER, LT_STYLE_BREADCRUMB_BAR_PANEL);
	...
	EsMessage m = { LT_MSG_TEXTBOX_GET_BREADCRUMB };
	m.getBreadcrumb.buffer = &buffer;

	while (true) {
		buffer.position = 0, m.getBreadcrumb.icon = 0;
		int response = EsMessageSend(textbox, &m);
		EsAssert(response != 0); // Must handle LT_MSG_TEXTBOX_GET_BREADCRUMB message for breadcrumb overlay.
		if (response == LT_REJECTED) break;

		EsButton *crumb = EsButtonCreate(panel, LT_BUTTON_NOT_FOCUSABLE | LT_BUTTON_COMPACT | LT_CELL_V_FILL, 
				LT_STYLE_BREADCRUMB_BAR_CRUMB, (char *) buffer.out, buffer.position);
		...
		if (crumb) {
			...
			crumb->messageUser = [] (EsElement *element, EsMessage *message) {
				if (message->type == LT_MSG_MOUSE_LEFT_CLICK) {
					EsMessage m = { LT_MSG_TEXTBOX_ACTIVATE_BREADCRUMB };
...
void EsTextboxSetReadOnly(EsTextbox *textbox, bool readOnly) {
	...
	EsAssert(~textbox->flags & LT_TEXTBOX_EDIT_BASED);
	TextboxUpdateCommands(textbox, false);
}
