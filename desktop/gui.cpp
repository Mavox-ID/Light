...
struct EsElement : EsElementPublic {
	...
	inline EsElement *GetChildByZ(uintptr_t index) {
		EsMessage m = { LT_MSG_Z_ORDER };
		m.zOrder.index = index, m.zOrder.child = GetChild(index);
		if (m.zOrder.child->flags & LT_ELEMENT_NON_CLIENT) return m.zOrder.child;
		if (LT_REJECTED == EsMessageSend(this, &m)) return nullptr;
		EsAssert(!m.zOrder.child || m.zOrder.child->parent == this); // Child obtained from LT_MSG_Z_ORDER had different parent.
		return m.zOrder.child;
	}

	inline EsRectangle GetInternalOffset() {
		return LT_RECT_4(internalOffsetLeft, internalOffsetRight, internalOffsetTop, internalOffsetBottom);
	}
	...
	bool IsFocusable() {
		if ((~flags & LT_ELEMENT_FOCUSABLE) || (flags & LT_ELEMENT_DISABLED) || (state & UI_STATE_DESTROYING)) {
			return false;
		}
		...
		while (element) {
			if ((element->flags & LT_ELEMENT_BLOCK_FOCUS) || (element->state & UI_STATE_BLOCK_INTERACTION) || (element->flags & LT_ELEMENT_HIDDEN)) {
				return false;
			}
	...
	bool IsTabTraversable() {
		return IsFocusable() && (~flags & LT_ELEMENT_NOT_TAB_TRAVERSABLE) && (!parent || ~parent->state & UI_STATE_RADIO_GROUP);
	}
	...
	inline EsRectangle GetBounds() { return LT_RECT_2S(width - internalOffsetLeft - internalOffsetRight, height - internalOffsetTop - internalOffsetBottom); }
...
#define PAINT_NO_BACKGROUND (1 << 4) // Don't paint the background.
	...
	void Repaint(bool all, EsRectangle region = LT_RECT_1(0) /* client coordinates */);
...
struct MeasurementCache {
	...
	bool Get(EsMessage *message, uint32_t *state) {
		...
		if (message->type == LT_MSG_GET_WIDTH) {
...
void WindowSnap(EsWindow *window, bool restored, bool dragging, uint8_t edge) {
	...
	EsRectangle screen;
	EsSyscall(LT_SYSCALL_SCREEN_WORK_AREA_GET, 0, (uintptr_t) &screen, 0, 0);
	...
	EsSyscall(LT_SYSCALL_WINDOW_MOVE, window->handle, (uintptr_t) &bounds, 0, 
			LT_WINDOW_MOVE_DYNAMIC | (edge == SNAP_EDGE_MAXIMIZE ? LT_WINDOW_MOVE_MAXIMIZED : 0));
...
void WindowRestore(EsWindow *window, bool dynamic = true) {
	...
	EsSyscall(LT_SYSCALL_WINDOW_MOVE, window->handle, (uintptr_t) &window->beforeMaximiseBounds, 0, dynamic ? LT_WINDOW_MOVE_DYNAMIC : LT_FLAGS_DEFAULT);
}
...
#define WINDOW_CLAMP_SIZE(_size, _direction, _side, _otherSide, _minimum) \
	...
	EsSyscall(LT_SYSCALL_WINDOW_MOVE, window->handle, (uintptr_t) &bounds, 0, LT_WINDOW_MOVE_DYNAMIC);
}

int ProcessWindowBorderMessage(EsWindow *window, EsMessage *message, EsRectangle bounds, bool addSolidInsets) {
	if (message->type == LT_MSG_GET_CURSOR) {
		EsPoint position = EsMouseGetPosition(window);
		message->cursorStyle = LT_CURSOR_NORMAL;
		...
		if (window->isMaximised) {
			...
			if (gui.resizing) {
				...
				message->cursorStyle = LT_CURSOR_RESIZE_DIAGONAL_1;
			} else if ((left && top) || (bottom && right)) {
				message->cursorStyle = LT_CURSOR_RESIZE_DIAGONAL_2;
			} else if (left || right) {
				message->cursorStyle = LT_CURSOR_RESIZE_HORIZONTAL;
			} else if (top || bottom) {
				message->cursorStyle = LT_CURSOR_RESIZE_VERTICAL;
			}
	...
	} else if (message->type == LT_MSG_MOUSE_LEFT_DOWN) {
	...
	} else if (message->type == LT_MSG_MOUSE_LEFT_DRAG) {
		EsPoint screenPosition = EsMouseGetPosition(nullptr);
	...
	} else if (message->type == LT_MSG_MOUSE_LEFT_UP) {
		if (window->restoreOnNextMove) {
			window->resetPositionOnNextMove = true;
		}
		...
		gui.resizing = false;
	} else if (message->type == LT_MSG_ANIMATE && window->animateToTargetBoundsAfterResize) {
...
void UIWindowNeedsUpdate(EsWindow *window) {
	if (!window->willUpdate && window->handle /* cleared in UIWindowDestroy, during InternalDestroy */) {
		EsMessage m = { LT_MSG_UPDATE_WINDOW };
		...
		// because the window cannot be deallocated until LT_MSG_WINDOW_DESTROYED is received, 
		// and this message will always be received first.
		EsSyscall(LT_SYSCALL_MESSAGE_POST, (uintptr_t) &m, (uintptr_t) window, LT_CURRENT_PROCESS, 0);
...
void UIWindowDestroy(EsWindow *window) {
	...
	AccessKeyModeExit();
	EsSyscall(LT_SYSCALL_WINDOW_CLOSE, window->handle, 0, 0, 0);
	EsHandleClose(window->handle);
	window->checkVisible.Free();
	window->sizeAlternatives.Free();
	window->updateActions.Free();
	window->dialogs.Free();
	window->handle = LT_INVALID_HANDLE;
}

EsElement *WindowGetMainPanel(EsWindow *window) {
	if (window->windowStyle == LT_WINDOW_MENU) {
		if (!window->children[0]->GetChildCount()) {
			EsMenuNextColumn((EsMenu *) window, LT_FLAGS_DEFAULT);
		}
...
int ProcessRootMessage(EsElement *element, EsMessage *message) {
	...
	if (window->windowStyle == LT_WINDOW_CONTAINER) {
		if (message->type == LT_MSG_LAYOUT) {
		...
		} else if (message->type == LT_MSG_UI_SCALE_CHANGED) {
			// This message is also sent when the window is created.
			EsRectangle solidInsets = LT_RECT_4(CONTAINER_SOLID_C, CONTAINER_SOLID_C, CONTAINER_SOLID_T, CONTAINER_SOLID_B);
			EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, LT_WINDOW_SOLID_TRUE, (uintptr_t) &solidInsets, LT_WINDOW_PROPERTY_SOLID);
			EsRectangle embedInsets = LT_RECT_4(CONTAINER_EMBED_C, CONTAINER_EMBED_C, CONTAINER_EMBED_T, CONTAINER_EMBED_B);
			EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, (uintptr_t) &embedInsets, 0, LT_WINDOW_PROPERTY_EMBED_INSETS);
			EsRectangle opaqueBounds = LT_RECT_4(CONTAINER_OPAQUE_C, window->windowWidth - CONTAINER_OPAQUE_C, 
					CONTAINER_OPAQUE_T, window->windowHeight - CONTAINER_OPAQUE_B);
			EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, (uintptr_t) &opaqueBounds, 0, LT_WINDOW_PROPERTY_OPAQUE_BOUNDS);
		} else {
			response = ProcessWindowBorderMessage(window, message, bounds, true);
		}
	} else if (window->windowStyle == LT_WINDOW_MENU) {
		if (message->type == LT_MSG_PAINT_BACKGROUND) {
			EsDrawClear(message->painter, message->painter->clip);
		} else if (message->type == LT_MSG_LAYOUT) {
			if (window->GetChildCount()) {
				EsElementMove(window->GetChild(0), 0, 0, bounds.r, bounds.b);
			}
		} else if (message->type == LT_MSG_TRANSITION_COMPLETE) {
			if (window->state & UI_STATE_MENU_EXITING) {
				EsElementDestroy(window);
			}
		}
	} else if (window->windowStyle == LT_WINDOW_INSPECTOR) {
		if (message->type == LT_MSG_LAYOUT) {
			if (window->GetChildCount()) {
	...
	} else if (window->windowStyle == LT_WINDOW_NORMAL) {
		if (message->type == LT_MSG_LAYOUT) {
			if (window->GetChildCount()) {
	...
	} else if (window->windowStyle == LT_WINDOW_TIP || window->windowStyle == LT_WINDOW_PLAIN) {
		if (message->type == LT_MSG_LAYOUT) {
			if (window->GetChildCount()) {
				EsElementMove(window->GetChild(0), 0, 0, bounds.r, bounds.b);
			}
	...
	if (message->type == LT_MSG_DESTROY) {
		if (window->windowStyle != LT_WINDOW_NORMAL) {
			EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, LT_FLAGS_DEFAULT, 0, LT_WINDOW_PROPERTY_SOLID);
		}

		if (window->windowStyle == LT_WINDOW_MENU) {
			EsAssert(window->state & UI_STATE_MENU_EXITING);
		}
	} else if (message->type == LT_MSG_MOUSE_LEFT_DOWN || message->type == LT_MSG_MOUSE_MIDDLE_DOWN || message->type == LT_MSG_MOUSE_RIGHT_DOWN) {
...
EsWindow *EsWindowCreate(EsInstance *instance, EsWindowStyle style) {
	...
	if (style == LT_WINDOW_NORMAL) {
		...
		window->handle = EsSyscall(LT_SYSCALL_WINDOW_CREATE, style, 0, (uintptr_t) window, 0);
	}

	window->id = EsSyscall(LT_SYSCALL_WINDOW_GET_ID, window->handle, 0, 0, 0);
	window->window = window;
	window->Initialise(nullptr, LT_CELL_FILL, ProcessRootMessage, 0);
	window->cName = "window";
	...
	if (style == LT_WINDOW_NORMAL) {
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, 0, (uintptr_t) window, LT_WINDOW_PROPERTY_OBJECT);
		window->activated = true;
		EsPanel *panel = EsPanelCreate(window, LT_ELEMENT_NON_CLIENT | LT_CELL_FILL | LT_PANEL_Z_STACK);
		panel->cName = "window stack";
		window->mainPanel = EsPanelCreate(panel, LT_CELL_FILL);
		window->mainPanel->cName = "window root";
		window->toolbarSwitcher = EsPanelCreate(window, LT_ELEMENT_NON_CLIENT | LT_PANEL_SWITCHER | LT_CELL_FILL, LT_STYLE_PANEL_TOOLBAR_ROOT);
		window->toolbarSwitcher->cName = "toolbar";
		EsElement *accessKeyLayer = EsCustomElementCreate(window, LT_ELEMENT_NON_CLIENT | LT_CELL_FILL | LT_ELEMENT_NO_HOVER, 0);
		accessKeyLayer->cName = "access key layer";
		accessKeyLayer->messageUser = AccessKeyLayerMessage;
		window->state |= UI_STATE_Z_STACK;
	} else if (style == LT_WINDOW_INSPECTOR) {
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, LT_WINDOW_SOLID_TRUE, 0, LT_WINDOW_PROPERTY_SOLID);
		window->SetStyle(LT_STYLE_PANEL_FILLED);
		window->mainPanel = EsPanelCreate(window, LT_ELEMENT_NON_CLIENT | LT_CELL_FILL);
		EsRectangle bounds = { 10, 600, 10, 500 };
		EsSyscall(LT_SYSCALL_WINDOW_MOVE, window->handle, (uintptr_t) &bounds, 0, LT_WINDOW_MOVE_ADJUST_TO_FIT_SCREEN);
	} else if (style == LT_WINDOW_TIP || style == LT_WINDOW_PLAIN) {
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, style == LT_WINDOW_PLAIN ? LT_WINDOW_SOLID_TRUE : LT_FLAGS_DEFAULT, 0, LT_WINDOW_PROPERTY_SOLID);
		window->mainPanel = EsPanelCreate(window, LT_ELEMENT_NON_CLIENT | LT_CELL_FILL, 0);
	} else if (style == LT_WINDOW_MENU) {
		window->SetStyle(LT_STYLE_MENU_ROOT);

		EsPanel *panel = EsPanelCreate(window, LT_ELEMENT_NON_CLIENT | LT_PANEL_HORIZONTAL | LT_CELL_FILL, LT_STYLE_MENU_CONTAINER);
		panel->cName = "menu";
		panel->separatorStylePart = LT_STYLE_MENU_SEPARATOR_VERTICAL;
		panel->separatorFlags = LT_CELL_V_FILL;
		...
		panel->messageUser = [] (EsElement *element, EsMessage *message) {
			if (message->type == LT_MSG_PAINT) {
				...
				EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, element->window->handle, (uintptr_t) &blurBounds, 0, LT_WINDOW_PROPERTY_BLUR_BOUNDS);
			}
		...
		EsElementStartTransition(window, LT_TRANSITION_FADE_IN);

		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, LT_WINDOW_SOLID_TRUE, (uintptr_t) &panel->style->insets, LT_WINDOW_PROPERTY_SOLID);
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, BLEND_WINDOW_MATERIAL_GLASS, 0, LT_WINDOW_PROPERTY_MATERIAL);
	}

	if (style == LT_WINDOW_INSPECTOR) {
...
void EsMenuAddSeparator(EsMenu *menu) {
	EsCustomElementCreate(menu, LT_CELL_H_FILL, LT_STYLE_MENU_SEPARATOR_HORIZONTAL)->cName = "menu separator";
}

void EsMenuNextColumn(EsMenu *menu, uint64_t flags) {
	EsPanelCreate(menu->children[0], LT_PANEL_VERTICAL | LT_CELL_V_TOP | flags, LT_STYLE_MENU_COLUMN);
}
...
EsMenu *EsMenuCreate(EsElement *source, uint32_t flags) {
	EsWindow *menu = (EsWindow *) EsWindowCreate(source->instance, LT_WINDOW_MENU);
...
void EsMenuAddCommandsFromToolbar(EsMenu *menu, EsElement *element) {
	for (uintptr_t i = 0; i < element->GetChildCount(); i++) {
		...
		if (child->flags & LT_ELEMENT_NON_CLIENT) {
			continue;
		}
...
void EsMenuClose(EsMenu *menu) {
	EsAssert(menu->windowStyle == LT_WINDOW_MENU);
	if (menu->state & UI_STATE_MENU_EXITING) return;
	EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, menu->handle, BLEND_WINDOW_MATERIAL_NONE, 0, LT_WINDOW_PROPERTY_MATERIAL);
	...
	menu->state |= UI_STATE_MENU_EXITING; // Set flag before EsElementStartTransition to receive LT_MSG_TRANSITION_COMPLETE when animations disabled.
	EsElementStartTransition(menu, LT_TRANSITION_ZOOM_OUT_LIGHT, LT_ELEMENT_TRANSITION_EXIT);
}
...
void EsMenuShow(EsMenu *menu, int fixedWidth, int fixedHeight) {
	...
	if (fixedHeight) {
		if (menu->flags & LT_MENU_MAXIMUM_HEIGHT) {
	...
	if (~menu->flags & LT_MENU_AT_CURSOR) {
		EsRectangle bounds = menu->source->GetScreenBounds(false);

		position = LT_POINT(bounds.l - (width + menuInsets.l - menuInsets.r) / 2 + menu->source->width / 2, 
	...
	EsRectangle bounds = LT_RECT_4(position.x, position.x + width, position.y, position.y + height);
	EsSyscall(LT_SYSCALL_WINDOW_MOVE, menu->handle, (uintptr_t) &bounds, 0, LT_WINDOW_MOVE_ADJUST_TO_FIT_SCREEN | LT_WINDOW_MOVE_ALWAYS_ON_TOP);
}

void EsMenuCloseAll() {
	for (uintptr_t i = 0; i < gui.allWindows.Length(); i++) {
		if (gui.allWindows[i]->windowStyle == LT_WINDOW_MENU) {
...
EsRectangle UIGetTransitionEffectRectangle(EsRectangle bounds, EsTransitionType type, double progress, bool to) {
	...
	if (type == LT_TRANSITION_FADE_IN || type == LT_TRANSITION_FADE_OUT || type == LT_TRANSITION_FADE_VIA_TRANSPARENT || type == LT_TRANSITION_FADE) {
		return bounds;
	} else if (!to) {
		if (type == LT_TRANSITION_SLIDE_UP) {
			return LT_RECT_4(bounds.l, bounds.r, 
					bounds.t - progress * height / 2, bounds.b - progress * height / 2);
		} else if (type == LT_TRANSITION_SLIDE_DOWN) {
			return LT_RECT_4(bounds.l, bounds.r, 
					bounds.t + progress * height / 2, bounds.b + progress * height / 2); 
		} else if (type == LT_TRANSITION_COVER_UP) {
			return LT_RECT_4(bounds.l, bounds.r, 
					bounds.t, bounds.b);
		} else if (type == LT_TRANSITION_COVER_DOWN) {
			return LT_RECT_4(bounds.l, bounds.r, 
					bounds.t, bounds.b);
		} else if (type == LT_TRANSITION_SQUISH_UP || type == LT_TRANSITION_REVEAL_UP) {
			return LT_RECT_4(bounds.l, bounds.r, 
					bounds.t, bounds.t + height * (1 - progress));
		} else if (type == LT_TRANSITION_SQUISH_DOWN || type == LT_TRANSITION_REVEAL_DOWN) {
			return LT_RECT_4(bounds.l, bounds.r, 
					bounds.t + height * progress, bounds.b);
		} else if (type == LT_TRANSITION_ZOOM_OUT) {
			return LT_RECT_4(bounds.l + 20 * progress, bounds.r - 20 * progress, 
					bounds.t + 20 * progress * ratio, bounds.b - 20 * progress * ratio);
		} else if (type == LT_TRANSITION_ZOOM_IN) {
			return LT_RECT_4(bounds.l - 20 * progress, bounds.r + 20 * progress, 
					bounds.t - 20 * progress * ratio, bounds.b + 20 * progress * ratio);
		} else if (type == LT_TRANSITION_ZOOM_OUT_LIGHT) {
			return LT_RECT_4(bounds.l + 5 * progress, bounds.r - 5 * progress, 
					bounds.t + 5 * progress * ratio, bounds.b - 5 * progress * ratio);
		} else if (type == LT_TRANSITION_ZOOM_IN_LIGHT) {
			return LT_RECT_4(bounds.l - 5 * progress, bounds.r + 5 * progress, 
					bounds.t - 5 * progress * ratio, bounds.b + 5 * progress * ratio);
		} else if (type == LT_TRANSITION_SLIDE_UP_OVER) {
			return LT_RECT_4(bounds.l, bounds.r, 
					bounds.t - progress * height / 4, bounds.b - progress * height / 4);
		} else if (type == LT_TRANSITION_SLIDE_DOWN_OVER) {
			return LT_RECT_4(bounds.l, bounds.r, 
					bounds.t + progress * height / 4, bounds.b + progress * height / 4); 
		} else if (type == LT_TRANSITION_SLIDE_UP_UNDER) {
			return LT_RECT_4(bounds.l, bounds.r, 
					bounds.t - progress * height / 2, bounds.b - progress * height / 2);
		} else if (type == LT_TRANSITION_SLIDE_DOWN_UNDER) {
			return LT_RECT_4(bounds.l, bounds.r, 
					bounds.t + progress * height / 2, bounds.b + progress * height / 2); 
		}
	} else {
		if (type == LT_TRANSITION_SLIDE_UP) {
			...
			return LT_RECT_4(bounds.l - 20 * (1 - progress), bounds.r + 20 * (1 - progress), 
					bounds.t - 20 * (1 - progress) * ratio, bounds.b + 20 * (1 - progress) * ratio);
		} else if (type == LT_TRANSITION_ZOOM_IN) {
			return LT_RECT_4(bounds.l + 20 * (1 - progress), bounds.r - 20 * (1 - progress) + 0.5, 
					bounds.t + 20 * (1 - progress) * ratio, bounds.b - 20 * (1 - progress) * ratio + 0.5);
		} else if (type == LT_TRANSITION_ZOOM_OUT_LIGHT) {
			return LT_RECT_4(bounds.l - 5 * (1 - progress), bounds.r + 5 * (1 - progress), 
					bounds.t - 5 * (1 - progress) * ratio, bounds.b + 5 * (1 - progress) * ratio);
		} else if (type == LT_TRANSITION_ZOOM_IN_LIGHT) {
			return LT_RECT_4(bounds.l + 5 * (1 - progress), bounds.r - 5 * (1 - progress) + 0.5, 
...
void UIDrawTransitionEffect(EsPainter *painter, EsPaintTarget *sourceSurface, EsRectangle bounds, EsTransitionType type, double progress, bool to) {
	...
	if ((type == LT_TRANSITION_FADE_OUT && to) || (type == LT_TRANSITION_FADE_IN && !to)) {
		return;
	}

	if (type == LT_TRANSITION_FADE_VIA_TRANSPARENT) {
		if (to) {
	...
	EsRectangle sourceRegion = LT_RECT_4(0, bounds.r - bounds.l, 0, bounds.b - bounds.t);
...
void EsElementStartTransition(EsElement *element, EsTransitionType transitionType, uint32_t flags, float timeMultiplier) {
	...
	if (!durationMs) {
		EsMessage m = { .type = LT_MSG_TRANSITION_COMPLETE };
		EsMessageSend(element, &m);
		return;
	}

	if (~element->state & UI_STATE_ENTERED) {
		flags |= LT_ELEMENT_TRANSITION_ENTRANCE;
	}

	if (transitionType == LT_TRANSITION_FADE_IN) {
		flags |= LT_ELEMENT_TRANSITION_ENTRANCE;
	} else if (transitionType == LT_TRANSITION_FADE_OUT) {
		flags |= LT_ELEMENT_TRANSITION_EXIT;
	}
	...
	if (~flags & LT_ELEMENT_TRANSITION_ENTRANCE) {
		...
		if (element->previousTransitionFrame) {
			EsPainter painter = {};
			painter.clip = LT_RECT_4(0, width, 0, height);
			...
			element->InternalPaint(&painter, PAINT_NO_TRANSITION | PAINT_NO_OFFSET 
					| ((flags & LT_ELEMENT_TRANSITION_CONTENT_ONLY) ? PAINT_NO_BACKGROUND : 0));
		}
	}
...
EsRectangle EsPainterBoundsClient(EsPainter *painter) {
	return LT_RECT_4(painter->offsetX, painter->offsetX + painter->width, painter->offsetY, painter->offsetY + painter->height);
}

EsRectangle EsPainterBoundsInset(EsPainter *painter) {
	UIStyle *style = (UIStyle *) painter->style;
	return LT_RECT_4(painter->offsetX + style->insets.l, painter->offsetX + painter->width - style->insets.r, 
...
void EsElement::InternalPaint(EsPainter *painter, int paintFlags) {
	if (width <= 0 || height <= 0 || (flags & LT_ELEMENT_HIDDEN)) {
		return;
	}
	...
	if (parent && parent->GetChildCount() && (parent->flags & LT_ELEMENT_AUTO_GROUP)) {
		if (parent->GetChildCount() == 1 && parent->GetChild(0) == this) {
			childType = THEME_CHILD_TYPE_ONLY;
		} else if (parent->GetChild(0) == this) {
			childType = (parent->flags & LT_ELEMENT_LAYOUT_HINT_REVERSE) ? THEME_CHILD_TYPE_LAST : THEME_CHILD_TYPE_FIRST;
		} else if (parent->GetChild(parent->GetChildCount() - 1) == this) {
			childType = (parent->flags & LT_ELEMENT_LAYOUT_HINT_REVERSE) ? THEME_CHILD_TYPE_FIRST : THEME_CHILD_TYPE_LAST;
		} else {
			childType = THEME_CHILD_TYPE_NONE;
		}

		if (parent->flags & LT_ELEMENT_LAYOUT_HINT_HORIZONTAL) {
			childType |= THEME_CHILD_TYPE_HORIZONTAL;
		}
	}

	if (paintFlags & PAINT_SHADOW) {
		interpolatedStyle->PaintLayers(painter, LT_RECT_2S(painter->width, painter->height), childType, THEME_LAYER_MODE_SHADOW);
	} else if (paintFlags & PAINT_OVERLAY) {
		interpolatedStyle->PaintLayers(painter, LT_RECT_2S(painter->width, painter->height), childType, THEME_LAYER_MODE_OVERLAY);

		// Paint layout bounds, if active.

		if (window->visualizeLayoutBounds) {
			EsDrawRectangle(painter, EsPainterBoundsClient(painter), 0, 0x7FFF0000, LT_RECT_1(1));
			EsDrawRectangle(painter, EsPainterBoundsInset(painter), 0, 0x7F0000FF, LT_RECT_1(1));
		}
		...
		if (transitionFlags & LT_ELEMENT_TRANSITION_CONTENT_ONLY) {
			EsMessage m;
			m.type = LT_MSG_PAINT_BACKGROUND;
			m.painter = painter;

			if (!EsMessageSend(this, &m)) {
				interpolatedStyle->PaintLayers(painter, LT_RECT_2S(painter->width, painter->height), childType, THEME_LAYER_MODE_BACKGROUND);
			}
		}
		...
		if (~transitionFlags & LT_ELEMENT_TRANSITION_EXIT) {
			if (EsPaintTargetTake(&target, targetWidth, targetHeight)) {
				EsPainter p = {};
				p.clip = LT_RECT_4(0, targetWidth, 0, targetHeight);
				p.offsetX = paintOutsets.l;
				p.offsetY = paintOutsets.t;
				p.target = &target;
				InternalPaint(&p, PAINT_NO_TRANSITION | PAINT_NO_OFFSET 
						| ((transitionFlags & LT_ELEMENT_TRANSITION_CONTENT_ONLY) ? PAINT_NO_BACKGROUND : 0));
		...
		if (style->metrics->clipEnabled && (~flags & LT_ELEMENT_NO_CLIP)) {
			Rectangle16 insets = style->metrics->clipInsets;
			EsRectangle content = LT_RECT_4(painter->offsetX + insets.l, painter->offsetX + width - insets.r, 
		...
		if (THEME_RECT_VALID(painter->clip)) {
			...
			if (!EsMessageSend(this, &m)) {
				...
				EsMessage zOrder;
				zOrder.type = LT_MSG_BEFORE_Z_ORDER;
				...
				if (isZStack) {
					...
					for (uintptr_t i = zOrder.beforeZOrder.start; i < zOrder.beforeZOrder.end; i++) {
						...
						child->InternalPaint(painter, LT_FLAGS_DEFAULT);
					...
					for (uintptr_t i = zOrder.beforeZOrder.start; i < zOrder.beforeZOrder.end; i++) {
						EsElement *child = GetChildByZ(i);
						if (child) child->InternalPaint(painter, LT_FLAGS_DEFAULT);
					}
				...
				zOrder.type = LT_MSG_AFTER_Z_ORDER;
				EsMessageSend(this, &zOrder);
			}
		}

		m.type = LT_MSG_PAINT_FOREGROUND;
		m.painter = painter;
		EsMessageSend(this, &m);
	...
	*painter = oldPainter;
	...
	if (window->visualizePaintSteps && LT_RECT_VALID(window->updateRegionInProgress) && painter->target->forWindowManager) {
		EsSyscall(LT_SYSCALL_WINDOW_SET_BITS, window->handle, (uintptr_t) &window->updateRegionInProgress, 
...
void ProcessAnimations() {
	...
	for (uintptr_t i = 0; i < gui.animatingElements.Length(); i++) {
		...
		if (ThemeAnimationStep(&element->animation, m.animate.deltaMs)) {
			element->Repaint(true, LT_RECT_1(0));
		}
		...
		if (element->transitionDurationMs) {
			...
			if (transitionComplete) {
				...
				if (element->transitionFlags & LT_ELEMENT_TRANSITION_HIDE_AFTER_COMPLETE) {
...
bool EsElement::RefreshStyleState() {
	...
	if (flags & LT_ELEMENT_DISABLED) {
		...
		if (((window->pressed == this && gui.lastClickButton == LT_MSG_MOUSE_LEFT_DOWN) 
...
EsRectangle LayoutCell(EsElement *element, int width, int height) {
	...
	int maximumWidth  = element->style->metrics->maximumWidth  ?: LT_PANEL_BAND_SIZE_DEFAULT;
	int minimumWidth  = element->style->metrics->minimumWidth  ?: LT_PANEL_BAND_SIZE_DEFAULT;
	int maximumHeight = element->style->metrics->maximumHeight ?: LT_PANEL_BAND_SIZE_DEFAULT;
	int minimumHeight = element->style->metrics->minimumHeight ?: LT_PANEL_BAND_SIZE_DEFAULT;

	if (layout & LT_CELL_H_EXPAND) maximumWidth = INT_MAX;
	if (layout & LT_CELL_H_SHRINK) minimumWidth = 0;
	if (layout & LT_CELL_V_EXPAND) maximumHeight = INT_MAX;
	if (layout & LT_CELL_V_SHRINK) minimumHeight = 0;

	if (maximumWidth == LT_PANEL_BAND_SIZE_DEFAULT || minimumWidth == LT_PANEL_BAND_SIZE_DEFAULT) {
		int width = element->GetWidth(height);
		if (maximumWidth == LT_PANEL_BAND_SIZE_DEFAULT) maximumWidth = width;
		if (minimumWidth == LT_PANEL_BAND_SIZE_DEFAULT) minimumWidth = width;
	}

	int preferredWidth = ClampInteger(minimumWidth,  maximumWidth,  width);

	if (maximumHeight == LT_PANEL_BAND_SIZE_DEFAULT || minimumHeight == LT_PANEL_BAND_SIZE_DEFAULT) {
		int height = element->GetHeight(preferredWidth);
		if (maximumHeight == LT_PANEL_BAND_SIZE_DEFAULT) maximumHeight = height;
		if (minimumHeight == LT_PANEL_BAND_SIZE_DEFAULT) minimumHeight = height;
	}

	int preferredHeight = ClampInteger(minimumHeight, maximumHeight, height);

	EsRectangle bounds = LT_RECT_4(0, width, 0, height);

	if ((layout & (LT_CELL_H_LEFT | LT_CELL_H_RIGHT)) == LT_CELL_H_LEFT) {
		bounds.r = bounds.l + preferredWidth;
	} else if ((layout & (LT_CELL_H_LEFT | LT_CELL_H_RIGHT)) == LT_CELL_H_RIGHT) {
	...
	if ((layout & (LT_CELL_V_TOP | LT_CELL_V_BOTTOM)) == LT_CELL_V_TOP) {
		bounds.b = bounds.t + preferredHeight;
	} else if ((layout & (LT_CELL_V_TOP | LT_CELL_V_BOTTOM)) == LT_CELL_V_BOTTOM) {
...
void LayoutTable(EsPanel *panel, EsMessage *message) {
	...
	if (message->type == LT_MSG_GET_WIDTH) {
		...
		has[1] = has[1];
	} else if (message->type == LT_MSG_GET_HEIGHT) {
	...
	for (int _axis = 0; _axis < 2; _axis++) {
		int axis = (~panel->flags & LT_PANEL_HORIZONTAL) ? (1 - _axis) : _axis;
		...
		for (uintptr_t i = 0; i < childCount; i++) {
			...
			if (child->flags & (LT_ELEMENT_HIDDEN | LT_ELEMENT_NON_CLIENT)) continue;

			// Step 1: Find the preferred size of the children for this axis.

			int size;

			if ((child->flags & (axis ? LT_CELL_V_PUSH : LT_CELL_H_PUSH)) && has[axis]) {
				size = 0;
			} else {
			...
			int maximumSizePerBand = maximumSizeValue ? (((int) maximumSizeValue - totalGapSize) / bandSpan) : LT_PANEL_BAND_SIZE_DEFAULT;
			int minimumSizePerBand = maximumSizeValue ? (((int) minimumSizeValue - totalGapSize) / bandSpan) : LT_PANEL_BAND_SIZE_DEFAULT;
			...
			for (int j = child->tableCell.from[axis]; j <= child->tableCell.to[axis]; j++) {
				...
				if (child->flags & (axis ? LT_CELL_V_PUSH : LT_CELL_H_PUSH)) {
					if (!band->push) {
						band->push = 1;
					}
				}

				if (!panel->bands[axis] || panel->bands[axis][j].preferredSize == LT_PANEL_BAND_SIZE_DEFAULT) {
					if (band->preferredSize != LT_PANEL_BAND_SIZE_DEFAULT) {
				...
				if (!panel->bands[axis] || panel->bands[axis][j].maximumSize == LT_PANEL_BAND_SIZE_DEFAULT) {
					if (maximumSizePerBand != LT_PANEL_BAND_SIZE_DEFAULT) {
						if (band->maximumSize != LT_PANEL_BAND_SIZE_DEFAULT) {
				...
				if (!panel->bands[axis] || panel->bands[axis][j].minimumSize == LT_PANEL_BAND_SIZE_DEFAULT) {
					if (minimumSizePerBand != LT_PANEL_BAND_SIZE_DEFAULT) {
						if (band->minimumSize != LT_PANEL_BAND_SIZE_DEFAULT) {
		...
		if (has[axis]) {
			...
			for (int i = 0; i < panel->bandCount[axis]; i++) {
				...
				if (band->minimumSize   == LT_PANEL_BAND_SIZE_DEFAULT) band->minimumSize   = 0;
				if (band->preferredSize == LT_PANEL_BAND_SIZE_DEFAULT) band->preferredSize = 0;
				if (band->maximumSize   == LT_PANEL_BAND_SIZE_DEFAULT) band->maximumSize   = INT_MAX;
			}
		...
		if ((axis ? (panel->flags & LT_PANEL_TABLE_V_JUSTIFY) : (panel->flags & LT_PANEL_TABLE_H_JUSTIFY))
				&& panel->bandCount[axis] > 1 && message->type == LT_MSG_LAYOUT) {
			int32_t usedSize = 0;
	...
	if (message->type == LT_MSG_GET_WIDTH) {
		...
		for (uintptr_t i = 0; i < childCount; i++) {
			...
			if (element->flags & LT_ELEMENT_NON_CLIENT) {
				continue;
			} else if (element->flags & LT_ELEMENT_HIDDEN) {
...
int LayoutStackDeterminePerPush(EsPanel *panel, int available, int secondary) {
	...
	for (uintptr_t i = 0; i < childCount; i++) {
		...
		if (panel->flags & LT_PANEL_HORIZONTAL) {
			if (child->flags & LT_CELL_H_PUSH) {
				fill++;
			} else if (available > 0) {
				available -= child->GetWidth(secondary);
			}
		} else {
			if (child->flags & LT_CELL_V_PUSH) {
				fill++;
			} else if (available > 0) {
...
void LayoutStackSecondary(EsPanel *panel, EsMessage *message) {
	bool horizontal = panel->flags & LT_PANEL_HORIZONTAL;
	size_t childCount = panel->GetChildCount();
	EsRectangle insets = panel->GetInsets();
	int size = 0;
	...
	for (uintptr_t i = 0; i < childCount; i++) {
		...
		if (horizontal) {
			int height = child->GetHeight((child->flags & LT_CELL_H_PUSH) ? perPush : 0);
			if (height > size) size = height;
		} else {
			int width = child->GetWidth((child->flags & LT_CELL_V_PUSH) ? perPush : 0);
...
void LayoutStackPrimary(EsPanel *panel, EsMessage *message) {
	bool horizontal = panel->flags & LT_PANEL_HORIZONTAL;
	bool reverse = panel->flags & LT_PANEL_REVERSE;
	EsRectangle bounds = panel->GetBounds();
	size_t childCount = panel->GetChildCount();
	...
	if (message->type != LT_MSG_LAYOUT && (panel->state & UI_STATE_INSPECTING)) {
	...
	int hBase = message->type == LT_MSG_GET_HEIGHT ? message->measure.width : Width(bounds);
	int vBase = message->type == LT_MSG_GET_WIDTH ? message->measure.height : Height(bounds);
	...
	for (uintptr_t i = 0; i < childCount; i++) {
		...
		if (horizontal) {
			int width = (child->flags & LT_CELL_H_PUSH) ? perPush : child->GetWidth(vSpace);

			if (reverse) {
				relative = LT_RECT_4(bounds.r - position - width, bounds.r - position, secondary1, secondary2);
			} else {
				relative = LT_RECT_4(position, position + width, insets.t, bounds.b - insets.b);
			}

			position += width + gap;
		} else {
			int height = (child->flags & LT_CELL_V_PUSH) ? perPush : child->GetHeight(hSpace);

			if (reverse) {
				relative = LT_RECT_4(secondary1, secondary2, bounds.b - position - height, bounds.b - position);
			} else {
				relative = LT_RECT_4(secondary1, secondary2, position, position + height);
			}
...
void LayoutStack(EsPanel *panel, EsMessage *message) {
	...
	if (message->type == LT_MSG_LAYOUT 
			|| (message->type == LT_MSG_GET_WIDTH && horizontal) 
			|| (message->type == LT_MSG_GET_HEIGHT && !horizontal)) {
...
int EsElement::GetWidth(int height) {
	...
	else if (style->preferredHeight && style->preferredHeight > height && (~flags & (LT_CELL_V_SHRINK))) height = style->preferredHeight;
	else if (style->preferredHeight && style->preferredHeight < height && (~flags & (LT_CELL_V_EXPAND))) height = style->preferredHeight;
...
int EsElement::GetHeight(int width) {
	...
	else if (style->preferredWidth && style->preferredWidth > width && (~flags & (LT_CELL_H_SHRINK))) width = style->preferredWidth;
	else if (style->preferredWidth && style->preferredWidth < width && (~flags & (LT_CELL_H_EXPAND))) width = style->preferredWidth;
...
void EsElement::InternalMove(int _width, int _height, int _offsetX, int _offsetY) {
	...
	if (!relayoutRequested && !hasSizeChanged) {
		...
		if (hasPositionChanged) {
			...
			if (parent) {
				EsRectangle paintOutsets = style->paintOutsets;
				EsRectangle rectangle = LT_RECT_4(oldOffsetX - paintOutsets.l, oldOffsetX + width + paintOutsets.r,
		...
		for (uintptr_t i = 0; i < children.Length(); i++) {
			EsAssert((children[i]->flags & LT_ELEMENT_HIDDEN) 
	...
	if (hasPositionChanged || hasSizeChanged) {
		InspectorNotifyElementMoved(this, LT_RECT_4(offsetX, offsetX + width, offsetY, offsetY + height));
	}
}
...
EsRectangle EsElementGetPreferredSize(EsElement *element) {
	...
	return LT_RECT_4(0, element->style->preferredWidth, 0, element->style->preferredHeight);
}
...
void EsElementUpdateContentSize(EsElement *element, uint32_t flags) {
	...
	if (!flags) flags = LT_ELEMENT_UPDATE_CONTENT_WIDTH | LT_ELEMENT_UPDATE_CONTENT_HEIGHT;
	...
	while (element && (~element->state & UI_STATE_IN_LAYOUT) && flags) {
		...
		if (element->style->preferredWidth || ((element->flags & LT_CELL_H_FILL) == LT_CELL_H_FILL)) {
			flags &= ~LT_ELEMENT_UPDATE_CONTENT_WIDTH;
		}

		if (element->style->preferredHeight || ((element->flags & LT_CELL_V_FILL) == LT_CELL_V_FILL)) {
			flags &= ~LT_ELEMENT_UPDATE_CONTENT_HEIGHT;
		}
...
void ScrollbarSetPosition(Scrollbar *scrollbar, double position, bool sendMovedMessage, bool smoothScroll) {
	...
	if (sendMovedMessage && scrollbar->oldPosition != (int) scrollbar->position) {
		EsMessage m = { LT_MSG_SCROLLBAR_MOVED };
...
Scrollbar *ScrollbarCreate(EsElement *parent, uint64_t flags) {
	...
	if (flags & LT_SCROLLBAR_HORIZONTAL) {
	...
	scrollbar->up = EsButtonCreate(scrollbar, LT_CELL_FILL);
	scrollbar->up->messageUser = ProcessScrollbarButtonMessage;
	scrollbar->down = EsButtonCreate(scrollbar, LT_CELL_FILL);
	scrollbar->down->messageUser = ProcessScrollbarButtonMessage;

	scrollbar->thumb->Initialise(scrollbar, LT_CELL_FILL, [] (EsElement *element, EsMessage *message) {
		Scrollbar *scrollbar = (Scrollbar *) element->parent;
		EsRectangle bounds = scrollbar->GetBounds();

		if (message->type == LT_MSG_MOUSE_LEFT_DRAG) {
			if (scrollbar->horizontal) {
		...
		} else if (message->type == LT_MSG_MOUSE_RIGHT_DOWN || message->type == LT_MSG_MOUSE_MIDDLE_DOWN) {
			// TODO.
		} else {
			return 0;
		}
	...
	if (scrollbar->horizontal) {
		scrollbar->up->SetStyle(LT_STYLE_PUSH_BUTTON_SCROLLBAR_LEFT);
		scrollbar->down->SetStyle(LT_STYLE_PUSH_BUTTON_SCROLLBAR_RIGHT);
		scrollbar->thumb->SetStyle(LT_STYLE_SCROLLBAR_THUMB_HORIZONTAL);
		scrollbar->SetStyle(LT_STYLE_SCROLLBAR_BAR_HORIZONTAL);
	} else {
		scrollbar->up->SetStyle(LT_STYLE_PUSH_BUTTON_SCROLLBAR_UP);
		scrollbar->down->SetStyle(LT_STYLE_PUSH_BUTTON_SCROLLBAR_DOWN);
		scrollbar->thumb->SetStyle(LT_STYLE_SCROLLBAR_THUMB_VERTICAL);
		scrollbar->SetStyle(LT_STYLE_SCROLLBAR_BAR_VERTICAL);
	}

	scrollbar->up->flags &= ~LT_ELEMENT_FOCUSABLE;
	scrollbar->down->flags &= ~LT_ELEMENT_FOCUSABLE;
...
void ScrollPane::Setup(EsElement *_parent, uint8_t _xMode, uint8_t _yMode, uint16_t _flags) {
	...
	if (mode[0] == LT_SCROLL_MODE_NONE) flags &= ~LT_SCROLL_X_DRAG;
	if (mode[1] == LT_SCROLL_MODE_NONE) flags &= ~LT_SCROLL_Y_DRAG;

	for (int axis = 0; axis < 2; axis++) {
		if (mode[axis] == LT_SCROLL_MODE_FIXED || mode[axis] == LT_SCROLL_MODE_AUTO) {
			uint64_t flags = LT_CELL_FILL | LT_ELEMENT_NON_CLIENT | (axis ? LT_SCROLLBAR_VERTICAL : LT_SCROLLBAR_HORIZONTAL);
			...
			bar[axis]->messageUser = [] (EsElement *element, EsMessage *message) {
				...
				if (message->type == LT_MSG_SCROLLBAR_MOVED) {
					int axis = (element->flags & LT_SCROLLBAR_HORIZONTAL) ? 0 : 1;
					EsMessage m = *message;
					m.type = axis ? LT_MSG_SCROLL_Y : LT_MSG_SCROLL_X;
	...
	if (bar[0] && bar[1]) {
		if (!pad) {
			pad = EsCustomElementCreate(parent, LT_CELL_FILL | LT_ELEMENT_NON_CLIENT, LT_STYLE_SCROLLBAR_PAD);
			...
			if (pad) {
				...
				pad->messageUser = [] (EsElement *, EsMessage *message) {
					...
					return message->type == LT_MSG_MOUSE_LEFT_DOWN 
						|| message->type == LT_MSG_MOUSE_MIDDLE_DOWN
						|| message->type == LT_MSG_MOUSE_RIGHT_DOWN ? LT_HANDLED : 0;
				};
			}
		}
	} else if (pad) {
...
int ScrollPane::ReceivedMessage(EsMessage *message) {
	if (message->type == LT_MSG_LAYOUT) {
		Refresh();
	} else if (message->type == LT_MSG_MOUSE_LEFT_DRAG || message->type == LT_MSG_MOUSE_RIGHT_DRAG || message->type == LT_MSG_MOUSE_MIDDLE_DRAG) {
		if (flags & (LT_SCROLL_X_DRAG | LT_SCROLL_Y_DRAG)) {
			parent->StartAnimating();
			dragScrolling = true;
		}
	} else if (message->type == LT_MSG_MOUSE_LEFT_UP || message->type == LT_MSG_MOUSE_RIGHT_UP || message->type == LT_MSG_MOUSE_MIDDLE_UP) {
		dragScrolling = false;
	} else if (message->type == LT_MSG_ANIMATE) {
		if (dragScrolling) {
			...
			if (deltaX && (flags & LT_SCROLL_X_DRAG)) SetX(position[0] + deltaX, true);
			if (deltaY && (flags & LT_SCROLL_Y_DRAG)) SetY(position[1] + deltaY, true);
			message->animate.complete = false;
		}
	} else if (message->type == LT_MSG_GET_HEIGHT && !message->measure.internalMeasurement) {
		...
		if (mode[0] == LT_SCROLL_MODE_FIXED) {
			horizontalScrollBarWillShow = true;
		} else if (mode[0] == LT_SCROLL_MODE_AUTO) {
			if (!width) {
		...
		return LT_HANDLED;
	} else if (message->type == LT_MSG_GET_WIDTH && !message->measure.internalMeasurement) {
		// Algorithm copied from above, in the GET_HEIGHT case.
		int height = message->measure.height;
		if (height && mode[0] == LT_SCROLL_MODE_FIXED) height -= bar[0]->style->preferredHeight;
		EsMessage m = { .type = LT_MSG_GET_WIDTH, .measure = { .height = mode[1] ? 0 : height, .internalMeasurement = true } };
		EsMessageSend(parent, &m);
		message->measure.width = m.measure.width; 
		bool verticalScrollBarWillShow = mode[1] == LT_SCROLL_MODE_FIXED;

		if (mode[1] == LT_SCROLL_MODE_AUTO && height) {
			EsMessage m = { .type = LT_MSG_GET_HEIGHT };
			EsMessageSend(parent, &m);
		...
		return LT_HANDLED;
	} else if (message->type == LT_MSG_SCROLL_WHEEL) {
		double oldPosition0 = position[0];
		double oldPosition1 = position[1];
		SetPosition(0, position[0] + 60 * message->scrollWheel.dx / LT_SCROLL_WHEEL_NOTCH, true);
		SetPosition(1, position[1] - 60 * message->scrollWheel.dy / LT_SCROLL_WHEEL_NOTCH, true);
		if (oldPosition0 != position[0] || oldPosition1 != position[1]) return LT_HANDLED;
	}
...
void ScrollPane::SetPosition(int axis, double newScroll, bool sendMovedMessage) {
	if (mode[axis] == LT_SCROLL_MODE_NONE) return;
...
bool ScrollPane::RefreshLimit(int axis, int64_t *contentSize) {
	if (mode[axis] != LT_SCROLL_MODE_NONE) {
		...
		*contentSize = axis ? m.measure.height : m.measure.width;
		...
		if (mode[axis] == LT_SCROLL_MODE_AUTO && limit[axis] > 0 && !(*internalOffset)) {
...
void ScrollPane::Refresh() {
	...
	parent->internalOffsetRight = mode[1] == LT_SCROLL_MODE_FIXED ? bar[1]->style->preferredWidth : 0;
	parent->internalOffsetBottom = mode[0] == LT_SCROLL_MODE_FIXED ? bar[0]->style->preferredHeight : 0;
	...
	if (bar[0]) {
		...
		enabled[0] = ~bar[0]->flags & LT_ELEMENT_DISABLED;
	}
	...
	if (bar[1]) {
		...
		enabled[1] = ~bar[1]->flags & LT_ELEMENT_DISABLED;
	}
...
void PanelSwitcherTransitionComplete(EsPanel *panel) {
	...
	panel->transitionType = LT_TRANSITION_NONE;
}
...
int ProcessPanelMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_LAYOUT) {
		if (panel->flags & LT_PANEL_TABLE) {
			LayoutTable(panel, message);
		} else if (panel->flags & LT_PANEL_SWITCHER) {
			EsRectangle insets = panel->GetInsets();
		...
		} else if (panel->flags & LT_PANEL_Z_STACK) {
			EsRectangle insets = panel->GetInsets();
	...
	} else if (message->type == LT_MSG_PAINT) {
	...
	} else if (message->type == LT_MSG_PAINT_CHILDREN) {
		if ((panel->flags & LT_PANEL_SWITCHER) && panel->transitionType != LT_TRANSITION_NONE) {
			...
			if (EsPaintTargetTake(&target, width, height)) {
				EsPainter painter = { .clip = LT_RECT_4(0, width, 0, height), .width = width, .height = height, .target = &target };

				// TODO 'Clip'-style transitions. LT_TRANSITION_REVEAL_UP/LT_TRANSITION_REVEAL_DOWN.

				if (panel->switchedFrom) {
					panel->switchedFrom->InternalPaint(&painter, PAINT_SHADOW);
					panel->switchedFrom->InternalPaint(&painter, LT_FLAGS_DEFAULT);
				...
				if (panel->switchedTo) {
					...
					panel->switchedTo->InternalPaint(&painter, LT_FLAGS_DEFAULT);
	...
	} else if (message->type == LT_MSG_GET_WIDTH) {
		if (!panel->measurementCache.Get(message, &panel->state)) {
			if (panel->flags & LT_PANEL_TABLE) {
				LayoutTable(panel, message);
			} else if (panel->flags & (LT_PANEL_Z_STACK | LT_PANEL_SWITCHER)) {
				int maximum = 0;

				for (uintptr_t i = 0; i < element->GetChildCount(); i++) {
					EsElement *child = element->GetChild(i);
					if (child->flags & LT_ELEMENT_NON_CLIENT) continue;
					if ((child->flags & LT_ELEMENT_HIDDEN) && (~panel->flags & LT_PANEL_SWITCHER_MEASURE_LARGEST)) continue;
	...
	} else if (message->type == LT_MSG_ENSURE_VISIBLE) {
		if (panel->scroll.enabled[0] || panel->scroll.enabled[1]) {
			...
			if (message->ensureVisible.center || !EsRectangleContainsAll(bounds, LT_RECT_4PD(offsetX, offsetY, child->width, child->height))) {
	...
	} else if (message->type == LT_MSG_PRE_ADD_CHILD) {
	...
	} else if (message->type == LT_MSG_ADD_CHILD) {
		if (panel->flags & LT_PANEL_TABLE) {
			...
			child->flags |= LT_ELEMENT_HIDDEN;
		}
	} else if (message->type == LT_MSG_SCROLL_X || message->type == LT_MSG_SCROLL_Y) {
		int delta = message->scroll.scroll - message->scroll.previous;
		int deltaX = message->type == LT_MSG_SCROLL_X ? delta : 0; 
		int deltaY = message->type == LT_MSG_SCROLL_Y ? delta : 0; 
		...
		if (panel->flags & LT_PANEL_SWITCHER) {
			panel->Repaint(true);
	...
	} else if (message->type == LT_MSG_DESTROY_CONTENTS) {
		if ((panel->flags & LT_PANEL_TABLE)) {
			panel->tableIndex = 0;
			panel->bandCount[(panel->flags & LT_PANEL_HORIZONTAL) ? 1 : 0] = 0;
		}
	} else if (message->type == LT_MSG_DESTROY) {
		if ((panel->flags & LT_PANEL_TABLE)) {
		...
		panel->movementItems.Free();
	} else if (message->type == LT_MSG_KEY_DOWN) {
		if (!(panel->flags & (LT_PANEL_TABLE | LT_PANEL_SWITCHER))
				&& panel->window->focused && panel->window->focused->parent == panel) {
			bool reverse = panel->flags & LT_PANEL_REVERSE;
			bool left = message->keyboard.scancode == LT_SCANCODE_LEFT_ARROW;
			bool right = message->keyboard.scancode == LT_SCANCODE_RIGHT_ARROW;
			bool up = message->keyboard.scancode == LT_SCANCODE_UP_ARROW;
			bool down = message->keyboard.scancode == LT_SCANCODE_DOWN_ARROW;
			...
			if ((left && !reverse) || (right && reverse)) {
				for (uintptr_t i = 0; i < panel->GetChildCount(); i++) {
					...
					if (child->flags & (LT_ELEMENT_HIDDEN | LT_ELEMENT_NON_CLIENT)) {
						continue;
					}
			...
			if (focus) {
				...
				if (panel->flags & LT_PANEL_RADIO_GROUP) {
					EsMessage m = { .type = LT_MSG_MOUSE_LEFT_CLICK };
	...
	} else if (message->type == LT_MSG_GET_INSPECTOR_INFORMATION) {
		EsBuffer *buffer = message->getContent.buffer;

		if (panel->flags & LT_PANEL_Z_STACK) {
			EsBufferFormat(buffer, "z-stack");
		} else if (panel->flags & LT_PANEL_SWITCHER) {
			EsBufferFormat(buffer, "switcher");
		} else if (panel->flags & LT_PANEL_TABLE) {
			EsBufferFormat(buffer, "table");
		} else {
			EsBufferFormat(buffer, "%z%z stack", 
					(panel->flags & LT_PANEL_REVERSE) ? "reverse " : "",
					(panel->flags & LT_PANEL_HORIZONTAL) ? "horizontal" : "vertical");
		}
	} else if (message->type == LT_MSG_BEFORE_Z_ORDER) {
		bool isStack = !(panel->flags & (LT_PANEL_TABLE | LT_PANEL_SWITCHER | LT_PANEL_Z_STACK));
		...
		if (isStack && panel->children.Length() > 100) {
			...
			while (childCount) {
				if (panel->children[childCount - 1]->flags & LT_ELEMENT_NON_CLIENT) {
					childCount--;
				} else {
					break;
				}
			...
			if (panel->flags & LT_PANEL_HORIZONTAL) {
				LT_MACRO_SEARCH(childCount, result = message->beforeZOrder.clip.l - panel->children[index]->offsetX;, position, found);
			} else {
				LT_MACRO_SEARCH(childCount, result = message->beforeZOrder.clip.t - panel->children[index]->offsetY;, position, found);
			}
...
EsPanel *EsPanelCreate(EsElement *parent, uint64_t flags, EsStyleID style) {
	...
	if (flags & LT_PANEL_Z_STACK)     panel->state |= UI_STATE_Z_STACK;
	if (flags & LT_PANEL_RADIO_GROUP) panel->state |= UI_STATE_RADIO_GROUP, panel->flags |= LT_ELEMENT_FOCUSABLE;
	if (flags & LT_PANEL_HORIZONTAL)  panel->flags |= LT_ELEMENT_LAYOUT_HINT_HORIZONTAL;
	if (flags & LT_PANEL_REVERSE)     panel->flags |= LT_ELEMENT_LAYOUT_HINT_REVERSE;

	panel->scroll.Setup(panel, 
			((flags & LT_PANEL_H_SCROLL_FIXED) ? LT_SCROLL_MODE_FIXED : (flags & LT_PANEL_H_SCROLL_AUTO) ? LT_SCROLL_MODE_AUTO : LT_SCROLL_MODE_NONE),
			((flags & LT_PANEL_V_SCROLL_FIXED) ? LT_SCROLL_MODE_FIXED : (flags & LT_PANEL_V_SCROLL_AUTO) ? LT_SCROLL_MODE_AUTO : LT_SCROLL_MODE_NONE),
			LT_FLAGS_DEFAULT);
...
void EsElementSetCellRange(EsElement *element, int xFrom, int yFrom, int xTo, int yTo) {
	...
	EsPanel *panel = (EsPanel *) element->parent;
	EsAssert(panel->messageClass == ProcessPanelMessage && panel->flags & LT_PANEL_TABLE); // Invalid parent for SetCellRange.
...
void EsPanelSetBands(EsPanel *panel, size_t columnCount, size_t rowCount, const EsPanelBand *columns, const EsPanelBand *rows) {
	EsMessageMutexCheck();
	EsAssert(panel->flags & LT_PANEL_TABLE); // Cannot set the bands layout for a non-table panel.
	EsHeapFree(panel->bands[0]);
	EsHeapFree(panel->bands[1]);
...
void EsPanelTableSetChildCells(EsPanel *panel) {
	EsMessageMutexCheck();
	EsAssert(panel->flags & LT_PANEL_TABLE);
...
void EsPanelSwitchTo(EsPanel *panel, EsElement *targetChild, EsTransitionType transitionType, uint32_t flags, float timeMultiplier) {
	...
	EsAssert(panel->flags & LT_PANEL_SWITCHER); // Cannot switch element for a non-switcher panel.
	...
	panel->destroyPreviousAfterTransitionCompletes = flags & LT_PANEL_SWITCHER_DESTROY_PREVIOUS_AFTER_TRANSITION;
	...
	if (transitionType == LT_TRANSITION_NONE || panel->switchedFrom == panel->switchedTo || !panel->transitionLengthMs) {
		PanelSwitcherTransitionComplete(panel);
	} else {
...
void EsPanelStartMovementAnimation(EsPanel *panel, float timeMultiplier) {
	...
	if (!timeMs) return;
	EsMessageMutexCheck();
	EsAssert(~panel->flags & LT_PANEL_SWITCHER); // Use EsPanelSwitchTo!
	panel->transitionTimeMs = 0;
	panel->transitionLengthMs = timeMs; 
	panel->StartAnimating();
	panel->movementItems.Free();
	...
	for (uintptr_t i = 0; i < panel->GetChildCount(); i++) {
		...
		item.oldBounds = LT_RECT_4(element->offsetX, element->offsetX + element->width, 
				element->offsetY, element->offsetY + element->height);
		item.wasHidden = element->flags & LT_ELEMENT_HIDDEN;
...
EsButton *EsPanelRadioGroupGetChecked(EsPanel *panel) {
	...
	for (uintptr_t i = 0; i < panel->GetChildCount(); i++) {
		if (LT_CHECK_CHECKED == EsButtonGetCheck((EsButton *) panel->GetChild(i))) {
...
int DialogClosingMessage(EsElement *element, EsMessage *message) {
	if (message->type == LT_MSG_TRANSITION_COMPLETE) {
...
void EsDialogClose(EsDialog *dialog) {
	...
	EsElementStartTransition(dialog->mainPanel, LT_TRANSITION_ZOOM_OUT_LIGHT, LT_ELEMENT_TRANSITION_EXIT, 1.0f);
...
EsDialog *EsDialogShow(EsWindow *window, const char *title, ptrdiff_t titleBytes, 
	...
	EsAssert(window->windowStyle == LT_WINDOW_NORMAL); // Can only show dialogs on normal windows.
	...
	EsElement *wrapper = EsPanelCreate(mainStack, LT_PANEL_VERTICAL | LT_CELL_FILL, LT_STYLE_DIALOG_WRAPPER);
	wrapper->cName = "dialog wrapper";
	dialog->mainPanel = EsPanelCreate(wrapper, LT_PANEL_VERTICAL | LT_CELL_SHRINK | LT_ELEMENT_AUTO_GROUP, LT_STYLE_DIALOG_SHADOW);
	dialog->mainPanel->cName = "dialog";
	EsElementStartTransition(dialog->mainPanel, LT_TRANSITION_ZOOM_OUT_LIGHT, LT_FLAGS_DEFAULT, 1.0f);
	window->dialogs.Add(dialog);

	EsElement *mainPanel = dialog->mainPanel;
	EsPanel *heading = EsPanelCreate(mainPanel, LT_CELL_H_FILL | LT_PANEL_HORIZONTAL, LT_STYLE_DIALOG_HEADING);

	if (iconID) {
		EsIconDisplayCreate(heading, LT_FLAGS_DEFAULT, 0, iconID);
	}

	EsTextDisplayCreate(heading, LT_CELL_H_FILL | LT_CELL_V_CENTER, LT_STYLE_TEXT_HEADING2, title, titleBytes)->cName = "dialog heading";

	dialog->contentArea = EsPanelCreate(mainPanel, LT_CELL_H_FILL | LT_PANEL_VERTICAL, LT_STYLE_DIALOG_CONTENT);
	EsTextDisplayCreate(dialog->contentArea, LT_CELL_H_FILL, LT_STYLE_TEXT_PARAGRAPH, content, contentBytes)->cName = "dialog contents";

	EsPanel *buttonArea = EsPanelCreate(mainPanel, LT_CELL_H_FILL | LT_PANEL_HORIZONTAL | LT_PANEL_REVERSE, LT_STYLE_DIALOG_BUTTON_AREA);
	dialog->buttonArea = buttonArea;

	if (flags & LT_DIALOG_ALERT_OK_BUTTON) {
		EsButton *button = EsButtonCreate(buttonArea, LT_BUTTON_DEFAULT | LT_BUTTON_CANCEL, 0, INTERFACE_STRING(CommonOK));
...
EsElement *CanvasPaneGetCanvas(EsElement *element) {
	for (uintptr_t i = 0; i < element->GetChildCount(); i++) {
		if (~element->GetChild(i)->flags & LT_ELEMENT_NON_CLIENT) {
...
int ProcessCanvasPaneMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_LAYOUT) {
		...
		if (widthFits) {
			uint64_t cellH = canvas->flags & (LT_CELL_H_LEFT | LT_CELL_H_RIGHT);
			if (cellH == LT_CELL_H_LEFT)   pane->panX = 0;
			if (cellH == LT_CELL_H_CENTER) pane->panX = width / 2 - Width(bounds) / pane->zoom / 2;
			if (cellH == LT_CELL_H_RIGHT)  pane->panX = width - Width(bounds) / pane->zoom;
		}

		if (heightFits) {
			uint64_t cellV = canvas->flags & (LT_CELL_V_TOP | LT_CELL_V_BOTTOM);
			if (cellV == LT_CELL_V_TOP)    pane->panY = 0;
			if (cellV == LT_CELL_V_CENTER) pane->panY = height / 2 - Height(bounds) / pane->zoom / 2;
			if (cellV == LT_CELL_V_BOTTOM) pane->panY = height - Height(bounds) / pane->zoom;
		}
		...
		if (element->flags & LT_CANVAS_PANE_SHOW_SHADOW) {
			UIStyle *style = GetStyle(MakeStyleKey(LT_STYLE_CANVAS_SHADOW, 0), true);
			EsRectangle shadow = LT_RECT_4PD(canvas->offsetX, canvas->offsetY, canvas->width, canvas->height);
			style->PaintLayers(message->painter, shadow, THEME_CHILD_TYPE_ONLY, LT_FLAGS_DEFAULT);
		}
	} else if (message->type == LT_MSG_MOUSE_MIDDLE_DOWN) {
		pane->lastPanPoint = EsMouseGetPosition(pane);
	} else if (message->type == LT_MSG_MOUSE_MIDDLE_DRAG) {
	...
	} else if (message->type == LT_MSG_GET_CURSOR && pane->window->dragged == pane) {
		message->cursorStyle = LT_CURSOR_HAND_DRAG;
	} else if (message->type == LT_MSG_GET_WIDTH) {
		EsElement *canvas = CanvasPaneGetCanvas(element);
...
EsCanvasPane *EsCanvasPaneCreate(EsElement *parent, uint64_t flags, EsStyleID style) {
	...
	pane->zoom = 1.0;
	pane->scroll.Setup(pane, LT_SCROLL_MODE_AUTO, LT_SCROLL_MODE_AUTO, LT_SCROLL_MANUAL);
	return pane;
}
...
int ProcessTextDisplayMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_PAINT) {
		...
		if (!display->plan || display->planWidth != textBounds.r - textBounds.l || display->planHeight != textBounds.b - textBounds.t) {
			...
			if (~display->flags & LT_TEXT_DISPLAY_PREFORMATTED) display->properties.flags |= LT_TEXT_PLAN_TRIM_SPACES;
			if (display->flags & LT_TEXT_DISPLAY_NO_FONT_SUBSTITUTION) display->properties.flags |= LT_TEXT_PLAN_NO_FONT_SUBSTITUTION;
	...
	} else if (message->type == LT_MSG_GET_WIDTH || message->type == LT_MSG_GET_HEIGHT) {
		if (!display->measurementCache.Get(message, &display->state)) {
			EsRectangle insets = EsElementGetInsets(element);

			if ((~display->style->textAlign & LT_TEXT_WRAP) && display->plan) {
				...
				if (display->plan) EsTextPlanDestroy(display->plan);
				display->properties.flags = display->style->textAlign | ((display->flags & LT_TEXT_DISPLAY_PREFORMATTED) ? 0 : LT_TEXT_PLAN_TRIM_SPACES);
				display->planWidth = message->type == LT_MSG_GET_HEIGHT && message->measure.width 
					? (message->measure.width - insets.l - insets.r) : 0;
				display->planHeight = 0;
				display->plan = EsTextPlanCreate(element, &display->properties, 
						LT_RECT_4(0, display->planWidth, 0, 0), 
...
void EsTextDisplaySetContents(EsTextDisplay *display, const char *string, ptrdiff_t stringBytes) {
	...
	if (display->flags & LT_TEXT_DISPLAY_RICH_TEXT) {
		EsHeapFree(display->contents);
...
EsTextDisplay *EsTextDisplayCreate(EsElement *parent, uint64_t flags, EsStyleID style, const char *label, ptrdiff_t labelBytes) {
	...
	display->Initialise(parent, flags, ProcessTextDisplayMessage, style ?: UIGetDefaultStyleVariant(LT_STYLE_TEXT_LABEL, parent));
...
int ProcessListDisplayMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_GET_HEIGHT) {
		int32_t height = 0;
		...
		char buffer[64];
		EsTextPlanProperties properties = {};
		properties.flags = LT_TEXT_H_RIGHT | LT_TEXT_V_TOP | LT_TEXT_PLAN_SINGLE_USE;
		EsTextRun textRun[2] = {};
		...
		uint8_t markerType = element->flags & LT_LIST_DISPLAY_MARKER_TYPE_MASK;

		EsMessage m = {};
		m.type = LT_MSG_LIST_DISPLAY_GET_MARKER;
		...
		for (uintptr_t i = 0; i < element->GetChildCount(); i++) {
			...
			textRun[0].style.figures = LT_TEXT_FIGURE_TABULAR;
		...
		display->itemCount++;
	} else if (message->type == LT_MSG_REMOVE_CHILD) {
		display->itemCount--;
	}
...
EsListDisplay *EsListDisplayCreate(EsElement *parent, uint64_t flags, EsStyleID style) {
	...
	if (!display) return nullptr;
	display->Initialise(parent, flags, ProcessListDisplayMessage, style ?: LT_STYLE_LIST_DISPLAY_DEFAULT);
...
int AnnouncementMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_ANIMATE) {
		...
		EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, 0xFF * inOut, 0, LT_WINDOW_PROPERTY_ALPHA);
		EsSyscall(LT_SYSCALL_WINDOW_MOVE, window->handle, (uintptr_t) &bounds, 0, 
				LT_WINDOW_MOVE_ADJUST_TO_FIT_SCREEN | LT_WINDOW_MOVE_ALWAYS_ON_TOP | LT_WINDOW_MOVE_UPDATE_SCREEN);
...
void EsAnnouncementShow(EsWindow *parent, uint64_t flags, int32_t x, int32_t y, const char *text, ptrdiff_t textBytes) {
	...
	EsWindow *window = EsWindowCreate(nullptr, LT_WINDOW_TIP);
	if (!window) return;
	window->messageUser = AnnouncementMessage;

	EsTextDisplay *display = EsTextDisplayCreate(window, LT_CELL_FILL, LT_STYLE_ANNOUNCEMENT, text, textBytes);
	...
	EsRectangle bounds = LT_RECT_4PD(x - width / 2 + parentBounds.l, y - height + parentBounds.t, width, height);
	EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, 0x00, 0, LT_WINDOW_PROPERTY_ALPHA);
	EsSyscall(LT_SYSCALL_WINDOW_MOVE, window->handle, (uintptr_t) &bounds, 0, LT_WINDOW_MOVE_ADJUST_TO_FIT_SCREEN | LT_WINDOW_MOVE_ALWAYS_ON_TOP);
...
int ProcessButtonMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_PAINT) {
		EsDrawContent(message->painter, element, 
			LT_RECT_2S(message->painter->width, message->painter->height), 
			button->label, button->labelBytes, button->iconID, 
			((button->flags & LT_BUTTON_DROPDOWN) ? LT_DRAW_CONTENT_MARKER_DOWN_ARROW : 0));
	} else if (message->type == LT_MSG_PAINT_ICON) {
		if (button->imageDisplay) {
			EsRectangle imageSize = LT_RECT_2S(button->imageDisplay->width, button->imageDisplay->height);
		...
		if (!button->measurementCache.Get(message, &button->state)) {
			...
			if (button->flags & LT_BUTTON_DROPDOWN) {
				int64_t width = 0;
				GetPreferredSizeFromStylePart(LT_STYLE_MARKER_DOWN_ARROW, &width, nullptr);
				contentWidth += width + button->style->gapMinor;
			}

			int minimumReportedWidth = GetConstantNumber("pushButtonMinimumReportedWidth");
			if (button->flags & LT_BUTTON_MENU_ITEM) minimumReportedWidth = GetConstantNumber("menuItemMinimumReportedWidth");
			if (!stringWidth || (button->flags & LT_BUTTON_COMPACT)) minimumReportedWidth = 0;
	...
	} else if (message->type == LT_MSG_MOUSE_LEFT_CLICK) {
		if (button->flags & LT_BUTTON_CHECKBOX) {
			button->customStyleState &= ~THEME_STATE_INDETERMINATE;
			button->customStyleState ^= THEME_STATE_CHECKED;
		} else if (button->flags & LT_BUTTON_RADIOBOX) {
			button->customStyleState |= THEME_STATE_CHECKED;

			EsMessage m = { LT_MSG_RADIO_GROUP_UPDATED };
		...
		if (button->flags & LT_BUTTON_MENU_ITEM) {
			EsAssert(button->window->windowStyle == LT_WINDOW_MENU);
			EsMenuClose((EsMenu *) button->window);
		} else {
			button->MaybeRefreshStyle();
		}
	} else if (message->type == LT_MSG_GET_ACCESS_KEY_HINT_BOUNDS && (button->flags & (LT_BUTTON_RADIOBOX | LT_BUTTON_CHECKBOX))) {
		...
		*message->accessKeyHintBounds = LT_RECT_4(x - width / 2, x + width / 2, y - height / 4, y + 3 * height / 4);
	} else if (message->type == LT_MSG_RADIO_GROUP_UPDATED && (button->flags & LT_BUTTON_RADIOBOX)) {
		EsButtonSetCheck(button, LT_CHECK_UNCHECKED);
	} else if (message->type == LT_MSG_FOCUSED_START) {
		if (button->window->defaultEnterButton && (button->flags & LT_BUTTON_PUSH)) {
	...
	} else if (message->type == LT_MSG_FOCUSED_END) {
...
EsButton *EsButtonCreate(EsElement *parent, uint64_t flags, EsStyleID style, const char *label, ptrdiff_t labelBytes) {
	...
	if (!style) {
		if (flags & LT_BUTTON_MENU_ITEM) {
			if (flags & LT_MENU_ITEM_HEADER) {
				style = LT_STYLE_MENU_ITEM_HEADER;
			} else {
				style = LT_STYLE_MENU_ITEM_NORMAL;
			}
		} else if (flags & LT_BUTTON_TOOLBAR) {
			style = LT_STYLE_PUSH_BUTTON_TOOLBAR;
		} else if (flags & LT_BUTTON_CHECKBOX) {
			style = LT_STYLE_CHECKBOX_NORMAL;
		} else if (flags & LT_BUTTON_RADIOBOX) {
			style = LT_STYLE_CHECKBOX_RADIOBOX;
		} else if (flags & LT_BUTTON_DEFAULT) {
			style = LT_STYLE_PUSH_BUTTON_NORMAL;
		} else {
			style = LT_STYLE_PUSH_BUTTON_NORMAL;
		}
	...
	if (style == LT_STYLE_PUSH_BUTTON_NORMAL || style == LT_STYLE_PUSH_BUTTON_DANGEROUS) {
		flags |= LT_BUTTON_PUSH;
	} else if (style == LT_STYLE_PUSH_BUTTON_TOOLBAR
			|| style == LT_STYLE_PUSH_BUTTON_TOOLBAR_BIG || style == LT_STYLE_PUSH_BUTTON_STATUS_BAR) {
		flags |= LT_BUTTON_COMPACT | LT_BUTTON_NOT_FOCUSABLE;
	} else if (style == LT_STYLE_CHECKBOX_NORMAL || style == LT_STYLE_CHECKBOX_RADIOBOX) {
		flags |= LT_BUTTON_COMPACT;
	}

	if (~flags & LT_BUTTON_NOT_FOCUSABLE) {
		flags |= LT_ELEMENT_FOCUSABLE;
	}

	button->Initialise(parent, flags, ProcessButtonMessage, style);
	button->cName = "button";

	if (flags & LT_BUTTON_DEFAULT) {
	...
	if ((flags & LT_BUTTON_MENU_ITEM) && (flags & LT_MENU_ITEM_HEADER)) {
...
EsCheckState EsButtonGetCheck(EsButton *button) {
	...
	if (button->customStyleState & THEME_STATE_CHECKED)       return LT_CHECK_CHECKED;
	if (button->customStyleState & THEME_STATE_INDETERMINATE) return LT_CHECK_INDETERMINATE;
	return LT_CHECK_UNCHECKED;
}
...
void EsButtonSetCheck(EsButton *button, EsCheckState checkState, bool sendUpdatedMessage) {
	...
	if (checkState == LT_CHECK_CHECKED)       button->customStyleState |= THEME_STATE_CHECKED;
	if (checkState == LT_CHECK_INDETERMINATE) button->customStyleState |= THEME_STATE_INDETERMINATE;

	if (sendUpdatedMessage) {
		EsMessage m = { LT_MSG_CHECK_UPDATED };
...
int ProcessMenuItemMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_PAINT) {
		...
		EsDrawContent(message->painter, element, 
			LT_RECT_2S(message->painter->width, message->painter->height), 
			button->label, button->labelBytes, 0, LT_FLAGS_DEFAULT);
		...
		EsDrawContent(message->painter, element, 
			LT_RECT_2S(message->painter->width, message->painter->height), 
			(const char *) _buffer, buffer.position, 0, LT_TEXT_H_RIGHT);
	} else if (message->type == LT_MSG_GET_WIDTH) {
		uint8_t _buffer[64];
		EsBuffer buffer = { .out = _buffer, .bytes = sizeof(_buffer) };
		MenuItemGetKeyboardShortcutString(button->command, &buffer);
		...
		if (~element->flags & LT_ELEMENT_DISABLED) {
...
MenuItem *MenuItemCreate(EsMenu *menu, uint64_t flags, const char *label, ptrdiff_t labelBytes) {
	...
	EsStyleID style = (flags & LT_MENU_ITEM_HEADER) ? LT_STYLE_MENU_ITEM_HEADER : LT_STYLE_MENU_ITEM_NORMAL;
	if (flags & LT_MENU_ITEM_HEADER) flags |= LT_ELEMENT_DISABLED;
	button->Initialise(menu, flags | LT_CELL_H_FILL, ProcessMenuItemMessage, style);
...
struct ColorPicker {
	...
	void Sync(EsElement *excluding) {
		...
		if (excluding != host.well && host.well) {
			...
			EsMessage m = { LT_MSG_COLOR_CHANGED };
...
void ColorPickerCreate(EsElement *parent, ColorPickerHost host, uint32_t initialColor, bool showTextbox) {
	...
	picker->panel = EsPanelCreate(parent, LT_PANEL_HORIZONTAL | LT_PANEL_TABLE, LT_STYLE_COLOR_PICKER_MAIN_PANEL);
	...
	picker->circle = EsCustomElementCreate(picker->panel, LT_ELEMENT_FOCUSABLE | LT_ELEMENT_NOT_TAB_TRAVERSABLE, LT_STYLE_COLOR_CIRCLE);
	...
	picker->circle->messageUser = [] (EsElement *element, EsMessage *message) {
		...
		if (message->type == LT_MSG_PAINT) {
		...
		} else if (message->type == LT_MSG_HIT_TEST) {
			...
			if ((message->keyboard.scancode == LT_SCANCODE_LEFT_SHIFT || message->keyboard.scancode == LT_SCANCODE_RIGHT_SHIFT) && !message->keyboard.repeat) {
	...
	picker->circlePoint = EsCustomElementCreate(picker->circle, LT_ELEMENT_NO_HOVER, LT_STYLE_COLOR_CHOSEN_POINT);
	...
	picker->slider = EsCustomElementCreate(picker->panel, LT_ELEMENT_FOCUSABLE | LT_ELEMENT_NOT_TAB_TRAVERSABLE, LT_STYLE_COLOR_SLIDER);
	...
	picker->sliderPoint = EsCustomElementCreate(picker->slider, LT_ELEMENT_NO_HOVER, LT_STYLE_COLOR_CHOSEN_POINT);
	...
	if (hasOpacity) {
		picker->opacitySlider = EsCustomElementCreate(picker->panel, LT_ELEMENT_FOCUSABLE | LT_ELEMENT_NOT_TAB_TRAVERSABLE, LT_STYLE_COLOR_SLIDER);
		...
		picker->opacitySliderPoint = EsCustomElementCreate(picker->opacitySlider, LT_ELEMENT_NO_HOVER, LT_STYLE_COLOR_CHOSEN_POINT);
	...
	if (showTextbox) {
		picker->textbox = EsTextboxCreate(picker->panel, LT_TEXTBOX_EDIT_BASED | LT_CELL_EXPAND | LT_TEXTBOX_NO_SMART_CONTEXT_MENUS, LT_STYLE_COLOR_HEX_TEXTBOX);
		...
		picker->textbox->messageUser = [] (EsElement *element, EsMessage *message) {
			...
			if (message->type == LT_MSG_TEXTBOX_UPDATED) {
				...
				EsHeapFree(string);
			} else if (message->type == LT_MSG_TEXTBOX_EDIT_START) {
				EsTextboxSetSelection((EsTextbox *) element, 0, 1, 0, -1);
			} else if (message->type == LT_MSG_TEXTBOX_EDIT_END) {
				picker->Sync(nullptr);
		...
		EsButton *eyedropperButton = EsButtonCreate(picker->panel, LT_CELL_EXPAND, 0);
		eyedropperButton->userData = picker;

		eyedropperButton->messageUser = [] (EsElement *element, EsMessage *message) {
			ColorPicker *picker = (ColorPicker *) element->userData.p;

			if (message->type == LT_MSG_MOUSE_LEFT_CLICK) {
				picker->indeterminateBeforeEyedrop = picker->host.indeterminate && *picker->host.indeterminate;
				EsSyscall(LT_SYSCALL_EYEDROP_START, (uintptr_t) element, picker->circle->window->handle, picker->color, 0);
			} else if (message->type == LT_MSG_EYEDROP_REPORT) {
		...
		EsButtonSetIcon(eyedropperButton, LT_ICON_COLOR_SELECT_SYMBOLIC);
...
uint32_t EsColorWellGetRGB(EsColorWell *well) {
	...
	return well->color & ((well->flags & LT_COLOR_WELL_HAS_OPACITY) ? 0xFFFFFFFF : 0x00FFFFFF);
}
...
int ProcessColorWellMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_PAINT) {
		...
		if (well->indeterminate) {
			...
			if (~well->flags & LT_COLOR_WELL_HAS_OPACITY) box.backgroundColor |= 0xFF000000;
			box.borderColor = EsColorBlend(well->color | 0xFF000000, 0x40000000, false);

			if ((well->flags & LT_COLOR_WELL_HAS_OPACITY) && ((well->color & 0xFF000000) != 0xFF000000)) {
				box.fragmentShader = [] (int x, int y, StyledBox *box) -> uint32_t {
		...
		EsMenu *menu = EsMenuCreate(well, LT_FLAGS_DEFAULT);
		if (!menu) return LT_HANDLED;
		ColorPickerHost host = { well, &well->indeterminate, (well->flags & LT_COLOR_WELL_HAS_OPACITY) ? true : false };
...
EsColorWell *EsColorWellCreate(EsElement *parent, uint64_t flags, uint32_t initialColor) {
	...
	well->Initialise(parent, flags | LT_ELEMENT_FOCUSABLE, ProcessColorWellMessage, LT_STYLE_PUSH_BUTTON_NORMAL_COLOR_WELL);
		...
		if (minimumPosition < maximumPosition) {
			...
			if (newPosition < minimumPosition) {
				if (newPosition > minimumPosition2 
						...
						&& panelBefore->flags & LT_CELL_COLLAPSABLE) {
				...
				if (newPosition < maximumPosition2 
						...
						&& (!fromKeyboard || newPosition > position)
						&& panelAfter->flags & LT_CELL_COLLAPSABLE) {
...
int ProcessSplitBarMessage(EsElement *element, EsMessage *message) {
	...
	} else if (message->type == LT_MSG_KEY_TYPED) {
		if (message->keyboard.scancode == (splitter->horizontal ? LT_SCANCODE_LEFT_ARROW : LT_SCANCODE_UP_ARROW)) {
			bar->Move(bar->position - GetConstantNumber("splitBarKeyboardMovementAmount"), true);
		} else if (message->keyboard.scancode == (splitter->horizontal ? LT_SCANCODE_RIGHT_ARROW : LT_SCANCODE_DOWN_ARROW)) {
			bar->Move(bar->position + GetConstantNumber("splitBarKeyboardMovementAmount"), true);
		} else {
			return 0;
		}
	} else if (message->type == LT_MSG_GET_ACCESS_KEY_HINT_BOUNDS) {
...
int ProcessSplitterMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_LAYOUT && splitter->GetChildCount()) {
		...
		uint64_t pushFlag = splitter->horizontal ? LT_CELL_H_PUSH : LT_CELL_V_PUSH;
	...
	} else if ((message->type == LT_MSG_GET_WIDTH && splitter->horizontal)
			|| (message->type == LT_MSG_GET_HEIGHT && !splitter->horizontal)) {
		int size = 0;
	...
	} else if (message->type == LT_MSG_PRE_ADD_CHILD && !splitter->addingSplitBar && splitter->GetChildCount()) {
		splitter->addingSplitBar = true;
		SplitBar *bar = (SplitBar *) EsHeapAllocate(sizeof(SplitBar), true);

		bar->Initialise(splitter, LT_ELEMENT_FOCUSABLE | LT_ELEMENT_NOT_TAB_TRAVERSABLE | LT_CELL_EXPAND, 
				ProcessSplitBarMessage, splitter->horizontal ? LT_STYLE_SPLIT_BAR_VERTICAL : LT_STYLE_SPLIT_BAR_HORIZONTAL);

		bar->cName = "split bar";
		bar->accessKey = 'Q';

		splitter->addingSplitBar = false;
	} else if (message->type == LT_MSG_REMOVE_CHILD && ((EsElement *) message->child)->messageClass != ProcessSplitBarMessage) {
		for (uintptr_t i = 0; i < splitter->GetChildCount(); i++) {
...
EsSplitter *EsSplitterCreate(EsElement *parent, uint64_t flags, EsStyleID style) {
	...
	splitter->horizontal = flags & LT_SPLITTER_HORIZONTAL;
	splitter->Initialise(parent, flags | LT_ELEMENT_NO_CLIP, ProcessSplitterMessage, 
			style ?: LT_STYLE_PANEL_WINDOW_BACKGROUND);
...
void EsImageDisplayPaint(EsImageDisplay *display, EsPainter *painter, EsRectangle bounds) {
	...
	source.fullAlpha = ~display->flags & LT_IMAGE_DISPLAY_FULLY_OPAQUE;
	EsDrawPaintTarget(painter, &source, bounds, LT_RECT_4(0, display->width, 0, display->height), 0xFF);
}
...
int ProcessImageDisplayMessage(EsElement *element, EsMessage *message) {
	...
	} else if (message->type == LT_MSG_NOT_VISIBLE) {
		EsHeapFree(display->bits);
...
void EsImageDisplayLoadFromMemory(EsImageDisplay *display, const void *buffer, size_t bufferBytes) {
	if (display->flags & LT_IMAGE_DISPLAY_DECODE_WHEN_NEEDED) {
		EsHeapFree(display->source);
		EsHeapFree(display->bits);
		display->bits = nullptr;
		...
		if (~display->flags & LT_IMAGE_DISPLAY_MANUAL_SIZE) {
...
int ProcessIconDisplayMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_PAINT) {
		EsDrawContent(message->painter, element, LT_RECT_2S(message->painter->width, message->painter->height), "", 0, display->iconID);
	}
...
EsIconDisplay *EsIconDisplayCreate(EsElement *parent, uint64_t flags, EsStyleID style, uint32_t iconID) {
	...
	if (!display) return nullptr;
	display->Initialise(parent, flags, ProcessIconDisplayMessage, style ?: LT_STYLE_ICON_DISPLAY);
	display->cName = "icon";
...
int ProcessSliderPointMessage(EsElement *element, EsMessage *message) {
	...
	} else if (message->type == LT_MSG_MOUSE_LEFT_UP && slider->inDrag) {
		slider->inDrag = false;
...
int ProcessSliderMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_LAYOUT) {
		...
		slider->point->customStyleState &= ~THEME_STATE_FOCUSED_ITEM;
		slider->point->MaybeRefreshStyle();
	} else if (message->type == LT_MSG_KEY_TYPED && message->keyboard.scancode == LT_SCANCODE_LEFT_ARROW) {
		EsSliderSetValue(slider, slider->value - (slider->steps ? 1.0 / slider->steps : 0.02));
	} else if (message->type == LT_MSG_KEY_TYPED && message->keyboard.scancode == LT_SCANCODE_RIGHT_ARROW) {
		EsSliderSetValue(slider, slider->value + (slider->steps ? 1.0 / slider->steps : 0.02));
	} else if (message->type == LT_MSG_KEY_TYPED && message->keyboard.scancode == LT_SCANCODE_HOME) {
		EsSliderSetValue(slider, 0.0);
	} else if (message->type == LT_MSG_KEY_TYPED && message->keyboard.scancode == LT_SCANCODE_END) {
...
void EsSliderSetValue(EsSlider *slider, double newValue, bool sendUpdatedMessage) {
	...
	if (sendUpdatedMessage) {
		EsMessage m = { LT_MSG_SLIDER_MOVED, .sliderMoved = { .value = newValue, .previous = previous, .inDrag = slider->inDrag } };
...
EsSlider *EsSliderCreate(EsElement *parent, uint64_t flags, EsStyleID style, double value, uint32_t steps) {
	...
	slider->Initialise(parent, flags | LT_ELEMENT_FOCUSABLE, ProcessSliderMessage, style ?: LT_STYLE_SLIDER_TRACK);
	slider->cName = "slider";
	slider->point = EsCustomElementCreate(slider, LT_FLAGS_DEFAULT, LT_STYLE_SLIDER_POINT);
...
const EsStyle styleFileMenuDocumentInformationPanel1 = {
	.metrics = {
		.mask = LT_THEME_METRICS_INSETS | LT_THEME_METRICS_GAP_MAJOR | LT_THEME_METRICS_PREFERRED_WIDTH,
		.insets = LT_RECT_4(10, 10, 5, 5),
		.preferredWidth = 230,
		.gapMajor = 5,
	},
};
...
int FileMenuNameTextboxMessage(EsElement *element, EsMessage *message) {
	if (message->type == LT_MSG_TEXTBOX_EDIT_END) {
		...
		if (!message->endEdit.rejected && !message->endEdit.unchanged) {
			...
			EsPanelSwitchTo(instance->fileMenuNameSwitcher, instance->fileMenuNamePanel, LT_TRANSITION_SLIDE_DOWN);
		}
...
void FileMenuRename(EsInstance *_instance, EsElement *, EsCommand *) {
	...
	EsPanelSwitchTo(instance->fileMenuNameSwitcher, instance->fileMenuNameTextbox, LT_TRANSITION_SLIDE_UP);
...
void EsFileMenuCreate(EsInstance *_instance, EsElement *element, uint32_t menuFlags) {
	...
	APIInstance *instance = (APIInstance *) _instance->_private;
	EsAssert(instance->instanceClass == LT_INSTANCE_CLASS_EDITOR);
	EsInstanceClassEditorSettings *editorSettings = &instance->editorSettings;
	...
	if (!menu) return;
	EsPanel *panel1 = EsPanelCreate(menu, LT_PANEL_HORIZONTAL | LT_CELL_H_LEFT, EsStyleIntern(&styleFileMenuDocumentInformationPanel1));
	if (!panel1) goto show;

	{
		...
		EsIconDisplayCreate(panel1, LT_CELL_V_TOP, 0, editorSettings->documentIconID);
		EsSpacerCreate(panel1, LT_FLAGS_DEFAULT, 0, 5, 0);

		EsPanel *panel2 = EsPanelCreate(panel1, LT_CELL_H_FILL, EsStyleIntern(&styleFileMenuDocumentInformationPanel2));
		if (!panel2) goto show;
		EsPanel *switcher = EsPanelCreate(panel2, LT_CELL_H_FILL | LT_PANEL_SWITCHER | LT_PANEL_SWITCHER_MEASURE_LARGEST);
		if (!switcher) goto show;
		EsPanel *panel3 = EsPanelCreate(switcher, LT_PANEL_HORIZONTAL | LT_CELL_H_FILL, EsStyleIntern(&styleFileMenuDocumentInformationPanel2));
		if (!panel3) goto show;

		instance->fileMenuNameTextbox = EsTextboxCreate(switcher, LT_CELL_H_FILL | LT_TEXTBOX_EDIT_BASED, EsStyleIntern(&styleFileMenuNameTextbox));

		instance->fileMenuNameSwitcher = switcher;
		instance->fileMenuNamePanel = panel3;
		EsPanelSwitchTo(instance->fileMenuNameSwitcher, instance->fileMenuNamePanel, LT_TRANSITION_NONE);
		
		if (newDocument) {
			EsTextDisplayCreate(panel3, LT_CELL_H_FILL, LT_STYLE_TEXT_LABEL, 
			...
			EsTextDisplayCreate(panel3, LT_CELL_H_FILL, LT_STYLE_TEXT_LABEL, name, nameBytes);
		}

		EsButton *renameButton = EsButtonCreate(panel3, LT_BUTTON_TOOLBAR);
		if (!renameButton) goto show;
		EsButtonSetIcon(renameButton, LT_ICON_DOCUMENT_EDIT_SYMBOLIC);
		EsButtonOnCommand(renameButton, FileMenuRename);

		if (!newDocument) {
			EsPanel *panel4 = EsPanelCreate(panel2, LT_PANEL_TABLE | LT_PANEL_HORIZONTAL | LT_CELL_H_LEFT, EsStyleIntern(&styleFileMenuDocumentInformationPanel2));
			if (!panel4) goto show;
			EsPanelSetBands(panel4, 2 /* columns */);

			EsTextDisplayCreate(panel4, LT_CELL_H_RIGHT, LT_STYLE_TEXT_LABEL_SECONDARY, INTERFACE_STRING(CommonFileMenuFileLocation));
			EsTextDisplayCreate(panel4, LT_CELL_H_LEFT, LT_STYLE_TEXT_LABEL, instance->startupInformation->containingFolder, 
			...
			EsTextDisplayCreate(panel4, LT_CELL_H_RIGHT, LT_STYLE_TEXT_LABEL_SECONDARY, INTERFACE_STRING(CommonFileMenuFileSize));
			EsTextDisplayCreate(panel4, LT_CELL_H_LEFT, LT_STYLE_TEXT_LABEL, buffer, bytes);

			// TODO Modification date, author, etc.
		}
	}

	if (instance->instanceClass == LT_INSTANCE_CLASS_EDITOR) {
		EsMenuAddSeparator(menu);

		if (!instance->commandSave.enabled && !newDocument) {
			EsMenuAddItem(menu, LT_ELEMENT_DISABLED, INTERFACE_STRING(CommonFileUnchanged));
		} else {
			EsMenuAddCommand(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(CommonFileSave), &instance->commandSave);
		}

		// EsMenuAddItem(menu, newDocument ? LT_ELEMENT_DISABLED : LT_FLAGS_DEFAULT, INTERFACE_STRING(CommonFileMakeCopy)); // TODO.
		// EsMenuAddSeparator(menu);

		// EsMenuAddItem(menu, newDocument ? LT_ELEMENT_DISABLED : LT_FLAGS_DEFAULT, INTERFACE_STRING(CommonFileShare)); // TODO.
		// EsMenuAddItem(menu, newDocument ? LT_ELEMENT_DISABLED : LT_FLAGS_DEFAULT, INTERFACE_STRING(CommonFileVersionHistory)); // TODO.
		EsMenuAddCommand(menu, newDocument ? LT_ELEMENT_DISABLED : LT_FLAGS_DEFAULT, INTERFACE_STRING(CommonFileShowInFileManager), &instance->commandShowInFileManager);
	}
...
void EsFileMenuAddToToolbar(EsElement *element, const EsFileMenuSettings *settings) {
	EsPanel *buttonGroup = EsPanelCreate(element, LT_PANEL_HORIZONTAL);
	EsButton *button = EsButtonCreate(buttonGroup, LT_BUTTON_DROPDOWN, 0, INTERFACE_STRING(CommonFileMenu));
	if (!button) return;
	button->accessKey = 'F';
...
void EsElement::Destroy(bool manual) {
	...
	if (parent) {
		EsMessage m = { LT_MSG_REMOVE_CHILD };
		m.child = this;
		EsMessageSend(parent, &m);
	}
	...
	if (window->hovered == this) {
		window->hovered = window;
		EsMessage m = {};
		m.type = LT_MSG_HOVERED_START;
		EsMessageSend(window, &m);
	}

	EsMessage m = { LT_MSG_DESTROY };
...
bool EsElement::InternalDestroy() {
	...
	if (flags & LT_ELEMENT_FREE_USER_DATA) {
	...
	ThemeAnimationDestroy(&animation);
	if (window == this) UIWindowDestroy(window); // Windows are deallocated after receiving LT_MSG_WINDOW_DESTROYED.
...
EsElement *EsWindowGetToolbar(EsWindow *window, bool createNew) {
	if (createNew || !window->toolbar) {
		...
		window->toolbar = EsPanelCreate(window->toolbarSwitcher, LT_PANEL_HORIZONTAL | LT_CELL_FILL, LT_STYLE_PANEL_TOOLBAR);
		...
		window->toolbar->messageClass = [] (EsElement *element, EsMessage *message) {
			if (message->type == LT_MSG_GET_CHILD_STYLE_VARIANT) {
				if (message->childStyleVariant == LT_STYLE_TEXT_LABEL) {
					message->childStyleVariant = LT_STYLE_TEXT_TOOLBAR;
					return LT_HANDLED;
				} else if (message->childStyleVariant == LT_STYLE_PUSH_BUTTON_NORMAL
						|| message->childStyleVariant == LT_STYLE_CHECKBOX_NORMAL
						|| message->childStyleVariant == LT_STYLE_CHECKBOX_RADIOBOX) {
					message->childStyleVariant = LT_STYLE_PUSH_BUTTON_TOOLBAR;
					return LT_HANDLED;
				}
			}
		...
		if (first) EsPanelSwitchTo(window->toolbarSwitcher, window->toolbar, LT_TRANSITION_NONE);
	}
...
EsRectangle EsWindowGetBounds(EsWindow *window) {
	EsRectangle bounds;
	EsSyscall(LT_SYSCALL_WINDOW_GET_BOUNDS, window->handle, (uintptr_t) &bounds, 0, 0);
	return bounds;
}
...
EsRectangle EsElementGetInsetBounds(EsElement *element) {
	EsMessageMutexCheck();
	EsRectangle insets = element->style->insets;
	return LT_RECT_4(insets.l, element->width - insets.r, 
...
EsRectangle EsElementGetInsetSize(EsElement *element) {
	...
	EsRectangle insets = element->style->insets;
	return LT_RECT_4(0, element->width - insets.l - insets.r, 
...
EsError EsMouseSetPosition(EsWindow *relativeWindow, int x, int y) {
	...
	return EsSyscall(LT_SYSCALL_CURSOR_POSITION_SET, x, y, 0, 0);
}
...
EsPoint EsMouseGetPosition(EsElement *relativeElement) {
	if (relativeElement) {
		...
		EsPoint position;
		EsSyscall(LT_SYSCALL_CURSOR_POSITION_GET, (uintptr_t) &position, 0, 0, 0);
		return position;
	}
}

EsStyleID UIGetDefaultStyleVariant(EsStyleID style, EsElement *parent) {
	EsMessage m = { .type = LT_MSG_GET_CHILD_STYLE_VARIANT, .childStyleVariant = style };
	EsElement *ancestor = parent;

	while (ancestor) {
		if (LT_HANDLED == EsMessageSend(ancestor, &m)) {
			break; 
		}
...
EsElement *UIFindHoverElementRecursively2(EsElement *element, int offsetX, int offsetY, EsPoint position, uintptr_t i) {
	...
	if ((response != LT_HANDLED || m.hitTest.inside) && response != LT_REJECTED) {
...
EsElement *UIFindHoverElementRecursively(EsElement *element, int offsetX, int offsetY, EsPoint position) {
	...
	EsMessage zOrder = { LT_MSG_BEFORE_Z_ORDER };
	zOrder.beforeZOrder.nonClient = zOrder.beforeZOrder.end = element->children.Length();
	zOrder.beforeZOrder.clip = Translate(LT_RECT_4(0, element->width, 0, element->height), -offsetX, -offsetY);
	EsMessageSend(element, &zOrder);

	EsElement *result = nullptr;

	if (~element->flags & LT_ELEMENT_NO_HOVER_DESCENDENTS) {
	...
	return result ? result : (element->flags & LT_ELEMENT_NO_HOVER) ? nullptr : element;
}
...
void UISendEnsureVisibleMessage(EsElement *element, EsGeneric) {
	...
	while (e->parent) {
		EsMessage m = { LT_MSG_ENSURE_VISIBLE };
		m.ensureVisible.descendent = child;
		m.ensureVisible.center = center;
		e = e->parent;

		if (LT_HANDLED == EsMessageSend(e, &m)) {
			child = e;
		}
	}
...
void EsElementFocus(EsElement *element, uint32_t flags) {
	...
	if (!element->IsFocusable() || (window->windowStyle == LT_WINDOW_CONTAINER)) return;
	...
	if ((flags & LT_ELEMENT_FOCUS_ONLY_IF_NO_FOCUSED_ELEMENT) && window->focused) return;
	...
	if (oldFocus) {
		...
		while (parent) {
			if (~parent->state & UI_STATE_TEMP) {
				...
				m.type = LT_MSG_FOCUS_WITHIN_END;
				EsMessageSend(parent, &m);
			}
	...
	while (parent) {
		if (~parent->state & UI_STATE_FOCUS_WITHIN) {
			parent->state |= UI_STATE_FOCUS_WITHIN;
			m.type = LT_MSG_FOCUS_WITHIN_START;
			EsMessageSend(parent, &m);
	...
	if ((flags & LT_ELEMENT_FOCUS_ENSURE_VISIBLE) && element) {
...
EsRectangle EsElementGetWindowBounds(EsElement *element, bool client) {
	...
	return LT_RECT_4(x, x + cw, y, y + ch);
}
...
void EsElement::Initialise(EsElement *_parent, uint64_t _flags, EsElementCallback _classCallback, EsStyleID _style) {
	...
	if (_parent) {
		if (!_parent->parent && (~_flags & LT_ELEMENT_NON_CLIENT)) {
		...
		if (parent->flags & LT_ELEMENT_DISABLED) flags |= LT_ELEMENT_DISABLED;

		if (~flags & LT_ELEMENT_NON_CLIENT) {
			EsMessage m = {};
			m.type = LT_MSG_PRE_ADD_CHILD;
			m.child = this;
			EsMessageSend(parent, &m);
		}
		...
		if (gui.insertAfter == _parent) {
			...
			gui.insertAfter = nullptr;
		} else if (flags & LT_ELEMENT_NON_CLIENT) {
			parent->children.Add(this);
		} else {
			for (uintptr_t i = parent->children.Length();; i--) {
				if (i == 0 || (~parent->children[i - 1]->flags & LT_ELEMENT_NON_CLIENT)) {
...
#define RECTANGLE_8_TO_LT_RECTANGLE(x) { (int32_t) (x).l, (int32_t) (x).r, (int32_t) (x).t, (int32_t) (x).b }
	m.insets = RECTANGLE_8_TO_LT_RECTANGLE(metrics->insets);
	m.clipInsets = RECTANGLE_8_TO_LT_RECTANGLE(metrics->clipInsets);
...
void EsElementSetHidden(EsElement *element, bool hidden) {
	...
	bool old = element->flags & LT_ELEMENT_HIDDEN;
	if (old == hidden) return;

	if (hidden) {
		element->flags |= LT_ELEMENT_HIDDEN;
	} else {
		element->flags &= ~LT_ELEMENT_HIDDEN;
	}
...
bool EsElementIsHidden(EsElement *element) {
	EsMessageMutexCheck();
	return element->flags & LT_ELEMENT_HIDDEN;
}
...
void EsElementSetDisabled(EsElement *element, bool disabled) {
	...
	for (uintptr_t i = 0; i < element->GetChildCount(); i++) {
		if (element->GetChild(i)->flags & LT_ELEMENT_NON_CLIENT) continue;
		EsElementSetDisabled(element->GetChild(i), disabled);
	}

	if ((element->flags & LT_ELEMENT_DISABLED) && disabled) return;
	if ((~element->flags & LT_ELEMENT_DISABLED) && !disabled) return;

	if (disabled) element->flags |= LT_ELEMENT_DISABLED;
	else element->flags &= ~LT_ELEMENT_DISABLED;
...
void EsElementDestroyContents(EsElement *element) {
	...
	EsMessage m = {};
	m.type = LT_MSG_DESTROY_CONTENTS;
	EsMessageSend(element, &m);
}

EsElement *UITabTraversalGetChild(EsElement *element, uintptr_t index) {
	if (element->flags & LT_ELEMENT_LAYOUT_HINT_REVERSE) {
...
bool EsMouseIsLeftHeld()     { return gui.mouseButtonDown && gui.lastClickButton == LT_MSG_MOUSE_LEFT_DOWN; }
bool EsMouseIsRightHeld()    { return gui.mouseButtonDown && gui.lastClickButton == LT_MSG_MOUSE_RIGHT_DOWN; }
bool EsMouseIsMiddleHeld()   { return gui.mouseButtonDown && gui.lastClickButton == LT_MSG_MOUSE_MIDDLE_DOWN; }
...
void _EsUISetFont(EsFontFamily id) {
	...
	EsMessage m = { LT_MSG_UI_SCALE_CHANGED };
...
void EsElementDraw(EsElement *element, EsPainter *painter) {
	...
	element->InternalPaint(painter, LT_FLAGS_DEFAULT);
...
void UIMouseDown(EsWindow *window, EsMessage *message) {
	...
	if ((~window->hovered->flags & LT_ELEMENT_DISABLED) && (~window->hovered->state & UI_STATE_BLOCK_INTERACTION)) {
		...
		EsMessage m = { LT_MSG_PRESSED_START };
		EsMessageSend(element, &m);
		...
		if (LT_REJECTED != UIMessageSendPropagateToAncestors(element, message, &window->dragged)) {
			if (window->dragged && (~window->dragged->flags & LT_ELEMENT_NO_FOCUS_ON_CLICK)) {
	...
	if (window->hovered != window->focused && window->focused && (~window->focused->state & UI_STATE_LOST_STRONG_FOCUS)) {
		EsMessage m = { LT_MSG_STRONG_FOCUS_END };
...
void UIMouseUp(EsWindow *window, EsMessage *message, bool sendClick) {
	...
	if (window->pressed) {
		...
		EsMessage m = { LT_MSG_PRESSED_END };
		EsMessageSend(pressed, &m);

		if (message && window->hovered == pressed && !gui.draggingStarted && sendClick) {
			if (message->type == LT_MSG_MOUSE_LEFT_UP) {
				m.type = LT_MSG_MOUSE_LEFT_CLICK;
				EsMessageSend(pressed, &m);
			} else if (message->type == LT_MSG_MOUSE_RIGHT_UP) {
				m.type = LT_MSG_MOUSE_RIGHT_CLICK;
				EsMessageSend(pressed, &m);
			} else if (message->type == LT_MSG_MOUSE_MIDDLE_UP) {
				m.type = LT_MSG_MOUSE_MIDDLE_CLICK;
				EsMessageSend(pressed, &m);
			}
		}
...
#define ADD_KEYBOARD_SHORTCUT_NAME(a, b) HashTablePutShort(&gui.keyboardShortcutNames, a, (void *) b)
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_A, "A");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_B, "B");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_C, "C");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_D, "D");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_E, "E");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F, "F");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_G, "G");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_H, "H");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_I, "I");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_J, "J");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_K, "K");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_L, "L");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_M, "M");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_N, "N");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_O, "O");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_P, "P");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_Q, "Q");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_R, "R");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_S, "S");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_T, "T");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_U, "U");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_V, "V");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_W, "W");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_X, "X");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_Y, "Y");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_Z, "Z");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_0, "0");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_1, "1");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_2, "2");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_3, "3");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_4, "4");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_5, "5");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_6, "6");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_7, "7");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_8, "8");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_9, "9");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_BACKSPACE, "Backspace");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_ESCAPE, "Esc");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_INSERT, "Ins");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_HOME, "Home");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_PAGE_UP, "PgUp");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_DELETE, "Del");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_END, "End");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_PAGE_DOWN, "PgDn");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_UP_ARROW, "Up");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_LEFT_ARROW, "Left");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_DOWN_ARROW, "Down");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_RIGHT_ARROW, "Right");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_SPACE, "Space");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_TAB, "Tab");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_ENTER, "Enter");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F1, "F1");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F2, "F2");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F3, "F3");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F4, "F4");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F5, "F5");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F6, "F6");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F7, "F7");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F8, "F8");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F9, "F9");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F10, "F10");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F11, "F11");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_F12, "F12");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_MM_NEXT, "Next Track");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_MM_PREVIOUS, "Previous Track");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_MM_STOP, "Stop Media");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_MM_PAUSE, "Play/Pause");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_MM_MUTE, "Mute");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_MM_QUIETER, "Quieter");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_MM_LOUDER, "Louder");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_MM_SELECT, "Open Media");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_WWW_SEARCH, "Search");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_WWW_HOME, "Homepage");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_WWW_BACK, "Back");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_WWW_FORWARD, "Forward");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_WWW_STOP, "Stop");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_WWW_REFRESH, "Refresh");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_WWW_STARRED, "Bookmark");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_SLASH, "/");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_PUNCTUATION_1, "\\");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_LEFT_BRACE, "(");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_RIGHT_BRACE, ")");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_EQUALS, "=");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_PUNCTUATION_5, "`");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_HYPHEN, "-");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_PUNCTUATION_3, ";");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_PUNCTUATION_4, "'");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_COMMA, ",");
	ADD_KEYBOARD_SHORTCUT_NAME(LT_SCANCODE_PERIOD, ".");
}
...
void AccessKeysCenterHint(EsElement *element, EsMessage *message) {
	...
	*message->accessKeyHintBounds = LT_RECT_4(x - style->preferredWidth / 2, x + style->preferredWidth / 2, 
			y - style->preferredHeight / 4, y + 3 * style->preferredHeight / 4);
}

void AccessKeysGather(EsElement *element) {
	if (element->flags & LT_ELEMENT_BLOCK_FOCUS) return;
	if (element->state & UI_STATE_BLOCK_INTERACTION) return;
	if (element->flags & LT_ELEMENT_HIDDEN) return;
	...
	if (element->state & UI_STATE_DESTROYING) return;
	if (element->flags & LT_ELEMENT_DISABLED) return;
	...
	EsRectangle hintBounds = LT_RECT_4(x - style->preferredWidth / 2, x + style->preferredWidth / 2, 
			y - style->preferredHeight / 4, y + 3 * style->preferredHeight / 4);

	EsMessage m = {};
	m.type = LT_MSG_GET_ACCESS_KEY_HINT_BOUNDS;
...
void AccessKeyHintsShow(EsPainter *painter) {
	for (uintptr_t i = 0; i < gui.accessKeys.entries.Length(); i++) {
		...
		style->PaintText(painter, gui.accessKeys.window, entry->bounds, &c, 1, 0, LT_FLAGS_DEFAULT);
	}
}

int AccessKeyLayerMessage(EsElement *element, EsMessage *message) {
	if (message->type == LT_MSG_PAINT && gui.accessKeyMode && gui.accessKeys.window == element->window) {
...
void AccessKeyModeEnter(EsWindow *window) {
	if (window->dialogs.Length() || gui.menuMode || gui.accessKeyMode || window->windowStyle != LT_WINDOW_NORMAL) {
		return;
	}

	if (!gui.accessKeys.hintStyle) {
		gui.accessKeys.hintStyle = GetStyle(MakeStyleKey(LT_STYLE_ACCESS_KEY_HINT, 0), true);
	}
...
void AccessKeyModeHandleKeyPress(EsMessage *message) {
	if (message->type == LT_MSG_KEY_UP || message->keyboard.scancode == LT_SCANCODE_LEFT_ALT) {
		return;
	}
	...
	const char *inputString = KeyboardLayoutLookup(message->keyboard.scancode, 
			message->keyboard.modifiers & LT_MODIFIER_SHIFT, message->keyboard.modifiers & LT_MODIFIER_ALT_GR, 
			false, false);
		...
		if (gui.accessKeys.numbers[ic - 'A'] > 1) {
			...
			for (uintptr_t i = 0; i < gui.accessKeys.entries.Length(); i++) {
				...
				if (entry->character == ic) {
					EsMessage m = { LT_MSG_MOUSE_LEFT_CLICK };
					EsMessageSend(entry->element, &m);
					EsElementFocus(entry->element, LT_ELEMENT_FOCUS_ENSURE_VISIBLE | LT_ELEMENT_FOCUS_FROM_KEYBOARD);

					keepAccessKeyModeActive = entry->element->flags & LT_ELEMENT_STICKY_ACCESS_KEY;
					regatherKeys = true;
				}
			}
		}
...
void UIRefreshPrimaryClipboard(EsWindow *window) {
	if (window->focused) {
		EsMessage m = {};
		m.type = LT_MSG_PRIMARY_CLIPBOARD_UPDATED;
		EsMessageSend(window->focused, &m);
	}
}

bool UIHandleKeyMessage(EsWindow *window, EsMessage *message) {
	if (message->type == LT_MSG_KEY_UP) {
		if (message->keyboard.scancode == LT_SCANCODE_LEFT_CTRL  ) gui.leftModifiers  &= ~LT_MODIFIER_CTRL;
		if (message->keyboard.scancode == LT_SCANCODE_LEFT_ALT   ) gui.leftModifiers  &= ~LT_MODIFIER_ALT;
		if (message->keyboard.scancode == LT_SCANCODE_LEFT_SHIFT ) gui.leftModifiers  &= ~LT_MODIFIER_SHIFT;
		if (message->keyboard.scancode == LT_SCANCODE_LEFT_FLAG  ) gui.leftModifiers  &= ~LT_MODIFIER_FLAG;
		if (message->keyboard.scancode == LT_SCANCODE_RIGHT_CTRL ) gui.rightModifiers &= ~LT_MODIFIER_CTRL;
		if (message->keyboard.scancode == LT_SCANCODE_RIGHT_ALT  ) gui.rightModifiers &= ~LT_MODIFIER_ALT_GR;
		if (message->keyboard.scancode == LT_SCANCODE_RIGHT_SHIFT) gui.rightModifiers &= ~LT_MODIFIER_SHIFT;
		if (message->keyboard.scancode == LT_SCANCODE_RIGHT_FLAG ) gui.rightModifiers &= ~LT_MODIFIER_FLAG;

		if (message->keyboard.scancode == LT_SCANCODE_LEFT_ALT && message->keyboard.single) {
			AccessKeyModeEnter(window);
			return true;
		} else if (window->focused) {
			return LT_HANDLED == EsMessageSend(window->focused, message);
		} else {
			return LT_HANDLED == EsMessageSend(window, message);
		}
	}
	...
	if (message->keyboard.scancode == LT_SCANCODE_F2 && message->keyboard.modifiers == LT_MODIFIER_ALT) {
		EnterDebugger();
		EsPrint("[Alt-F2]\n");
	}
	...
	if (window->windowStyle == LT_WINDOW_MENU && message->keyboard.scancode == LT_SCANCODE_ESCAPE) {
	...
	if (window->pressed) {
		if (message->keyboard.scancode == LT_SCANCODE_ESCAPE) {
	...
	if (window->focused) {
		message->type = LT_MSG_KEY_TYPED;

		if (EsMessageSend(window->focused, message) == LT_HANDLED /* allow messageUser to reject input */) {
			return true;
		}

		EsElement *element = window->focused;
		message->type = LT_MSG_KEY_DOWN;
	...
	if (message->keyboard.scancode == LT_SCANCODE_TAB && (message->keyboard.modifiers & ~LT_MODIFIER_SHIFT) == 0) {
		EsElement *element = window->focused ?: window;
		EsElement *start = element;
		bool backwards = message->keyboard.modifiers & LT_MODIFIER_SHIFT;
		...
		EsElementFocus(element, LT_ELEMENT_FOCUS_ENSURE_VISIBLE | LT_ELEMENT_FOCUS_FROM_KEYBOARD);
		return true;
	}

	if (window->focused) {
		if (message->keyboard.scancode == LT_SCANCODE_SPACE && message->keyboard.modifiers == 0) {
			EsMessage m = { LT_MSG_MOUSE_LEFT_CLICK };
			EsMessageSend(window->focused, &m);
			return true;
		}
	} else {
		if (LT_HANDLED == EsMessageSend(window, message)) {
			return true;
		}
	}
	...
	if (window->enterButton && message->keyboard.scancode == LT_SCANCODE_ENTER && !message->keyboard.modifiers
			&& window->enterButton->onCommand && (~window->enterButton->flags & LT_ELEMENT_DISABLED)) {
		window->enterButton->onCommand(window->instance, window->enterButton, window->enterButton->command);
		return true;
	} else if (window->escapeButton && message->keyboard.scancode == LT_SCANCODE_ESCAPE && !message->keyboard.modifiers
			&& window->escapeButton->onCommand && (~window->escapeButton->flags & LT_ELEMENT_DISABLED)) {
	...
	if (!window->dialogs.Length()) {
		...
		if (shortcutName && window->instance && window->instance->_private) {
			...
			size_t bytes = EsStringFormat(keyboardShortcutString, 128, "%z%z%z%z%c",
					(message->keyboard.modifiers & LT_MODIFIER_CTRL)  ? "Ctrl+"  : "", 
					(message->keyboard.modifiers & LT_MODIFIER_SHIFT) ? "Shift+" : "", 
					(message->keyboard.modifiers & LT_MODIFIER_ALT)   ? "Alt+"   : "", 
					shortcutName, 0) - 1;
...
void UIWindowPaintNow(EsWindow *window, ProcessMessageTiming *timing, bool afterResize) {
	...
	EsRectangle bounds = LT_RECT_4(0, window->windowWidth, 0, window->windowHeight);
	...
	if (THEME_RECT_VALID(updateRegion)) {
		...
		target.fullAlpha = window->windowStyle != LT_WINDOW_NORMAL;
		...
		painter.clip = LT_RECT_4(0, target.width, 0, target.height);
		painter.target = &target;

		window->updateRegionInProgress = updateRegion;
		window->InternalPaint(&painter, LT_FLAGS_DEFAULT);
		window->updateRegionInProgress = {};
		if (timing) timing->endPaint = EsTimeStampMs();

		if (window->visualizeRepaints) {
			EsDrawRectangle(&painter, painter.clip, 0, EsRandomU64(), LT_RECT_1(3));
		}

		// Update the screen.
		if (timing) timing->startUpdate = EsTimeStampMs();
		EsSyscall(LT_SYSCALL_WINDOW_SET_BITS, window->handle, (uintptr_t) &updateRegion, (uintptr_t) target.bits,
	...
	window->updateRegion = LT_RECT_4(window->windowWidth, 0, window->windowHeight, 0);
}
...
void UIWindowLayoutNow(EsWindow *window, ProcessMessageTiming *timing) {
	...
	if (window->processCheckVisible) {
		for (uintptr_t i = 0; i < window->checkVisible.Length(); i++) {
			...
			i--;
			EsMessage m = { LT_MSG_NOT_VISIBLE };
			EsMessageSend(element, &m);
		}
...
bool UISetCursor(EsWindow *window) {
	...
	EsCursorStyle cursorStyle = LT_CURSOR_NORMAL;
	EsElement *element = window->dragged ?: window->pressed ?: window->hovered;

	if (element) {
		EsMessage m = { LT_MSG_GET_CURSOR };

		if (LT_HANDLED == EsMessageSend(element, &m)) {
...
#define CURSOR(_constant, _x, _y, _ox, _oy, _w, _h) _constant: { x = _x; y = _y; ox = -_ox; oy = -_oy; w = _w; h = _h; } break
	...
	switch (cursorStyle) {
		CURSOR(case LT_CURSOR_TEXT, 84, 60, 4, 10, 11, 22);
		CURSOR(case LT_CURSOR_RESIZE_VERTICAL, 44, 12, 4, 11, 11, 24);
		CURSOR(case LT_CURSOR_RESIZE_HORIZONTAL, 68, 15, 11, 4, 24, 11);
		CURSOR(case LT_CURSOR_RESIZE_DIAGONAL_1, 55, 35, 8, 8, 19, 19);
		CURSOR(case LT_CURSOR_RESIZE_DIAGONAL_2, 82, 35, 8, 8, 19, 19);
		CURSOR(case LT_CURSOR_SPLIT_VERTICAL, 37, 55, 4, 10, 12, 24);
		CURSOR(case LT_CURSOR_SPLIT_HORIZONTAL, 56, 60, 10, 4, 24, 12);
		CURSOR(case LT_CURSOR_HAND_HOVER, 131, 14, 8, 1, 21, 26);
		CURSOR(case LT_CURSOR_HAND_DRAG, 107, 18, 7, 1, 20, 22);
		CURSOR(case LT_CURSOR_HAND_POINT, 159, 14, 5, 1, 21, 26);
		CURSOR(case LT_CURSOR_SCROLL_UP_LEFT, 217, 89, 9, 9, 16, 16);
		CURSOR(case LT_CURSOR_SCROLL_UP, 193, 87, 5, 11, 13, 18);
		CURSOR(case LT_CURSOR_SCROLL_UP_RIGHT, 234, 89, 4, 9, 16, 16);
		CURSOR(case LT_CURSOR_SCROLL_LEFT, 175, 93, 11, 5, 18, 13);
		CURSOR(case LT_CURSOR_SCROLL_CENTER, 165, 51, 12, 12, 28, 28);
		CURSOR(case LT_CURSOR_SCROLL_RIGHT, 194, 106, 4, 4, 18, 13);
		CURSOR(case LT_CURSOR_SCROLL_DOWN_LEFT, 216, 106, 10, 4, 16, 16);
		CURSOR(case LT_CURSOR_SCROLL_DOWN, 182, 107, 4, 3, 12, 16);
		CURSOR(case LT_CURSOR_SCROLL_DOWN_RIGHT, 234, 106, 4, 4, 16, 16);
		CURSOR(case LT_CURSOR_SELECT_LINES, 7, 19, 10, 0, 16, 23);
		CURSOR(case LT_CURSOR_DROP_TEXT, 11, 48, 1, 1, 21, 28);
		CURSOR(case LT_CURSOR_CROSS_HAIR_PICK, 104, 56, 11, 11, 26, 26);
		CURSOR(case LT_CURSOR_CROSS_HAIR_RESIZE, 134, 53, 11, 11, 26, 26);
		CURSOR(case LT_CURSOR_MOVE_HOVER, 226, 13, 10, 10, 24, 32);
		CURSOR(case LT_CURSOR_MOVE_DRAG, 197, 13, 10, 10, 24, 24);
		CURSOR(case LT_CURSOR_ROTATE_HOVER, 228, 49, 9, 10, 24, 32);
		CURSOR(case LT_CURSOR_ROTATE_DRAG, 198, 49, 9, 10, 22, 22);
		CURSOR(case LT_CURSOR_BLANK, 0, 0, 0, 0, 1, 1);
		CURSOR(default, 24, 19, 0, 0, 11, 20);
	}

	bool shadow = cursorStyle != LT_CURSOR_TEXT && api.global->showCursorShadow;

	return EsSyscall(LT_SYSCALL_WINDOW_SET_CURSOR, window->handle, 
...
void UIProcessWindowManagerMessage(EsWindow *window, EsMessage *message, ProcessMessageTiming *timing) {
	...
	if (message->type == LT_MSG_WINDOW_DESTROYED) {
		if (window->instance) {
		...
		EsAssert(window->handle == LT_INVALID_HANDLE);
		EsHeapFree(window);
		return;
	} else if (window->handle == LT_INVALID_HANDLE) {
		return;
	}
	...
	if (api.global->swapLeftAndRightButtons) {
		if      (message->type == LT_MSG_MOUSE_LEFT_DOWN ) message->type = LT_MSG_MOUSE_RIGHT_DOWN;
		else if (message->type == LT_MSG_MOUSE_RIGHT_DOWN) message->type = LT_MSG_MOUSE_LEFT_DOWN;
		else if (message->type == LT_MSG_MOUSE_LEFT_UP   ) message->type = LT_MSG_MOUSE_RIGHT_UP;
		else if (message->type == LT_MSG_MOUSE_RIGHT_UP  ) message->type = LT_MSG_MOUSE_LEFT_UP;
	}

	if (message->type == LT_MSG_MOUSE_MOVED) {
		window->mousePosition.x = message->mouseMoved.newPositionX;
		window->mousePosition.y = message->mouseMoved.newPositionY;

		if ((!window->activated || window->targetMenu) && window->windowStyle == LT_WINDOW_NORMAL) {
			...
			if (gui.draggingStarted || DistanceSquared(message->mouseMoved.newPositionX - gui.lastClickX, 
				...
				message->type = gui.lastClickButton == LT_MSG_MOUSE_LEFT_DOWN ? LT_MSG_MOUSE_LEFT_DRAG
					: gui.lastClickButton == LT_MSG_MOUSE_RIGHT_DOWN ? LT_MSG_MOUSE_RIGHT_DRAG
					: gui.lastClickButton == LT_MSG_MOUSE_MIDDLE_DOWN ? LT_MSG_MOUSE_MIDDLE_DRAG : (EsMessageType) 0;
				...
				if (LT_HANDLED == EsMessageSend(window->dragged, message)) {
		...
		window->hovering = true;
	} else if (message->type == LT_MSG_MOUSE_EXIT) {
		window->hovering = false;
	} else if (message->type == LT_MSG_MOUSE_LEFT_DOWN || message->type == LT_MSG_MOUSE_RIGHT_DOWN || message->type == LT_MSG_MOUSE_MIDDLE_DOWN) {
	...
	} else if (message->type == LT_MSG_KEY_UP || message->type == LT_MSG_KEY_DOWN) {
		...
		if (scancode == LT_SCANCODE_NUM_DIVIDE  ) { numpad = LT_SCANCODE_SLASH; }
		if (scancode == LT_SCANCODE_NUM_MULTIPLY) { numpad = LT_SCANCODE_8; nshift = 1; }
		if (scancode == LT_SCANCODE_NUM_SUBTRACT) { numpad = LT_SCANCODE_HYPHEN; }
		if (scancode == LT_SCANCODE_NUM_ADD	) { numpad = LT_SCANCODE_EQUALS; nshift = 1; }
		if (scancode == LT_SCANCODE_NUM_ENTER	) { numpad = LT_SCANCODE_ENTER; }

		if (message->keyboard.numlock) {
			if (scancode == LT_SCANCODE_NUM_POINT) { numpad = LT_SCANCODE_PERIOD; }
			if (scancode == LT_SCANCODE_NUM_0    ) { numpad = LT_SCANCODE_0; }
			if (scancode == LT_SCANCODE_NUM_1    ) { numpad = LT_SCANCODE_1; }
			if (scancode == LT_SCANCODE_NUM_2    ) { numpad = LT_SCANCODE_2; }
			if (scancode == LT_SCANCODE_NUM_3    ) { numpad = LT_SCANCODE_3; }
			if (scancode == LT_SCANCODE_NUM_4    ) { numpad = LT_SCANCODE_4; }
			if (scancode == LT_SCANCODE_NUM_5    ) { numpad = LT_SCANCODE_5; }
			if (scancode == LT_SCANCODE_NUM_6    ) { numpad = LT_SCANCODE_6; }
			if (scancode == LT_SCANCODE_NUM_7    ) { numpad = LT_SCANCODE_7; }
			if (scancode == LT_SCANCODE_NUM_8    ) { numpad = LT_SCANCODE_8; }
			if (scancode == LT_SCANCODE_NUM_9    ) { numpad = LT_SCANCODE_9; }
		} else {
			if (scancode == LT_SCANCODE_NUM_POINT) { numpad = LT_SCANCODE_DELETE; allowShift = true; }
			if (scancode == LT_SCANCODE_NUM_0    ) { numpad = LT_SCANCODE_INSERT; allowShift = true; }
			if (scancode == LT_SCANCODE_NUM_1    ) { numpad = LT_SCANCODE_END; allowShift = true; }
			if (scancode == LT_SCANCODE_NUM_2    ) { numpad = LT_SCANCODE_DOWN_ARROW; allowShift = true; }
			if (scancode == LT_SCANCODE_NUM_3    ) { numpad = LT_SCANCODE_PAGE_DOWN; allowShift = true; }
			if (scancode == LT_SCANCODE_NUM_4    ) { numpad = LT_SCANCODE_LEFT_ARROW; allowShift = true; }
			if (scancode == LT_SCANCODE_NUM_6    ) { numpad = LT_SCANCODE_RIGHT_ARROW; allowShift = true; }
			if (scancode == LT_SCANCODE_NUM_7    ) { numpad = LT_SCANCODE_HOME; allowShift = true; }
			if (scancode == LT_SCANCODE_NUM_8    ) { numpad = LT_SCANCODE_UP_ARROW; allowShift = true; }
			if (scancode == LT_SCANCODE_NUM_9    ) { numpad = LT_SCANCODE_PAGE_UP; allowShift = true; }
		}

		if (numpad && ((~message->keyboard.modifiers & LT_MODIFIER_SHIFT) || allowShift)) {
			EsMessage m = *message;
			m.type = message->type;
			m.keyboard.modifiers = message->keyboard.modifiers | (nshift ? LT_MODIFIER_SHIFT : 0);
	...
	} else if (message->type == LT_MSG_WINDOW_RESIZED) {
		AccessKeyModeExit();
		...
		if (window->windowStyle == LT_WINDOW_CONTAINER) {
			...
			EsRectangle opaqueBounds = LT_RECT_2S(window->windowWidth, window->windowHeight);
			EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, window->handle, (uintptr_t) &opaqueBounds, 0, LT_WINDOW_PROPERTY_OPAQUE_BOUNDS);
		}
		...
		EsSyscall(LT_SYSCALL_CURSOR_POSITION_GET, (uintptr_t) &window->mousePosition, 0, 0, 0);
		...
		window->hovering = true;
	} else if (message->type == LT_MSG_WINDOW_DEACTIVATED) {
		if (window->activated) {
			if (window->pressed) {
				UIMouseUp(window, nullptr, false);
			}
	...
	} else if (message->type == LT_MSG_WINDOW_ACTIVATED) {
	...
	if (window->receivedFirstResize /* don't try to layout an embedded window until its size is known */
			|| window->windowStyle != LT_WINDOW_NORMAL) {
	...
	if (window->width == (int) window->windowWidth && window->height == (int) window->windowHeight 
		...
		UIWindowPaintNow(window, timing, message->type == LT_MSG_WINDOW_RESIZED);
	} else if (changedCursor) {
		EsSyscall(LT_SYSCALL_SCREEN_FORCE_UPDATE, 0, 0, 0, 0);
	}
