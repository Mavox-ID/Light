...
void *EsMemoryReserve(size_t size, EsMemoryProtection protection, uint32_t flags) {
	intptr_t result = EsSyscall(LT_SYSCALL_MEMORY_ALLOCATE, size, flags, protection, 0);
...
EsHandle EsMemoryCreateShareableRegion(size_t bytes) {
	return EsSyscall(LT_SYSCALL_MEMORY_ALLOCATE, bytes, 0, 0, 1);
}

void EsMemoryUnreserve(void *address, size_t size) {
	EsSyscall(LT_SYSCALL_MEMORY_FREE, (uintptr_t) address, size, 0, 0);
}

bool EsMemoryCommit(void *pointer, size_t bytes) {
	EsAssert(((uintptr_t) pointer & (LT_PAGE_SIZE - 1)) == 0 && (bytes & (LT_PAGE_SIZE - 1)) == 0); // Misaligned pointer/bytes in EsMemoryCommit.
	return LT_SUCCESS == (intptr_t) EsSyscall(LT_SYSCALL_MEMORY_COMMIT, (uintptr_t) pointer >> LT_PAGE_BITS, bytes >> LT_PAGE_BITS, 0, 0);
}

void EsMemoryFaultRange(const void *pointer, size_t bytes, uint32_t flags) {
	EsSyscall(LT_SYSCALL_MEMORY_FAULT_RANGE, (uintptr_t) pointer, bytes, flags, 0);
}

bool EsMemoryDecommit(void *pointer, size_t bytes) {
	EsAssert(((uintptr_t) pointer & (LT_PAGE_SIZE - 1)) == 0 && (bytes & (LT_PAGE_SIZE - 1)) == 0); // Misaligned pointer/bytes in EsMemoryDecommit.
	return LT_SUCCESS == (intptr_t) EsSyscall(LT_SYSCALL_MEMORY_COMMIT, (uintptr_t) pointer >> LT_PAGE_BITS, bytes >> LT_PAGE_BITS, 1, 0);
}
...
EsError EsProcessCreate(const EsProcessCreationArguments *arguments, EsProcessInformation *information) {
	...
	EsError error = EsSyscall(LT_SYSCALL_PROCESS_CREATE, (uintptr_t) arguments, 0, (uintptr_t) information, 0);

	if (error == LT_SUCCESS && information == &_information) {
...
EsError EsMessagePost(EsElement *target, EsMessage *message) {
	...
	if (api.postBox.Length() == 1 && success) {
		EsMessage m;
		m.type = LT_MSG_WAKEUP;
		success = LT_SUCCESS == (EsError) EsSyscall(LT_SYSCALL_MESSAGE_POST, (uintptr_t) &m, 0, LT_CURRENT_PROCESS, 0);
	}
...
EsError EsMessagePostRemote(EsHandle process, EsMessage *message) {
	return EsSyscall(LT_SYSCALL_MESSAGE_POST, (uintptr_t) message, 0, process, 0);
}

EsHandle EsEventCreate(bool autoReset) {
	return EsSyscall(LT_SYSCALL_EVENT_CREATE, autoReset, 0, 0, 0);
}

void EsEventSet(EsHandle handle) {
	EsSyscall(LT_SYSCALL_EVENT_SET, handle, 0, 0, 0);
}

void EsEventReset(EsHandle handle) {
	EsSyscall(LT_SYSCALL_EVENT_RESET, handle, 0, 0, 0);
}

EsError EsHandleClose(EsHandle handle) {
	return EsSyscall(LT_SYSCALL_HANDLE_CLOSE, handle, 0, 0, 0);
}

void EsThreadTerminate(EsHandle thread) {
	EsSyscall(LT_SYSCALL_THREAD_TERMINATE, thread, 0, 0, 0);
}

void EsProcessTerminate(EsHandle process, int status) {
	EsSyscall(LT_SYSCALL_PROCESS_TERMINATE, process, status, 0, 0);
}

void EsProcessTerminateCurrent() {
	EsSyscall(LT_SYSCALL_PROCESS_TERMINATE, LT_CURRENT_PROCESS, 0, 0, 0);
}

int EsProcessGetExitStatus(EsHandle process) {
	return EsSyscall(LT_SYSCALL_PROCESS_GET_STATUS, process, 0, 0, 0);
}
...
EsError EsThreadCreate(EsThreadEntryCallback entryFunction, EsThreadInformation *information, EsGeneric argument) {
	...
	EsError error = EsSyscall(LT_SYSCALL_THREAD_CREATE, (uintptr_t) ThreadEntry, (uintptr_t) entryFunction, (uintptr_t) information, argument.u);

	if (error == LT_SUCCESS && information == &discard) {
		EsHandleClose(information->handle);
	}
...
EsHandle EsMemoryShare(EsHandle sharedMemoryRegion, EsHandle targetProcess, bool readOnly) {
	return EsSyscall(LT_SYSCALL_HANDLE_SHARE, sharedMemoryRegion, targetProcess, readOnly, 0);
}

void *EsMemoryMapObject(EsHandle sharedMemoryRegion, uintptr_t offset, size_t size, unsigned flags) {
	intptr_t result = EsSyscall(LT_SYSCALL_MEMORY_MAP_OBJECT, sharedMemoryRegion, offset, size, flags);
...
uintptr_t EsWait(const EsHandle *handles, size_t count, uintptr_t timeoutMs) {
	return EsSyscall(LT_SYSCALL_WAIT, (uintptr_t) handles, count, timeoutMs, 0);
}

void EsProcessPause(EsHandle process, bool resume) {
	EsSyscall(LT_SYSCALL_PROCESS_PAUSE, process, resume, 0, 0);
}

EsObjectID EsThreadGetID(EsHandle thread) {
	if (thread == LT_CURRENT_THREAD) {
		return GetThreadLocalStorage()->id;
	} else {
		EsObjectID id;
		EsSyscall(LT_SYSCALL_THREAD_GET_ID, thread, (uintptr_t) &id, 0, 0);
		return id;
	}
}
...
EsObjectID EsProcessGetID(EsHandle process) {
	EsObjectID id;
	EsSyscall(LT_SYSCALL_THREAD_GET_ID, process, (uintptr_t) &id, 0, 0);
	return id;
}
...
void EsBatch(EsBatchCall *calls, size_t count) {
	...
	EsSyscall(LT_SYSCALL_BATCH, (uintptr_t) calls, count, 0, 0);
}

void EsConstantBufferRead(EsHandle buffer, void *output) {
	EsSyscall(LT_SYSCALL_CONSTANT_BUFFER_READ, buffer, (uintptr_t) output, 0, 0);
}

void EsProcessGetState(EsHandle process, EsProcessState *state) {
	EsSyscall(LT_SYSCALL_PROCESS_GET_STATE, process, (uintptr_t) state, 0, 0);
}

void EsSchedulerYield() {
	EsSyscall(LT_SYSCALL_YIELD_SCHEDULER, 0, 0, 0, 0);
}

void EsSleep(uint64_t milliseconds) {
	EsSyscall(LT_SYSCALL_SLEEP, milliseconds >> 32, milliseconds & 0xFFFFFFFF, 0, 0);
}

EsHandle EsTakeSystemSnapshot(int type, size_t *bufferSize) {
	return EsSyscall(LT_SYSCALL_SYSTEM_TAKE_SNAPSHOT, type, (uintptr_t) bufferSize, 0, 0);
}

EsHandle EsProcessOpen(uint64_t pid) {
	// TODO This won't work correctly if arguments to system call are 32-bit.
	return EsSyscall(LT_SYSCALL_PROCESS_OPEN, pid, 0, 0, 0);
}

EsHandle EsConstantBufferShare(EsHandle constantBuffer, EsHandle targetProcess) {
	return EsSyscall(LT_SYSCALL_HANDLE_SHARE, constantBuffer, targetProcess, 0, 0);
}

EsHandle EsConstantBufferCreate(const void *data, size_t dataBytes, EsHandle targetProcess) {
	return EsSyscall(LT_SYSCALL_CONSTANT_BUFFER_CREATE, (uintptr_t) data, targetProcess, dataBytes, 0);
}

size_t EsConstantBufferGetSize(EsHandle buffer) {
	return EsSyscall(LT_SYSCALL_CONSTANT_BUFFER_READ, buffer, 0, 0, 0);
}

EsError EsAddressResolve(const char *domain, ptrdiff_t domainBytes, uint32_t flags, EsAddress *address) {
	return EsSyscall(LT_SYSCALL_DOMAIN_NAME_RESOLVE, (uintptr_t) domain, domainBytes, (uintptr_t) address, flags);
}
...
EsError EsConnectionOpen(EsConnection *connection, uint32_t flags) {
	...
	EsError error = EsSyscall(LT_SYSCALL_CONNECTION_OPEN, (uintptr_t) connection, flags, 0, 0);

	if (error == LT_SUCCESS && (flags & LT_CONNECTION_OPEN_WAIT)) {
		while (!connection->open && connection->error == LT_SUCCESS) {
...
void EsConnectionPoll(EsConnection *connection) {
	EsSyscall(LT_SYSCALL_CONNECTION_POLL, (uintptr_t) connection, 0, 0, connection->handle);
}

void EsConnectionNotify(EsConnection *connection) {
	EsSyscall(LT_SYSCALL_CONNECTION_NOTIFY, 0, connection->sendWritePointer, connection->receiveReadPointer, connection->handle);
}
...
EsError EsConnectionWriteSync(EsConnection *connection, const void *_data, size_t dataBytes) {
	...
	while (dataBytes) {
		...
		if (connection->error != LT_SUCCESS) {
			return connection->error;
		}
...
size_t EsGameControllerStatePoll(EsGameControllerState *buffer) {
	return EsSyscall(LT_SYSCALL_GAME_CONTROLLER_STATE_POLL, (uintptr_t) buffer, 0, 0, 0);
}
...
void MessageDesktop(void *message, size_t messageBytes, EsHandle embeddedWindow = LT_INVALID_HANDLE, EsBuffer *responseBuffer = nullptr) {
	static EsMutex messageDesktopMutex = {};

	EsObjectID embeddedWindowID = embeddedWindow ? EsSyscall(LT_SYSCALL_WINDOW_GET_ID, embeddedWindow, 0, 0, 0) : 0;
...
EsError EsClipboardAddText(EsClipboard clipboard, const char *text, ptrdiff_t textBytes) {
	...
	if (fileStore) {
		...
		return EsClipboardCloseAndAdd(clipboard, LT_CLIPBOARD_FORMAT_TEXT, fileStore);
	} else {
...
bool EsClipboardHasFormat(EsClipboard clipboard, EsClipboardFormat format) {
	...
	if (information.error != LT_SUCCESS) return false;

	if (format == LT_CLIPBOARD_FORMAT_TEXT) {
		return information.format == LT_CLIPBOARD_FORMAT_TEXT || information.format == LT_CLIPBOARD_FORMAT_PATH_LIST;
	} else {
...
bool EsClipboardHasData(EsClipboard clipboard) {
	...
	ClipboardGetInformation(&file, &information);
	if (file) EsHandleClose(file);
	return information.error == LT_SUCCESS && information.format != LT_CLIPBOARD_FORMAT_INVALID;
}
...
char *EsClipboardReadText(EsClipboard clipboard, size_t *bytes, uint32_t *flags) {
	...
	*bytes = 0;
	...
	if (file) {
		if (information.format == LT_CLIPBOARD_FORMAT_TEXT || information.format == LT_CLIPBOARD_FORMAT_PATH_LIST) {
...
void EsPipeCreate(EsHandle *readEnd, EsHandle *writeEnd) {
	*readEnd = *writeEnd = LT_INVALID_HANDLE;
	EsSyscall(LT_SYSCALL_PIPE_CREATE, (uintptr_t) readEnd, (uintptr_t) writeEnd, 0, 0);
}
...
size_t EsPipeRead(EsHandle pipe, void *buffer, size_t bytes, bool allowShortReads) {
	if (!bytes) {
		...
		return EsSyscall(LT_SYSCALL_PIPE_READ, pipe, (uintptr_t) buffer, bytes, 0);
	} else {
		size_t position = 0;
...
size_t EsPipeWrite(EsHandle pipe, const void *buffer, size_t bytes) {
	if (bytes) {
		return EsSyscall(LT_SYSCALL_PIPE_WRITE, pipe, (uintptr_t) buffer, bytes, 0);
	} else {
		return 0;
	}
}
...
EsError EsDeviceControl(EsHandle handle, EsDeviceControlType type, void *dp, void *dq) {
	return EsSyscall(LT_SYSCALL_DEVICE_CONTROL, handle, type, (uintptr_t) dp, (uintptr_t) dq);
}

uintptr_t _EsDebugCommand(uintptr_t a, uintptr_t b, uintptr_t c, uintptr_t d) {
	return EsSyscall(LT_SYSCALL_DEBUG_COMMAND, a, b, c, d);
}
