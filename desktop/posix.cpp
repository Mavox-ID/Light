...
long EsPOSIXSystemCall(long n, long a1, long a2, long a3, long a4, long a5, long a6) {
	...
	if (!posixMountPointBase) {
		...
		if (!posixMountPointBase) {
			...
			EsAssert(LT_SUCCESS == NodeOpen(EsLiteral("|POSIX:/"), LT_NODE_DIRECTORY | LT__NODE_DIRECTORY_WRITE, &node));
	...
	switch (n) {
		case SYS_open: {
			...
			returnValue = EsSyscall(LT_SYSCALL_POSIX, (uintptr_t) &syscall, 0, 0, 0);
			// EsPrint("SYS_open '%s' with handle %d\n", pathBytes, path, returnValue);
			EsHeapFree(path);
		} break;

		case SYS_vfork: {
			long result = EsSyscall(LT_SYSCALL_POSIX, (uintptr_t) &syscall, 0, 0, 0);
		...
		case SYS_getpid: {
			...
			EsObjectID id;
			EsSyscall(LT_SYSCALL_THREAD_GET_ID, LT_CURRENT_PROCESS, (uintptr_t) &id, 0, 0);
			returnValue = id;
		} break;

		case SYS_gettid: {
			returnValue = EsThreadGetID(LT_CURRENT_THREAD);
		} break;
		...
		case SYS_unlink: {
			...
			size_t pathBytes;
			char *path = EsPOSIXConvertPath((const char *) a1, &pathBytes, false);
			EsError error = EsSyscall(LT_SYSCALL_NODE_OPEN, (uintptr_t) path, pathBytes, LT_NODE_FAIL_IF_NOT_FOUND | LT_FILE_WRITE, (uintptr_t) &node);
			EsHeapFree(path);
			if (error == LT_ERROR_FILE_DOLT_NOT_EXIST) returnValue = -ENOENT;
			else if (error == LT_ERROR_PATH_NOT_TRAVERSABLE) returnValue = -ENOTDIR;
			else if (error == LT_ERROR_OPERATION_BLOCKED) returnValue = -EBUSY;
			else if (error == LT_ERROR_HARDWARE_FAILURE || error == LT_ERROR_CORRUPT_DATA) returnValue = -EIO;
			else if (error != LT_SUCCESS) returnValue = -EACCES;
			else {
				error = EsSyscall(LT_SYSCALL_NODE_DELETE, node.handle, 0, 0, 0);
				EsHandleClose(node.handle);
				if (error == LT_ERROR_HARDWARE_FAILURE || error == LT_ERROR_CORRUPT_DATA) returnValue = -EIO;
				else if (error != LT_SUCCESS) returnValue = -EACCES;
			}
		} break;
		...
		case SYS_truncate: {
			...
			else if (node.type == LT_NODE_DIRECTORY) { returnValue = -EISDIR; EsHandleClose(node.handle); }
			else {
		...
		case SYS_readlink: {
			if (0 == EsMemoryCompare((void *) a1, EsLiteral("/proc/self/fd/"))) {
				...
				syscall.index = LT_POSIX_SYSCALL_GET_POSIX_FD_PATH;
		...
		case SYS_mmap: {
			...
			if (a4 & MAP_FIXED) {
				...
				returnValue = (long) EsMemoryReserve(a2, LT_MEMORY_PROTECTION_READ_WRITE, none ? 0 : LT_MEMORY_RESERVE_COMMIT_ALL);
			} else {
		...
		case SYS_munmap: {
			...
			if (length == 0 || ((uintptr_t) address & (LT_PAGE_SIZE - 1))) {
				returnValue = -EINVAL;
			} else {
		...
		case SYS_prlimit64: {
			...
			if (a1 && a1 != (long) EsProcessGetID(LT_CURRENT_PROCESS)) {
				returnValue = -EPERM;
				break;
			}
			...
			if (a2 == RLIMIT_STACK) {
				...
				EsError error = EsSyscall(LT_SYSCALL_THREAD_STACK_SIZE, LT_CURRENT_THREAD, 
				...
				if (error != LT_SUCCESS) returnValue = -EINVAL;
			...
			if (!foundChild) {
				...
				if (~options & 1 /* WNOHANG */) {
					if (a1 == -1) {
						...
						EsWait(handles, childProcesses.Length(), LT_WAIT_NO_TIMEOUT);
					} else {
				...
				for (uintptr_t i = 0; i < childProcesses.Length(); i++) {
					...
					if (state.flags & LT_PROCESS_STATE_ALL_THREADS_TERMINATED) {
		...
		case SYS_mkdir: {
			...
			if (error == LT_ERROR_INSUFFICIENT_RESOURCES) returnValue = -ENOMEM;
			else if (error == LT_ERROR_ALREADY_EXISTS) returnValue = -EEXIST;
			else if (error == LT_ERROR_PATH_NOT_TRAVERSABLE) returnValue = -ENOENT;
			else if (error == LT_ERROR_PATH_NOT_WITHIN_MOUNTED_VOLUME) returnValue = -ENOENT;
			else if (error == LT_ERROR_FILE_ON_READ_ONLY_VOLUME) returnValue = -EPERM;
			EsHeapFree(path);
		} break;

		case SYS_uname: {
			struct utsname *buffer = (struct utsname *) a1;
			EsCRTstrcpy(buffer->sysname, "Light");
		...
		case -1000: {
			...
			EsSyscall(LT_SYSCALL_THREAD_SET_TLS, a1, 0, 0, 0);
			tlsStorageOffset = -a2;
...
void EsPOSIXInitialise(int *argc, char ***argv) {
	...
	if (environmentHandle) {
		EsAssert(startupInformation->data.subsystemID == LT_SUBSYSTEM_ID_POSIX);
