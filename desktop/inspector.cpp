...
int InspectorElementItemCallback(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_HOVERED_START) {
	...
	} else if (message->type == LT_MSG_HOVERED_END || message->type == LT_MSG_DESTROY) {
...
void InspectorUpdateEditor(InspectorWindow *inspector) {
	...
	bool isStack = e && e->messageClass == ProcessPanelMessage && !(e->flags & (LT_PANEL_Z_STACK | LT_PANEL_TABLE | LT_PANEL_SWITCHER));
	bool alignHLeft = e ? (e->flags & LT_CELL_H_LEFT) : false, alignHRight = e ? (e->flags & LT_CELL_H_RIGHT) : false;
	bool alignHExpand = e ? (e->flags & LT_CELL_H_EXPAND) : false, alignHShrink = e ? (e->flags & LT_CELL_H_SHRINK) : false; 
	bool alignHPush = e ? (e->flags & LT_CELL_H_PUSH) : false;
	bool alignVTop = e ? (e->flags & LT_CELL_V_TOP) : false, alignVBottom = e ? (e->flags & LT_CELL_V_BOTTOM) : false;
	bool alignVExpand = e ? (e->flags & LT_CELL_V_EXPAND) : false, alignVShrink = e ? (e->flags & LT_CELL_V_SHRINK) : false; 
	bool alignVPush = e ? (e->flags & LT_CELL_V_PUSH) : false;
	bool stackHorizontal = isStack && (e->flags & LT_PANEL_HORIZONTAL);
	bool stackReverse = isStack && (e->flags & LT_PANEL_REVERSE);
...
int InspectorElementListCallback(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_LIST_VIEW_GET_CONTENT) {
		...
		if (column == 0) {
			...
			EsMessage m = *message;
			m.type = LT_MSG_GET_INSPECTOR_INFORMATION;
			EsMessageSend(entry->element, &m);
		}

		return LT_HANDLED;
	} else if (message->type == LT_MSG_LIST_VIEW_GET_INDENT) {
...
void InspectorNotifyElementPainted(EsElement *element, EsPainter *painter) {
	...
	EsRectangle bounds = LT_RECT_4(painter->offsetX, painter->offsetX + painter->width, 
	...
	if (entry->element == element) {
		...
		if ((element->flags & LT_CELL_FILL) != LT_CELL_FILL) {
...
INSPECTOR_ALIGN_COMMAND(InspectorHAlignLeft, LT_CELL_H_LEFT | LT_CELL_H_RIGHT, LT_CELL_H_LEFT, false);
INSPECTOR_ALIGN_COMMAND(InspectorHAlignCenter, LT_CELL_H_LEFT | LT_CELL_H_RIGHT, LT_CELL_H_LEFT | LT_CELL_H_RIGHT, false);
INSPECTOR_ALIGN_COMMAND(InspectorHAlignRight, LT_CELL_H_LEFT | LT_CELL_H_RIGHT, LT_CELL_H_RIGHT, false);
INSPECTOR_ALIGN_COMMAND(InspectorHAlignExpand, 0, LT_CELL_H_EXPAND, true);
INSPECTOR_ALIGN_COMMAND(InspectorHAlignShrink, 0, LT_CELL_H_SHRINK, true);
INSPECTOR_ALIGN_COMMAND(InspectorHAlignPush, 0, LT_CELL_H_PUSH, true);
INSPECTOR_ALIGN_COMMAND(InspectorVAlignTop, LT_CELL_V_TOP | LT_CELL_V_BOTTOM, LT_CELL_V_TOP, false);
INSPECTOR_ALIGN_COMMAND(InspectorVAlignCenter, LT_CELL_V_TOP | LT_CELL_V_BOTTOM, LT_CELL_V_TOP | LT_CELL_V_BOTTOM, false);
INSPECTOR_ALIGN_COMMAND(InspectorVAlignBottom, LT_CELL_V_TOP | LT_CELL_V_BOTTOM, LT_CELL_V_BOTTOM, false);
INSPECTOR_ALIGN_COMMAND(InspectorVAlignExpand, 0, LT_CELL_V_EXPAND, true);
INSPECTOR_ALIGN_COMMAND(InspectorVAlignShrink, 0, LT_CELL_V_SHRINK, true);
INSPECTOR_ALIGN_COMMAND(InspectorVAlignPush, 0, LT_CELL_V_PUSH, true);
INSPECTOR_ALIGN_COMMAND(InspectorDirectionLeft, LT_PANEL_HORIZONTAL | LT_PANEL_REVERSE | LT_ELEMENT_LAYOUT_HINT_HORIZONTAL | LT_ELEMENT_LAYOUT_HINT_REVERSE, 
		LT_PANEL_HORIZONTAL | LT_PANEL_REVERSE | LT_ELEMENT_LAYOUT_HINT_HORIZONTAL | LT_ELEMENT_LAYOUT_HINT_REVERSE, false);
INSPECTOR_ALIGN_COMMAND(InspectorDirectionRight, LT_PANEL_HORIZONTAL | LT_PANEL_REVERSE | LT_ELEMENT_LAYOUT_HINT_HORIZONTAL | LT_ELEMENT_LAYOUT_HINT_REVERSE, 
		LT_PANEL_HORIZONTAL | LT_ELEMENT_LAYOUT_HINT_HORIZONTAL, false);
INSPECTOR_ALIGN_COMMAND(InspectorDirectionUp, LT_PANEL_HORIZONTAL | LT_PANEL_REVERSE | LT_ELEMENT_LAYOUT_HINT_HORIZONTAL | LT_ELEMENT_LAYOUT_HINT_REVERSE, 
		LT_PANEL_REVERSE | LT_ELEMENT_LAYOUT_HINT_REVERSE, false);
INSPECTOR_ALIGN_COMMAND(InspectorDirectionDown, LT_PANEL_HORIZONTAL | LT_PANEL_REVERSE | LT_ELEMENT_LAYOUT_HINT_HORIZONTAL | LT_ELEMENT_LAYOUT_HINT_REVERSE, 0, false);
...
void InspectorVisualizeRepaints(EsInstance *instance, EsElement *, EsCommand *) {
	...
	EsButtonSetCheck(inspector->visualizeRepaints, window->visualizeRepaints ? LT_CHECK_CHECKED : LT_CHECK_UNCHECKED, false);
}
...
void InspectorVisualizePaintSteps(EsInstance *instance, EsElement *, EsCommand *) {
	...
	EsButtonSetCheck(inspector->visualizePaintSteps, window->visualizePaintSteps ? LT_CHECK_CHECKED : LT_CHECK_UNCHECKED, false);
}
...
void InspectorVisualizeLayoutBounds(EsInstance *instance, EsElement *, EsCommand *) {
	...
	EsButtonSetCheck(inspector->visualizeLayoutBounds, window->visualizeLayoutBounds ? LT_CHECK_CHECKED : LT_CHECK_UNCHECKED, false);
...
void InspectorAddElement(EsInstance *, EsElement *element, EsCommand *) {
	EsMenu *menu = EsMenuCreate(element, LT_FLAGS_DEFAULT);
...
void InspectorSetup(EsWindow *window) {
	...
	EsSplitter *splitter = EsSplitterCreate(window, LT_CELL_FILL | LT_SPLITTER_VERTICAL);
	EsPanel *panel1 = EsPanelCreate(splitter, LT_CELL_FILL, LT_STYLE_PANEL_FILLED);
	EsPanel *panel2 = EsPanelCreate(splitter, LT_CELL_FILL, LT_STYLE_PANEL_FILLED);

	{
		EsPanel *toolbar = EsPanelCreate(panel1, LT_CELL_H_FILL | LT_PANEL_HORIZONTAL, LT_STYLE_PANEL_TOOLBAR);
		inspector->visualizeRepaints = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR, 0, "Visualize repaints");
		EsButtonOnCommand(inspector->visualizeRepaints, InspectorVisualizeRepaints);
		inspector->visualizeLayoutBounds = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR, 0, "Visualize layout bounds");
		EsButtonOnCommand(inspector->visualizeLayoutBounds, InspectorVisualizeLayoutBounds);
		inspector->visualizePaintSteps = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR, 0, "Visualize paint steps");
		EsButtonOnCommand(inspector->visualizePaintSteps, InspectorVisualizePaintSteps);
		EsSpacerCreate(toolbar, LT_CELL_H_FILL);
	}

	inspector->elementList = EsListViewCreate(panel1, LT_CELL_FILL | LT_LIST_VIEW_COLUMNS | LT_LIST_VIEW_SINGLE_SELECT);
...

	{
		...
		EsTextDisplayCreate(toolbar, LT_FLAGS_DEFAULT, 0, "Horizontal:");
		EsSpacerCreate(toolbar, LT_FLAGS_DEFAULT, 0, 5, 0);
		inspector->alignH[0] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED);
		EsButtonSetIcon(inspector->alignH[0], LT_ICON_ALIGN_HORIZONTAL_LEFT);
		EsButtonOnCommand(inspector->alignH[0], InspectorHAlignLeft);
		inspector->alignH[1] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED);
		EsButtonSetIcon(inspector->alignH[1], LT_ICON_ALIGN_HORIZONTAL_CENTER);
		EsButtonOnCommand(inspector->alignH[1], InspectorHAlignCenter);
		inspector->alignH[2] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED);
		EsButtonSetIcon(inspector->alignH[2], LT_ICON_ALIGN_HORIZONTAL_RIGHT);
		EsButtonOnCommand(inspector->alignH[2], InspectorHAlignRight);
		inspector->alignH[3] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED, 0, "Expand");
		EsButtonOnCommand(inspector->alignH[3], InspectorHAlignExpand);
		inspector->alignH[4] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED, 0, "Shrink");
		EsButtonOnCommand(inspector->alignH[4], InspectorHAlignShrink);
		inspector->alignH[5] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED, 0, "Push");
		EsButtonOnCommand(inspector->alignH[5], InspectorHAlignPush);
		EsSpacerCreate(toolbar, LT_FLAGS_DEFAULT, 0, 5, 0);
		EsTextDisplayCreate(toolbar, LT_FLAGS_DEFAULT, 0, "Vertical:");
		EsSpacerCreate(toolbar, LT_FLAGS_DEFAULT, 0, 5, 0);
		inspector->alignV[0] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED);
		EsButtonSetIcon(inspector->alignV[0], LT_ICON_ALIGN_VERTICAL_TOP);
		EsButtonOnCommand(inspector->alignV[0], InspectorVAlignTop);
		inspector->alignV[1] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED);
		EsButtonSetIcon(inspector->alignV[1], LT_ICON_ALIGN_VERTICAL_CENTER);
		EsButtonOnCommand(inspector->alignV[1], InspectorVAlignCenter);
		inspector->alignV[2] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED);
		EsButtonSetIcon(inspector->alignV[2], LT_ICON_ALIGN_VERTICAL_BOTTOM);
		EsButtonOnCommand(inspector->alignV[2], InspectorVAlignBottom);
		inspector->alignV[3] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED, 0, "Expand");
		EsButtonOnCommand(inspector->alignV[3], InspectorVAlignExpand);
		inspector->alignV[4] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED, 0, "Shrink");
		EsButtonOnCommand(inspector->alignV[4], InspectorVAlignShrink);
		inspector->alignV[5] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED, 0, "Push");
...

	{
		EsPanel *toolbar = EsPanelCreate(panel1, LT_CELL_H_FILL | LT_PANEL_HORIZONTAL, LT_STYLE_PANEL_TOOLBAR);
		EsSpacerCreate(toolbar, LT_FLAGS_DEFAULT, 0, 5, 0);
		EsTextDisplayCreate(toolbar, LT_FLAGS_DEFAULT, 0, "Stack:");
		EsSpacerCreate(toolbar, LT_FLAGS_DEFAULT, 0, 5, 0);
		inspector->direction[0] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED);
		EsButtonSetIcon(inspector->direction[0], LT_ICON_GO_PREVIOUS);
		EsButtonOnCommand(inspector->direction[0], InspectorDirectionLeft);
		inspector->direction[1] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED);
		EsButtonSetIcon(inspector->direction[1], LT_ICON_GO_NEXT);
		EsButtonOnCommand(inspector->direction[1], InspectorDirectionRight);
		inspector->direction[2] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED);
		EsButtonSetIcon(inspector->direction[2], LT_ICON_GO_UP);
		EsButtonOnCommand(inspector->direction[2], InspectorDirectionUp);
		inspector->direction[3] = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_ELEMENT_DISABLED);
		EsButtonSetIcon(inspector->direction[3], LT_ICON_GO_DOWN);
		EsButtonOnCommand(inspector->direction[3], InspectorDirectionDown);
		EsSpacerCreate(toolbar, LT_FLAGS_DEFAULT, 0, 25, 0);
		inspector->addChildButton = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_BUTTON_DROPDOWN | LT_ELEMENT_DISABLED | LT_BUTTON_COMPACT, 0, "Add child... ");
		EsButtonOnCommand(inspector->addChildButton, InspectorAddElement);
		inspector->addSiblingButton = EsButtonCreate(toolbar, LT_BUTTON_TOOLBAR | LT_BUTTON_DROPDOWN | LT_ELEMENT_DISABLED | LT_BUTTON_COMPACT, 0, "Add sibling... ");
...

	{
		EsPanel *toolbar = EsPanelCreate(panel1, LT_CELL_H_FILL | LT_PANEL_HORIZONTAL, LT_STYLE_PANEL_TOOLBAR);
		EsSpacerCreate(toolbar, LT_FLAGS_DEFAULT, 0, 5, 0);
		EsTextDisplayCreate(toolbar, LT_FLAGS_DEFAULT, 0, "Content:");
		inspector->contentTextbox = EsTextboxCreate(toolbar, LT_ELEMENT_DISABLED | LT_TEXTBOX_EDIT_BASED);
		inspector->contentTextbox->messageUser = InspectorContentTextboxCallback;
		EsSpacerCreate(toolbar, LT_FLAGS_DEFAULT, 0, 25, 0);
		EsTextDisplayCreate(toolbar, LT_FLAGS_DEFAULT, 0, "Event category filter:");
		inspector->textboxCategoryFilter = EsTextboxCreate(toolbar, LT_ELEMENT_DISABLED);
		inspector->textboxCategoryFilter->messageUser = InspectorTextboxCategoryFilterCallback;
	}

	{
		inspector->listEvents = EsListViewCreate(panel2, LT_CELL_FILL | LT_LIST_VIEW_CHOICE_SELECT | LT_LIST_VIEW_FIXED_ITEMS, LT_STYLE_LIST_CHOICE_BORDERED);
	}
