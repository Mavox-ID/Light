...
void FontShapeText(EsTextPlan *plan, const char *string, size_t stringBytes, 
	...
	if (plan->font.type == FONT_TYPE_BITMAP) {
		...
		while (text < string + sectionOffsetBytes + sectionCountBytes) {
			...
			LT_MACRO_SEARCH(header->glyphCount, result = codepoint - ((const BitmapFontGlyph *) ((const uint8_t *) 
...
void FontInitialiseShaping(EsTextPlan *plan) {
	...
	plan->segmentProperties.direction = (plan->properties.flags & LT_TEXT_PLAN_RTL) ? HB_DIRECTION_RTL : HB_DIRECTION_LTR;
...
EsFontFamily FontApplySubstitution(EsTextPlanProperties *properties, EsFontFamily family, uint32_t script) {
	...
	if (properties->flags & LT_TEXT_PLAN_NO_FONT_SUBSTITUTION) {
		return family;
	}
...
Font FontGet(EsFont key) {
	...
	for (uintptr_t i = 0; i < 18; i++) {
		if (entry->files[i]) {
			int weight = (i % 9) + 1;
			bool italic = i >= 9;
			int distance = ((italic != ((key.flags & LT_FONT_ITALIC) ? true : false)) ? 10 : 0) + AbsoluteInteger(weight - key.weight);
	...
	size_t size;
	void *data = EsFileStoreMap(file, &size, LT_MEMORY_MAP_OBJECT_READ_ONLY);
...
IconPackImage *IconPackReadImage(uint32_t id, uint32_t size, int *type) {
	...
	*type = (EsBufferReadInt(&iconManagement.pack) == 1) ? CHARACTER_RECOLOR : CHARACTER_IMAGE;
	...
	// bool rtl = api.systemConstants[LT_SYSTEM_CONSTANT_RIGHT_TO_LEFT];
	bool rtl = false;
	bool found = false;
	uint32_t variant = 0;
...
bool EsDrawStandardIcon(EsPainter *painter, uint32_t id, int size, EsRectangle region, uint32_t color) {
	...
	DrawSingleCharacter(cacheEntry->width, cacheEntry->height, 0, 0, 
			LT_POINT(left, top), region, painter->target,  
...
void TextTrimSpaces(EsTextPlan *plan) {
	if (~plan->properties.flags & LT_TEXT_PLAN_TRIM_SPACES) {
		return;
	}
...
void TextPlaceEmergencyBreaks(EsTextPlan *plan, int32_t maximumLineWidth) {
	if ((plan->properties.flags & LT_TEXT_PLAN_CLIP_UNBREAKABLE_LINES) || maximumLineWidth == -1) {
		return;
	}
...
void TextAddEllipsis(EsTextPlan *plan, int32_t maximumLineWidth, bool needFinalEllipsis, int32_t boundsWidth) {
	if (!boundsWidth || (~plan->properties.flags & LT_TEXT_ELLIPSIS)) {
		return;
	}
...
#ifdef USE_FREETYPE_AND_HARFBUZZ
		if (plan->currentTextStyle->figures == LT_TEXT_FIGURE_OLD) hb_feature_from_string("onum", -1, features + (featureCount++));
		if (plan->currentTextStyle->figures == LT_TEXT_FIGURE_TABULAR) hb_feature_from_string("tnum", -1, features + (featureCount++));
#endif
...
EsTextPlan *EsTextPlanCreate(EsElement *element, EsTextPlanProperties *properties, EsRectangle bounds, const char *string, const EsTextRun *formatRuns, size_t formatRunCount) {
	...
	plan.string = string;
	plan.singleUse = properties->flags & LT_TEXT_PLAN_SINGLE_USE;
	...
	int32_t maximumLineWidth = Width(bounds) && (properties->flags & LT_TEXT_WRAP) ? Width(bounds) * FREETYPE_UNIT_SCALE : -1;
	Break previousBreak = {};
	bool needEllipsis = false;
	...
	while (previousBreak.position != plan.breaker.bytes) {
		...
		while (!plan.breaker.error && !nextBreak.forced && (~properties->flags & LT_TEXT_WRAP)) {
...
void DrawTextPiece(EsPainter *painter, EsTextPlan *plan, TextPiece *piece, TextLine *line,
	...
	for (uintptr_t i = 0; i < piece->glyphCount; i++) {
		...
		DrawSingleCharacter(entry->width, entry->height, entry->xoff, entry->yoff, 
				LT_POINT(positionX, positionY + line->ascent / FREETYPE_UNIT_SCALE), 
	...
	if (!selection->hideCaret && caret >= piece->start 
		...
		EsDrawInvert(painter, LT_RECT_4(caretX, caretX + caretWidth, caretY, selectionBackgroundBottom));
	}
...

	{
		...
		if (piece->style->decorations & LT_TEXT_DECORATION_STRIKE_THROUGH) {
		...
		if (piece->style->decorations & LT_TEXT_DECORATION_UNDERLINE) {
...
void EsDrawText(EsPainter *painter, EsTextPlan *plan, EsRectangle bounds, const EsRectangle *_clip, const EsTextSelection *_selection) {
	...
	int32_t cursorY = (plan->properties.flags & LT_TEXT_V_CENTER) ? (maximumHeight - EsTextPlanGetHeight(plan)) / 2 
		: (plan->properties.flags & LT_TEXT_V_BOTTOM) ? maximumHeight - EsTextPlanGetHeight(plan) : 0;
	...
	for (uintptr_t i = 0; i < plan->lines.Length(); i++) {
		...
		int32_t cursorX = (plan->properties.flags & LT_TEXT_H_CENTER) ? ((maximumLineWidth * FREETYPE_UNIT_SCALE - line->width) / 2)
			: (plan->properties.flags & LT_TEXT_H_RIGHT) ? (maximumLineWidth * FREETYPE_UNIT_SCALE - line->width) : 0;

		int32_t selectionBackgroundBottom;
		
		if (plan->lines.Length() == 1 && (plan->properties.flags & LT_TEXT_V_CENTER)) {
...
void EsDrawTextSimple(EsPainter *painter, EsElement *element, EsRectangle bounds, const char *string, ptrdiff_t stringBytes, EsTextStyle style, uint32_t flags) {
	EsTextPlanProperties properties = {};
	properties.flags = LT_TEXT_PLAN_SINGLE_USE | flags;
	EsTextRun textRuns[2] = {};
		...
		if (inString[i] == '\a') {
			...
			for (; i < inStringBytes; i++) {
				...
				if (c == ']') {
					...
					textRun->style.font.flags |= LT_FONT_ITALIC;
				} else if (c == 's' /* size */) {
					...
					textRun->style.font.family = LT_FONT_MONOSPACED;
				} else if (c == '-' /* strike-through */) {
					textRun->style.decorations |= LT_TEXT_DECORATION_STRIKE_THROUGH;
				} else if (c == '_' /* underline */) {
					textRun->style.decorations |= LT_TEXT_DECORATION_UNDERLINE;
...
Array<EsTextRun> TextApplySyntaxHighlighting(const EsTextStyle *baseStyle, int language, uint32_t *colors, Array<EsTextRun> runs, const char *string, size_t bytes) {
	...
	for (uintptr_t index = 0; index < bytes; index++) {
		...
		if (language == LT_SYNTAX_HIGHLIGHTING_LANGUAGE_C || language == LT_SYNTAX_HIGHLIGHTING_LANGUAGE_SCRIPT) {
			if (lexState == 4) {
				lexState = 0;
			} else if (lexState == 1) {
			...
			} else if (lexState == 3 || lexState == 6 || (lexState == 5 && language == LT_SYNTAX_HIGHLIGHTING_LANGUAGE_SCRIPT)) {
		...
		} else if (language == LT_SYNTAX_HIGHLIGHTING_LANGUAGE_INI) {
			if (c == ';' && !index) {
				lexState = 1;
			} else if (c == '[' && !index) {
				lexState = 6;
