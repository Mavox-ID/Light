...
uintptr_t APISyscallCheckForCrash(uintptr_t argument0, uintptr_t argument1, uintptr_t argument2, uintptr_t unused, uintptr_t argument3, uintptr_t argument4) {
	...
	_APISyscall(LT_SYSCALL_PROCESS_GET_STATE, LT_CURRENT_PROCESS, (uintptr_t) &state, 0, 0, 0);
	volatile int x = 1;
	if (state.flags & LT_PROCESS_STATE__PAUSED_FROM_CRASH) while (x);
	return returnValue;
}
#endif
...
void EsApplicationStart(EsInstance *instance, const EsApplicationStartupRequest *request) {
	...
	if (!buffer.error) {
		MessageDesktop(buffer.out, buffer.position, instance ? instance->window->handle : LT_INVALID_HANDLE);
	}
...
void EsInstanceSetClassEditor(EsInstance *_instance, const EsInstanceClassEditorSettings *settings) {
	...
	instance->instanceClass = LT_INSTANCE_CLASS_EDITOR;
...
void EsInstanceSetClassViewer(EsInstance *_instance, const EsInstanceClassViewerSettings *) {
	APIInstance *instance = (APIInstance *) _instance->_private;
	instance->instanceClass = LT_INSTANCE_CLASS_VIEWER;
}
...
void EsMessageMutexAcquire() {
	...
	api.messageMutexThreadID = EsThreadGetID(LT_CURRENT_THREAD);
}
...
void EsMessageMutexCheck() {
	EsAssert(api.messageMutexThreadID == EsThreadGetID(LT_CURRENT_THREAD)); // Expected message mutex to be acquired.
}
...
int EsMessageSend(EsElement *element, EsMessage *message) {
	...
	if (message->type >= LT_MSG_STATE_CHANGE_MESSAGE_START && message->type <= LT_MSG_STATE_CHANGE_MESSAGE_END) {
...
void InstanceClose(EsInstance *instance) {
	if (!EsCommandByID(instance, LT_COMMAND_SAVE)->enabled) {
	...
	EsDialog *dialog = EsDialogShow(instance->window, cTitle, -1, content, contentBytes, LT_ICON_DIALOG_WARNING);

	if (!apiInstance->startupInformation->filePathBytes) {
		EsPanel *row = EsPanelCreate(dialog->contentArea, LT_PANEL_HORIZONTAL, LT_STYLE_PANEL_FORM_TABLE);
		EsTextDisplayCreate(row, LT_FLAGS_DEFAULT, LT_STYLE_TEXT_LABEL, INTERFACE_STRING(FileCloseNewName));
	...
	EsDialogAddButton(dialog, LT_BUTTON_CANCEL, 0, INTERFACE_STRING(CommonCancel), 
			[] (EsInstance *instance, EsElement *, EsCommand *) { 
		EsDialogClose(instance->window->dialogs.Last()); 
	});

	EsDialogAddButton(dialog, LT_FLAGS_DEFAULT, LT_STYLE_PUSH_BUTTON_DANGEROUS, INTERFACE_STRING(FileCloseWithModificationsDelete), 
			[] (EsInstance *instance, EsElement *, EsCommand *) { 
		EsDialogClose(instance->window->dialogs.Last()); 
		EsInstanceClose(instance);
	});

	EsButton *button = EsDialogAddButton(dialog, LT_BUTTON_DEFAULT, 0, INTERFACE_STRING(FileCloseWithModificationsSave), 
			[] (EsInstance *instance, EsElement *, EsCommand *) { 
		EsDialogClose(instance->window->dialogs.Last()); 
...
APIInstance *InstanceSetup(EsInstance *instance) {
	...
	EsCommandRegister(&apiInstance->commandDelete, instance, INTERFACE_STRING(CommonSelectionDelete), nullptr, LT_COMMAND_DELETE, "Del");
	EsCommandRegister(&apiInstance->commandSelectAll, instance, INTERFACE_STRING(CommonSelectionSelectAll), nullptr, LT_COMMAND_SELECT_ALL, "Ctrl+A");
	EsCommandRegister(&apiInstance->commandCopy, instance, INTERFACE_STRING(CommonClipboardCopy), nullptr, LT_COMMAND_COPY, "Ctrl+C|Ctrl+Ins");
	EsCommandRegister(&apiInstance->commandCut, instance, INTERFACE_STRING(CommonClipboardCut), nullptr, LT_COMMAND_CUT, "Ctrl+X|Shift+Del");
	EsCommandRegister(&apiInstance->commandPaste, instance, INTERFACE_STRING(CommonClipboardPaste), nullptr, LT_COMMAND_PASTE, "Ctrl+V|Shift+Ins");
	EsCommandRegister(&apiInstance->commandUndo, instance, INTERFACE_STRING(CommonUndo), nullptr, LT_COMMAND_UNDO, "Ctrl+Z");
	EsCommandRegister(&apiInstance->commandRedo, instance, INTERFACE_STRING(CommonRedo), nullptr, LT_COMMAND_REDO, "Ctrl+Y");
	EsCommandRegister(&apiInstance->commandSave, instance, INTERFACE_STRING(CommonFileSave), nullptr, LT_COMMAND_SAVE, "Ctrl+S");
	EsCommandRegister(&apiInstance->commandShowInFileManager, instance, INTERFACE_STRING(CommonFileShowInFileManager), nullptr, LT_COMMAND_SHOW_IN_FILE_MANAGER, "Ctrl+Shift+O");
...
EsInstance *_EsInstanceCreate(size_t bytes, EsMessage *message, const char *applicationName, ptrdiff_t applicationNameBytes) {
	...
	if (message && message->createInstance.data != LT_INVALID_HANDLE && message->createInstance.dataBytes > 1) {
	...
	if (message) {
		...
		instance->window = EsWindowCreate(instance, LT_WINDOW_NORMAL);
		...
		if (apiInstance->startupInformation && apiInstance->startupInformation->readHandle) {
			...
			// TODO Can the posted message be raced by a LT_MSG_INSTANCE_DOCUMENT_UPDATED?
			EsMessage m = { LT_MSG_INSTANCE_OPEN_DELAYED };
...
void EsInstanceCloseReference(EsInstance *_instance) {
	...
	if (!instance->referenceCount) {
		if (_instance->callback) {
			EsMessage m = {};
			m.type = LT_MSG_INSTANCE_DESTROY;
...
void InstanceSendOpenMessage(EsInstance *instance, bool update) {
	...
	EsMessage m = { .type = LT_MSG_INSTANCE_OPEN };
...
EsError GetMessage(_EsMessageWithObject *message) {
	...
	// so that messages like LT_MSG_WINDOW_DESTROYED are received last.
	...
	if (gotMessage) return LT_SUCCESS;

	return EsSyscall(LT_SYSCALL_MESSAGE_GET, (uintptr_t) message, 0, 0, 0);
}
...
EsMessage *EsMessageReceive() {
	...
	while (true) {
		if (message.message.type == LT_MSG_INSTANCE_CREATE) {
			if (message.message.createInstance.data != LT_INVALID_HANDLE) {
				EsHandleClose(message.message.createInstance.data);
			}
		} else if (message.message.type == LT_MSG_APPLICATION_EXIT) {
			if (api.startupInformation->isDesktop) {
				...
				EsPrint("LT_MSG_APPLICATION_EXIT - Heap allocation count: %d (%d from malloc).\n", heap.allocationsCount, mallocCount);
#endif
		...
		while (error != LT_SUCCESS) {
			if (!gui.animationSleep && gui.animatingElements.Length()) {
				EsMessageMutexAcquire();
				ProcessAnimations();
				EsMessageMutexRelease();
			} else {
				EsSyscall(LT_SYSCALL_MESSAGE_WAIT, LT_WAIT_NO_TIMEOUT, 0, 0, 0);
			}
		...
		if (type == LT_MSG_EYEDROP_REPORT) {
...
#endif
		} else if (type == LT_MSG_TAB_INSPECT_UI) {
			...
			if (_instance) {
				...
				if (instance->attachedInspector) {
					EsSyscall(LT_SYSCALL_WINDOW_SET_PROPERTY, instance->attachedInspector->window->handle, 0, 0, LT_WINDOW_PROPERTY_FOCUSED);
				} else {
					EsWindowCreate(_instance, LT_WINDOW_INSPECTOR);
				}
			}
		} else if (type == LT_MSG_TAB_CLOSE_REQUEST) {
			EsInstance *instance = InstanceFromWindowID(message.message.tabOperation.id);

			if (instance) {
				InstanceClose(instance);
			}
		} else if (type == LT_MSG_INSTANCE_SAVE_RESPONSE) {
			// TODO Support multithreaded file operations.

			EsMessage m = {};
			m.type = LT_MSG_INSTANCE_SAVE;
			...
			if (m.instanceSave.file) {
				...
				if (m.instanceSave.file->error == LT_SUCCESS && _instance->callback && _instance->callback(_instance, &m)) {
		...
		} else if (type == LT_MSG_INSTANCE_RENAME_RESPONSE) {
			EsInstance *instance = InstanceFromWindowID(message.message.tabOperation.id);

			if (instance) {
				APIInstance *apiInstance = (APIInstance *) instance->_private;

				if (message.message.tabOperation.error == LT_SUCCESS) {
					EsAnnouncementShow(instance->window, LT_FLAGS_DEFAULT, -1, -1, INTERFACE_STRING(FileRenameSuccess));
				} else {
					char buffer[512];
					...
					switch (message.message.tabOperation.error) {
						case LT_ERROR_FILE_DOLT_NOT_EXIST: 
						case LT_ERROR_NODE_DELETED: 
						case LT_ERROR_PERMISSION_NOT_GRANTED: 
						case LT_ERROR_INCORRECT_NODE_TYPE:
							errorMessage = interfaceString_FileSaveErrorFileDeleted;
							break;
						case LT_ERROR_CORRUPT_DATA:
							errorMessage = interfaceString_FileSaveErrorCorrupt;
							break;
						case LT_ERROR_HARDWARE_FAILURE:
							errorMessage = interfaceString_FileSaveErrorDrive;
							break;
						case LT_ERROR_INSUFFICIENT_RESOURCES:
							errorMessage = interfaceString_FileSaveErrorResourcesLow;
							break;

						case LT_ERROR_ALREADY_EXISTS: {
							errorMessage = buffer;
					...
					EsDialogShow(instance->window, INTERFACE_STRING(FileCannotRename), 
							errorMessage, errorMessageBytes, LT_ICON_DIALOG_ERROR, LT_DIALOG_ALERT_OK_BUTTON);
				}

				EsHeapFree(apiInstance->newName);
				apiInstance->newName = nullptr;
			}
		} else if (type == LT_MSG_INSTANCE_DOCUMENT_RENAMED) {
			...
			EsHeapFree(buffer);
			EsHandleClose(message.message.tabOperation.handle);
		} else if (type == LT_MSG_INSTANCE_DOCUMENT_UPDATED) {
			EsInstance *_instance = InstanceFromWindowID(message.message.tabOperation.id);
		...
		} else if (type == LT_MSG_DEVICE_CONNECTED) {
			api.connectedDevices.Add(message.message.device);
			return &message.message;
		} else if (type == LT_MSG_DEVICE_DISCONNECTED) {
...
void EsInstanceSetModified(EsInstance *instance, bool modified) {
	if (((APIInstance *) instance->_private)->instanceClass != LT_INSTANCE_CLASS_EDITOR) {
		// The modified flag should only be used if this is an editor-style application.
		return;
	}

	EsCommandSetEnabled(EsCommandByID(instance, LT_COMMAND_SAVE), modified);
...
void EsInstanceOpenComplete(EsInstance *instance, EsFileStore *file, bool success, const char *errorText, ptrdiff_t errorTextBytes) {
	if (!success || file->error != LT_SUCCESS) {
		if (errorText && errorTextBytes) {
			...
			EsDialogShow(instance->window, INTERFACE_STRING(FileCannotOpen), 
					errorMessage, -1, LT_ICON_DIALOG_ERROR, LT_DIALOG_ALERT_OK_BUTTON);
		}
...
void EsInstanceSaveComplete(EsInstance *instance, EsFileStore *file, bool success) {
	...
	if (success) {
		file->error = EsFileControl(file->handle, LT_FILE_CONTROL_FLUSH, nullptr, 0);
	...
	if (instance) {
		...
		EsMessage m = { .type = LT_MSG_INSTANCE_SAVE_COMPLETE_DELAYED, ._argument = instance };
		EsInstanceOpenReference(instance);
		EsMessagePost(nullptr, &m); 
		...
		if (success) {
			...
			EsAnnouncementShow(instance->window, LT_FLAGS_DEFAULT, -1, -1, message, messageBytes);
			EsHeapFree(message);
			EsCommandSetDisabled(&apiInstance->commandShowInFileManager, false);
...
uintptr_t EsSystemGetOptimalWorkQueueThreadCount() {
	return EsSyscall(LT_SYSCALL_PROCESSOR_COUNT, 0, 0, 0, 0);
}

__attribute__((no_instrument_function))
void ThreadInitialise(ThreadLocalStorage *local) {
	EsMemoryZero(local, sizeof(ThreadLocalStorage));
	EsSyscall(LT_SYSCALL_THREAD_GET_ID, LT_CURRENT_THREAD, (uintptr_t) &local->id, 0, 0);
	local->self = local;
	EsSyscall(LT_SYSCALL_THREAD_SET_TLS, (uintptr_t) local - tlsStorageOffset, 0, 0, 0);
	EsSyscall(LT_SYSCALL_THREAD_SET_TIMER_ADJUST_ADDRESS, (uintptr_t) &local->timerAdjustTicks, 0, 0, 0);
}
...
extern "C" void _start(EsProcessStartupInformation *_startupInformation) {
	...
	if (isDesktop) {
		...
		EsMemoryCopy(LT_API_BASE, apiTable, sizeof(apiTable));
	}
	...
	api.global = (GlobalData *) EsMemoryMapObject(api.startupInformation->globalDataRegion, 
			0, sizeof(GlobalData), isDesktop ? LT_MEMORY_MAP_OBJECT_READ_WRITE : LT_MEMORY_MAP_OBJECT_READ_ONLY);
	theming.scale = api.global->uiScale; // We'll receive LT_MSG_UI_SCALE_CHANGED when this changes.
	...
	if (isDesktop) {
		...
		NodeOpen(path, EsCStringLength(path), LT_NODE_DIRECTORY, &node);
		...
		if (api.desktopRequestPipe) {
			...
			MessageDesktop(&m, 1, LT_INVALID_HANDLE, &responseBuffer);
	...
	EsThreadTerminate(LT_CURRENT_THREAD);
}
...
void EsPanic(const char *cFormat, ...) {
	...
	while (x);
#endif
	EsSyscall(LT_SYSCALL_PROCESS_CRASH, 0, 0, 0, 0);
}
...
uint32_t EsIconIDFromDriveType(uint8_t driveType) {
	if (driveType == LT_DRIVE_TYPE_HDD             ) return LT_ICON_DRIVE_HARDDISK;
	if (driveType == LT_DRIVE_TYPE_SSD             ) return LT_ICON_DRIVE_HARDDISK_SOLIDSTATE;
	if (driveType == LT_DRIVE_TYPE_CDROM           ) return LT_ICON_MEDIA_OPTICAL;
	if (driveType == LT_DRIVE_TYPE_USB_MASS_STORAGE) return LT_ICON_DRIVE_REMOVABLE_MEDIA_USB;
	return LT_ICON_DRIVE_HARDDISK;
}
...
void EsUndoPop(EsUndoManager *manager) {
	...
	UndoItemFooter *footer = (UndoItemFooter *) (stack->array + stack->Length()) - 1;
	EsMessage m = {};
	m.type = LT_MSG_UNDO_CANCEL;
	footer->callback((uint8_t *) footer - footer->bytes, manager, &m);
	stack->SetLength(oldLength - footer->bytes - sizeof(UndoItemFooter));
	EsCommandSetDisabled(EsCommandByID(manager->instance, LT_COMMAND_UNDO), !manager->undoStack.Length());
	EsCommandSetDisabled(EsCommandByID(manager->instance, LT_COMMAND_REDO), !manager->redoStack.Length());
}
...
void UserTaskThread(EsGeneric _task) {
	...
	EsMessageMutexAcquire();
	EsMessageMutexRelease();
	EsSyscall(LT_SYSCALL_WINDOW_CLOSE, task->taskHandle, 0, 0, 0);
...
EsError EsUserTaskStart(EsUserTaskCallback callback, EsGeneric data, const char *title, ptrdiff_t titleBytes, uint32_t iconID) {
	...
	MessageDesktop(&m, 1, LT_INVALID_HANDLE, &response);
	EsHandle handle;
...
void TimersThread(EsGeneric) {
	...
	while (true) {
		...
		int64_t waitFor = LT_WAIT_NO_TIMEOUT;
...
void WorkThread(EsGeneric) {
	while (true) {
		EsWait(&api.workAvailable, 1, LT_WAIT_NO_TIMEOUT);
...
EsError EsWorkQueue(EsWorkCallback callback, EsGeneric context) {
	...
	return success ? LT_SUCCESS : LT_ERROR_INSUFFICIENT_RESOURCES;
}
...
void EsPOSIXInitialise(int *, char ***) {
	while (true) {
		...
		if (message->type == LT_MSG_INSTANCE_CREATE) {
			...
			EsPanel *panel = EsPanelCreate((EsElement *) instance->window, LT_PANEL_VERTICAL | LT_CELL_FILL, LT_STYLE_PANEL_WINDOW_BACKGROUND);
			EsTextDisplayCreate(panel, LT_CELL_H_CENTER | LT_CELL_V_FILL | LT_TEXT_DISPLAY_RICH_TEXT, 0, INTERFACE_STRING(POSIXUnavailable));
		}
	}
}
