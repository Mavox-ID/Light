...
bool BasicFileOperations() {
	...
	if (error != LT_ERROR_FILE_DOLT_NOT_EXIST) {
...
bool CRTMathFunctions() {
	...
	for (float i = 0.0f; i <= 10.0f; i += 0.1f) CHECK(AreFloatsRoughlyEqual(EsCRTcosf(i), EsCRTcosf(i + 2 * LT_PI)));
	for (double i = 0.0; i <= 10.0; i += 0.1) CHECK(AreDoublesRoughlyEqual(EsCRTcos(i), EsCRTcos(-i)));
	for (double i = 0.0; i <= 10.0; i += 0.1) CHECK(AreDoublesRoughlyEqual(EsCRTcos(i), EsCRTcos(i + 2 * LT_PI)));
	for (double i = 0.0; i <= 10.0; i += 0.1) CHECK(AreFloatsRoughlyEqual(EsCRTcos(i), EsCRTcosf(i)));
	CHECK(AreFloatsRoughlyEqual(EsCRTacosf(-1.0f), LT_PI));
	CHECK(AreFloatsRoughlyEqual(EsCRTacosf(0.0f), LT_PI / 2.0f));
	CHECK(AreFloatsRoughlyEqual(EsCRTacosf(1.0f), 0.0f));
	CHECK(AreFloatsRoughlyEqual(EsCRTcosf(0.0f), 1.0f));
	CHECK(AreFloatsRoughlyEqual(EsCRTcosf(LT_PI / 2.0f), 0.0f));
	CHECK(AreFloatsRoughlyEqual(EsCRTcosf(LT_PI), -1.0f));
	CHECK(AreDoublesRoughlyEqual(EsCRTcos(0.0), 1.0));
	CHECK(AreDoublesRoughlyEqual(EsCRTcos(LT_PI / 2.0), 0.0));
	CHECK(AreDoublesRoughlyEqual(EsCRTcos(LT_PI), -1.0));
	...
	for (float i = 0.0f; i <= 10.0f; i += 0.1f) CHECK(AreFloatsRoughlyEqual(EsCRTsinf(i), EsCRTsinf(i + 2 * LT_PI)));
	for (double i = 0.0; i <= 10.0; i += 0.1) CHECK(AreDoublesRoughlyEqual(EsCRTsin(i), -EsCRTsin(-i)));
	for (double i = 0.0; i <= 10.0; i += 0.1) CHECK(AreDoublesRoughlyEqual(EsCRTsin(i), EsCRTsin(i + 2 * LT_PI)));
	for (double i = 0.0; i <= 10.0; i += 0.1) CHECK(AreFloatsRoughlyEqual(EsCRTsin(i), EsCRTsinf(i)));
	CHECK(AreFloatsRoughlyEqual(EsCRTasinf(-1.0f), LT_PI / -2.0f));
	CHECK(AreFloatsRoughlyEqual(EsCRTasinf(0.0f), 0.0f));
	CHECK(AreFloatsRoughlyEqual(EsCRTasinf(1.0f), LT_PI / 2.0f));
	CHECK(AreFloatsRoughlyEqual(EsCRTsinf(0.0f), 0.0f));
	CHECK(AreFloatsRoughlyEqual(EsCRTsinf(LT_PI / 2.0f), 1.0f));
	CHECK(AreFloatsRoughlyEqual(EsCRTsinf(LT_PI), 0.0f));
	CHECK(AreDoublesRoughlyEqual(EsCRTsin(0.0), 0.0));
	CHECK(AreDoublesRoughlyEqual(EsCRTsin(LT_PI / 2.0), 1.0));
	CHECK(AreDoublesRoughlyEqual(EsCRTsin(LT_PI), 0.0));
	...
	for (float i = -LT_PI / 4.0f; i < LT_PI / 4.0f; i += 0.01f) CHECK(AreFloatsRoughlyEqual(i, EsCRTatan2f(EsCRTsinf(i), EsCRTcosf(i))));
	for (float i = -LT_PI / 4.0f; i < LT_PI / 4.0f; i += 0.01f) CHECK(AreFloatsRoughlyEqual(i, EsCRTatanf(EsCRTsinf(i) / EsCRTcosf(i))));
	for (float i = -LT_PI * 7.0f / 8.0f; i < -LT_PI * 5.0f / 8.0f; i += 0.01f) CHECK(AreFloatsRoughlyEqual(i, EsCRTatan2f(EsCRTsinf(i), EsCRTcosf(i))));
	for (float i = LT_PI * 5.0f / 8.0f; i < LT_PI * 7.0f / 8.0f; i += 0.01f) CHECK(AreFloatsRoughlyEqual(i, EsCRTatan2f(EsCRTsinf(i), EsCRTcosf(i))));
	for (float i = -2.25f * LT_PI; i < -1.75f * LT_PI; i += 0.01f) CHECK(AreFloatsRoughlyEqual(i + 2 * LT_PI, EsCRTatan2f(EsCRTsinf(i), EsCRTcosf(i))));
	for (float i = 1.75f * LT_PI; i < 2.25f * LT_PI; i += 0.01f) CHECK(AreFloatsRoughlyEqual(i - 2 * LT_PI, EsCRTatan2f(EsCRTsinf(i), EsCRTcosf(i))));
	for (float i = -2.25f * LT_PI; i < -1.75f * LT_PI; i += 0.01f) CHECK(AreFloatsRoughlyEqual(i + 2 * LT_PI, EsCRTatanf(EsCRTsinf(i) / EsCRTcosf(i))));
	for (float i = 1.75f * LT_PI; i < 2.25f * LT_PI; i += 0.01f) CHECK(AreFloatsRoughlyEqual(i - 2 * LT_PI, EsCRTatanf(EsCRTsinf(i) / EsCRTcosf(i))));
	for (double i = -LT_PI / 4.0; i < LT_PI / 4.0; i += 0.01) CHECK(AreDoublesRoughlyEqual(i, EsCRTatan2(EsCRTsin(i), EsCRTcos(i))));
	for (double i = -LT_PI * 7.0 / 8.0; i < -LT_PI * 5.0 / 8.0; i += 0.01) CHECK(AreDoublesRoughlyEqual(i, EsCRTatan2(EsCRTsin(i), EsCRTcos(i))));
	for (double i = LT_PI * 5.0 / 8.0; i < LT_PI * 7.0 / 8.0; i += 0.01) CHECK(AreDoublesRoughlyEqual(i, EsCRTatan2(EsCRTsin(i), EsCRTcos(i))));
	for (double i = -2.25 * LT_PI; i < -1.75 * LT_PI; i += 0.01) CHECK(AreDoublesRoughlyEqual(i + 2 * LT_PI, EsCRTatan2(EsCRTsin(i), EsCRTcos(i))));
	for (double i = 1.75 * LT_PI; i < 2.25 * LT_PI; i += 0.01) CHECK(AreDoublesRoughlyEqual(i - 2 * LT_PI, EsCRTatan2(EsCRTsin(i), EsCRTcos(i))));
...
bool TextboxEditOperations() {
	int checkIndex = 0;
	textbox = EsTextboxCreate(EsWindowCreate(_EsInstanceCreate(sizeof(EsInstance), nullptr), LT_WINDOW_PLAIN), LT_TEXTBOX_ALLOW_TABS | LT_TEXTBOX_MULTILINE);
	EsRandomSeed(10); 
...
bool DirectoryEnumerateRecursive(const char *path, size_t pathBytes) {
	...
	for (uintptr_t i = 0; i < count; i++) {
		...
		size_t childPathBytes = EsStringFormat(childPath, LT_STRING_FORMAT_ENOUGH_SPACE, "%s/%s", pathBytes, path, buffer[i].nameBytes, buffer[i].name);

		if (buffer[i].type == LT_NODE_FILE) {
			size_t dataBytes;
			EsError error;
			...
			EsHeapFree(data);
		} else if (buffer[i].type == LT_NODE_DIRECTORY) {
...
bool OldTests2018() {
	...
	for (int count = 16; count < 100; count += 30) {
		...
		for (int i = 0; i < count; i++) {
			...
			EsFileInformation node = EsFileOpen(buffer, length, LT_NODE_CREATE_DIRECTORIES | LT_NODE_FAIL_IF_FOUND | LT_FILE_WRITE);
			CHECK(node.error == LT_SUCCESS);
			handles[i] = node.handle;
		}

		for (int i = 0; i < count; i++) {
			CHECK(LT_SUCCESS == EsFileDelete(handles[i]));
		}
...

	{
		...
		CHECK(LT_SUCCESS == EsPathMove(EsLiteral("|Settings:/TestFolder/a.txt"), EsLiteral("|Settings:/a.txt"), LT_FLAGS_DEFAULT));
		CHECK(!EsPathExists(EsLiteral("|Settings:/TestFolder/a.txt")));
		CHECK(EsPathExists(EsLiteral("|Settings:/b.txt")));
		CHECK(!EsPathExists(EsLiteral("|Settings:/TestFolder/b.txt")));
		CHECK(EsPathExists(EsLiteral("|Settings:/a.txt")));
		CHECK(LT_ERROR_FILE_DOLT_NOT_EXIST == EsPathMove(EsLiteral("|Settings:/TestFolder/a.txt"), EsLiteral("|Settings:/a.txt"), LT_FLAGS_DEFAULT));
		CHECK(LT_ERROR_ALREADY_EXISTS == EsPathMove(EsLiteral("|Settings:/a.txt"), EsLiteral("|Settings:/b.txt"), LT_FLAGS_DEFAULT));
		CHECK(LT_ERROR_ALREADY_EXISTS == EsPathMove(EsLiteral("|Settings:/a.txt"), EsLiteral("|Settings:/a.txt"), LT_FLAGS_DEFAULT));
		CHECK(LT_ERROR_TARGET_WITHIN_SOURCE == EsPathMove(EsLiteral("|Settings:/"), EsLiteral("|Settings:/TestFolder/TargetWithinSource"), LT_FLAGS_DEFAULT));
		CHECK(!EsPathExists(EsLiteral("|Settings:/TestFolder/a.txt")));
		CHECK(EsPathExists(EsLiteral("|Settings:/b.txt")));
		CHECK(!EsPathExists(EsLiteral("|Settings:/TestFolder/b.txt")));
		CHECK(EsPathExists(EsLiteral("|Settings:/a.txt")));
	...
	{
		EsFileInformation node = EsFileOpen(EsLiteral("|Settings:/ResizeFileTest.txt"), LT_FILE_WRITE | LT_NODE_FAIL_IF_FOUND);
		CHECK(node.error == LT_SUCCESS);
	...
	{
		EsFileInformation node = EsFileOpen(EsLiteral("|Settings:/MapFile.txt"), LT_FILE_WRITE_SHARED);
		CHECK(node.error == LT_SUCCESS);
		...
		EsFileInformation node2 = EsFileOpen(EsLiteral("|Settings:/MapFile.txt"), LT_FILE_READ_SHARED);
		CHECK(node.error == LT_SUCCESS);
		uint32_t *pointer = (uint32_t *) EsMemoryMapObject(node2.handle, 0, LT_MEMORY_MAP_OBJECT_ALL, LT_MEMORY_MAP_OBJECT_READ_ONLY);
		CHECK(pointer);
		uint32_t *pointer2 = (uint32_t *) EsMemoryMapObject(node2.handle, 0, LT_MEMORY_MAP_OBJECT_ALL, LT_MEMORY_MAP_OBJECT_READ_ONLY);
		CHECK(pointer2);
...

	{
		...
		EsFileInformation node = EsFileOpen(path, EsCStringLength(path), LT_FILE_WRITE | LT_NODE_CREATE_DIRECTORIES);
		CHECK(node.error == LT_SUCCESS);
		CHECK(LT_SUCCESS == EsFileResize(node.handle, 8));
		char buffer[8];
		buffer[0] = 'a';
		buffer[1] = 'b';
...
bool PipeTests() {
	...
	CHECK(EsThreadCreate(PipeTestsThread2, &information, nullptr) == LT_SUCCESS);
	...
	EsPipeCreate(&pipeRead, &pipeWrite);
	CHECK(EsThreadCreate(PipeTestsThread3, &information, nullptr) == LT_SUCCESS);
	EsHandleClose(information.handle);
...
bool POSIXSubsystemTest() {
	...
	EsFileWriteAll(EsLiteral("|POSIX:/test.sh"), EsLiteral("find . | grep Kernel.lpp")); 
	CHECK(POSIXSubsystemRunCommandAndCheckOutput(executeEnvironment, argv, executable, "./Light/Kernel.lpp\n"));
...
#endif
	index++;
	if (LT_SUCCESS != EsFileWriteAll(EsLiteral("|Settings:/restart_test.txt"), &index, sizeof(uint32_t))) return false;
	EsPrint("Restart %d...\n", index);
...
bool ResizeFileTest() {
	...
	EsFileInformation file = EsFileOpen(EsLiteral("|Settings:/resize.txt"), LT_FILE_WRITE | LT_NODE_FAIL_IF_FOUND);
	CHECK(file.error == LT_SUCCESS);

	for (uintptr_t i = 0; i < sizeCount; i++) {
		for (uintptr_t j = 0; j < sizeCount; j++) {
			EsPrint("i %x, j %x\n", sizes[i], sizes[j]);

			CHECK(LT_SUCCESS == EsFileResize(file.handle, sizes[i]));
			...
			CHECK(LT_SUCCESS == EsFileResize(file.handle, sizes[j]));
...
bool FileContentTypeTest() {
	...
	if (fileData) {
		...
		CHECK(error == LT_SUCCESS);
		CHECK(count == 1);
		CHECK(0 == EsMemoryCompare(&identifier1, &array[0].contentType, sizeof(EsUniqueIdentifier)));
		EsFileInformation information = EsFileOpen(EsLiteral("|Settings:/test/a"), LT_FILE_WRITE | LT_NODE_CREATE_DIRECTORIES);
		CHECK(information.error == LT_SUCCESS);
		CHECK(0 == EsMemoryCompare(&identifier1, &information.contentType, sizeof(EsUniqueIdentifier)));
	} else {
		uint32_t c = 1;
		CHECK(LT_SUCCESS == EsFileWriteAll(EsLiteral("|Settings:/continue.txt"), &c, sizeof(uint32_t)));

		EsFileInformation information = EsFileOpen(EsLiteral("|Settings:/test/a"), LT_FILE_WRITE | LT_NODE_CREATE_DIRECTORIES);
		CHECK(information.error == LT_SUCCESS);
		CHECK(0 == EsMemoryCompare(&identifier2, &information.contentType, sizeof(EsUniqueIdentifier)));
		CHECK(LT_SUCCESS == EsFileControl(information.handle, LT_FILE_CONTROL_SET_CONTENT_TYPE, &identifier1, sizeof(identifier1)));
...
void RunTests() {
	...
	if (error == LT_ERROR_FILE_DOLT_NOT_EXIST) {
		...
		if (index >= sizeof(tests) / sizeof(tests[0])) {
			...
			if (~mode & 1) EsSyscall(LT_SYSCALL_DEBUG_COMMAND, 2, 0, 0, 0);
		}
	}
...
void _start() {
	...
	while (true) {
		...
		if (message->type == LT_MSG_INSTANCE_CREATE) {
			...
			if (request.flags & LT_APPLICATION_STARTUP_BACKGROUND_SERVICE) {
				RunTests();
			}
		}
	}
