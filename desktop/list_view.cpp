...
#define LIST_SORT_DIRECTION_DESCENDING (2)
	...
	int64_t MeasureItems(EsListViewIndex groupIndex, EsListViewIndex firstIndex, EsListViewIndex count) {
		...
		bool variableSize = flags & LT_LIST_VIEW_VARIABLE_SIZE;
		...
		if (!variableSize) {
			...
			if ((group->flags & LT_LIST_VIEW_GROUP_HAS_HEADER) && firstIndex == 0) {
				normalCount--;
				additionalSize += fixedHeaderSize;
			}

			if ((group->flags & LT_LIST_VIEW_GROUP_HAS_FOOTER) && firstIndex + count == group->itemCount) {
				normalCount--;
				additionalSize += fixedFooterSize;
			}

			return additionalSize + normalCount * (flags & LT_LIST_VIEW_HORIZONTAL ? fixedWidth : fixedHeight);
		}

		if (count > 1) {
			EsMessage m = { LT_MSG_LIST_VIEW_MEASURE_RANGE };
			m.itemRange.group = groupIndex;
			m.itemRange.firstIndex = firstIndex;
			m.itemRange.count = count;

			if (LT_HANDLED == EsMessageSend(this, &m)) {
		...
		while (true) {
			EsMessage m2 = { LT_MSG_LIST_VIEW_MEASURE_ITEM };
			m2.measureItem.group = groupIndex;
			m2.measureItem.index = m.iterateIndex.index;
			EsAssert(LT_HANDLED == EsMessageSend(this, &m2)); // Variable height list view must be able to measure items.
	...
	void GetItemPosition(EsListViewIndex groupIndex, EsListViewIndex index, int64_t *_position, int64_t *_itemSize) {
		int64_t gapBetweenGroup = style->gapMajor,
			gapBetweenItems = (flags & LT_LIST_VIEW_TILED) ? style->gapWrap : style->gapMinor,
			fixedSize       = (flags & LT_LIST_VIEW_VARIABLE_SIZE) ? 0 : (flags & LT_LIST_VIEW_HORIZONTAL ? fixedWidth : fixedHeight),
			startInset 	= flags & LT_LIST_VIEW_HORIZONTAL ? style->insets.l : style->insets.t;

		int64_t position = (flags & LT_LIST_VIEW_HORIZONTAL ? -scroll.position[0] : -scroll.position[1]) + startInset, 
			itemSize = 0;
		...
		if ((group->flags & LT_LIST_VIEW_GROUP_HAS_HEADER) && index == 0) {
			itemSize = fixedHeaderSize;
		} else if ((group->flags & LT_LIST_VIEW_GROUP_HAS_HEADER) && index == group->itemCount - 1) {
			position += group->totalSize - fixedFooterSize;
			itemSize = fixedFooterSize;
		} else if (~flags & LT_LIST_VIEW_VARIABLE_SIZE) {
			intptr_t linearIndex = index;

			if (group->flags & LT_LIST_VIEW_GROUP_HAS_HEADER) {
				linearIndex--;
			...
			EsAssert(~flags & LT_LIST_VIEW_TILED); // Tiled list views must be fixed-size.

			EsMessage index = {};
			index.type = LT_MSG_LIST_VIEW_FIND_POSITION;
			index.iterateIndex.group = groupIndex;
			index.iterateIndex.index = targetIndex;

			if (LT_HANDLED == EsMessageSend(this, &index)) {
				...
				if (reference && reference->group == groupIndex && !closerToStartThanReference && !closerToEndThanReference) {
					...
					position = (flags & LT_LIST_VIEW_HORIZONTAL) ? reference->element->offsetX : reference->element->offsetY;
	...
	void _EnsureItemVisible(EsListViewIndex groupIndex, EsListViewIndex index, uint8_t visibleFlags) {
		...
		int64_t startInset = flags & LT_LIST_VIEW_HORIZONTAL ? style->insets.l : style->insets.t,
			endInset = flags & LT_LIST_VIEW_HORIZONTAL ? style->insets.r : style->insets.b,
			contentSize = flags & LT_LIST_VIEW_HORIZONTAL ? Width(contentBounds) : Height(contentBounds);
		...
		if (visibleFlags & ENSURE_VISIBLE_ALIGN_FOR_SCROLL_ITEM) {
			if (flags & LT_LIST_VIEW_HORIZONTAL) {
	...
	EsMessage FindFirstVisibleItem(int64_t *_position, int64_t position, ListViewItem *reference, bool *noItems) {
		...
		if (~flags & LT_LIST_VIEW_VARIABLE_SIZE) {
			...
			EsListViewIndex itemsPerBand = (flags & LT_LIST_VIEW_TILED) ? GetItemsPerBand() : 1;
		...
		EsAssert(~flags & LT_LIST_VIEW_TILED); // Trying to use TILED mode with VARIABLE_SIZE mode.

		// Try asking the application to find the item.

		index.type = LT_MSG_LIST_VIEW_FIND_INDEX;
		...
		if (reference && reference->group == groupIndex) {
			int64_t referencePosition = (flags & LT_LIST_VIEW_HORIZONTAL) ? reference->element->offsetX : reference->element->offsetY;
	...
	void _Populate() {
		...
		EsRectangle contentBounds = GetListBounds();
		int64_t contentSize = flags & LT_LIST_VIEW_HORIZONTAL ? Width(contentBounds) : Height(contentBounds);
		int64_t scroll = EsCRTfloor(flags & LT_LIST_VIEW_HORIZONTAL ? (this->scroll.position[0] - style->insets.l) 
		...
		int64_t fixedMinorSize = (flags & LT_LIST_VIEW_HORIZONTAL) ? fixedHeight : fixedWidth;
		intptr_t itemsPerBand = GetItemsPerBand();
		intptr_t itemInBand = 0;
		int64_t computedMinorGap = style->gapMinor;
		int64_t minorPosition = 0;
		int64_t centerOffset = (flags & LT_LIST_VIEW_CENTER_TILES) 
		...
		while (visibleIndex < visibleItems.Length()) {
			...
			int64_t visibleItemPosition = flags & LT_LIST_VIEW_HORIZONTAL ? visibleItem->element->offsetX : visibleItem->element->offsetY;
		...
		while (position < contentSize && !noItems) {
			...
			if (visibleItem && visibleItem->index == currentItem.iterateIndex.index
				...
				if (~flags & LT_LIST_VIEW_TILED) {
					int64_t expectedPosition = (flags & LT_LIST_VIEW_HORIZONTAL)
				...
				if ((group->flags & LT_LIST_VIEW_GROUP_HAS_HEADER) && visibleItem->index == 0) {
					style = headerItemStyle;
					visibleItem->isHeader = true;
				} else if ((group->flags & LT_LIST_VIEW_GROUP_HAS_FOOTER) && visibleItem->index == (intptr_t) group->itemCount - 1) {
					style = footerItemStyle;
					visibleItem->isFooter = true;
				} else {
					if (group->flags & LT_LIST_VIEW_GROUP_INDENT) {
						visibleItem->indent++;
					}
				...
				visibleItem->element->Initialise(this, LT_CELL_FILL, nullptr, style);
				visibleItem->element->index = visibleIndex;
				...
				m.type = LT_MSG_LIST_VIEW_IS_SELECTED;
			...
			if ((flags & LT_LIST_VIEW_TILED) && !visibleItem->isHeader && !visibleItem->isFooter) {
				if (flags & LT_LIST_VIEW_HORIZONTAL) {
				...
				bool endOfGroup = ((group->flags & LT_LIST_VIEW_GROUP_HAS_FOOTER) && currentItem.iterateIndex.index == group->itemCount - 2)
					|| (currentItem.iterateIndex.index == group->itemCount - 1);

				if (itemInBand == itemsPerBand || endOfGroup) {
					minorPosition = 0;
					itemInBand = 0;
					position += (flags & LT_LIST_VIEW_HORIZONTAL) ? visibleItem->element->width : visibleItem->element->height;
					if (!endOfGroup || (group->flags & LT_LIST_VIEW_GROUP_HAS_FOOTER)) position += style->gapWrap;
				}
			} else {
				if (flags & LT_LIST_VIEW_HORIZONTAL) {
				...
				} else if ((flags & LT_LIST_VIEW_COLUMNS) && ((~flags & LT_LIST_VIEW_CHOICE_SELECT) || (this->scroll.enabled[0]))) {
				...
				if ((flags & LT_LIST_VIEW_TILED) && (group->flags & LT_LIST_VIEW_GROUP_HAS_HEADER) && currentItem.iterateIndex.index == 0) {
			...
			position += previousGroup == currentItem.iterateIndex.group ? (flags & LT_LIST_VIEW_TILED ? 0 : style->gapMinor) : style->gapMajor;
		}
	...
	void Wrap(bool sizeChanged) {
		if (~flags & LT_LIST_VIEW_TILED) return;
		...
		for (uintptr_t i = 0; i < groups.Length(); i++) {
			...
			if (group->flags & LT_LIST_VIEW_GROUP_HAS_FOOTER) {
				groupSize += fixedFooterSize + style->gapWrap;
				itemCount--;
			}

			intptr_t bandsInGroup = (itemCount + itemsPerBand - 1) / itemsPerBand;
			groupSize += (((flags & LT_LIST_VIEW_HORIZONTAL) ? fixedWidth : fixedHeight) + style->gapWrap) * bandsInGroup;
	...
	void SetSelected(EsListViewIndex fromGroup, EsListViewIndex fromIndex, EsListViewIndex toGroup, EsListViewIndex toIndex, 
		...
		if (!select && (flags & LT_LIST_VIEW_CHOICE_SELECT)) {
			return;
		}

		if (!select && (flags & LT_LIST_VIEW_SINGLE_SELECT)) {
			EsMessage m = {};
			m.type = LT_MSG_LIST_VIEW_SELECT;
		...
		for (; start.iterateIndex.group <= toGroup; start.iterateIndex.group++, end.iterateIndex.group++) {
			...
			EsMessage m = { LT_MSG_LIST_VIEW_SELECT_RANGE };
			...
			while (true) {
				...
				if (flags & LT_LIST_VIEW_FIXED_ITEMS) {
	...
	void SelectPreview(intptr_t singleItem = -1) {
		...
		for (uintptr_t i = 0; i < visibleItems.Length(); i++) {
			...
			EsMessage m = { LT_MSG_LIST_VIEW_IS_SELECTED };
	...
	void SelectBox(int64_t x1, int64_t x2, int64_t y1, int64_t y2, bool toggle) {
		...
		} else if (flags & LT_LIST_VIEW_COLUMNS) {
		...
		int r1 = (flags & LT_LIST_VIEW_HORIZONTAL) ? style->insets.l - x1 : style->insets.t - y1 + scroll.fixedViewport[1];
		int r2 = (flags & LT_LIST_VIEW_HORIZONTAL) ? style->insets.l - x2 : style->insets.t - y2 + scroll.fixedViewport[1];
		...
		if (flags & LT_LIST_VIEW_TILED) {
			...
			int64_t fixedMinorSize = (flags & LT_LIST_VIEW_HORIZONTAL) ? fixedHeight : fixedWidth;
			intptr_t itemsPerBand = GetItemsPerBand();
			int64_t centerOffset = (flags & LT_LIST_VIEW_CENTER_TILES) ? (wrapLimit - itemsPerBand * (fixedMinorSize + style->gapMinor) + style->gapMinor) / 2 : 0;
			int64_t minorStartOffset = centerOffset + ((flags & LT_LIST_VIEW_HORIZONTAL) ? style->insets.t : style->insets.l);

			int64_t s0 = (flags & LT_LIST_VIEW_HORIZONTAL) ? y1 : x1;
			int64_t s1 = (flags & LT_LIST_VIEW_HORIZONTAL) ? y2 : x2;
			...
			if (adjustStart) {
				...
				if (((group->flags & LT_LIST_VIEW_GROUP_HAS_HEADER) && start.iterateIndex.index == 0)
						|| ((group->flags & LT_LIST_VIEW_GROUP_HAS_FOOTER) && start.iterateIndex.index == group->itemCount - 1)) {
					IterateForwards(&start);
				} else {
					for (intptr_t i = 0; i < itemsPerBand; i++) {
						if ((group->flags & LT_LIST_VIEW_GROUP_HAS_FOOTER) && start.iterateIndex.index == group->itemCount - 1) {
							break;
						}
			...
			if (adjustEnd) {
				...
				if (((group->flags & LT_LIST_VIEW_GROUP_HAS_HEADER) && end.iterateIndex.index == 0)
						|| ((group->flags & LT_LIST_VIEW_GROUP_HAS_FOOTER) && end.iterateIndex.index == group->itemCount - 1)) {
				} else {
					for (intptr_t i = 0; i < itemsPerBand - 1; i++) {
						if ((group->flags & LT_LIST_VIEW_GROUP_HAS_FOOTER) && end.iterateIndex.index == group->itemCount - 1) {
	...
	void Select(EsListViewIndex group, EsListViewIndex index, bool range, bool toggle, bool moveAnchorOnly) {
		if ((~flags & LT_LIST_VIEW_SINGLE_SELECT) && (~flags & LT_LIST_VIEW_MULTI_SELECT) && (~flags & LT_LIST_VIEW_CHOICE_SELECT)) {
			return;
		}
		...
		if (!hasAnchorItem || (~flags & LT_LIST_VIEW_MULTI_SELECT)) {
			range = false;
		}
	...
	int ProcessItemMessage(uintptr_t visibleIndex, EsMessage *message, ListViewItemElement *element) {
		...
		if (message->type == LT_MSG_PAINT) {
			EsMessage m = { LT_MSG_LIST_VIEW_GET_CONTENT };
			uint8_t _buffer[512];
			...
			if (flags & LT_LIST_VIEW_COLUMNS) {
				...
				for (uintptr_t i = item->startAtSecondColumn ? 1 : 0; i < activeColumns.Length(); i++) {
					...
					if (EsRectangleClip(drawBounds, message->painter->clip, nullptr) 
							&& LT_HANDLED == EsMessageSend(this, &m)) {
						bool useSelectedCellStyle = (item->element->customStyleState & THEME_STATE_SELECTED) && (flags & LT_LIST_VIEW_CHOICE_SELECT);
				...
				if (flags & LT_LIST_VIEW_TILED) {
					m.type = LT_MSG_LIST_VIEW_GET_SUMMARY;
					if (LT_HANDLED == EsMessageSend(this, &m)) goto standardPaint;
					m.type = LT_MSG_LIST_VIEW_GET_CONTENT;
				}
		...
		} else if (message->type == LT_MSG_MOUSE_LEFT_DOWN || message->type == LT_MSG_MOUSE_MIDDLE_CLICK) {
			EsElementFocus(this);
			...
			if (message->type == LT_MSG_MOUSE_MIDDLE_CLICK) {
				Select(item->group, item->index, false, false, false);
				EsMessage m = { LT_MSG_LIST_VIEW_CHOOSE_ITEM };
				m.chooseItem.group = item->group;
				m.chooseItem.index = item->index;
				m.chooseItem.source = LT_LIST_VIEW_CHOOSE_ITEM_MIDDLE_CLICK;
				EsMessageSend(this, &m);
				...
				m.chooseItem.group = item->group;
				m.chooseItem.index = item->index;
				m.chooseItem.source = LT_LIST_VIEW_CHOOSE_ITEM_DOUBLE_CLICK;
				EsMessageSend(this, &m);
			}
		} else if (message->type == LT_MSG_MOUSE_MIDDLE_DOWN) {
		} else if (message->type == LT_MSG_MOUSE_RIGHT_DOWN) {
			EsMessage m = { LT_MSG_LIST_VIEW_IS_SELECTED };
			...
			m.type = LT_MSG_LIST_VIEW_CONTEXT_MENU;
			EsMessageSend(this, &m);
		} else if (message->type == LT_MSG_MOUSE_LEFT_DRAG) {
			if (flags & LT_LIST_VIEW_CHOICE_SELECT) {
				window->pressed = this;
	...
	inline int GetWrapLimit() {
		EsRectangle bounds = GetListBounds();
		return (flags & LT_LIST_VIEW_HORIZONTAL) 
	...
	bool Search() {
		...
		if (!hasFocusedItem) {
			...
			KeyInput(LT_SCANCODE_DOWN_ARROW, false, false, false, true);
			if (!hasFocusedItem) return false;
		}

		EsMessage m = { LT_MSG_LIST_VIEW_SEARCH };
		...
		if (response == LT_HANDLED) {
			...
			EnsureItemVisible(focusedItemGroup, focusedItemIndex, LT_FLAGS_DEFAULT);
		}
	...
	bool KeyInput(int scancode, bool ctrl, bool alt, bool shift, bool keepSearchBuffer = false) {
		...
		if (scancode == LT_SCANCODE_BACKSPACE && searchBufferBytes) {
			searchBufferBytes = 0;
			Search();
			return true;
		}
		...
		bool isNext = false, 
		     ...
		     isHome = scancode == LT_SCANCODE_HOME,
		     isEnd = scancode == LT_SCANCODE_END,
		     isPageUp = scancode == LT_SCANCODE_PAGE_UP,
		     isPageDown = scancode == LT_SCANCODE_PAGE_DOWN,
		     isSpace = scancode == LT_SCANCODE_SPACE,
		     isEnter = scancode == LT_SCANCODE_ENTER;

		if (flags & LT_LIST_VIEW_HORIZONTAL) {
			isNext = scancode == LT_SCANCODE_DOWN_ARROW;
			isNextBand = scancode == LT_SCANCODE_RIGHT_ARROW;
			isPrevious = scancode == LT_SCANCODE_UP_ARROW;
			isPreviousBand = scancode == LT_SCANCODE_LEFT_ARROW;
		} else {
			isNext = scancode == LT_SCANCODE_RIGHT_ARROW;
			isNextBand = scancode == LT_SCANCODE_DOWN_ARROW;
			isPrevious = scancode == LT_SCANCODE_LEFT_ARROW;
			isPreviousBand = scancode == LT_SCANCODE_UP_ARROW;
		}

		if (hasSelectionBoxAnchor) {
			if (scancode == LT_SCANCODE_UP_ARROW)     scroll.SetY(scroll.position[1] - GetConstantNumber("scrollKeyMovement"));
			if (scancode == LT_SCANCODE_DOWN_ARROW)   scroll.SetY(scroll.position[1] + GetConstantNumber("scrollKeyMovement"));
			if (scancode == LT_SCANCODE_LEFT_ARROW)   scroll.SetX(scroll.position[0] - GetConstantNumber("scrollKeyMovement"));
			if (scancode == LT_SCANCODE_RIGHT_ARROW)  scroll.SetX(scroll.position[0] + GetConstantNumber("scrollKeyMovement"));
			if (scancode == LT_SCANCODE_PAGE_UP)      scroll.SetY(scroll.position[1] - Height(GetBounds()));
			if (scancode == LT_SCANCODE_PAGE_DOWN)    scroll.SetY(scroll.position[1] + Height(GetBounds()));
			
			if (flags & LT_LIST_VIEW_HORIZONTAL) {
				if (scancode == LT_SCANCODE_HOME) scroll.SetX(0);
				if (scancode == LT_SCANCODE_END)  scroll.SetX(scroll.limit[0]);
			} else {
				if (scancode == LT_SCANCODE_HOME) scroll.SetY(0);
				if (scancode == LT_SCANCODE_END)  scroll.SetY(scroll.limit[1]);
			}
			...
			EnsureItemVisible(focusedItemGroup, focusedItemIndex, (isPrevious || isHome || isPageUp || isPreviousBand) ? ENSURE_VISIBLE_ALIGN_TOP : LT_FLAGS_DEFAULT);
			...
			m.chooseItem.source = LT_LIST_VIEW_CHOOSE_ITEM_ENTER;
			EsMessageSend(this, &m);
			return true;
		} else if (!ctrl && !alt) {
	...
	void MoveInlineTextbox(ListViewItem *item) {
		...
		} else if (flags & LT_LIST_VIEW_TILED) {
	...
	int ProcessMessage(EsMessage *message) {
		...
		if (message->type == LT_MSG_GET_WIDTH || message->type == LT_MSG_GET_HEIGHT) {
			...
			if ((message->type == LT_MSG_SCROLL_X) == ((flags & LT_LIST_VIEW_HORIZONTAL) ? true : false)) {
				for (uintptr_t i = 0; i < visibleItems.Length(); i++) {
					if (flags & LT_LIST_VIEW_HORIZONTAL) visibleItems[i].element->offsetX -= delta;
			...
			if (columnHeader && message->type == LT_MSG_SCROLL_X) {
			...
			for (uintptr_t i = 0; i < registeredColumns.Length(); i++) {
				if ((registeredColumns[i].flags & LT_LIST_VIEW_COLUMN_DATA_MASK) == LT_LIST_VIEW_COLUMN_DATA_STRINGS) {
			...
			if (EsElementIsFocused(this)) {
				EsCommandSetCallback(EsCommandByID(instance, LT_COMMAND_SELECT_ALL), nullptr);
			}
		} else if (message->type == LT_MSG_KEY_UP) {
			if (message->keyboard.scancode == LT_SCANCODE_LEFT_CTRL || message->keyboard.scancode == LT_SCANCODE_RIGHT_CTRL) {
				SelectPreview();
			}
			...
			if (message->keyboard.modifiers & ~(LT_MODIFIER_CTRL | LT_MODIFIER_ALT | LT_MODIFIER_SHIFT)) {
				// Unused modifier.
				return 0;
			}

			return KeyInput(message->keyboard.scancode, 
					message->keyboard.modifiers & LT_MODIFIER_CTRL, 
					message->keyboard.modifiers & LT_MODIFIER_ALT, 
					message->keyboard.modifiers & LT_MODIFIER_SHIFT)
				? LT_HANDLED : 0;
		} else if (message->type == LT_MSG_FOCUSED_START) {
			if (!hasFocusedItem && groups.Length() && (message->focus.flags & LT_ELEMENT_FOCUS_FROM_KEYBOARD)) {
				hasFocusedItem = true;
				focusedItemGroup = 0;
				focusedItemIndex = 0;
			}
			...
			EsCommand *command = EsCommandByID(instance, LT_COMMAND_SELECT_ALL);
			command->data = this;
			...
			if (selectionBox) {
				Repaint(false, LT_RECT_4(selectionBox->offsetX, selectionBox->offsetX + selectionBox->width,
				...
				if (!hasSelectionBoxAnchor) {
					...
					if (gui.lastClickButton == LT_MSG_MOUSE_LEFT_DOWN) {
						Select(-1, 0, EsKeyboardIsShiftHeld(), EsKeyboardIsCtrlHeld(), false);
					}
				}
				...
				SelectPreview();
			} else if (flags & LT_LIST_VIEW_CHOICE_SELECT) {
				DragSelect();
			}
		...
		} else if (message->type == LT_MSG_Z_ORDER) {
			...
			if (selectionBox)  { if (index == 0) return message->zOrder.child = selectionBox,  LT_HANDLED; else index--; }
			if (columnHeader)  { if (index == 0) return message->zOrder.child = columnHeader,  LT_HANDLED; else index--; }
			if (inlineTextbox) { if (index == 0) return message->zOrder.child = inlineTextbox, LT_HANDLED; else index--; }

			message->zOrder.child = nullptr;
		} else if (message->type == LT_MSG_PAINT && !totalItemCount && emptyMessageBytes) {
			EsDrawTextThemed(message->painter, this, EsPainterBoundsInset(message->painter), emptyMessage, emptyMessageBytes, 
					LT_STYLE_TEXT_LABEL_SECONDARY, LT_TEXT_H_CENTER | LT_TEXT_V_CENTER | LT_TEXT_WRAP);
		} else if (message->type == LT_MSG_ANIMATE) {
			if (scroll.dragScrolling && (flags & LT_LIST_VIEW_CHOICE_SELECT)) {
				DragSelect();
			}
		...
		} else if (message->type == LT_MSG_AFTER_Z_ORDER) {
			zOrderItems.Free();
			...
			primaryCellStyle = GetStyle(MakeStyleKey(LT_STYLE_LIST_PRIMARY_CELL, 0), false);
			secondaryCellStyle = GetStyle(MakeStyleKey(LT_STYLE_LIST_SECONDARY_CELL, 0), false);
			selectedCellStyle = GetStyle(MakeStyleKey(LT_STYLE_LIST_SELECTED_CHOICE_CELL, 0), false);

			EsListViewChangeStyles(this, 0, 0, 0, 0, LT_FLAGS_DEFAULT, LT_FLAGS_DEFAULT);
		} else if (message->type == LT_MSG_LIST_VIEW_GET_CONTENT && (activeColumns.Length() || (flags & LT_LIST_VIEW_FIXED_ITEMS))) {
			uintptr_t index = message->getContent.index;

			ListViewFixedItemData data = {};

			ListViewColumn *column = &registeredColumns[(flags & LT_LIST_VIEW_COLUMNS) ? activeColumns[message->getContent.activeColumnIndex] : 0];
			uint32_t format = column->flags & LT_LIST_VIEW_COLUMN_FORMAT_MASK;
			uint32_t type = column->flags & LT_LIST_VIEW_COLUMN_DATA_MASK;
			...
			if (flags & LT_LIST_VIEW_FIXED_ITEMS) {
				...
				EsMessage m = { .type = LT_MSG_LIST_VIEW_GET_ITEM_DATA };
				...
				if (type == LT_LIST_VIEW_COLUMN_DATA_STRINGS) {
					data.s.string = (char *) m.getItemData.s;
					data.s.bytes = m.getItemData.sBytes;
				} else if (type == LT_LIST_VIEW_COLUMN_DATA_DOUBLES) {
					data.d = m.getItemData.d;
				} else if (type == LT_LIST_VIEW_COLUMN_DATA_INTEGERS) {
...
#define BOOLEAN_FORMAT(trueString, falseString) \
	if (type == LT_LIST_VIEW_COLUMN_DATA_INTEGERS) { \
...
#define UNIT_FORMAT(unitString1, unitString2, unitString3) \
	double d = type == LT_LIST_VIEW_COLUMN_DATA_INTEGERS ? data.i : type == LT_LIST_VIEW_COLUMN_DATA_DOUBLES ? data.d : 0; \
	if (d < 10000)         EsBufferFormat(message->getContent.buffer, "%F%z",     d,           interfaceString_ ## unitString1); \
			...
			if (format == LT_LIST_VIEW_COLUMN_FORMAT_DEFAULT) {
				if (type == LT_LIST_VIEW_COLUMN_DATA_STRINGS) {
			...
			} else if (format == LT_LIST_VIEW_COLUMN_FORMAT_BYTES) {
				if (type == LT_LIST_VIEW_COLUMN_DATA_INTEGERS) {
					EsBufferFormat(message->getContent.buffer, "%D", data.i);
				} else {
					EsAssert(false);
				}
			} else if (format == LT_LIST_VIEW_COLUMN_FORMAT_ENUM_STRING) {
				if (type == LT_LIST_VIEW_COLUMN_DATA_INTEGERS) {
			...
			} else if (format == LT_LIST_VIEW_COLUMN_FORMAT_YLT_NO) {
				BOOLEAN_FORMAT(CommonBooleanYes, CommonBooleanNo);
			} else if (format == LT_LIST_VIEW_COLUMN_FORMAT_ON_OFF) {
				BOOLEAN_FORMAT(CommonBooleanOn, CommonBooleanOff);
			} else if (format == LT_LIST_VIEW_COLUMN_FORMAT_PERCENTAGE) {
				NUMBER_FORMAT(CommonUnitPercent);
			} else if (format == LT_LIST_VIEW_COLUMN_FORMAT_BITS) {
				NUMBER_FORMAT(CommonUnitBits);
			} else if (format == LT_LIST_VIEW_COLUMN_FORMAT_PIXELS) {
				NUMBER_FORMAT(CommonUnitPixels);
			} else if (format == LT_LIST_VIEW_COLUMN_FORMAT_DPI) {
				NUMBER_FORMAT(CommonUnitDPI);
			} else if (format == LT_LIST_VIEW_COLUMN_FORMAT_SECONDS) {
				NUMBER_FORMAT(CommonUnitSeconds);
			} else if (format == LT_LIST_VIEW_COLUMN_FORMAT_HERTZ) {
				UNIT_FORMAT(CommonUnitHz, CommonUnitKHz, CommonUnitMHz);
			} else if (format == LT_LIST_VIEW_COLUMN_FORMAT_BYTE_RATE) {
...
#undef BOOLEAN_FORMAT
		} else if (message->type == LT_MSG_LIST_VIEW_IS_SELECTED && (flags & LT_LIST_VIEW_FIXED_ITEMS)) {
			message->selectItem.isSelected = message->selectItem.index == fixedItemSelection;
		} else if (message->type == LT_MSG_LIST_VIEW_COLUMN_MENU && (flags & LT_LIST_VIEW_FIXED_ITEMS)) {
			...
			uint32_t sortMode = column->flags & LT_LIST_VIEW_COLUMN_SORT_MASK;
			uint64_t checkAscending  = (fixedItemSortDirection == LIST_SORT_DIRECTION_ASCENDING  && column->id == fixedItemSortColumnID) ? LT_MENU_ITEM_CHECKED : 0;
			uint64_t checkDescending = (fixedItemSortDirection == LIST_SORT_DIRECTION_DESCENDING && column->id == fixedItemSortColumnID) ? LT_MENU_ITEM_CHECKED : 0;

			if (sortMode != LT_LIST_VIEW_COLUMN_SORT_NONE) {
				EsMenuAddItem(menu, LT_MENU_ITEM_HEADER, INTERFACE_STRING(CommonSortHeader));

				if (sortMode == LT_LIST_VIEW_COLUMN_SORT_DEFAULT) {
				...
				} else if (sortMode == LT_LIST_VIEW_COLUMN_SORT_TIME) {
				...
				} else if (sortMode == LT_LIST_VIEW_COLUMN_SORT_SIZE) {
...
#if 0
	if (element->flags & LT_ELEMENT_DEBUG) {
...
int ListViewColumnHeaderMessage(EsElement *element, EsMessage *message) {
	...
	} else if (message->type == LT_MSG_MOUSE_LEFT_DOWN || message->type == LT_MSG_MOUSE_RIGHT_DOWN) {
		return LT_HANDLED;
	}
...
void EsListViewChangeStyles(EsListView *view, EsStyleID style, EsStyleID itemStyle, 
	...
	bool wasTiledView = view->flags & LT_LIST_VIEW_TILED;

	EsAssert(!(addFlags & removeFlags));
	view->flags |= addFlags;
	view->flags &= ~(uint64_t) removeFlags;

	bool horizontal = view->flags & LT_LIST_VIEW_HORIZONTAL;
	...
	if ((view->flags & LT_LIST_VIEW_MULTI_SELECT) && !view->selectionBox) {
		view->selectionBox = EsCustomElementCreate(view, LT_CELL_FILL | LT_ELEMENT_DISABLED | LT_ELEMENT_NO_HOVER, LT_STYLE_LIST_SELECTION_BOX);
		view->selectionBox->cName = "selection box";
	} else if ((~view->flags & LT_LIST_VIEW_MULTI_SELECT) && view->selectionBox) {
		EsElementDestroy(view->selectionBox);
		view->selectionBox = nullptr;
	}

	if ((view->flags & LT_LIST_VIEW_COLUMNS) && !view->columnHeader) {
		view->columnHeader = EsCustomElementCreate(view, LT_CELL_FILL, LT_STYLE_LIST_COLUMN_HEADER);
	...
	} else if ((~view->flags & LT_LIST_VIEW_COLUMNS) && view->columnHeader) {
	...
	// It's safe to use LT_SCROLL_MODE_AUTO even in tiled mode,
	// because decreasing the secondary axis can only increase the primary axis.

	uint8_t scrollXMode = 0, scrollYMode = 0;

	if (view->flags & LT_LIST_VIEW_COLUMNS) {
		scrollXMode = LT_SCROLL_MODE_AUTO;
		scrollYMode = LT_SCROLL_MODE_AUTO;
	} else if (view->flags & LT_LIST_VIEW_HORIZONTAL) {
		scrollXMode = LT_SCROLL_MODE_AUTO;
	} else {
		scrollYMode = LT_SCROLL_MODE_AUTO;
	}

	view->scroll.Setup(view, scrollXMode, scrollYMode, LT_SCROLL_X_DRAG | LT_SCROLL_Y_DRAG);
...
EsListView *EsListViewCreate(EsElement *parent, uint64_t flags, EsStyleID style, 
	...
	view->primaryCellStyle = GetStyle(MakeStyleKey(LT_STYLE_LIST_PRIMARY_CELL, 0), false);
	view->secondaryCellStyle = GetStyle(MakeStyleKey(LT_STYLE_LIST_SECONDARY_CELL, 0), false);
	view->selectedCellStyle = GetStyle(MakeStyleKey(LT_STYLE_LIST_SELECTED_CHOICE_CELL, 0), false); // Only used for choice list views.

	view->Initialise(parent, flags | LT_ELEMENT_FOCUSABLE, ListViewProcessMessage, style ?: LT_STYLE_LIST_VIEW);
	...
	if (!itemStyle) {
		if (flags & LT_LIST_VIEW_CHOICE_SELECT) itemStyle = LT_STYLE_LIST_CHOICE_ITEM;
		else if (flags & LT_LIST_VIEW_TILED) itemStyle = LT_STYLE_LIST_ITEM_TILE;
		else itemStyle = LT_STYLE_LIST_ITEM;
	}

	EsListViewChangeStyles(view, 0, itemStyle, headerItemStyle ?: LT_STYLE_LIST_ITEM_GROUP_HEADER, 
			footerItemStyle ?: LT_STYLE_LIST_ITEM_GROUP_FOOTER, LT_FLAGS_DEFAULT, LT_FLAGS_DEFAULT);
...
void EsListViewInsertGroup(EsListView *view, EsListViewIndex group, uint32_t flags) {
	...
	int64_t additionalItems = ((flags & LT_LIST_VIEW_GROUP_HAS_HEADER) ? 1 : 0) + ((flags & LT_LIST_VIEW_GROUP_HAS_FOOTER) ? 1 : 0);
...
int ListViewColumnHeaderItemMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_PAINT) {
		EsMessage m = { LT_MSG_LIST_VIEW_GET_COLUMN_SORT };
		m.getColumnSort.index = element->userData.u;
		int sort = EsMessageSend(view, &m);
		EsDrawContent(message->painter, element, element->GetBounds(), 
				column->title, column->titleBytes, 0, 
				sort == LT_LIST_VIEW_COLUMN_SORT_ASCENDING ? LT_DRAW_CONTENT_MARKER_UP_ARROW
				: sort == LT_LIST_VIEW_COLUMN_SORT_DESCENDING ? LT_DRAW_CONTENT_MARKER_DOWN_ARROW : LT_FLAGS_DEFAULT);
	} else if (message->type == LT_MSG_MOUSE_LEFT_CLICK && (column->flags & LT_LIST_VIEW_COLUMN_HAS_MENU)) {
		EsMessage m = { LT_MSG_LIST_VIEW_COLUMN_MENU };
...
void EsListViewAddAllColumns(EsListView *view) {
	...
	EsAssert(view->flags & LT_LIST_VIEW_COLUMNS); // List view does not have columns flag set.
	view->activeColumns.Free();

	for (uintptr_t i = 0; i < view->registeredColumns.Length(); i++) {
		view->activeColumns.Add(i);

		EsStyleID style = (view->registeredColumns[i].flags & LT_LIST_VIEW_COLUMN_HAS_MENU) ? LT_STYLE_LIST_COLUMN_HEADER_ITEM_HAS_MENU : LT_STYLE_LIST_COLUMN_HEADER_ITEM;
		EsElement *columnHeaderItem = EsCustomElementCreate(view->columnHeader, LT_CELL_FILL, style);
		...
		EsElement *splitter = EsCustomElementCreate(view->columnHeader, LT_CELL_FILL, LT_STYLE_LIST_COLUMN_HEADER_SPLITTER);
...
void EsListViewFocusItem(EsListView *view, EsListViewIndex group, EsListViewIndex index) {
	...
	view->EnsureItemVisible(group, index, LT_FLAGS_DEFAULT);
}
...
#define LIST_VIEW_SORT_FUNCTION(_name, _line) \
	LT_MACRO_SORT(_name, EsListViewIndex, { \
...
ListViewSortFunction ListViewGetSortFunction(ListViewColumn *column, uint8_t direction) {
	if ((column->flags & LT_LIST_VIEW_COLUMN_DATA_MASK) == LT_LIST_VIEW_COLUMN_DATA_STRINGS) {
		return (direction == LIST_SORT_DIRECTION_DESCENDING ? ListViewSortByStringsDescending : ListViewSortByStringsAscending);
	} else if ((column->flags & LT_LIST_VIEW_COLUMN_DATA_MASK) == LT_LIST_VIEW_COLUMN_DATA_INTEGERS) {
		if ((column->flags & LT_LIST_VIEW_COLUMN_FORMAT_MASK) == LT_LIST_VIEW_COLUMN_FORMAT_ENUM_STRING) {
	...
	} else if ((column->flags & LT_LIST_VIEW_COLUMN_DATA_MASK) == LT_LIST_VIEW_COLUMN_DATA_DOUBLES) {
...
EsListViewIndex EsListViewFixedItemInsert(EsListView *view, EsGeneric data, EsListViewIndex index, uint32_t iconID) {
	EsAssert(view->flags & LT_LIST_VIEW_FIXED_ITEMS);

	if (!view->groups.Length()) {
		EsListViewInsertGroup(view, 0, LT_FLAGS_DEFAULT);
	}
...
void ListViewFixedItemSetInternal(EsListView *view, EsListViewIndex index, uint32_t columnID, ListViewFixedItemData data, uint32_t dataType) {
	...
	EsAssert(column);
	EsAssert((column->flags & LT_LIST_VIEW_COLUMN_DATA_MASK) == dataType);
	...
	if (dataType == LT_LIST_VIEW_COLUMN_DATA_STRINGS) {
		changed = EsStringCompareRaw(column->items[index].s.string, column->items[index].s.bytes, data.s.string, data.s.bytes);
	} else if (dataType == LT_LIST_VIEW_COLUMN_DATA_DOUBLES) {
		changed = column->items[index].d != data.d;
	} else if (dataType == LT_LIST_VIEW_COLUMN_DATA_INTEGERS) {
...
void EsListViewFixedItemSetString(EsListView *view, EsListViewIndex index, uint32_t columnID, const char *string, ptrdiff_t stringBytes) {
	...
	if (outBytes == fixedString.bytes) {
		ListViewFixedItemSetInternal(view, index, columnID, { .s = fixedString }, LT_LIST_VIEW_COLUMN_DATA_STRINGS);
	}
}

void EsListViewFixedItemSetDouble(EsListView *view, EsListViewIndex index, uint32_t columnID, double number) {
	ListViewFixedItemSetInternal(view, index, columnID, { .d = number }, LT_LIST_VIEW_COLUMN_DATA_DOUBLES);
}

void EsListViewFixedItemSetInteger(EsListView *view, EsListViewIndex index, uint32_t columnID, int64_t number) {
	ListViewFixedItemSetInternal(view, index, columnID, { .i = number }, LT_LIST_VIEW_COLUMN_DATA_INTEGERS);
}
...
EsTextbox *EsListViewCreateInlineTextbox(EsListView *view, EsListViewIndex group, EsListViewIndex index, uint32_t flags) {
	...
	uint64_t textboxFlags = LT_CELL_FILL | LT_TEXTBOX_EDIT_BASED | LT_TEXTBOX_ALLOW_TABS;
	
	if (flags & LT_LIST_VIEW_INLINE_TEXTBOX_REJECT_EDIT_IF_FOCUS_LOST) {
		textboxFlags |= LT_TEXTBOX_REJECT_EDIT_IF_LOST_FOCUS;
	}

	view->inlineTextbox = EsTextboxCreate(view, textboxFlags, LT_STYLE_TEXTBOX_INLINE);
	...
	if (flags & LT_LIST_VIEW_INLINE_TEXTBOX_COPY_EXISTING_TEXT) {
		EsMessage m = { LT_MSG_LIST_VIEW_GET_CONTENT };
		uint8_t _buffer[256];
		EsBuffer buffer = { .out = _buffer, .bytes = sizeof(_buffer) };
		m.getContent.buffer = &buffer;
...
void EsListViewScrollToEnd(EsListView *view) {
	if (view->flags & LT_LIST_VIEW_HORIZONTAL) {
...
EsPoint EsListViewGetAnnouncementPointForSelection(EsListView *view) {
	...
	return LT_POINT((bounding.l + bounding.r) / 2, (bounding.t + bounding.b) / 2);
}
