...
#if defined(LT_ARCH_X86_32) || defined(LT_ARCH_X86_64)
...
bool KEventWait(KEvent *event, uint64_t timeoutMs = LT_WAIT_NO_TIMEOUT); // See KEventWaitMultiple to wait for multiple events. Returns false if the wait timed out.
...
bool KSemaphoreTake(KSemaphore *semaphore, uintptr_t units = 1, uintptr_t timeoutMs = LT_WAIT_NO_TIMEOUT);
...
#define K_PAGE_BITS LT_PAGE_BITS
#define K_PAGE_SIZE LT_PAGE_SIZE
...
void *MMMapShared(MMSpace *space, MMSharedRegion *sharedRegion, uintptr_t offset, size_t bytes, uint32_t additionalFlags = LT_FLAGS_DEFAULT, void *baseAddresses = nullptr);
...
#define K_DEVICE_VISIBLE_TO_USER (1 << 1)   // A LT_MSG_DEVICE_CONNECTED message was sent to Desktop for this device.
	uint8_t flags;
	uint32_t handles;
	EsDeviceType type;
	EsObjectID objectID;
...
EsError (*control)(KDevice *device, EsDeviceControlType type, void *dp, void *dq); // Handle EsDeviceControl calls.
...
void KDeviceOpenHandle(KDevice *device, uint32_t handleFlags = LT_FLAGS_DEFAULT);
void KDeviceDestroy(KDevice *device); // Call if initialisation of the device failed. Otherwise use KDeviceCloseHandle.
void KDeviceCloseHandle(KDevice *device, uint32_t handleFlags = LT_FLAGS_DEFAULT); // The device creator is responsible for one handle after the creating it. The device is destroyed once all handles are closed.
void KDeviceRemoved(KDevice *device); // Call when a child device is removed. Must be called only once!
void KDeviceSendConnectedMessage(KDevice *device, EsDeviceType type, uint32_t handleFlags = LT_FLAGS_DEFAULT); // Send a message to Desktop to inform it the device was connected.
...
struct KNodeMetadata {
	...
	EsFileOffsetDifference directoryChildren; // LT_DIRECTORY_CHILDREN_UNKNOWN if not supported by the file system.
...
#define FS_BLOCK_ACCESS_SOFT_ERRORS (2)
	...
	EsVolumeFlags volumeFlags; // Note: LT_VOLUME_READ_ONLY will be automatically set if you don't set the write() callback pointer.
...
EsError FSDirectoryEntryFound(KNode *parentDirectory, KNodeMetadata *metadata /* ignored if the entry is already cached */, 
		...
		bool update /* set to true if you don't want to insert an new entry if it isn't already cached; returns LT_SUCCESS or LT_ERROR_FILE_DOLT_NOT_EXIST only */,
