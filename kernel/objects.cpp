...
struct HandleTable {
	...
	// If this fails, the handle is closed and LT_INVALID_HANDLE is returned.
	EsHandle OpenHandle(void *_object, uint32_t _flags, KernelObjectType _type, EsHandle at = LT_INVALID_HANDLE);
...
bool OpenHandleToObject(void *object, KernelObjectType type, uint32_t flags) {
	...
	switch (type) {
		...
		case KERNEL_OBJECT_NODE: {
			failed = LT_SUCCESS != FSNodeOpenHandle((KNode *) object, flags, FS_NODE_OPEN_HANDLE_STANDARD);
		} break;
...
uintptr_t HandleShare(Handle share, Process *process, uint32_t mode, EsHandle at = LT_INVALID_HANDLE) {
#define HANDLE_SHARE_TYPE_MASK ((KernelObjectType) (KERNEL_OBJECT_SHMEM | KERNEL_OBJECT_CONSTANT_BUFFER | KERNEL_OBJECT_PROCESS \
	...
	if (share.type == KERNEL_OBJECT_SHMEM) {
		...
		sharedFlags = (mode & 1) && (share.flags & (LT_FILE_WRITE_SHARED | LT_FILE_WRITE)) ? LT_FILE_READ_SHARED : share.flags;
		if (mode & 2) sharedFlags &= ~LT__NODE_DIRECTORY_WRITE;
...
uint8_t HandleTable::ResolveHandle(Handle *outHandle, EsHandle inHandle, KernelObjectType typeMask) {
	// Special handles.
	if (inHandle == LT_CURRENT_THREAD && (typeMask & KERNEL_OBJECT_THREAD)) {
		outHandle->type = KERNEL_OBJECT_THREAD;
		outHandle->object = GetCurrentThread();
		outHandle->flags = 0;
		return RESOLVE_HANDLE_NO_CLOSE;
	} else if (inHandle == LT_CURRENT_PROCESS && (typeMask & KERNEL_OBJECT_PROCESS)) {
		outHandle->type = KERNEL_OBJECT_PROCESS;
		outHandle->object = GetCurrentThread()->process;
		outHandle->flags = 0;
		return RESOLVE_HANDLE_NO_CLOSE;
	} else if (inHandle == LT_INVALID_HANDLE && (typeMask & KERNEL_OBJECT_NONE)) {
...
EsHandle ConstantBufferCreate(K_USER_BUFFER const void *data, size_t bytes, Process *process) {
	...
	return object ? process->handleTable.OpenHandle(object, 0, KERNEL_OBJECT_CONSTANT_BUFFER) : LT_INVALID_HANDLE; 
}
...
size_t Pipe::Access(void *_buffer, size_t bytes, bool write, bool user) {
	...
	while (bytes) {
		...
		if (write) {
			...
			KEventWait(&canWrite, LT_WAIT_NO_TIMEOUT);
		} else {
			// Wait until we can read from the pipe.
			KEventWait(&canRead, LT_WAIT_NO_TIMEOUT);
		}
...
#define MERGE_MESSAGES(variable, change) \
	...
	if (_message->message.type == LT_MSG_MOUSE_MOVED) {
		MERGE_MESSAGES(mouseMovedMessage, true);
	} else if (_message->message.type == LT_MSG_WINDOW_RESIZED) {
		MERGE_MESSAGES(windowResizedMessage, true);
	} else if (_message->message.type == LT_MSG_EYEDROP_REPORT) {
		MERGE_MESSAGES(eyedropResultMessage, true);
	} else if (_message->message.type == LT_MSG_KEY_DOWN && _message->message.keyboard.repeat) {
		...
		if (_message->message.type == LT_MSG_PING) {
			pinged = true;
		}
	}
