...
void SendMessageToWindow(Window *window, EsMessage *message) {
	...
	if (window->style != LT_WINDOW_CONTAINER || !window->embed) {
		window->owner->messageQueue.SendMessage(window->apiWindow, message);
		return;
	}

	if (message->type == LT_MSG_WINDOW_RESIZED) {
		message->windowResized.content = LT_RECT_2S(window->width, window->height);
		window->owner->messageQueue.SendMessage(window->apiWindow, message);
		message->windowResized.content = LT_RECT_2S(window->width - window->embedInsets.l - window->embedInsets.r, 
	...
	} else if (message->type == LT_MSG_WINDOW_DEACTIVATED || message->type == LT_MSG_WINDOW_ACTIVATED) {
		window->owner->messageQueue.SendMessage(window->apiWindow, message);
		window->embed->owner->messageQueue.SendMessage(window->embed->apiWindow, message);
	} else if (message->type == LT_MSG_MOUSE_MOVED) {
		EsRectangle embedRegion = LT_RECT_4(window->embedInsets.l, window->width - window->embedInsets.r, 
		...
		if (inEmbed) {
			...
			if (!windowManager.pressedWindow && !window->hoveringOverEmbed) {
				message->type = LT_MSG_MOUSE_EXIT;
				window->owner->messageQueue.SendMessage(window->apiWindow, message);
			}
		} else {
			window->owner->messageQueue.SendMessage(window->apiWindow, message);
	...
	} else if (message->type >= LT_MSG_MOUSE_LEFT_DOWN && message->type <= LT_MSG_MOUSE_MIDDLE_UP) {
		...
		if ((message->keyboard.modifiers & (LT_MODIFIER_CTRL | LT_MODIFIER_ALT | LT_MODIFIER_FLAG | LT_MODIFIER_ALT_GR))
				|| message->keyboard.scancode == LT_SCANCODE_LEFT_CTRL || message->keyboard.scancode == LT_SCANCODE_RIGHT_CTRL
				|| message->keyboard.scancode == LT_SCANCODE_LEFT_SHIFT || message->keyboard.scancode == LT_SCANCODE_RIGHT_SHIFT
				|| message->keyboard.scancode == LT_SCANCODE_LEFT_ALT || message->keyboard.scancode == LT_SCANCODE_RIGHT_ALT
				|| message->keyboard.scancode == LT_SCANCODE_LEFT_FLAG || message->keyboard.scancode == LT_SCANCODE_RIGHT_FLAG) {
...
Window *WindowManager::FindWindowAtPosition(int cursorX, int cursorY, EsObjectID exclude) {
	...
	for (intptr_t i = windows.Length() - 1; i >= 0; i--) {
		...
		EsRectangle bounds = LT_RECT_4PD(window->position.x, window->position.y, window->width, window->height);
...
void WindowManager::EndEyedrop(bool cancelled) {
	...
	EsMessage m = { LT_MSG_EYEDROP_REPORT };
	...
	Redraw(LT_POINT(0, 0), graphics.width, graphics.height);
}
...
void WindowManager::PressKey(uint32_t scancode) {
	...
	if (scancode == LT_SCANCODE_NUM_DIVIDE && (leftModifiers & LT_MODIFIER_FLAG)) {
		KernelPanic("WindowManager::PressKey - Panic key pressed.\n");
	}

	if (eyedropping) {
		if (scancode == (LT_SCANCODE_ESCAPE | K_SCANCODE_KEY_RELEASED)) {
			EndEyedrop(true);
		}
	...
	if (scancode == LT_SCANCODE_LEFT_CTRL) leftModifiers |= LT_MODIFIER_CTRL;
	if (scancode == (LT_SCANCODE_LEFT_CTRL | K_SCANCODE_KEY_RELEASED)) leftModifiers &= ~LT_MODIFIER_CTRL;
	if (scancode == LT_SCANCODE_LEFT_SHIFT) leftModifiers |= LT_MODIFIER_SHIFT;
	if (scancode == (LT_SCANCODE_LEFT_SHIFT | K_SCANCODE_KEY_RELEASED)) leftModifiers &= ~LT_MODIFIER_SHIFT;
	if (scancode == LT_SCANCODE_LEFT_ALT) leftModifiers |= LT_MODIFIER_ALT;
	if (scancode == (LT_SCANCODE_LEFT_ALT | K_SCANCODE_KEY_RELEASED)) leftModifiers &= ~LT_MODIFIER_ALT;
	if (scancode == LT_SCANCODE_LEFT_FLAG) leftModifiers |= LT_MODIFIER_FLAG;
	if (scancode == (LT_SCANCODE_LEFT_FLAG | K_SCANCODE_KEY_RELEASED)) leftModifiers &= ~LT_MODIFIER_FLAG;

	if (scancode == LT_SCANCODE_RIGHT_CTRL) leftModifiers |= LT_MODIFIER_CTRL;
	if (scancode == (LT_SCANCODE_RIGHT_CTRL | K_SCANCODE_KEY_RELEASED)) leftModifiers &= ~LT_MODIFIER_CTRL;
	if (scancode == LT_SCANCODE_RIGHT_SHIFT) leftModifiers |= LT_MODIFIER_SHIFT;
	if (scancode == (LT_SCANCODE_RIGHT_SHIFT | K_SCANCODE_KEY_RELEASED)) leftModifiers &= ~LT_MODIFIER_SHIFT;
	if (scancode == LT_SCANCODE_RIGHT_ALT) leftModifiers |= LT_MODIFIER_ALT_GR;
	if (scancode == (LT_SCANCODE_RIGHT_ALT | K_SCANCODE_KEY_RELEASED)) leftModifiers &= ~LT_MODIFIER_ALT_GR;
	if (scancode == LT_SCANCODE_RIGHT_FLAG) leftModifiers |= LT_MODIFIER_FLAG;
	if (scancode == (LT_SCANCODE_RIGHT_FLAG | K_SCANCODE_KEY_RELEASED)) leftModifiers &= ~LT_MODIFIER_FLAG;

	if (scancode == LT_SCANCODE_NUM_LOCK) {
		numlock = !numlock;
	...
	message.type = (scancode & K_SCANCODE_KEY_RELEASED) ? LT_MSG_KEY_UP : LT_MSG_KEY_DOWN;
	...
	if (message.type == LT_MSG_KEY_DOWN && (keysHeldBitSet[message.keyboard.scancode / 8] & (1 << (message.keyboard.scancode % 8)))) {
	...
	if ((((leftModifiers | rightModifiers) & LT_MODIFIER_CTRL) && ((leftModifiers | rightModifiers) & LT_MODIFIER_FLAG)) || !activeWindow) {
...
Window *WindowManager::FindWindowToActivate(Window *excluding) {
	for (uintptr_t i = windows.Length(); i > 0; i--) {
		if (!windows[i - 1]->hidden 
				&& windows[i - 1]->style == LT_WINDOW_CONTAINER 
...
void WindowManager::HideWindow(Window *window) {
	...
	Redraw(LT_POINT(window->position.x, window->position.y), window->width, window->height);
}
...
void WindowManager::ChangeWindowDepth(Window *window, bool redraw, ptrdiff_t _newZDepth) {
	...
	if (oldZDepth != newZDepth || redraw) {
		...
		windowManager.Redraw(LT_POINT(window->position.x, window->position.y), window->width, window->height);
	}
}
...
bool WindowManager::CloseMenus() {
	...
	for (uintptr_t i = 0; i < windows.Length(); i++) {
		...
		if (windows[i]->style == LT_WINDOW_MENU) {
			message.type = LT_MSG_WINDOW_DEACTIVATED;
...
bool WindowManager::ActivateWindow(Window *window) {
	...
	if (window && (window->style == LT_WINDOW_TIP || window->style == LT_WINDOW_MENU || window->closed)) {
		return false;
	}
	...
	if (window) {
		...
		message.type = LT_MSG_WINDOW_ACTIVATED;
	...
	if (eyedropping && delta) {
		...
		if (buttons == delta) {
			...
			if (window && window->noClickActivate) {
				...
				if (window->style != LT_WINDOW_MENU) {
...

		{
			...
			if (delta & K_LEFT_BUTTON) message.type = (buttons & K_LEFT_BUTTON) ? LT_MSG_MOUSE_LEFT_DOWN : LT_MSG_MOUSE_LEFT_UP;
			if (delta & K_RIGHT_BUTTON) message.type = (buttons & K_RIGHT_BUTTON) ? LT_MSG_MOUSE_RIGHT_DOWN : LT_MSG_MOUSE_RIGHT_UP;
			if (delta & K_MIDDLE_BUTTON) message.type = (buttons & K_MIDDLE_BUTTON) ? LT_MSG_MOUSE_MIDDLE_DOWN : LT_MSG_MOUSE_MIDDLE_UP;

			// TODO Setting pressedWindow if holding with other mouse buttons.

			if (message.type == LT_MSG_MOUSE_LEFT_DOWN || message.type == LT_MSG_MOUSE_MIDDLE_DOWN || message.type == LT_MSG_MOUSE_RIGHT_DOWN) {
				if (!pressedWindow) {
			...
			if (message.type == LT_MSG_MOUSE_LEFT_UP || message.type == LT_MSG_MOUSE_MIDDLE_UP || message.type == LT_MSG_MOUSE_RIGHT_UP) {
				if (pressedWindow) {
...
void WindowManager::MoveCursor(int64_t xMovement, int64_t yMovement) {
	...
	if ((leftModifiers & LT_MODIFIER_ALT) && (cursorProperties & CURSOR_USE_ALT_SLOW)) {
	...
	if (eyedropping) {
		...
		if (window) {
			EsMessage message;
			EsMemoryZero(&message, sizeof(EsMessage));
			message.type = LT_MSG_MOUSE_MOVED;
...
void WindowManager::ScrollWheel(int32_t dx, int32_t dy) {
	...
	EsMessage message;
	EsMemoryZero(&message, sizeof(EsMessage));
	message.type = LT_MSG_SCROLL_WHEEL;
...
void *WindowManager::CreateWindow(Process *process, void *apiWindow, EsWindowStyle style) {
	...
	window->apiWindow = apiWindow;
	window->alpha = 0xFF;
	window->position = LT_POINT(-8, -8);
	window->owner = process;
	window->handles = 2; // One handle for the window manager, and one handle for the calling process.
	window->hidden = true;
	window->id = ++currentWindowID;

	if (style == LT_WINDOW_INSPECTOR) windowManager.inspectorWindowCount++;

	// Insert the window into the window array.

	uintptr_t insertionPoint = style == LT_WINDOW_TIP ? windows.Length() : GetActivationZIndex();
...
bool Window::Move(EsRectangle rectangle, uint32_t flags) {
	...
	if ((flags & LT_WINDOW_MOVE_DYNAMIC) 
			&& (~flags & LT_WINDOW_MOVE_MAXIMIZED) && !isMaximised
	...
	isMaximised = flags & LT_WINDOW_MOVE_MAXIMIZED;
	alwaysOnTop = flags & LT_WINDOW_MOVE_ALWAYS_ON_TOP;

	// TS("Move window\n");

	if (flags & LT_WINDOW_MOVE_ADJUST_TO_FIT_SCREEN) {
	...
	if (!hidden) {
		...
		windowManager.Redraw(LT_POINT(position.x, position.y), width, height, this);
	}

	hidden = false;
	position = LT_POINT(rectangle.l, rectangle.t);
	bool changedSize = width != newWidth || height != newHeight;
	width = newWidth, height = newHeight;

	if (changedSize) {
		if (style == LT_WINDOW_CONTAINER && embed) {
		...
		message.windowResized.content = LT_RECT_4(0, width, 0, height);
	...
	if (flags & LT_WINDOW_MOVE_AT_BOTTOM) {
		windowManager.ChangeWindowDepth(this, false, 0);
	}

	if ((flags & LT_WINDOW_MOVE_DYNAMIC) && changedSize && style == LT_WINDOW_CONTAINER && !windowManager.resizeSlow) {
...
void EmbeddedWindow::Close() {
	...
	m.object = apiWindow;
	m.message.type = LT_MSG_WINDOW_DESTROYED;
	owner->messageQueue.SendMessage(&m); 
	SetEmbedOwner(nullptr);
	m.object = nullptr;
	m.message.type = LT_MSG_EMBEDDED_WINDOW_DESTROYED;
...
void Window::Close() {
	...
	EsMessage message;
	EsMemoryZero(&message, sizeof(EsMessage));
	message.type = LT_MSG_WINDOW_DESTROYED;
	...
	windowManager.Redraw(LT_POINT(position.x, position.y), width, height);
...
void WindowManager::Redraw(EsPoint position, int width, int height, Window *except, int startingAt, bool addToModifiedRegion) {
	...
	for (int index = startingAt; index < (int) windows.Length(); index++) {
		...
		EsRectangle rectangle = LT_RECT_4PD(window->position.x, window->position.y, window->width, window->height);
		rectangle = EsRectangleIntersection(rectangle, LT_RECT_2S(graphics.frameBuffer.width, graphics.frameBuffer.height));
		rectangle = EsRectangleIntersection(rectangle, LT_RECT_4PD(position.x, position.y, width, height));
		...
		if (!LT_RECT_VALID(rectangle)) continue;

		Surface *surface = &window->surface;

		EsRectangle transparentRegions[] = {
			EsRectangleIntersection(rectangle, LT_RECT_4(0, window->opaqueBounds.l, 0, window->height)),
			EsRectangleIntersection(rectangle, LT_RECT_4(window->opaqueBounds.r, window->width, 0, window->height)),
			EsRectangleIntersection(rectangle, LT_RECT_4(window->opaqueBounds.l, window->opaqueBounds.r, 0, window->opaqueBounds.t)),
			EsRectangleIntersection(rectangle, LT_RECT_4(window->opaqueBounds.l, window->opaqueBounds.r, window->opaqueBounds.b, window->height)),
		};
		...
		for (uintptr_t i = 0; i < 4; i++) {
			if (LT_RECT_VALID(transparentRegions[i])) {
				EsPoint point = LT_POINT(window->position.x + transparentRegions[i].l, window->position.y + transparentRegions[i].t);
				graphics.frameBuffer.BlendWindow(surface, point, transparentRegions[i], window->material, window->alpha, blurRegion);
			}
		}

		if (LT_RECT_VALID(opaqueRegion)) {
			EsPoint point = LT_POINT(window->position.x + opaqueRegion.l, window->position.y + opaqueRegion.t);
...
bool Window::UpdateDirect(K_USER_BUFFER void *bits, uintptr_t stride, EsRectangle region) {
	...
	EsRectangle thisBounds = LT_RECT_4(position.x, position.x + width, position.y, position.y + height);

	for (uintptr_t i = z + 1; i < windowManager.windows.Length(); i++) {
		Window *other = windowManager.windows[i];
		EsRectangle otherBounds = LT_RECT_4(other->position.x, other->position.x + other->width,
...
void Window::Update(EsRectangle *_region, bool addToModifiedRegion) {
	...
	// TS("Update window %x within %R\n", this, _region ? *_region : LT_RECT_1(0));

	EsRectangle region = _region ? *_region : LT_RECT_4(0, width, 0, height);
	...
	if (alpha == 0xFF) {
		...
		EsRectangleClip(region, LT_RECT_4(translucentBorder.l, translucentBorder.r, translucentBorder.t, translucentBorder.b), &r);
		windowManager.Redraw(LT_POINT(position.x + r.l, position.y + r.t), r.r - r.l, r.b - r.t, nullptr, z, addToModifiedRegion);

		// Draw the transparent regions after everything below them, then everything above them.
		EsRectangleClip(region, LT_RECT_4(0, translucentBorder.l, 0, height), &r);
		windowManager.Redraw(LT_POINT(position.x + r.l, position.y + r.t), r.r - r.l, r.b - r.t, nullptr, 0, addToModifiedRegion);
		EsRectangleClip(region, LT_RECT_4(translucentBorder.r, width, 0, height), &r);
		windowManager.Redraw(LT_POINT(position.x + r.l, position.y + r.t), r.r - r.l, r.b - r.t, nullptr, 0, addToModifiedRegion);
		EsRectangleClip(region, LT_RECT_4(translucentBorder.l, translucentBorder.r, 0, translucentBorder.t), &r);
		windowManager.Redraw(LT_POINT(position.x + r.l, position.y + r.t), r.r - r.l, r.b - r.t, nullptr, 0, addToModifiedRegion);
		EsRectangleClip(region, LT_RECT_4(translucentBorder.l, translucentBorder.r, translucentBorder.b, height), &r);
		windowManager.Redraw(LT_POINT(position.x + r.l, position.y + r.t), r.r - r.l, r.b - r.t, nullptr, 0, addToModifiedRegion);
	} else {
		// The whole window is translucent; draw it.
		EsRectangleClip(region, LT_RECT_4(0, width, 0, height), &r);
		windowManager.Redraw(LT_POINT(position.x + r.l, position.y + r.t), r.r - r.l, r.b - r.t, nullptr, 0, addToModifiedRegion);
	}
}
...
void Window::ResizeEmbed() {
	...
	EsMemoryZero(&message, sizeof(EsMessage));
	message.type = LT_MSG_WINDOW_RESIZED;
	message.windowResized.content = LT_RECT_2S(width - embedInsets.l - embedInsets.r, height - embedInsets.t - embedInsets.b);
...
void Window::SetEmbed(EmbeddedWindow *newEmbed) {
	...
	if (embed) {
		...
		EsMemoryZero(&message, sizeof(EsMessage));
		message.type = LT_MSG_WINDOW_RESIZED;
		message.windowResized.content = LT_RECT_4(0, 0, 0, 0);
	...
	if (embed) {
		...
		message.type = windowManager.activeWindow == this ? LT_MSG_WINDOW_ACTIVATED : LT_MSG_WINDOW_DEACTIVATED;
...
void KKeyboardUpdate(uint16_t *keysDown, size_t keysDownCount) {
	...
	for (uintptr_t i = 0; i < keysDownCount; i++) {
		...
		if (!found && keysDown[i]) {
			if (keysDown[i] == LT_SCANCODE_PAUSE) {
...
uint64_t KGameControllerConnect() {
	...
	if (windowManager.gameControllerCount != LT_GAME_CONTROLLER_MAX_COUNT) {
