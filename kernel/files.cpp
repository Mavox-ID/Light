...
EsError FSReadIntoCache(CCSpace *fileCache, void *buffer, EsFileOffset offset, EsFileOffset count) {
	...
	if (node->fsZeroAfter < offset + count) {
		...
		if (LT_CHECK_ERROR(count)) {
			node->error = count;
		}
	}

	return LT_CHECK_ERROR(count) ? count : LT_SUCCESS;
}
...
EsError FSFileCreateAndResizeOnFileSystem(FSFile *node, EsFileOffset fileSize) {
	...
	if (node->fsFileSize != fileSize || node->fsZeroAfter != fileSize) {
		...
		EsError error = LT_ERROR_UNKNOWN;
		KWriterLockTake(&entry->parent->writerLock, K_LOCK_EXCLUSIVE);
		...
		if (node->fsFileSize != fileSize) {
			return LT_ERROR_COULD_NOT_RESIZE_FILE;
		}
...
ptrdiff_t FSFileReadSync(KNode *node, K_USER_BUFFER void *buffer, EsFileOffset offset, EsFileOffset bytes, uint32_t accessFlags) {
	...
	if (offset > file->directoryEntry->totalSize) return LT_ERROR_ACCESS_NOT_WITHIN_FILE_BOUNDS;
	...
	return error == LT_SUCCESS ? bytes : error;
}
...
EsError FSFileResize(KNode *node, EsFileOffset newSize, bool growOnly) {
	...
	if (node->directoryEntry->type != LT_NODE_FILE) {
	...
	if (growOnly && newSize <= file->directoryEntry->totalSize) {
		...
		error = LT_ERROR_OPERATION_BLOCKED;
	} else if (!file->fileSystem->resize) {
		error = LT_ERROR_FILE_ON_READ_ONLY_VOLUME;
	} else {
...
ptrdiff_t FSFileWriteSync(KNode *node, K_USER_BUFFER const void *buffer, EsFileOffset offset, EsFileOffset bytes, uint32_t flags) {
	...
	if (LT_SUCCESS != FSFileResize(node, offset + bytes, true /* growOnly */)) {
		return LT_ERROR_COULD_NOT_RESIZE_FILE;
	}
	...
	if (!file->fileSystem->write) return LT_ERROR_FILE_ON_READ_ONLY_VOLUME;
	if (offset > file->directoryEntry->totalSize) return LT_ERROR_ACCESS_NOT_WITHIN_FILE_BOUNDS;
	if (bytes > file->directoryEntry->totalSize - offset) bytes = file->directoryEntry->totalSize - offset;
	if (!bytes) return 0;
...
EsError FSFileControlSetContentType(KNode *node, EsUniqueIdentifier identifier) {
	if (node->fileSystem->flags & LT_VOLUME_READ_ONLY) {
		return LT_ERROR_FILE_ON_READ_ONLY_VOLUME;
	} else if (~node->fileSystem->flags & LT_VOLUME_STORLT_CONTENT_TYPE) {
		return LT_ERROR_UNSUPPORTED;
	} else {
		KWriterLockTake(&node->writerLock, K_LOCK_EXCLUSIVE);
...
EsError FSNodeDelete(KNode *node) {
	...
	FSFile *file = entry->type == LT_NODE_FILE ? (FSFile *) node : nullptr;

	if (!parent) return LT_ERROR_PERMISSION_NOT_GRANTED;
	...
	if (LT_SUCCESS != FSNodeOpenHandle(parent, LT_FLAGS_DEFAULT, FS_NODE_OPEN_HANDLE_DIRECTORY_TEMPORARY)) {
		return LT_ERROR_INSUFFICIENT_RESOURCES;
	}

	EsDefer(FSNodeCloseHandle(parent, LT_FLAGS_DEFAULT));
	...
	if (file) {
		...
		if (file->blockResize) {
			KWriterLockReturn(&file->resizeLock, K_LOCK_EXCLUSIVE);
			return LT_ERROR_OPERATION_BLOCKED;
		}
	...
	if (node->flags & NODE_DELETED) {
		error = LT_ERROR_NODE_DELETED;
	} else if (entry->type == LT_NODE_DIRECTORY && ((FSDirectory *) node)->entryCount) {
		error = LT_ERROR_DIRECTORY_NOT_EMPTY;
...
EsError FSNodeMove(KNode *node, KNode *_newParent, const char *newName, size_t newNameBytes) {
	...
	if (newParent->directoryEntry->type != LT_NODE_DIRECTORY) {
		return LT_ERROR_INCORRECT_NODE_TYPE;
	}

	if (!oldParent || oldParent->fileSystem != newParent->fileSystem || oldParent->fileSystem != node->fileSystem) {
		return LT_ERROR_VOLUME_MISMATCH;
	}

	if (!newNameBytes || newNameBytes > LT_MAX_DIRECTORY_CHILD_NAME_LENGTH) {
		return LT_ERROR_INVALID_NAME;
	}
	...
	if (LT_SUCCESS != FSNodeOpenHandle(oldParent, LT_FLAGS_DEFAULT, FS_NODE_OPEN_HANDLE_DIRECTORY_TEMPORARY)) {
		return LT_ERROR_INSUFFICIENT_RESOURCES;
	}

	EsDefer(FSNodeCloseHandle(oldParent, LT_FLAGS_DEFAULT));
	...
	while (newAncestor) {
		if (newAncestor == node) {
			...
			error = LT_ERROR_TARGET_WITHIN_SOURCE;
			goto fail;
		}
	...
	if (oldParent->directoryEntry->directoryChildren != LT_DIRECTORY_CHILDREN_UNKNOWN) {
		oldParent->directoryEntry->directoryChildren--;
	}

	newParent->entryCount++;

	if (newParent->directoryEntry->directoryChildren != LT_DIRECTORY_CHILDREN_UNKNOWN) {
...
void _FSDirectoryEnumerateVisit(AVLItem<FSDirectoryEntry> *item, K_USER_BUFFER EsDirectoryChild *buffer, size_t bufferSize, uintptr_t *position) {
	...
	*position = *position + 1;
	...
	size_t nameBytes = entry->item.key.longKeyBytes > LT_MAX_DIRECTORY_CHILD_NAME_LENGTH ? LT_MAX_DIRECTORY_CHILD_NAME_LENGTH : entry->item.key.longKeyBytes;
...
ptrdiff_t FSDirectoryEnumerate(KNode *node, K_USER_BUFFER EsDirectoryChild *buffer, size_t bufferSize) {
	...
	if (node->directoryEntry->type != LT_NODE_DIRECTORY) {
...
void FSNodeFree(KNode *node) {
	...
	if (entry->type == LT_NODE_FILE) {
		CCSpaceDestroy(&((FSFile *) node)->cache);
	} else if (entry->type == LT_NODE_DIRECTORY) {
	...
	EsHeapFree(node, entry->type == LT_NODE_DIRECTORY ? sizeof(FSDirectory) : sizeof(FSFile), K_FIXED);
...
void FSNodeScanAndLoadComplete(KNode *node, bool success) {
	if (success) {
		...
		} else if (node->directoryEntry->type == LT_NODE_DIRECTORY && ((FSDirectory *) node)->entryCount) {
...
EsError FSDirectoryEntryAllocateNode(FSDirectoryEntry *entry, KFileSystem *fileSystem, bool createdOnFileSystem, bool inDirectoryEntryCache) {
	{
		...
		if (entry->removingThisFromCache) {
			...
			return LT_ERROR_DIRECTORY_ENTRY_BEING_REMOVED;
		}
	...
	KNode *node = (KNode *) EsHeapAllocate(entry->type == LT_NODE_DIRECTORY ? sizeof(FSDirectory) : sizeof(FSFile), true, K_FIXED);
	...
	if (entry->type == LT_NODE_DIRECTORY) {
		FSDirectory *directory = (FSDirectory *) node;
	...
	} else if (entry->type == LT_NODE_FILE) {
		FSFile *file = (FSFile *) node;
...
EsError FSDirectoryEntryOpenHandleToNode(FSDirectoryEntry *directoryEntry) {
	...
	if (!directoryEntry->node || directoryEntry->removingNodeFromCache) {
		return LT_ERROR_NODE_NOT_LOADED;
	}
	...
	if (!node->handles) {
		if (node->directoryEntry->type == LT_NODE_DIRECTORY && ((FSDirectory *) node)->entryCount) {
...
EsError FSNodeOpenHandle(KNode *node, uint32_t flags, uint8_t mode) {
	{
		...
		if (node->directoryEntry->type == LT_NODE_FILE) {
			...
			if (flags & LT_FILE_READ) {
				if (file->countWrite > 0) return LT_ERROR_OPERATION_BLOCKED; 
			} else if (flags & LT_FILE_WRITE) {
				if (flags & LT__NODE_FROM_WRITE_EXCLUSIVE) {
					if (!file->countWrite || (~file->flags & NODE_HAS_EXCLUSIVE_WRITER)) {
						KernelPanic("FSNodeOpenHandle - File %x is invalid state for a handle to have the LT__NODE_FROM_WRITE_EXCLUSIVE flag.\n", file);
					}
				} else {
					if (file->countWrite) {
						return LT_ERROR_OPERATION_BLOCKED; 
			...
			} else if (flags & LT_FILE_WRITE_SHARED) {
				if ((file->flags & NODE_HAS_EXCLUSIVE_WRITER) || file->countWrite < 0) return LT_ERROR_OPERATION_BLOCKED;
			}

			if (flags & (LT_FILE_WRITE_SHARED | LT_FILE_WRITE)) {
				if (!file->fileSystem->write) {
					return LT_ERROR_FILE_ON_READ_ONLY_VOLUME;
				}
			}

			if (flags & (LT_FILE_WRITE_SHARED | LT_FILE_WRITE)) file->countWrite++;
			if (flags & LT_FILE_READ) file->countWrite--;
			if (flags & LT_FILE_WRITE) __sync_fetch_and_or(&node->flags, NODE_HAS_EXCLUSIVE_WRITER);
		}
	...
	if (node->directoryEntry->type == LT_NODE_FILE && (flags & LT_NODE_PREVENT_RESIZE)) {
...
void FSNodeCloseHandle(KNode *node, uint32_t flags) {
	...
	if (node->directoryEntry->type == LT_NODE_FILE) {
		...
		if ((flags & LT_FILE_WRITE) && file->countWrite == 0) {
	...
	bool hasEntries = node->directoryEntry->type == LT_NODE_DIRECTORY && ((FSDirectory *) node)->entryCount;
...
EsError FSNodeTraverseLayer(uintptr_t *sectionEnd, 
	...
	*sectionEnd = *sectionEnd + 1;
	...
	if (!nameBytes) {
		...
		return LT_ERROR_PATH_NOT_TRAVERSABLE;
	}
...

	{
		...
		if (treeItem) {
			...
			if (error == LT_ERROR_NODE_NOT_LOADED) {
				error = LT_SUCCESS; // Proceed to use exclusive access.
			} else {
	...
	if (!treeItem) {
		...
		if (flags & LT__NODE_NO_WRITE_BASE) {
			error = LT_ERROR_PERMISSION_NOT_GRANTED;
			goto failed;
		}

		if (*sectionEnd == pathBytes && isFinalPath) {
			if (~flags & LT_NODE_FAIL_IF_NOT_FOUND) {
				error = FSNodeCreate(directory, name, nameBytes, flags & LT_NODE_DIRECTORY);
				if (error != LT_SUCCESS) goto failed;
				treeItem = TreeFind(&directory->entries, key, TREE_SEARCH_EXACT);
				flags &= ~LT_NODE_FAIL_IF_FOUND;
				*createdNode = true;
			}

			if (!treeItem) {
				error = LT_ERROR_FILE_DOLT_NOT_EXIST;
				goto failed;
			}
		} else {
			if (flags & LT_NODE_CREATE_DIRECTORIES) {
				error = FSNodeCreate(directory, name, nameBytes, LT_NODE_DIRECTORY);
				if (error != LT_SUCCESS) goto failed;
				treeItem = TreeFind(&directory->entries, key, TREE_SEARCH_EXACT);
			}

			if (!treeItem) {
				error = LT_ERROR_PATH_NOT_TRAVERSABLE;
				goto failed;
			}
		}
	}
	...
	if (!entry->node) {
		...
		if (error == LT_ERROR_DIRECTORY_ENTRY_BEING_REMOVED) {
			...
			// Similar to the strategy for the LT_ERROR_NODE_NOT_LOADED race below.
			KWriterLockReturn(&directory->writerLock, K_LOCK_EXCLUSIVE);
			KYield(); 
			goto tryAgain;
		}
		...
		if (LT_SUCCESS != FSNodeOpenHandle(entry->node, 0, FS_NODE_OPEN_HANDLE_FIRST)) {
	...
	if (*sectionEnd != pathBytes || !isFinalPath) {
		if (entry->node->directoryEntry->type != LT_NODE_DIRECTORY) {
			FSNodeCloseHandle(directory, 0);
			return LT_ERROR_PATH_NOT_TRAVERSABLE;
		}
	}
...
KNodeInformation FSNodeOpen(const char *path, size_t pathBytes, uint32_t flags, KNode *baseDirectory) {
	if ((1 << (flags & 0xF)) & ~(0x117)) {
		...
		return { LT_ERROR_PERMISSION_NOT_GRANTED }; 
	}

	if (fs.shutdown) return { LT_ERROR_PATH_NOT_TRAVERSABLE };
	...
	if (node->directoryEntry->type != LT_NODE_DIRECTORY && (flags & LT_NODE_DIRECTORY)) {
		return { LT_ERROR_INCORRECT_NODE_TYPE }; 
	}

	if ((flags & LT_NODE_FAIL_IF_FOUND) && !createdNode) {
		error = LT_ERROR_ALREADY_EXISTS;
	} else {
	...
	if (error != LT_SUCCESS) node = nullptr;
	return { error, node };
}
...
bool FSTrimCachedDirectoryEntry(MMObjectCache *cache) {
	...
	if (entry) {
		if (!entry->parent) {
			...
			FSDirectoryEntryFree(entry);
		} else if (LT_SUCCESS == FSNodeOpenHandle(entry->parent, LT_FLAGS_DEFAULT, FS_NODE_OPEN_HANDLE_DIRECTORY_TEMPORARY)) {
			KWriterLockTake(&entry->parent->writerLock, K_LOCK_EXCLUSIVE);
			...
			KWriterLockReturn(&entry->parent->writerLock, K_LOCK_EXCLUSIVE);
			FSNodeCloseHandle(entry->parent, LT_FLAGS_DEFAULT); // This will put the parent in the node cache if needed.
			FSDirectoryEntryFree(entry);
		} else {
...
EsError FSBlockDeviceAccess(KBlockDeviceAccessRequest request) {
	...
	if (device->information.readOnly && request.operation == K_ACCESS_WRITE) {
		if (request.flags & FS_BLOCK_ACCESS_SOFT_ERRORS) return LT_ERROR_BLOCK_ACCESS_INVALID;
	...
	if (request.dispatchGroup == &fakeDispatchGroup) {
		return fakeDispatchGroup.Wait() ? LT_SUCCESS : LT_ERROR_HARDWARE_FAILURE;
	} else {
		return LT_SUCCESS;
	}
}
...
EsError FSReadIntoBlockCache(CCSpace *cache, void *buffer, EsFileOffset offset, EsFileOffset count) {
	...
	return fileSystem->Access(offset, count, K_ACCESS_READ, buffer, LT_FLAGS_DEFAULT, nullptr);
}

EsError FSWriteFromBlockCache(CCSpace *cache, const void *buffer, EsFileOffset offset, EsFileOffset count) {
	KFileSystem *fileSystem = EsContainerOf(KFileSystem, cacheSpace, cache);
	return fileSystem->Access(offset, count, K_ACCESS_WRITE, (void *) buffer, LT_FLAGS_DEFAULT, nullptr);
}
...
EsError KFileSystem::Access(EsFileOffset offset, size_t count, int operation, void *buffer, uint32_t flags, KWorkGroup *dispatchGroup) {
	if (this->flags & K_DEVICE_REMOVED) {
		...
		return LT_ERROR_DEVICE_REMOVED;
	}
	...
	if (blockDeviceCachedEnabled && (flags & FS_BLOCK_ACCESS_CACHED)) {
		...
		if (dispatchGroup) {
			dispatchGroup->End(result == LT_SUCCESS);
		}
	...
	if (MBRGetPartitions(device->signatureBlock, device->information.sectorCount, partitions)) {
		...
		for (uintptr_t i = 0; i < 4; i++) {
			if (partitions[i].present) {
				...
				FSPartitionDeviceCreate(device, partitions[i].offset, partitions[i].count, LT_FLAGS_DEFAULT, EsLiteral("MBR partition"));
				foundAny = true;
			}
		}
	...
	if (GPTGetPartitions(device->signatureBlock, device->information.sectorCount, device->information.sectorSize, partitions)) {
		for (uintptr_t i = 0; i < GPT_PARTITION_COUNT; i++) {
			if (partitions[i].present) {
				...
				FSPartitionDeviceCreate(device, partitions[i].offset, partitions[i].count, LT_FLAGS_DEFAULT, EsLiteral("GPT partition"));
			}
		}
...
bool FSFileSystemInitialise(KFileSystem *fileSystem) {
	...
	rootEntry->type = LT_NODE_DIRECTORY;
	if (LT_SUCCESS != FSDirectoryEntryAllocateNode(rootEntry, nullptr, true, false)) goto error;
...
void FSDetectFileSystem(KBlockDevice *device) {
	...
	if (signatureBlock) {
		...
		if (LT_SUCCESS != FSBlockDeviceAccess(request)) {
...
void FSRegisterBootFileSystem(KFileSystem *fileSystem, EsUniqueIdentifier identifier) {
	...
	if (!EsMemoryCompare(&identifier, &installationID, sizeof(EsUniqueIdentifier))) {
		...
		if (!fs.bootFileSystem) {
			...
			FSNodeOpenHandle(fileSystem->rootDirectory, LT_FLAGS_DEFAULT, FS_NODE_OPEN_HANDLE_FIRST);
		} else {
...
void FSTrackUserFileSystemHandle(KDevice *device, bool opened) {
	...
	if (opened) FSNodeOpenHandle(fileSystem->rootDirectory, LT_FLAGS_DEFAULT, FS_NODE_OPEN_HANDLE_STANDARD);
	else FSNodeCloseHandle(fileSystem->rootDirectory, LT_FLAGS_DEFAULT);
}

void FSRegisterFileSystem(KFileSystem *fileSystem) {
	if (!fileSystem->write) fileSystem->volumeFlags |= LT_VOLUME_READ_ONLY;
	...
	FSNodeOpenHandle(fileSystem->rootDirectory, LT_FLAGS_DEFAULT, fileSystem->isBootFileSystem ? FS_NODE_OPEN_HANDLE_STANDARD : FS_NODE_OPEN_HANDLE_FIRST);
	KDeviceSendConnectedMessage(fileSystem, LT_DEVICE_FILE_SYSTEM, K_DEVICE_HANDLE_TRACKED);
	FSNodeCloseHandle(fileSystem->rootDirectory, LT_FLAGS_DEFAULT);
}
...
void FSRegisterBlockDevice(KBlockDevice *device) {
	KThreadCreate("FSDetect", [] (uintptr_t context) {
		...
		KDeviceSendConnectedMessage(device, LT_DEVICE_BLOCK);
	}, (uintptr_t) device);
}
