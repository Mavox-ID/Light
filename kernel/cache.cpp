...
EsError CCSpaceAccess(CCSpace *cache, K_USER_BUFFER void *buffer, EsFileOffset offset, EsFileOffset count, uint32_t flags, 
		MMSpace *mapSpace = nullptr, unsigned mapFlags = LT_FLAGS_DEFAULT);
		...
		for (uintptr_t i = firstPage; i < section->pageCount; i++) {
			...
			if (section->data[i] & MM_SHARED_ENTRY_PRESENT) {
				...
				if (pmm.pageFrames[page >> K_PAGE_BITS].state != MMPageFrame::ACTIVE) {
					MMPhysicalActivatePages(page >> K_PAGE_BITS, 1, LT_FLAGS_DEFAULT);
				}
...
void CCDereferenceActiveSection(CCActiveSection *section, uintptr_t startingPage) {
	...
	if (!startingPage) {
		MMArchUnmapPages(kernelMMSpace, 
				...
				CC_ACTIVE_SECTION_SIZE / K_PAGE_SIZE, MM_UNMAP_PAGLT_BALANCE_FILE);
		...
		MMArchUnmapPages(kernelMMSpace, 
				...
				(CC_ACTIVE_SECTION_SIZE / K_PAGE_SIZE - startingPage), MM_UNMAP_PAGLT_BALANCE_FILE);
...
EsError CCSpaceAccess(CCSpace *cache, K_USER_BUFFER void *_buffer, EsFileOffset offset, EsFileOffset count, uint32_t flags, 
	...
	for (EsFileOffset sectionOffset = firstSection; sectionOffset < lastSection; sectionOffset += CC_ACTIVE_SECTION_SIZE) {
		if (MM_AVAILABLE_PAGES() < MM_CRITICAL_AVAILABLE_PAGLT_THRESHOLD && !GetCurrentThread()->isPageGenerator) {
		...
		while (true) {
			...
			for (uintptr_t i = pageStart; i < pageEnd; i++) {
				...
				if ((entry & MM_SHARED_ENTRY_PRESENT) && (~section->referencedPages[i >> 3] & (1 << (i & 7)))) {
					...
					if (frame->state == MMPageFrame::STANDBY) {
						...
						MMPhysicalActivatePages(entry / K_PAGE_SIZE, 1, LT_FLAGS_DEFAULT);
			...
			for (uintptr_t i = pageStart; i < pageEnd; i++) {
				...
				pageFrames[i] = MMPhysicalAllocate(LT_FLAGS_DEFAULT);

				MMPageFrame *frame = pmm.pageFrames + (pageFrames[i] >> K_PAGE_BITS);
				frame->active.references = 1;
				frame->cacheReference = pagesToLoad[i];

				MMArchMapPage(kernelMMSpace, pageFrames[i], (uintptr_t) sectionBase + i * K_PAGE_SIZE, LT_FLAGS_DEFAULT);
			}
			...
			if ((flags & CC_ACCESS_WRITE) && (~flags & CC_ACCESS_USER_BUFFER_MAPPED)) {
				...
				if (error == LT_SUCCESS && (start & (K_PAGE_SIZE - 1)) && pagesToLoad[pageStart]) {
				...
				if (error == LT_SUCCESS && (end & (K_PAGE_SIZE - 1)) && !(pageStart == pageEnd - 1 && loadedStart) && pagesToLoad[pageEnd - 1]) {
			...
			if (error != LT_SUCCESS) {
				...
				for (uintptr_t i = pageStart; i < pageEnd; i++) {
					...
					MMArchUnmapPages(kernelMMSpace, (uintptr_t) sectionBase + i * K_PAGE_SIZE, 1, LT_FLAGS_DEFAULT);
...
void CCWriteBehindThread() {
	...
	while (true) {
		...
		if (MM_AVAILABLE_PAGES() > MM_LOW_AVAILABLE_PAGLT_THRESHOLD && !scheduler.shutdown) {
...
void CCInitialise() {
	...
	activeSectionManager.writeBackThread = ThreadSpawn("CCWriteBehind", (uintptr_t) CCWriteBehindThread, 0, LT_FLAGS_DEFAULT);
