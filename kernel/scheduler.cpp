...
struct Thread {
	...
	union {
		...
		struct {
			...
			KEvent *volatile events[LT_MAX_WAIT_COUNT];
...
struct Process {
	...
	char cExecutableName[LT_SNAPSHOT_MAX_PROCESS_NAME_LENGTH + 1];
...
Thread *ThreadSpawn(const char *cName, uintptr_t startAddress, uintptr_t argument1 = 0, 
		uint32_t flags = LT_FLAGS_DEFAULT, Process *process = nullptr, uintptr_t argument2 = 0);
...
Thread *ThreadSpawn(const char *cName, uintptr_t startAddress, uintptr_t argument1, uint32_t flags, Process *process, uintptr_t argument2) {
...
#if defined(LT_BITS_64)
	uintptr_t kernelStackSize = 0x5000 /* 20KB */;
#elif defined(LT_BITS_32)
	...
	if (userland) {
		stack = (uintptr_t) MMStandardAllocate(process->vmm, userStackReserve, LT_FLAGS_DEFAULT, nullptr, false);
...
void ProcessLoadExecutable() {
	...
	api.isDesktop = true;
	EsError loadError = LT_SUCCESS;

	{
		uint64_t fileFlags = LT_FILE_READ | LT_NODE_FAIL_IF_NOT_FOUND;
		KNodeInformation node = FSNodeOpen(EsLiteral(K_DESKTOP_EXECUTABLE), fileFlags);
		loadError = node.error;

		if (node.error == LT_SUCCESS) {
			if (node.node->directoryEntry->type != LT_NODE_FILE) {
				loadError = LT_ERROR_INCORRECT_NODE_TYPE;
			} else {
	...
	if (success) {
		startupInformation = (EsProcessStartupInformation *) MMStandardAllocate(
				thisProcess->vmm, sizeof(EsProcessStartupInformation), LT_FLAGS_DEFAULT);
		...
		if (!startupInformation) {
			...
			uint32_t globalDataRegionFlags = thisProcess->type == PROCESS_DESKTOP ? LT_SHARED_MEMORY_READ_WRITE : LT_FLAGS_DEFAULT;
	...
	if (success) {
		...
		if (thisProcess->creationFlags & LT_PROCESS_CREATE_PAUSED) threadFlags |= SPAWN_THREAD_PAUSED;

		thisProcess->executableState = LT_PROCESS_EXECUTABLE_LOADED;
	...
	if (!success) {
		...
		thisProcess->executableState = LT_PROCESS_EXECUTABLE_FAILED_TO_LOAD;
	}
...
bool ProcessStartWithNode(Process *process, KNode *node) {
	...
	if (byteCount > LT_SNAPSHOT_MAX_PROCESS_NAME_LENGTH) byteCount = LT_SNAPSHOT_MAX_PROCESS_NAME_LENGTH;
	...
	if (!OpenHandleToObject(node, KERNEL_OBJECT_NODE, LT_FILE_READ)) {
	...
	Thread *loadExecutableThread = ThreadSpawn("ExecLoad", (uintptr_t) ProcessLoadExecutable, 0, LT_FLAGS_DEFAULT, process);
	KMutexRelease(&scheduler.allProcessesMutex);
	...
	KEventWait(&process->executableLoadAttemptComplete, LT_WAIT_NO_TIMEOUT);

	if (process->executableState == LT_PROCESS_EXECUTABLE_FAILED_TO_LOAD) {
...
bool ProcessStart(Process *process, char *imagePath, size_t imagePathLength, KNode *baseDirectory = nullptr) {
	uint64_t flags = LT_FILE_READ | LT_NODE_FAIL_IF_NOT_FOUND;
	KNodeInformation node = FSNodeOpen(imagePath, imagePathLength, flags, baseDirectory);
	bool result = false;

	if (!LT_CHECK_ERROR(node.error)) {
		if (node.node->directoryEntry->type == LT_NODE_FILE) {
...
Process *ProcessSpawn(ProcessType processType) {
	...
	process->permissions = LT_PERMISSION_ALL;
...
void ProcessRemove(Process *process) {
	...
	if (process->executableNode) {
		...
		CloseHandleToObject(process->executableNode, KERNEL_OBJECT_NODE, LT_FILE_READ);
...
void ProcessCrash(Process *process, EsCrashReason *crashReason) {
	...
	if (!process->crashed) {
		...
		if (!scheduler.shutdown) {
			...
			m.message.type = LT_MSG_APPLICATION_CRASH;
...
EsHandle DesktopOpenHandle(void *object, uint32_t flags, KernelObjectType type) {
	EsHandle result = LT_INVALID_HANDLE;
	bool close = false;
	KMutexAcquire(&desktopMutex);
