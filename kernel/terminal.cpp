...
uintptr_t DebugReadNumber() {
	...
	for (uintptr_t i = 0; i < 2 * sizeof(uintptr_t); i++) {
		...
		while (true) {
			int key = KWaitKey();
			if (key == LT_SCANCODE_0) { EsPrint("0"); value |= 0; }
			else if (key == LT_SCANCODE_1) { EsPrint("1"); value |= 1; }
			else if (key == LT_SCANCODE_2) { EsPrint("2"); value |= 2; }
			else if (key == LT_SCANCODE_3) { EsPrint("3"); value |= 3; }
			else if (key == LT_SCANCODE_4) { EsPrint("4"); value |= 4; }
			else if (key == LT_SCANCODE_5) { EsPrint("5"); value |= 5; }
			else if (key == LT_SCANCODE_6) { EsPrint("6"); value |= 6; }
			else if (key == LT_SCANCODE_7) { EsPrint("7"); value |= 7; }
			else if (key == LT_SCANCODE_8) { EsPrint("8"); value |= 8; }
			else if (key == LT_SCANCODE_9) { EsPrint("9"); value |= 9; }
			else if (key == LT_SCANCODE_A) { EsPrint("A"); value |= 10; }
			else if (key == LT_SCANCODE_B) { EsPrint("B"); value |= 11; }
			else if (key == LT_SCANCODE_C) { EsPrint("C"); value |= 12; }
			else if (key == LT_SCANCODE_D) { EsPrint("D"); value |= 13; }
			else if (key == LT_SCANCODE_E) { EsPrint("E"); value |= 14; }
			else if (key == LT_SCANCODE_F) { EsPrint("F"); value |= 15; }
			else if (key == LT_SCANCODE_ENTER) { value >>= 4; return value; }
			else continue;
			break;
		}
	}
...
void KernelPanic(const char *format, ...) {
	...
	while (true) {
		...
		if (key == LT_SCANCODE_0) {
			...
			while (position < kernelLogEnd) {
				...
				if (y == 
					...
					while (true) {
						...
						if (key == LT_SCANCODE_SPACE || key == LT_SCANCODE_DOWN_ARROW) {
							position = nextPosition;
							break;
						} else if (key == LT_SCANCODE_UP_ARROW) {
							position = nextPosition;
			...
			KWaitKey();
		} else if (key == LT_SCANCODE_1) {
			ProcessorReset();
		} else if (key == LT_SCANCODE_2) {
			...
			while (KWaitKey() != LT_SCANCODE_ENTER);
		} else if (key == LT_SCANCODE_3) {
			EsPrint("Enter address: ");
			uintptr_t address = DebugReadNumber();
			uintptr_t offset = address & (K_PAGE_SIZE - 1);
