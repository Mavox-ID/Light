...
#define TCP_MAKE_STANDARD_REPLY(_flags) \
	{ \
		...
		if (buffer.error) { NetTaskComplete(task, LT_ERROR_INSUFFICIENT_RESOURCES); return; } \
		TCP_PREPARE_REPLY_2(nullptr, 0, nullptr, 0, _flags, 5); \
		if (!NetTransmit(interface, &buffer, NET_PACKET_ETHERNET)) NetTaskComplete(task, LT_ERROR_INSUFFICIENT_RESOURCES); \
	}
...
bool NetARPLookup(NetTask *task, KIPAddress targetIP, KMACAddress *targetMAC) {
	...
	if (buffer.error) {
		NetTaskComplete(task, LT_ERROR_INSUFFICIENT_RESOURCES);
		return false;
	}
...
void NetInterfaceReceive(NetInterface *interface, const uint8_t *data, size_t dataBytes, NetPacketType packetType) {
	...
	if (interface->addressSetupTask.changedState) {
		...
		interface->hasIP = interface->addressSetupTask.error == LT_SUCCESS;
...
void NetDomainNameResolve(NetTask *_task, void *_buffer) {
	...
	if (task->completed) {
		...
		if ((flags & 15) == 3) {
			error = LT_ERROR_NO_ADDRESS_FOR_DOMAIN_NAME;
		} else if ((flags & 15) == 0) {
			error = LT_SUCCESS;
		}
...
void NetEchoRequest(NetTask *_task, void *_buffer) {
	...
	if (task->completed) {
		...
		buffer.Write(task->data, LT_ECHO_REQUEST_MAX_LENGTH);
		...
		if (dataBytes > LT_ECHO_REQUEST_MAX_LENGTH) {
			dataBytes = LT_ECHO_REQUEST_MAX_LENGTH;
		}

		EsMemoryZero(task->data, LT_ECHO_REQUEST_MAX_LENGTH);
		EsMemoryCopy(task->data, data, dataBytes);

		NetTaskComplete(task, LT_SUCCESS);
	} else {
...
void NetTCPConnection(NetTask *_task, void *_data) {
	...
	if (task->step == 0) {
		...
		if ((data->flags & TCP_ACK) && !NetTCPIsBetween(task->sendUnacknowledged, data->ackNumber, task->sendNext)) {
			...
			NetTaskComplete(task, LT_ERROR_CONNECTION_RESET);
		...
		if (data->flags & TCP_RST) {
			if (task->step == TCP_STEP_SYN_RECEIVED) {
				NetTaskComplete(task, LT_ERROR_CONNECTION_REFUSED);
...
void NetAddressSetup(NetTask *_task, void *_buffer) {
	...
	if (messageType == DHCPOFFER) {
		...
		task->changedState = true;
		NetTaskComplete(task, LT_ERROR_LOST_IP_ADDRESS);
	}
}
...
void NetTaskBegin(NetTask *task) {
	...
	if (!task->interface) {
		NetTaskComplete(task, LT_ERROR_NO_CONNECTED_NETWORK_INTERFACES);
	} else {
		task->index = 0xFFFF;
