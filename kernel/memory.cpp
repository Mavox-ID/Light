...
#define MM_OBJECT_CACHE_SHOULD_TRIM() (pmm.approximateTotalObjectCacheBytes / K_PAGE_SIZE > MM_OBJECT_CACHE_PAGLT_MAXIMUM())
...
#define MM_MAP_PAGE_COMMIT_TABLLT_NOW 		(1 << 3)
...
// MMArchUnmapPages.
#define MM_UNMAP_PAGLT_FREE 			(1 << 0)
#define MM_UNMAP_PAGLT_FREE_COPIED		(1 << 1)
#define MM_UNMAP_PAGLT_BALANCE_FILE		(1 << 2)
...
void *MMMapFile(MMSpace *space, struct FSFile *node, EsFileOffset offset, size_t bytes, 
		int protection, void *baseAddress, size_t zeroedBytes = 0, uint32_t additionalFlags = LT_FLAGS_DEFAULT);
...
void MMUpdateAvailablePageCount(bool increase) {
	if (MM_AVAILABLE_PAGES() >= MM_CRITICAL_AVAILABLE_PAGLT_THRESHOLD) {
	...
	if (MM_AVAILABLE_PAGES() >= MM_LOW_AVAILABLE_PAGLT_THRESHOLD) {
...
bool MMHandlePageFault(MMSpace *space, uintptr_t address, unsigned faultFlags) {
	...
	if (!lockAcquired && MM_AVAILABLE_PAGES() < MM_CRITICAL_AVAILABLE_PAGLT_THRESHOLD && GetCurrentThread() && !GetCurrentThread()->isPageGenerator) {
	...
	unsigned flags = LT_FLAGS_DEFAULT;
	...
	if (region->flags & MM_REGION_PHYSICAL) {
		...
		if (copyOnWrite) {
			...
			if (existingTranslation) {
				...
				uintptr_t page = MMPhysicalAllocate(LT_FLAGS_DEFAULT);
				PMCopy(page, (void *) address, 1);
				MMArchUnmapPages(space, address, 1, MM_UNMAP_PAGLT_BALANCE_FILE);
...
void MMUnreserve(MMSpace *space, MMRegion *remove, bool unmapPages, bool guardRegion) {
	...
	if (unmapPages) {
		MMArchUnmapPages(space, remove->baseAddress, remove->pageCount, LT_FLAGS_DEFAULT);
	}
...
void *MMMapFile(MMSpace *space, FSFile *node, EsFileOffset offset, size_t bytes, int protection, void *baseAddress, size_t zeroedBytes, uint32_t additionalFlags) {
	if (protection != LT_MEMORY_MAP_OBJECT_READ_ONLY && protection != LT_MEMORY_MAP_OBJECT_COPY_ON_WRITE) {
		return nullptr;
	}
	...
	uint64_t fileHandleFlags = LT_NODE_PREVENT_RESIZE 
		| (protection == LT_MEMORY_MAP_OBJECT_READ_WRITE ? LT_FILE_WRITE_SHARED : LT_FILE_READ_SHARED);
	bool decommit = false;
	...
	region = MMReserve(space, bytes + zeroedBytes, MM_REGION_FILE | additionalFlags
			| ((protection == LT_MEMORY_MAP_OBJECT_READ_ONLY || protection == LT_MEMORY_MAP_OBJECT_COPY_ON_WRITE) ? MM_REGION_READ_ONLY : 0)
			| (protection == LT_MEMORY_MAP_OBJECT_COPY_ON_WRITE ? MM_REGION_COPY_ON_WRITE : 0),
	...
	if (protection == LT_MEMORY_MAP_OBJECT_COPY_ON_WRITE) {
...
bool MMCommit(uint64_t bytes, bool fixed) {
	...
	if (pmm.commitLimit) {
		if (fixed) {
			...
			if (MM_AVAILABLE_PAGES() - pagesNeeded < MM_CRITICAL_AVAILABLE_PAGLT_THRESHOLD && !GetCurrentThread()->isPageGenerator) {
				KernelLog(LOG_ERROR, "Memory", "failed fixed commit", 
						...
						pagesNeeded, pmm.countZeroedPages, pmm.countFreePages, pmm.countStandbyPages, MM_CRITICAL_AVAILABLE_PAGLT_THRESHOLD);
				return false;
			}
...
bool MMDecommitRange(MMSpace *space, MMRegion *region, uintptr_t pageOffset, size_t pageCount) {
	...
	space->commit -= delta;
	region->data.normal.commitPageCount -= delta;
	MMArchUnmapPages(space, region->baseAddress + pageOffset * K_PAGE_SIZE, pageCount, MM_UNMAP_PAGLT_FREE);
...
void MMAllowWriteCombiningCaching(MMSpace *space, void *virtualAddress) {
	...
	MMArchUnmapPages(space, region->baseAddress, region->pageCount, LT_FLAGS_DEFAULT);

	KMutexRelease(&space->reserveMutex);

	for (uintptr_t i = 0; i < region->pageCount; i++) {
		MMHandlePageFault(space, region->baseAddress + i * K_PAGE_SIZE, LT_FLAGS_DEFAULT);
	}
}
...
bool MMFree(MMSpace *space, void *address, size_t expectedSize, bool userOnly) {
...

	{
		...
		if (region->flags & MM_REGION_NORMAL) {
			...
			MMArchUnmapPages(space, region->baseAddress, region->pageCount, MM_UNMAP_PAGLT_FREE_COPIED | MM_UNMAP_PAGLT_BALANCE_FILE);
			unmapPages = false;
...
void MMBalanceThread() {
	...
	while (true) {
		...
		if (MM_AVAILABLE_PAGES() >= targetAvailablePages) {
			...
			targetAvailablePages = MM_LOW_AVAILABLE_PAGLT_THRESHOLD + MM_PAGLT_TO_FIND_BALANCE;
		}
#else
		...
		OpenHandleToObject(process, KERNEL_OBJECT_PROCESS, LT_FLAGS_DEFAULT);
...

			{
				...
				for (; i < PHYSICAL_MEMORY_MANIPULATION_REGION_PAGES; i++) {
					if (pmm.firstFreePage) {
						...
						MMPhysicalActivatePages(pages[i], 1, LT_FLAGS_DEFAULT);
					} else {
						done = true;
						break;
					}
...
bool MMFaultRange(uintptr_t address, uintptr_t byteCount, uint32_t flags = LT_FLAGS_DEFAULT) {
...
void MMInitialise() {
...

	{
		...
		KMutexAcquire(&kernelMMSpace->reserveMutex);
		pmm.pmManipulationRegion = (void *) MMReserve(kernelMMSpace, PHYSICAL_MEMORY_MANIPULATION_REGION_PAGES * K_PAGE_SIZE, LT_FLAGS_DEFAULT)->baseAddress; 
		KMutexRelease(&kernelMMSpace->reserveMutex);
...

	{
		...
		ThreadSpawn("MMBalance", (uintptr_t) MMBalanceThread, 0, LT_FLAGS_DEFAULT)->isPageGenerator = true;
		ThreadSpawn("MMObjTrim", (uintptr_t) MMObjectCacheTrimThread, 0, LT_FLAGS_DEFAULT);
	}
