// TODO Replace LT_ERROR_UNKNOWN with proper errors.
...
#define SYSCALL_BUFFER(address, length, index, write) \
	...
	if (!_region ## index) SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_BUFFER, true); \
	...
	if (write && (_region ## index->flags & MM_REGION_READ_ONLY) && (~_region ## index->flags & MM_REGION_COPY_ON_WRITE)) \
		SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_BUFFER, true);
#define SYSCALL_HANDLE_2(handle, _type, out) \
	Handle _ ## out; \
	uint8_t status_ ## out = currentProcess->handleTable.ResolveHandle(&_ ## out, handle, _type); \
	if (status_ ## out == RESOLVE_HANDLE_FAILED) SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_HANDLE, true); \
...
#define SYSCALL_READ_HEAP(destination, source, length) \
	*(void **) &(destination) = EsHeapAllocate((length), false, K_FIXED); \
	if ((length) != 0 && !(destination)) SYSCALL_RETURN(LT_ERROR_INSUFFICIENT_RESOURCES, false); \
...
#define SYSCALL_PERMISSION(x) do { if ((x) != (currentProcess->permissions & (x))) { *fatalError = true; return LT_FATAL_ERROR_INSUFFICIENT_PERMISSIONS; } } while (0)
typedef uintptr_t (*SyscallFunction)(SYSCALL_ARGUMENTS);
#pragma GCC diagnostic ignored "-Wunused-parameter" push

SYSCALL_IMPLEMENT(LT_SYSCALL_COUNT) {
	SYSCALL_RETURN(LT_FATAL_ERROR_UNKNOWN_SYSCALL, true);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_PRINT) {
	char *buffer; 
	if (argument1 > SYSCALL_BUFFER_LIMIT) SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_BUFFER, true);
	SYSCALL_READ_HEAP(buffer, argument0, argument1);
	EsPrint("%s", argument1, buffer);
	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_MEMORY_ALLOCATE) {
	if (argument3) {
		if (argument0 > LT_SHARED_MEMORY_MAXIMUM_SIZE) SYSCALL_RETURN(LT_FATAL_ERROR_OUT_OF_RANGE, true);
		MMSharedRegion *region = MMSharedCreateRegion(argument0, false, 0);
		if (!region) SYSCALL_RETURN(LT_ERROR_INSUFFICIENT_RESOURCES, false);
		SYSCALL_RETURN(currentProcess->handleTable.OpenHandle(region, LT_SHARED_MEMORY_READ_WRITE, KERNEL_OBJECT_SHMEM), false);
	} else {
		EsMemoryProtection protection = (EsMemoryProtection) argument2;
		uint32_t flags = MM_REGION_USER;
		if (protection == LT_MEMORY_PROTECTION_READ_ONLY) flags |= MM_REGION_READ_ONLY;
		if (protection == LT_MEMORY_PROTECTION_EXECUTABLE) flags |= MM_REGION_EXECUTABLE;
		uintptr_t address = (uintptr_t) MMStandardAllocate(currentVMM, argument0, flags, nullptr, argument1 & LT_MEMORY_RESERVE_COMMIT_ALL);
		SYSCALL_RETURN(address, false);
	}
}

SYSCALL_IMPLEMENT(LT_SYSCALL_MEMORY_FREE) {
	if (!MMFree(currentVMM, (void *) argument0, argument1, true /* only allow freeing regions marked with MM_REGION_USER */)) {
		SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_MEMORY_REGION, true);
	}

	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_MEMORY_COMMIT) {
	...
	SYSCALL_RETURN(success ? LT_SUCCESS : LT_ERROR_INSUFFICIENT_RESOURCES, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_MEMORY_FAULT_RANGE) {
	bool success = MMFaultRange(argument0, argument1);
	SYSCALL_RETURN(success ? LT_SUCCESS : (intptr_t) LT_FATAL_ERROR_INVALID_BUFFER, !success);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_MEMORY_GET_AVAILABLE) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_PROCESS_CREATE) {
	...
	if (executableObject->directoryEntry->type != LT_NODE_FILE) {
		SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_NODE_TYPE, true);
	}
	...
	if (!handles && arguments.handleCount) {
		SYSCALL_RETURN(LT_ERROR_INSUFFICIENT_RESOURCES, K_PAGED);
	}
	...
	for (uintptr_t i = 0; i < arguments.handleCount; i++) {
		if (RESOLVE_HANDLE_NORMAL != currentProcess->handleTable.ResolveHandle(&handles[i], inHandles[i], HANDLE_SHARE_TYPE_MASK)) {
			...
			error = LT_FATAL_ERROR_INVALID_HANDLE;
		}
	}
	...
	if (error == LT_SUCCESS) {
		...
		if (!process) {
			...
			if (error != LT_SUCCESS || !ProcessStartWithNode(process, executableObject)) {
				error = LT_ERROR_UNKNOWN; // TODO.
...
SYSCALL_IMPLEMENT(LT_SYSCALL_SCREEN_FORCE_UPDATE) {
	if (argument0) {
		SYSCALL_PERMISSION(LT_PERMISSION_SCREEN_MODIFY);
	}

	KMutexAcquire(&windowManager.mutex);

	if (argument0) {
		windowManager.Redraw(LT_POINT(0, 0), graphics.frameBuffer.width, graphics.frameBuffer.height, nullptr);
	}
...
SYSCALL_IMPLEMENT(LT_SYSCALL_MESSAGE_GET) {
	...
	if (currentProcess->messageQueue.GetMessage(&message)) {
		...
		SYSCALL_RETURN(LT_ERROR_NO_MESSAGES_AVAILABLE, false);
	}
}

SYSCALL_IMPLEMENT(LT_SYSCALL_MESSAGE_WAIT) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_WINDOW_CREATE) {
	SYSCALL_PERMISSION(LT_PERMISSION_WINDOW_MANAGER);

	if (argument0 == LT_WINDOW_NORMAL) {
		...
		if (!_window) {
			SYSCALL_RETURN(LT_ERROR_INSUFFICIENT_RESOURCES, false);
		} else {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_WINDOW_CLOSE) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_WINDOW_SET_PROPERTY) {
	...
	if (property == LT_WINDOW_PROPERTY_SOLID) {
		window->solid = (argument1 & LT_WINDOW_SOLID_TRUE) != 0;
		window->noClickActivate = (argument1 & LT_WINDOW_SOLID_NO_ACTIVATE) != 0;
		window->noBringToFront = (argument1 & LT_WINDOW_SOLID_NO_BRING_TO_FRONT) != 0;
		EsRectangle solidInsets = LT_RECT_1(0);
		if (argument2) SYSCALL_READ(&solidInsets, argument2, sizeof(EsRectangle));
		window->solidOffsets = LT_RECT_4(solidInsets.l, -solidInsets.r, solidInsets.t, -solidInsets.b);
		KMutexAcquire(&windowManager.mutex);
		windowManager.MoveCursor(0, 0);
		KMutexRelease(&windowManager.mutex);
	} else if (property == LT_WINDOW_PROPERTY_OPAQUE_BOUNDS) {
		SYSCALL_READ(&window->opaqueBounds, argument1, sizeof(EsRectangle));
	} else if (property == LT_WINDOW_PROPERTY_BLUR_BOUNDS) {
		SYSCALL_READ(&window->blurBounds, argument1, sizeof(EsRectangle));
	} else if (property == LT_WINDOW_PROPERTY_ALPHA) {
		window->alpha = argument1 & 0xFF;
	} else if (property == LT_WINDOW_PROPERTY_FOCUSED) {
		KMutexAcquire(&windowManager.mutex);
		windowManager.ActivateWindow(window);
		windowManager.MoveCursor(0, 0);
		KMutexRelease(&windowManager.mutex);
	} else if (property == LT_WINDOW_PROPERTY_MATERIAL) {
		window->material = argument1;
	} else if (property == LT_WINDOW_PROPERTY_EMBED) {
		...
		KMutexRelease(&windowManager.mutex);
	} else if (property == LT_WINDOW_PROPERTY_EMBED_INSETS) {
		KMutexAcquire(&windowManager.mutex);
		SYSCALL_READ(&window->embedInsets, argument1, sizeof(EsRectangle));
		window->ResizeEmbed();
		KMutexRelease(&windowManager.mutex);
	} else if (property == LT_WINDOW_PROPERTY_OBJECT) {
		...
		KMutexRelease(&windowManager.mutex);
	} else if (property == LT_WINDOW_PROPERTY_EMBED_OWNER) {
		...
		SYSCALL_RETURN(LT_FATAL_ERROR_OUT_OF_RANGE, true);
	}

	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_WINDOW_REDRAW) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_WINDOW_SET_BITS) {
	SYSCALL_HANDLE_2(argument0, (KernelObjectType) (KERNEL_OBJECT_WINDOW | KERNEL_OBJECT_EMBEDDED_WINDOW), _window);

	EsRectangle region;
	SYSCALL_READ(&region, argument1, sizeof(EsRectangle));

	if (!LT_RECT_VALID(region)) {
		SYSCALL_RETURN(LT_SUCCESS, false);
	}
	...
	EsRectangle clippedRegion = EsRectangleIntersection(region, LT_RECT_2S(surface->width, surface->height));

	EsRectangle directUpdateSubRegion;

	if (window->style == LT_WINDOW_CONTAINER && !isEmbed) {
		directUpdateSubRegion = LT_RECT_4(0, window->width, 0, insets.t);
	} else if (window->style == LT_WINDOW_CONTAINER && isEmbed) {
		directUpdateSubRegion = LT_RECT_4(insets.l, window->width - insets.r, insets.t, window->height - insets.b);
	} else {
		directUpdateSubRegion = LT_RECT_4(0, window->width, 0, window->height);
	}
...
EsRectangle subRegion = EsRectangleIntersection(clippedRegion, LT_RECT_4(__VA_ARGS__)); \
if (LT_RECT_VALID(subRegion)) { surface->SetBits((K_USER_BUFFER const uint8_t *) argument2 \
+ stride * (subRegion.t - clippedRegion.t) + 4 * (subRegion.l - clippedRegion.l), stride, subRegion); } }
	...
	if (resizeQueued) {
		window->Move(windowManager.resizeQueuedRectangle, LT_WINDOW_MOVE_DYNAMIC);
	}
...
SYSCALL_IMPLEMENT(LT_SYSCALL_EVENT_CREATE) {
	KEvent *event = (KEvent *) EsHeapAllocate(sizeof(KEvent), true, K_FIXED);
	if (!event) SYSCALL_RETURN(LT_ERROR_INSUFFICIENT_RESOURCES, false);
	event->handles = 1;
	event->autoReset = argument0;
	SYSCALL_RETURN(currentProcess->handleTable.OpenHandle(event, 0, KERNEL_OBJECT_EVENT), false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_HANDLE_CLOSE) {
	if (!currentProcess->handleTable.CloseHandle(argument0)) {
		SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_HANDLE, true);
	}

	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_THREAD_TERMINATE) {
	bool self = false;
...
SYSCALL_IMPLEMENT(LT_SYSCALL_PROCESS_TERMINATE) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_THREAD_CREATE) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_MEMORY_MAP_OBJECT) {
	SYSCALL_HANDLE_2(argument0, (KernelObjectType) (KERNEL_OBJECT_SHMEM | KERNEL_OBJECT_NODE), object);

	if (((argument3 & LT_MEMORY_MAP_OBJECT_READ_WRITE) ? 1 : 0)
			+ ((argument3 & LT_MEMORY_MAP_OBJECT_READ_ONLY) ? 1 : 0)
			+ ((argument3 & LT_MEMORY_MAP_OBJECT_COPY_ON_WRITE) ? 1 : 0) != 1) {
		SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_FILE_ACCESS, true);
	}

	if (object.type == KERNEL_OBJECT_SHMEM) {
		MMSharedRegion *region = (MMSharedRegion *) object.object;

		if (argument2 == LT_MEMORY_MAP_OBJECT_ALL) {
			argument2 = region->sizeBytes;
		}

		if ((argument3 == LT_MEMORY_MAP_OBJECT_READ_WRITE && (~object.flags & LT_SHARED_MEMORY_READ_WRITE))
				|| argument3 == LT_MEMORY_MAP_OBJECT_COPY_ON_WRITE) {
			SYSCALL_RETURN(LT_FATAL_ERROR_INSUFFICIENT_PERMISSIONS, true);
		}

		uint32_t flags = MM_REGION_USER | ((argument3 & LT_MEMORY_MAP_OBJECT_READ_ONLY) ? MM_REGION_READ_ONLY : 0);
		...
		if (file->directoryEntry->type != LT_NODE_FILE) SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_NODE_TYPE, true);

		if (argument3 == LT_MEMORY_MAP_OBJECT_READ_WRITE) {
			if (!(object.flags & (LT_FILE_WRITE_SHARED | LT_FILE_WRITE))) {
				SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_FILE_ACCESS, true);
			}
		} else {
			if (!(object.flags & (LT_FILE_READ | LT_FILE_READ_SHARED))) {
				SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_FILE_ACCESS, true);
			}
		}
	...
	KernelPanic("LT_SYSCALL_MEMORY_MAP_OBJECT - Unhandled case.\n");
	SYSCALL_RETURN(0, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_CONSTANT_BUFFER_CREATE) {
	if (argument2 > SYSCALL_BUFFER_LIMIT) SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_BUFFER, true);
	SYSCALL_HANDLE(argument1, KERNEL_OBJECT_PROCESS, process, Process);
...
SYSCALL_IMPLEMENT(LT_SYSCALL_HANDLE_SHARE) {
	SYSCALL_HANDLE_2(argument0, HANDLE_SHARE_TYPE_MASK, share);
	SYSCALL_HANDLE(argument1, KERNEL_OBJECT_PROCESS, process, Process);
	SYSCALL_RETURN(HandleShare(share, process, argument2), false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_VOLUME_GET_INFORMATION) {
	if (~currentProcess->permissions & LT_PERMISSION_GET_VOLUME_INFORMATION) {
		SYSCALL_RETURN(LT_ERROR_PERMISSION_NOT_GRANTED, false);
	}
...
SYSCALL_IMPLEMENT(LT_SYSCALL_NODE_OPEN) {
	size_t pathLength = (size_t) argument1;
	uint64_t flags = (uint64_t) argument2;

	flags &= ~LT__NODE_FROM_WRITE_EXCLUSIVE | LT__NODE_NO_WRITE_BASE;

	bool needWritePermission = flags & (LT_FILE_WRITE | LT_FILE_WRITE_SHARED | LT__NODE_DIRECTORY_WRITE);

	char *path;
	if (argument1 > K_MAX_PATH) SYSCALL_RETURN(LT_FATAL_ERROR_OUT_OF_RANGE, true);
	...
	if (_directory.type == KERNEL_OBJECT_DEVICE) {
		...
		if (device->type == LT_DEVICE_FILE_SYSTEM) {
			KFileSystem *fileSystem = (KFileSystem *) device;
			directory = fileSystem->rootDirectory;
			directoryFlags = LT__NODE_DIRECTORY_WRITE;
		} else {
			SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_NODE_TYPE, true);
		}
	...
	if (directory->directoryEntry->type != LT_NODE_DIRECTORY) {
		SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_NODE_TYPE, true);
	}

	if ((~directoryFlags & LT__NODE_DIRECTORY_WRITE) && needWritePermission) {
		SYSCALL_RETURN(LT_ERROR_PERMISSION_NOT_GRANTED, false);
	}

	if (~directoryFlags & LT__NODE_DIRECTORY_WRITE) {
		flags |= LT__NODE_NO_WRITE_BASE;
	}
	...
	if (flags & LT_FILE_WRITE) {
		...
		flags |= LT__NODE_FROM_WRITE_EXCLUSIVE;
	}
...
SYSCALL_IMPLEMENT(LT_SYSCALL_NODE_DELETE) {
	SYSCALL_HANDLE_2(argument0, KERNEL_OBJECT_NODE, handle);
	KNode *node = (KNode *) handle.object; 

	if (handle.flags & LT__NODE_NO_WRITE_BASE) {
		SYSCALL_RETURN(LT_ERROR_PERMISSION_NOT_GRANTED, false);
	}
	
	if (node->directoryEntry->type == LT_NODE_FILE && (~handle.flags & LT_FILE_WRITE)) {
		SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_FILE_ACCESS, true);
	}

	if (node->directoryEntry->type == LT_NODE_DIRECTORY || node->directoryEntry->type == LT_NODE_FILE) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_NODE_MOVE) {
	...
	char *newPath;
	if (argument3 > SYSCALL_BUFFER_LIMIT) SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_BUFFER, true);
	SYSCALL_READ_HEAP(newPath, argument2, argument3);
	SYSCALL_RETURN(FSNodeMove(file, directory, newPath, (size_t) argument3), false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_FILE_READ_SYNC) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_FILE_WRITE_SYNC) {
	if (!argument2) SYSCALL_RETURN(0, false);
	...
	if (handle.flags & (LT_FILE_WRITE_SHARED | LT_FILE_WRITE)) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_FILE_GET_SIZE) {
	SYSCALL_HANDLE(argument0, KERNEL_OBJECT_NODE, file, KNode);
	if (file->directoryEntry->type != LT_NODE_FILE) SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_NODE_TYPE, true);
	SYSCALL_RETURN(file->directoryEntry->totalSize, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_FILE_RESIZE) {
	SYSCALL_HANDLE_2(argument0, KERNEL_OBJECT_NODE, handle);
	KNode *file = (KNode *) handle.object; 
...
SYSCALL_IMPLEMENT(LT_SYSCALL_EVENT_SET) {
	SYSCALL_HANDLE(argument0, KERNEL_OBJECT_EVENT, event, KEvent);
	KEventSet(event, true);
	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_EVENT_RESET) {
	SYSCALL_HANDLE(argument0, KERNEL_OBJECT_EVENT, event, KEvent);
	KEventReset(event);
	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_SLEEP) {
	KTimer timer = {};
#ifdef LT_BITS_64
...
#endif
	currentThread->terminatableState = THREAD_USER_BLOCK_REQUEST;
	KEventWait(&timer.event, LT_WAIT_NO_TIMEOUT);
	currentThread->terminatableState = THREAD_IN_SYSCALL;
	KTimerRemove(&timer);
	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_WAIT) {
	if (argument1 >= LT_MAX_WAIT_COUNT - 1 /* leave room for timeout timer */) {
		SYSCALL_RETURN(LT_FATAL_ERROR_OUT_OF_RANGE, true);
	}

	EsHandle handles[LT_MAX_WAIT_COUNT];
	KEvent *events[LT_MAX_WAIT_COUNT];
	Handle _objects[LT_MAX_WAIT_COUNT];
	uint8_t status[LT_MAX_WAIT_COUNT];
	...
	if (!handleErrors) {
		...
		if (argument2 != (uintptr_t) LT_WAIT_NO_TIMEOUT) {
		...
		if (waitReturnValue == argument1) {
			waitReturnValue = LT_ERROR_TIMEOUT_REACHED;
		}
...
SYSCALL_IMPLEMENT(LT_SYSCALL_WINDOW_SET_CURSOR) {
	SYSCALL_HANDLE_2(argument0, (KernelObjectType) (KERNEL_OBJECT_WINDOW | KERNEL_OBJECT_EMBEDDED_WINDOW), _window);
	...
	if (!ignore && !window->closed && different && !windowManager.eyedropping && (windowManager.hoverWindow == window || !windowManager.hoverWindow)) {
		...
		if (windowManager.cursorSurface.Resize(width, height)
			...
			windowManager.cursorSurface.SetBits((K_USER_BUFFER const void *) argument1, argument3 & 0xFFFFFF, 
					LT_RECT_4PD(CURSOR_PADDING_L, CURSOR_PADDING_T, imageWidth, imageHeight));
...
SYSCALL_IMPLEMENT(LT_SYSCALL_WINDOW_MOVE) {
	SYSCALL_HANDLE(argument0, KERNEL_OBJECT_WINDOW, window, Window);
	...
	if (argument3 & LT_WINDOW_MOVE_HIDDEN) {
		windowManager.HideWindow(window);
	} else {
		window->Move(rectangle, argument3);
	}

	if (argument3 & LT_WINDOW_MOVE_UPDATE_SCREEN) {
		GraphicsUpdateScreen();
	}

	KMutexRelease(&windowManager.mutex);

	SYSCALL_RETURN(success ? LT_SUCCESS : LT_ERROR_INVALID_DIMENSIONS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_WINDOW_TRANSFER_PRESS) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_WINDOW_FIND_BY_POINT) {
	SYSCALL_PERMISSION(LT_PERMISSION_SCREEN_MODIFY);
	KMutexAcquire(&windowManager.mutex);
...
SYSCALL_IMPLEMENT(LT_SYSCALL_CURSOR_POSITION_GET) {
	EsPoint point = LT_POINT(windowManager.cursorX, windowManager.cursorY);
	SYSCALL_WRITE(argument0, &point, sizeof(EsPoint));
	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_CURSOR_POSITION_SET) {
	KMutexAcquire(&windowManager.mutex);
	...
	KMutexRelease(&windowManager.mutex);
	SYSCALL_RETURN(allowed ? LT_SUCCESS : LT_ERROR_PERMISSION_NOT_GRANTED, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_CURSOR_PROPERTILT_SET) {
	SYSCALL_PERMISSION(LT_PERMISSION_SCREEN_MODIFY);
	KMutexAcquire(&windowManager.mutex);
...
SYSCALL_IMPLEMENT(LT_SYSCALL_GAME_CONTROLLER_STATE_POLL) {
	EsGameControllerState gameControllers[LT_GAME_CONTROLLER_MAX_COUNT];
...
SYSCALL_IMPLEMENT(LT_SYSCALL_WINDOW_GET_BOUNDS) {
	SYSCALL_HANDLE_2(argument0, (KernelObjectType) (KERNEL_OBJECT_WINDOW | KERNEL_OBJECT_EMBEDDED_WINDOW), _window);
...
SYSCALL_IMPLEMENT(LT_SYSCALL_MESSAGE_POST) {
	...
	if (process->messageQueue.SendMessage(&message)) {
		...
		SYSCALL_RETURN(LT_ERROR_MESSAGE_QUEUE_FULL, false);
	}
}

SYSCALL_IMPLEMENT(LT_SYSCALL_THREAD_GET_ID) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_THREAD_STACK_SIZE) {
	SYSCALL_HANDLE(argument0, KERNEL_OBJECT_THREAD, thread, Thread);
	...
	if (argument3) {
		...
		if (argument3 >= K_PAGE_SIZE /* see LT_SYSCALL_THREAD_SET_TIMER_ADJUST_ADDRESS */
...
SYSCALL_IMPLEMENT(LT_SYSCALL_DIRECTORY_ENUMERATE) {
	SYSCALL_HANDLE(argument0, KERNEL_OBJECT_NODE, node, KNode);
	
	if (node->directoryEntry->type != LT_NODE_DIRECTORY) SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_NODE_TYPE, true);

	if (argument2 > SYSCALL_BUFFER_LIMIT / sizeof(EsDirectoryChild)) SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_BUFFER, true);
...
SYSCALL_IMPLEMENT(LT_SYSCALL_FILE_CONTROL) {
	SYSCALL_HANDLE_2(argument0, KERNEL_OBJECT_NODE, handle);
	KNode *file = (KNode *) handle.object; 

	if (file->directoryEntry->type != LT_NODE_FILE) {
		SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_NODE_TYPE, true);
	}

	EsError error = LT_ERROR_UNSUPPORTED;

	if (argument1 == LT_FILE_CONTROL_FLUSH) {
		if ((handle.flags & (LT_FILE_WRITE_SHARED | LT_FILE_WRITE)) == 0) {
			SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_FILE_ACCESS, true);
		}

		error = FSFileControlFlush(file);
	} else if (argument1 == LT_FILE_CONTROL_SET_CONTENT_TYPE) {
		if ((handle.flags & (LT_FILE_WRITE_SHARED | LT_FILE_WRITE)) == 0) {
			SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_FILE_ACCESS, true);
		}
	...
	} else if (argument1 == LT_FILE_CONTROL_GET_CONTENT_TYPE) {
		EsUniqueIdentifier identifier;
		KWriterLockTake(&file->writerLock, K_LOCK_SHARED);

		if (file->fileSystem->flags & LT_VOLUME_STORLT_CONTENT_TYPE) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_BATCH) {
	EsBatchCall *calls;
	if (argument1 > SYSCALL_BUFFER_LIMIT / sizeof(EsBatchCall)) SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_BUFFER, true);
	...
	for (uintptr_t i = 0; i < count; i++) {
		...
		if (calls[i].stopBatchIfError && LT_CHECK_ERROR(_returnValue)) break;
...
SYSCALL_IMPLEMENT(LT_SYSCALL_CONSTANT_BUFFER_READ) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_PROCESS_GET_STATE) {
	SYSCALL_HANDLE(argument0, KERNEL_OBJECT_PROCESS, process, Process);
	...
	state.flags = (process->allThreadsTerminated ? LT_PROCESS_STATE_ALL_THREADS_TERMINATED : 0)
		| (process->preventNewThreads ? LT_PROCESS_STATE_TERMINATING : 0)
		| (process->crashed ? LT_PROCESS_STATE_CRASHED : 0)
#ifdef PAUSE_ON_USERLAND_CRASH
		| (process->pausedFromCrash ? LT_PROCESS_STATE__PAUSED_FROM_CRASH : 0)
#endif
		| (process->messageQueue.pinged ? LT_PROCESS_STATE__PINGED : 0);

	SYSCALL_WRITE(argument1, &state, sizeof(EsProcessState));
	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_SHUTDOWN) {
	SYSCALL_PERMISSION(LT_PERMISSION_SHUTDOWN);
	shutdownAction = argument0;
	KEventSet(&shutdownEvent, true);
	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_WINDOW_GET_ID) {
	SYSCALL_HANDLE_2(argument0, (KernelObjectType) (KERNEL_OBJECT_WINDOW | KERNEL_OBJECT_EMBEDDED_WINDOW), _window);
...
SYSCALL_IMPLEMENT(LT_SYSCALL_YIELD_SCHEDULER) {
	ProcessorFakeTimerInterrupt();
	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_SYSTEM_TAKE_SNAPSHOT) {
	SYSCALL_PERMISSION(LT_PERMISSION_TAKE_SYSTEM_SNAPSHOT);
	...
	switch (type) {
		case LT_SYSTEM_SNAPSHOT_PROCESSES: {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_PROCESSOR_COUNT) {
	SYSCALL_RETURN(scheduler.nextProcessorID, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_PROCESS_OPEN) {
	SYSCALL_PERMISSION(LT_PERMISSION_PROCESS_OPEN);
	...
	if (process) {
		...
		SYSCALL_RETURN(LT_INVALID_HANDLE, false);
	}
}

SYSCALL_IMPLEMENT(LT_SYSCALL_THREAD_SET_TLS) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_THREAD_SET_TIMER_ADJUST_ADDRESS) {
#ifdef K_ARCH_STACK_GROWS_DOWN
...
SYSCALL_IMPLEMENT(LT_SYSCALL_PROCESS_GET_TLS) {
	SYSCALL_RETURN(currentThread->tlsAddress, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_SCREEN_GET_BITS) {
	SYSCALL_PERMISSION(LT_PERMISSION_SCREEN_MODIFY);
	EsRectangle rectangle;
	SYSCALL_READ(&rectangle, argument1, sizeof(EsRectangle));
	...
	EsRectangle source = EsRectangleIntersection(rectangle, LT_RECT_2S(graphics.frameBuffer.width, graphics.frameBuffer.height));
	uint32_t *bits = (uint32_t *) graphics.frameBuffer.bits;
	uint32_t *destination = (uint32_t *) argument2;

	if (LT_RECT_VALID(source)) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_SCREEN_BOUNDS_GET) {
	EsRectangle rectangle;
	EsMemoryZero(&rectangle, sizeof(EsRectangle));
...
SYSCALL_IMPLEMENT(LT_SYSCALL_SCREEN_WORK_AREA_SET) {
	SYSCALL_PERMISSION(LT_PERMISSION_SCREEN_MODIFY);
	EsRectangle rectangle;
	SYSCALL_READ(&rectangle, argument1, sizeof(EsRectangle));
...
SYSCALL_IMPLEMENT(LT_SYSCALL_SCREEN_WORK_AREA_GET) {
...
#ifdef ENABLE_POSIX_SUBSYSTEM
SYSCALL_IMPLEMENT(LT_SYSCALL_POSIX) {
	SYSCALL_PERMISSION(LT_PERMISSION_POSIX_SUBSYSTEM);
	...
	if (syscall.index == 2 /* open */ || syscall.index == 59 /* execve */) {
		...
		if (~node.flags & LT__NODE_DIRECTORY_WRITE) SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_HANDLE, true);
...
SYSCALL_IMPLEMENT(LT_SYSCALL_PROCESS_GET_STATUS) {
	SYSCALL_HANDLE(argument0, KERNEL_OBJECT_PROCESS, process, Process);
	SYSCALL_RETURN(process->exitStatus, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_PIPE_CREATE) {
	Pipe *pipe = (Pipe *) EsHeapAllocate(sizeof(Pipe), true, K_PAGED);
	if (!pipe) SYSCALL_RETURN(LT_ERROR_INSUFFICIENT_RESOURCES, false);
...
SYSCALL_IMPLEMENT(LT_SYSCALL_PIPE_READ) {
	if (!argument2) SYSCALL_RETURN(LT_SUCCESS, false);
	SYSCALL_HANDLE_2(argument0, KERNEL_OBJECT_PIPE, _pipe);
	Pipe *pipe = (Pipe *) _pipe.object;
	if ((~_pipe.flags & PIPE_READER)) SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_FILE_ACCESS, true);
...
SYSCALL_IMPLEMENT(LT_SYSCALL_PIPE_WRITE) {
	if (!argument2) SYSCALL_RETURN(LT_SUCCESS, false);
	SYSCALL_HANDLE_2(argument0, KERNEL_OBJECT_PIPE, _pipe);
	Pipe *pipe = (Pipe *) _pipe.object;
	if ((~_pipe.flags & PIPE_WRITER)) SYSCALL_RETURN(LT_FATAL_ERROR_INCORRECT_FILE_ACCESS, true);
	SYSCALL_BUFFER(argument1, argument2, 2, true /* write */);
	SYSCALL_RETURN(pipe->Access((void *) argument1, argument2, true, true), false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_DOMAIN_NAME_RESOLVE) {
	SYSCALL_PERMISSION(LT_PERMISSION_NETWORKING);

	if (argument1 > LT_DOMAIN_NAME_MAX_LENGTH) {
		SYSCALL_RETURN(LT_ERROR_BAD_DOMAIN_NAME, false);
	}

	char domainName[LT_DOMAIN_NAME_MAX_LENGTH];
...
SYSCALL_IMPLEMENT(LT_SYSCALL_ECHO_REQUEST) {
	SYSCALL_PERMISSION(LT_PERMISSION_NETWORKING);

	if (argument1 > LT_ECHO_REQUEST_MAX_LENGTH) {
		SYSCALL_RETURN(LT_FATAL_ERROR_INVALID_BUFFER, true);
	}
...
SYSCALL_IMPLEMENT(LT_SYSCALL_CONNECTION_OPEN) {
	SYSCALL_PERMISSION(LT_PERMISSION_NETWORKING);
	...
	if (connection.sendBufferBytes < 1024 || connection.receiveBufferBytes < 1024) {
		SYSCALL_RETURN(LT_ERROR_BUFFER_TOO_SMALL, false);
	}
...
SYSCALL_IMPLEMENT(LT_SYSCALL_CONNECTION_POLL) {
	...
	connection->error = netConnection->task.completed ? netConnection->task.error : LT_SUCCESS;

	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_CONNECTION_NOTIFY) {
	SYSCALL_HANDLE(argument3, KERNEL_OBJECT_CONNECTION, netConnection, NetConnection);
	NetConnectionNotify(netConnection, argument1, argument2);
	SYSCALL_RETURN(LT_SUCCESS, false);
}

SYSCALL_IMPLEMENT(LT_SYSCALL_DEVICE_CONTROL) {
	...
	if (device->type == LT_DEVICE_BLOCK) {
		...
		if (type == LT_DEVICE_CONTROL_BLOCK_GET_INFORMATION) {
			SYSCALL_WRITE(dp, &block->information, sizeof(EsBlockDeviceInformation));
		} else if (type == LT_DEVICE_CONTROL_BLOCK_READ || type == LT_DEVICE_CONTROL_BLOCK_WRITE) {
			bool write = type == LT_DEVICE_CONTROL_BLOCK_WRITE;
		...
		} else if (type == LT_DEVICE_CONTROL_BLOCK_DETECT_FS) {
			KDeviceOpenHandle(block);
			FSDetectFileSystem(block); // Closes handle.
		} else {
			SYSCALL_RETURN(LT_FATAL_ERROR_UNKNOWN_SYSCALL, true);
		}
	} else if (device->type == LT_DEVICE_CLOCK) {
		KClockDevice *clock = (KClockDevice *) device;

		if (type == LT_DEVICE_CONTROL_CLOCK_READ) {
	...
	} else if (device->type == LT_DEVICE_FILE_SYSTEM) {
		KFileSystem *fileSystem = (KFileSystem *) device;

		if (type == LT_DEVICE_CONTROL_FS_IS_BOOT) {
...
SYSCALL_IMPLEMENT(LT_SYSCALL_DEBUG_COMMAND) {
	SYSCALL_PERMISSION(LT_PERMISSION_TAKE_SYSTEM_SNAPSHOT);
	...
	if (argument0 == 3) {
		...
		statistics.maximumObjectCachePages = MM_OBJECT_CACHE_PAGLT_MAXIMUM();
...
const SyscallFunction syscallFunctions[LT_SYSCALL_COUNT + 1] {
...
uintptr_t DoSyscall(EsSyscallType index, uintptr_t argument0, uintptr_t argument1, uintptr_t argument2, uintptr_t argument3,
	...
	EsError returnValue = LT_FATAL_ERROR_UNKNOWN_SYSCALL;
	bool fatalError = true;

	if (index < LT_SYSCALL_COUNT) {
		SyscallFunction function = syscallFunctions[index];

		if (batched && index == LT_SYSCALL_BATCH) {
