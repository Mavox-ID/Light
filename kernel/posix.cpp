...
#include <errno.h>
	...
	intptr_t Read(POSIXFile *file, K_USER_BUFFER void *base, size_t length, bool baseMappedToFile) {
		...
		if (file->type == POSIX_FILE_TERMINAL) {
			...
			if (LT_CHECK_ERROR(count)) return -EIO;
	...
	uintptr_t DoSyscall(_EsPOSIXSyscall syscall, uintptr_t *userStackPointer) {
		...
		switch (syscall.index) {
			case SYS_open: {
				...
				uint64_t openFlags = LT_FLAGS_DEFAULT;
				...
				if ((EsCStringLength(devZero) == pathLength && 0 == EsMemoryCompare(path, devZero, pathLength))) {
					...
					if (flags & O_EXCL) openFlags |= LT_NODE_FAIL_IF_FOUND;
					else if (flags & O_CREAT) {}
					else openFlags |= LT_NODE_FAIL_IF_NOT_FOUND;
					if (flags & O_DIRECTORY) openFlags |= LT_NODE_DIRECTORY;
					if (flags & O_APPEND) openFlags |= LT_FILE_WRITE_SHARED;
					else if (flags & O_RDWR) openFlags |= LT_FILE_WRITE_SHARED;
					else if (flags & O_WRONLY) openFlags |= LT_FILE_WRITE_SHARED;
					else if (!(flags & O_PATH)) openFlags |= LT_FILE_READ;
					...
					if (!information.node) {
						...
						return (information.error == LT_ERROR_FILE_DOLT_NOT_EXIST || information.error == LT_ERROR_PATH_NOT_TRAVERSABLE) ? -ENOENT : -EACCES;
					}

					if (information.node->directoryEntry->type == LT_NODE_DIRECTORY && !(flags & O_DIRECTORY) && !(flags & O_PATH)) {
						EsHeapFree(file, sizeof(POSIXFile), K_FIXED);
					...
					file->type = file->node->directoryEntry->type == LT_NODE_DIRECTORY ? POSIX_FILE_DIRECTORY : POSIX_FILE_NORMAL;
				}
				...
				// EsPrint("OPEN '%s' %z\n", pathLength, path, (openFlags & LT_NODE_WRITE_ACCESS) ? "Write" : ((openFlags & LT_NODE_READ_ACCESS) ? "Read" : "None"));

				return handleTable->OpenHandle(file, (flags & O_CLOEXEC) ? FD_CLOEXEC : 0, KERNEL_OBJECT_POSIX_FD) ?: -ENFILE;
			} break;

			case LT_POSIX_SYSCALL_GET_POSIX_FD_PATH: {
			...
			case SYS_write: {
				...
				if (file->type == POSIX_FILE_NORMAL && !(file->openFlags & (LT_FILE_WRITE_SHARED | LT_FILE_WRITE))) {
					return -EACCES;
				}
			...
			case SYS_execve: {
				...
				process->data.subsystemID = LT_SUBSYSTEM_ID_POSIX;
				...
				OpenHandleToObject((void *) syscall.arguments[4], KERNEL_OBJECT_NODE, LT__NODE_DIRECTORY_WRITE);
				systemData.mountPoint.base = process->handleTable.OpenHandle((void *) syscall.arguments[4], LT__NODE_DIRECTORY_WRITE, KERNEL_OBJECT_NODE);
			...
			case SYS_getdents64: {
				...
				if (!file->offsetIntoFile || !file->directoryBuffer) {
					...
					for (uintptr_t i = 0; i < count; i++) {
						...
						((uint8_t *) position)[18] = entry->type == LT_NODE_DIRECTORY ? 4 /* DT_DIR */
							: entry->type == LT_NODE_FILE ? 8 /* DT_REG */ : 0 /* DT_UNKNOWN */;
