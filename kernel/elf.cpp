...
EsError KLoadELF(KNode *node, KLoadedExecutable *executable) {
...

	{
		...
		if (header.signature == BUNDLE_SIGNATURE 
			...
			if (!MMMapFile(thisProcess->vmm, (FSFile *) node, 
					0, fileSize, LT_MEMORY_MAP_OBJECT_READ_ONLY, 
	...
	for (uintptr_t i = 0; i < header.programHeaderEntries; i++) {
		...
		if (header->type == 1 /* PT_LOAD */) {
			...
			success = MMStandardAllocate(thisProcess->vmm, RoundUp(header->segmentSize, K_PAGE_SIZE), LT_FLAGS_DEFAULT, 
			...
			if (success) {
				...
				if (bytesRead != header->dataInFile) return LT_ERROR_UNSUPPORTED;
			}
#else
			...
			success = MMMapFile(thisProcess->vmm, (FSFile *) node, 
					...
					LT_MEMORY_MAP_OBJECT_COPY_ON_WRITE, 
			...
			if (!success) return LT_ERROR_INSUFFICIENT_RESOURCES;
...
EsError KLoadELFModule(KModule *module) {
	...
	if (node.error != LT_SUCCESS) return node.error;
	...
	for (uintptr_t i = 0; i < header->sectionHeaderEntries; i++) {
		...
		if (!memory) return LT_ERROR_INSUFFICIENT_RESOURCES; // TODO Free allocations.
	...
	if (unresolvedSymbols) {
		return LT_ERROR_COULD_NOT_RESOLVE_SYMBOL;
	}
