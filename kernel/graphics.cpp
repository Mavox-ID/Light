...
void GraphicsUpdateScreen(K_USER_BUFFER void *bits, EsRectangle *bounds, uintptr_t bitsStride) {
	...
	if (bits) {
		...
		_bounds = LT_RECT_4(0, sourceSurface->width, 0, sourceSurface->height);
		bounds = &_bounds;
	}

	EsRectangle cursorBounds = LT_RECT_4(cursorX, cursorX + windowManager.cursorSwap.width, cursorY, cursorY + windowManager.cursorSwap.height);
	EsRectangleClip(LT_RECT_4(0, Width(*bounds), 0, Height(*bounds)), cursorBounds, &cursorBounds);

	windowManager.cursorSwap.Copy(sourceSurface, LT_POINT(0, 0), cursorBounds, true);
	...
	sourceSurface->Draw(&windowManager.cursorSurface, LT_RECT_4(cursorX, cursorX + cursorImageWidth, cursorY, cursorY + cursorImageHeight), 0, 0, 0xFF);
	...
	sourceSurface->Copy(&windowManager.cursorSwap, LT_POINT(cursorBounds.l, cursorBounds.t), LT_RECT_4(0, Width(cursorBounds), 0, Height(cursorBounds)), true);
}
...
void KRegisterGraphicsTarget(KGraphicsTarget *target) {
	...
	KDeviceSendConnectedMessage(target, LT_DEVICE_GRAPHICS_TARGET);
#endif
}
...
bool Surface::Resize(size_t newResX, size_t newResY, uint32_t clearColor, bool copyOldBits) {
	...
	if (copyOldBits) {
		EsDrawBitmap(&painter, LT_RECT_4(0, oldWidth, 0, oldHeight), (uint32_t *) oldBits, oldStride, LT_DRAW_BITMAP_OPAQUE);

		if (clearColor) {
			EsDrawBlock(&painter, LT_RECT_4(oldWidth, width, 0, height), clearColor);
			EsDrawBlock(&painter, LT_RECT_4(0, oldWidth, oldHeight, height), clearColor);
		} else {
			EsDrawClear(&painter, LT_RECT_4(oldWidth, width, 0, height));
			EsDrawClear(&painter, LT_RECT_4(0, oldWidth, oldHeight, height));
		}
	}
...
void Surface::Copy(Surface *source, EsPoint destinationPoint, EsRectangle sourceRegion, bool addToModifiedRegion) {
	EsRectangle destinationRegion = LT_RECT_4(destinationPoint.x, destinationPoint.x + Width(sourceRegion), 
	...
	if (addToModifiedRegion) {
		...
		EsRectangleClip(modifiedRegion, LT_RECT_4(0, width, 0, height), &modifiedRegion);
	}
	...
	EsDrawBitmap(&painter, destinationRegion, (uint32_t *) sourceBits, source->stride, LT_DRAW_BITMAP_OPAQUE);
}
...
#if 1
	if (!EsRectangleClip(region, LT_RECT_4(0, width, 0, height), &region)) {
		return;
	}
