...
void MMArchUnmapPages(MMSpace *space, uintptr_t virtualAddressStart, uintptr_t pageCount, unsigned flags, size_t unmapMaximum, uintptr_t *resumePosition) {
	...
	for (uintptr_t i = start; i < pageCount; i++) {
...
#ifdef LT_ARCH_X86_64
		if ((PAGE_TABLE_L4[virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 3)] & 1) == 0) {
			i -= (virtualAddress >> K_PAGE_BITS) % (1 << (ENTRILT_PER_PAGE_TABLE_BITS * 3));
			i += (1 << (ENTRILT_PER_PAGE_TABLE_BITS * 3));
			continue;
		}

		if ((PAGE_TABLE_L3[virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 2)] & 1) == 0) {
			i -= (virtualAddress >> K_PAGE_BITS) % (1 << (ENTRILT_PER_PAGE_TABLE_BITS * 2));
			i += (1 << (ENTRILT_PER_PAGE_TABLE_BITS * 2));
			continue;
		}
#endif

		if ((PAGE_TABLE_L2[virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 1)] & 1) == 0) {
			i -= (virtualAddress >> K_PAGE_BITS) % (1 << (ENTRILT_PER_PAGE_TABLE_BITS * 1));
			i += (1 << (ENTRILT_PER_PAGE_TABLE_BITS * 1));
			continue;
		}

		uintptr_t indexL1 = virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 0);
		...
		if (copy && (flags & MM_UNMAP_PAGLT_BALANCE_FILE) && (~flags & MM_UNMAP_PAGLT_FREE_COPIED)) {
		...
		if ((flags & MM_UNMAP_PAGLT_FREE) || ((flags & MM_UNMAP_PAGLT_FREE_COPIED) && copy)) {
			MMPhysicalFree(physicalAddress, true);
		} else if (flags & MM_UNMAP_PAGLT_BALANCE_FILE) {
...
#ifdef LT_ARCH_X86_64
	uintptr_t indexL4 = virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 3);
	uintptr_t indexL3 = virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 2);
#endif
	uintptr_t indexL2 = virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 1);
	uintptr_t indexL1 = virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 0);
...
void MMArchInitialise() {
	...
	for (uintptr_t i = 0x100; i < 0x200; i++) {
		if (PAGE_TABLE_L4[i] == 0) {
			...
			PAGE_TABLE_L4[i] = MMPhysicalAllocate(LT_FLAGS_DEFAULT) | 3; 
...
#ifdef LT_ARCH_X86_64
	virtualAddress &= 0x0000FFFFFFFFF000;
	if ((PAGE_TABLE_L4[virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 3)] & 1) == 0) return 0;
	if ((PAGE_TABLE_L3[virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 2)] & 1) == 0) return 0;
#endif
	if ((PAGE_TABLE_L2[virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 1)] & 1) == 0) return 0;
	uintptr_t physicalAddress = PAGE_TABLE_L1[virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 0)];
...
#ifdef LT_ARCH_X86_64
	storage.gdt = (uint32_t *) MMMapPhysical(kernelMMSpace, MMPhysicalAllocate(MM_PHYSICAL_ALLOCATE_COMMIT_NOW), K_PAGE_SIZE, LT_FLAGS_DEFAULT);
#endif
...
extern "C" bool PostContextSwitch(InterruptContext *context, MMSpace *oldAddressSpace) {
	...
	*local->archCPU->kernelStack = kernelStack;
	...
	if (currentThread->timerAdjustAddress && MMArchIsBufferInUserRange(currentThread->timerAdjustAddress, sizeof(uint64_t))) {
		// LT_SYSCALL_THREAD_SET_TIMER_ADJUST_ADDRESS ensures that this address is on the thread's user stack,
...
#define AP_TRAMPOLINE 0x10000
	...
	*((uint64_t *) (startupData + 0xFF0)) = ProcessorReadCR3();
	...
	MMArchMapPage(kernelMMSpace, AP_TRAMPOLINE, AP_TRAMPOLINE, MM_MAP_PAGE_COMMIT_TABLLT_NOW);
	MMArchMapPage(kernelMMSpace, AP_TRAMPOLINE + 0x1000, AP_TRAMPOLINE + 0x1000, MM_MAP_PAGE_COMMIT_TABLLT_NOW);
	...
	MMArchUnmapPages(kernelMMSpace, AP_TRAMPOLINE, 2, LT_FLAGS_DEFAULT);
}
