...
#define ENTRILT_PER_PAGE_TABLE (512)
#define ENTRILT_PER_PAGE_TABLE_BITS (9)

uint8_t coreL1Commit[(0xFFFF800200000000 - 0xFFFF800100000000) >> (/* ENTRILT_PER_PAGE_TABLE_BITS */ 9 + K_PAGE_BITS + 3)];
...
bool MMArchCommitPageTables(MMSpace *space, MMRegion *region) {
	...
	for (uintptr_t i = base; i < end; i += 1L << (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 3)) {
		uintptr_t indexL4 = i >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 3);
		if (!(data->l3Commit[indexL4 >> 3] & (1 << (indexL4 & 7)))) needed++;
		i = indexL4 << (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 3);
	}

	for (uintptr_t i = base; i < end; i += 1L << (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 2)) {
		uintptr_t indexL3 = i >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 2);
		if (!(data->l2Commit[indexL3 >> 3] & (1 << (indexL3 & 7)))) needed++;
		i = indexL3 << (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 2);
	}

	uintptr_t previousIndexL2I = -1;

	for (uintptr_t i = base; i < end; i += 1L << (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 1)) {
		uintptr_t indexL2 = i >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 1);
		...
		i = indexL2 << (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 1);
	}
...
bool MMArchHandlePageFault(uintptr_t address, uint32_t flags) {
	...
	if (address < K_PAGE_SIZE) {
		...
		MMArchMapPage(kernelMMSpace, address - LOW_MEMORY_MAP_START, address, MM_MAP_PAGE_COMMIT_TABLLT_NOW);
		return true;
		...
		MMArchMapPage(kernelMMSpace, MMPhysicalAllocate(MM_PHYSICAL_ALLOCATE_ZEROED), address, MM_MAP_PAGE_COMMIT_TABLLT_NOW);
		return true;
...
#define INVALIDATE_ALL_PAGLT_THRESHOLD (1024)

void TLBShootdownCallback() {
	uintptr_t page = tlbShootdownVirtualAddress;

	if (tlbShootdownPageCount > INVALIDATE_ALL_PAGLT_THRESHOLD) { 
...
bool MMArchInitialiseUserSpace(MMSpace *space, MMRegion *region) {
	...
	space->data.cr3 = MMPhysicalAllocate(LT_FLAGS_DEFAULT);
	...
	uint64_t *pageTable = (uint64_t *) MMMapPhysical(kernelMMSpace, (uintptr_t) space->data.cr3, K_PAGE_SIZE, LT_FLAGS_DEFAULT);
...
void MMArchFreeVAS(MMSpace *space) {
	for (uintptr_t i = 0; i < 256; i++) {
		...
		for (uintptr_t j = i * ENTRILT_PER_PAGE_TABLE; j < (i + 1) * ENTRILT_PER_PAGE_TABLE; j++) {
			if (!PAGE_TABLE_L3[j]) continue;

			for (uintptr_t k = j * ENTRILT_PER_PAGE_TABLE; k < (j + 1) * ENTRILT_PER_PAGE_TABLE; k++) {
	...
	MMArchUnmapPages(coreMMSpace, l1CommitRegion->baseAddress, l1CommitRegion->pageCount, MM_UNMAP_PAGLT_FREE);
...
extern "C" void InterruptHandler(InterruptContext *context) {
	...
	if (interrupt < 0x20) {
		...
		if (!supervisor) {
			...
			crashReason.errorCode = LT_FATAL_ERROR_PROCESSOR_EXCEPTION;
...
EsError ArchApplyRelocation(uintptr_t type, uint8_t *buffer, uintptr_t offset, uintptr_t result) {
	...
	else return LT_ERROR_UNSUPPORTED;
	return LT_SUCCESS;
}
