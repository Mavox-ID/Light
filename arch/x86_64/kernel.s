...
InterruptHandler%1:
	push	qword 0 ; A fake error code
...
ProcessorAPStartup: ; This function must be less than 4KB in length (see drivers/acpi.cpp)
	mov	ax,0x1000
	mov	ds,ax
	mov 	byte [abs 0xFC0],1 ; Indicate we've started.
	mov	eax,[abs 0xFF0]
	mov	cr3,eax
	lgdt	[abs 0x1000 + gdt_data.gdt - gdt_data]
	mov	eax,cr0
	or	eax,1
	mov	cr0,eax
...
[bits 64]
	...
	mov	byte [abs 0x10FC0],2 ; Indicate the BSP can start the next processor.
	and	rsp,~0xF
	jmp	ProcessorReady
