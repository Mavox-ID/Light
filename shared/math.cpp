...
EsRectangle EsRectangleLinearInterpolate(EsRectangle from, EsRectangle to, float progress) {
	return LT_RECT_4(LinearInterpolate(from.l, to.l, progress), LinearInterpolate(from.r, to.r, progress), 
...
double EsCRTsin(double x) {
	...
	x -= 2 * LT_PI * EsCRTfloor(x / (2 * LT_PI));
	...
	double y = x < LT_PI / 4 ? _Sine(x) : _Cosine(LT_PI / 2 - x);
	return negate ? -y : y;
}
...
float EsCRTsinf(float x) {
	...
	x -= 2 * LT_PI * EsCRTfloorf(x / (2 * LT_PI));
	...
	float y = x < LT_PI / 4 ? _SineFloat(x) : _CosineFloat(LT_PI / 2 - x);
	return negate ? -y : y;
}
...
double EsCRTcos(double x) {
	...
	double y = x < LT_PI / 4 ? _Cosine(x) : _Sine(LT_PI / 2 - x);
	return negate ? -y : y;
}
...
float EsCRTcosf(float x) {
	...
	float y = x < LT_PI / 4 ? _CosineFloat(x) : _SineFloat(LT_PI / 2 - x);
	return negate ? -y : y;
}
...
double EsCRTtan(double x) {
	...
	double y = x < LT_PI / 4 ? _Tangent(x) : (1.0 / _Tangent(LT_PI / 2 - x));
	return negate ? -y : y;
}
...
float EsCRTtanf(float x) {
	...
	float y = x < LT_PI / 4 ? _TangentFloat(x) : (1.0 / _TangentFloat(LT_PI / 2 - x));
	return negate ? -y : y;
}
...
double EsCRTasin(double x) {
	...
	if (x < 0.5) {
		...
		y = LT_PI / 2 - 2 * _ArcSine(EsCRTsqrt(0.5 - 0.5 * x));
	}
...
float EsCRTasinf(float x) {
	...
	if (x < 0.5) {
		...
		y = LT_PI / 2 - 2 * _ArcSineFloat(EsCRTsqrtf(0.5 - 0.5 * x));
	}
...
double EsCRTacos(double x) {
	return EsCRTasin(-x) + LT_PI / 2;
}

float EsCRTacosf(float x) {
	return EsCRTasinf(-x) + LT_PI / 2;
}
...
double EsCRTatan2(double y, double x) {
	if (x == 0) return y > 0 ? LT_PI / 2 : -LT_PI / 2;
	else if (x > 0) return EsCRTatan(y / x);
	else if (y >= 0) return LT_PI + EsCRTatan(y / x);
	else return -LT_PI + EsCRTatan(y / x);
}

float EsCRTatan2f(float y, float x) {
	if (x == 0) return y > 0 ? LT_PI / 2 : -LT_PI / 2;
	else if (x > 0) return EsCRTatanf(y / x);
	else if (y >= 0) return LT_PI + EsCRTatanf(y / x);
	else return -LT_PI + EsCRTatanf(y / x);
}
