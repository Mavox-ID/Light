...
void *ArenaAllocate(Arena *arena, bool zero) {
	if (!arena->firstEmptySlot) {
		...
		block->data = (uint8_t *) MMStandardAllocate(kernelMMSpace, arena->blockSize, LT_FLAGS_DEFAULT, nullptr, true /* commitAll */);
#else
		ArenaBlock *block = (ArenaBlock *) EsHeapAllocate(arena->slotsPerBlock * sizeof(ArenaSlot) + sizeof(ArenaBlock), false);
		block->data = (uint8_t *) EsMemoryReserve(arena->blockSize, LT_MEMORY_PROTECTION_READ_WRITE); 
#endif
