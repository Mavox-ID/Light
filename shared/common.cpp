...
EsRectangle EsRectangleFit(EsRectangle parent, EsRectangle child, bool allowScalingUp) {
	...
	if (childMaximumWidth > parentWidth) {
		return EsRectangleCenter(parent, LT_RECT_2S(parentWidth, childMaximumHeight));
	} else {
		return EsRectangleCenter(parent, LT_RECT_2S(childMaximumWidth, parentHeight));
	}
}
...
bool EsRectangleContains(EsRectangle a, int32_t x, int32_t y) {
	return LT_RECT_VALID(a) && a.l <= x && a.r > x && a.t <= y && a.b > y;
}

bool EsRectangleContainsAll(EsRectangle parent, EsRectangle child) {
	return LT_RECT_VALID(parent) && child.l >= parent.l && child.r <= parent.r && child.t >= parent.t && child.b <= parent.b;
}
...
void EsDrawBitmap(EsPainter *painter, EsRectangle region, const uint32_t *sourceBits, uintptr_t sourceStride, uint16_t mode) {
	...
	for (int i = 0; i < bounds.b - bounds.t; i++, lineStart += stride, sourceLineStart += sourceStride) {
		...
		} else if (mode == LT_DRAW_BITMAP_XOR) {
			while (j >= 4) {
		...
		} else if (mode == LT_DRAW_BITMAP_OPAQUE) {
...
void EsDrawRectangle(EsPainter *painter, EsRectangle r, uint32_t mainColor, uint32_t borderColor, EsRectangle borderSize) {
	EsDrawBlock(painter, LT_RECT_4(r.l, r.r, r.t, r.t + borderSize.t), borderColor);
	EsDrawBlock(painter, LT_RECT_4(r.l, r.l + borderSize.l, r.t + borderSize.t, r.b - borderSize.b), borderColor);
	EsDrawBlock(painter, LT_RECT_4(r.r - borderSize.r, r.r, r.t + borderSize.t, r.b - borderSize.b), borderColor);
	EsDrawBlock(painter, LT_RECT_4(r.l, r.r, r.b - borderSize.b, r.b), borderColor);
	EsDrawBlock(painter, LT_RECT_4(r.l + borderSize.l, r.r - borderSize.r, r.t + borderSize.t, r.b - borderSize.b), mainColor);
}
...
void EsDrawBitmapScaled(EsPainter *painter, EsRectangle destinationRegion, EsRectangle sourceRegion, const uint32_t *sourceBits, uintptr_t sourceStride, uint16_t alpha) {
	...
	for (int32_t y = bounds.t; y < bounds.b; y++) {
		...
		for (int32_t x = bounds.l; x < bounds.r; x++) {
			...
			if (alpha == LT_DRAW_BITMAP_OPAQUE) {
				*destinationPixel = modified;
			} else {
				if (alpha != 0xFF) {
		...
		if (c == '%') {
			...
			switch (c) {
				case 'd': {
					...
					_FormatInteger(callback, callbackData, value, pad, flags & LT_STRING_FORMAT_SIMPLE);
				} break;
				...
				case 'x': {
					uintptr_t value = va_arg(arguments, uintptr_t);
					bool simple = flags & LT_STRING_FORMAT_SIMPLE;
				...
				case 'F': {
					...
					// Integer digits.
					_FormatInteger(callback, callbackData, integer, pad, flags & LT_STRING_FORMAT_SIMPLE);
...
void StringFormatCallback(int character, void *_fsi) {
	...
	if (fsi->buffer) {
		if (fsi->bytesRemaining < bytes && fsi->bytesRemaining != (size_t) LT_STRING_FORMAT_ENOUGH_SPACE) {
			fsi->full = true;
			return;
		} else {
			...
			if (fsi->bytesRemaining != (size_t) LT_STRING_FORMAT_ENOUGH_SPACE) fsi->bytesRemaining -= bytes;
		}
	}
}
...
void PrintCallback(int character, void *) {
	if (printing.bufferPosition >= PRINT_BUFFER_SIZE - 16) {
		EsSyscall(LT_SYSCALL_PRINT, (uintptr_t) printing.buffer, printing.bufferPosition, 0, 0);
...
void EsPrintDirect(const char *string, ptrdiff_t stringLength) {
	...
	EsSyscall(LT_SYSCALL_PRINT, (uintptr_t) string, stringLength, 0, 0);
}
...
void EsMutexAcquire(EsMutex *mutex) {
	...
	while (true) {
		...
		if (mutex->event == LT_INVALID_HANDLE) {
...
void EsCRTexit(int status) {
	EsProcessTerminate(LT_CURRENT_PROCESS, status);
}
