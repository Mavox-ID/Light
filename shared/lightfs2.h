...
#define LIGHTFS_BOOT_SUPER_BLOCK_SIZE 			(8192)			// The bootloader and superblock take up 16KB.
#define LIGHTFS_DRIVE_MINIMUM_SIZE 			(1048576)		// The minimum drive size that can be formatted.
#define LIGHTFS_DRIVER_VERSION 				(10)			// The current driver version.
#define LIGHTFS_MAXIMUM_VOLUME_NAME_LENGTH 		(32)			// The volume name limit.

#define LIGHTFS_CORE_NODE_KERNEL				(0)			// The kernel core node.
#define LIGHTFS_CORE_NODE_ROOT				(1)			// The root directory core node.
#define LIGHTFS_CORE_NODE_COUNT				(2)			// The number of core nodes.

#define LIGHTFS_SIGNATURE_STRING        			("!LightFS2-------")	// The signature in the superblock.
#define LIGHTFS_DIRECTORY_ENTRY_SIGNATURE 			("DirEntry")		// The signature in directory entries.
#define LIGHTFS_GROUP_DESCRIPTOR_SIGNATURE			("GDTE")		// The signature in a group descriptor.
#define LIGHTFS_INDEX_VERTEX_SIGNATURE			("INXE")		// The signature in a index vertex.

#define LIGHTFS_NODE_TYPE_FILE 				(1)			// DirectoryEntry.nodeType: a file.
#define LIGHTFS_NODE_TYPE_DIRECTORY 			(2)			// DirectoryEntry.nodeType: a directory.

#define LIGHTFS_ATTRIBUTE_DATA				(1)			// Contains the data of the file, or a list of DirectoryEntries.
#define LIGHTFS_ATTRIBUTE_FILENAME				(2)			// The UTF-8 filename.
#define LIGHTFS_ATTRIBUTE_DIRECTORY			(3)			// Additional information about the directory.

#define LIGHTFS_INDIRECTION_DIRECT				(1)			// The data is stored in the attribute.
#define LIGHTFS_INDIRECTION_L1				(2)			// The attribute contains a extent list that points to the data.

#define LIGHTFS_INDEX_MAX_DEPTH				(16)			// The maximum depth of the index tree. I'd be surprised if this gets past 8.
#define LIGHTFS_VERTEX_KEY(vertex, key) 			((IndexKey *) ((uint8_t *) vertex + vertex->offset) + key)
...
typedef struct AttributeFilename {
	/*  0 */ uint16_t type;						// LIGHTFS_ATTRIBUTE_FILENAME.
	/*  2 */ uint16_t size;						// The size in bytes. Must be 8 byte aligned.
	/*  4 */ uint16_t length;					// The length of the filename in bytes.
	/*  6 */ uint16_t _unused;					// Unused.

#define LIGHTFS_FILENAME_HEADER_SIZE (8)						// The size of the header of a AttributeFilename.
	/*  8 */ uint8_t filename[1];					// The UTF-8 filename.
} AttributeFilename;

typedef struct AttributeDirectory {
	/*  0 */ uint16_t type;						// LIGHTFS_ATTRIBUTE_DIRECTORY.
	/*  2 */ uint16_t size;						// The size in bytes. Must be 8 byte aligned.
...
typedef struct AttributeData {
	/*  0 */ uint16_t type;						// LIGHTFS_ATTRIBUTE_DATA.
	/*  2 */ uint16_t size;						// The size in bytes. Must be 8 byte aligned.
...
typedef struct DirectoryEntry {
	/*  0 */ char signature[8];					// Must be LIGHTFS_DIRECTORY_ENTRY_SIGNATURE.
...
#define LIGHTFS_ATTRIBUTE_OFFSET (96)
	/* 96 */ uint8_t attributes[1024 - LIGHTFS_ATTRIBUTE_OFFSET];	// Attribute list.
} DirectoryEntry;

typedef struct GroupDescriptor {
	/*  0 */ char signature[4];					// Must be LIGHTFS_GROUP_DESCRIPTOR_SIGNATURE.
...
typedef struct IndexVertex {
	/*  0 */ char signature[4];					// Must be LIGHTFS_INDEX_VERTEX_SIGNATURE.
...
#define LIGHTFS_INDEX_KEY_OFFSET (32)
	/* 32 */ IndexKey keys[1];					// There are this.count keys.
} IndexVertex;

typedef struct Superblock {
	/*   0 */ char signature[16];					// The filesystem signature; should be LIGHTFS_SIGNATURE_STRING.
	/*  16 */ char volumeName[LIGHTFS_MAXIMUM_VOLUME_NAME_LENGTH];	// The name of the volume.
	...
	/* 152 */ EsUniqueIdentifier osInstallation;			// The unique identifier of the Light installation this volume was made for. All zero for a non-installation volume.
...
bool AccessNode(DirectoryEntry *node, void *buffer, uint64_t offsetIntoFile, uint64_t totalCount, DirectoryEntryReference *reference, bool read) {
	...
	AttributeData *dataAttribute = (AttributeData *) FindAttribute(node, LIGHTFS_ATTRIBUTE_DATA);

	if (dataAttribute->indirection == LIGHTFS_INDIRECTION_DIRECT && read) {
		memcpy(buffer, dataAttribute->data + offsetIntoFile, totalCount);
		return true;
	} else if (dataAttribute->indirection == LIGHTFS_INDIRECTION_DIRECT && !read) {
		assert(!reference);
		memcpy(dataAttribute->data + offsetIntoFile, buffer, totalCount);
		return true;
	}

	assert(dataAttribute->indirection == LIGHTFS_INDIRECTION_L1);
...
bool ResizeNode(DirectoryEntry *entry, uint64_t newSize) {
	...
	AttributeData *dataAttribute = (AttributeData *) FindAttribute(entry, LIGHTFS_ATTRIBUTE_DATA);

	if (newSize < (uint64_t) (dataAttribute->size - dataAttribute->dataOffset) && entry->nodeType == LIGHTFS_NODE_TYPE_FILE) {
		dataAttribute->indirection = LIGHTFS_INDIRECTION_DIRECT;
	...
	dataAttribute->indirection = LIGHTFS_INDIRECTION_L1;
...
void NewDirectoryEntry(DirectoryEntry *entry, uint8_t nodeType, EsUniqueIdentifier parentUID, const char *name, EsUniqueIdentifier contentType) {
	memcpy(entry->signature, LIGHTFS_DIRECTORY_ENTRY_SIGNATURE, 8);
	...
	size_t newFilenameSize = ((strlen(name) + LIGHTFS_FILENAME_HEADER_SIZE - 1) & ~7) + 8; // Size of name + size of header, rounded up to the nearest 8 bytes.

	if (nodeType == LIGHTFS_NODE_TYPE_DIRECTORY) {
	...
	data->indirection = nodeType == LIGHTFS_NODE_TYPE_DIRECTORY ? LIGHTFS_INDIRECTION_L1 : LIGHTFS_INDIRECTION_DIRECT;
...
bool AddNode(const char *name, uint8_t nodeType, DirectoryEntry *outputEntry, DirectoryEntryReference *outputReference, 
	...
	AttributeData *dataAttribute = (AttributeData *) FindAttribute(&directory, LIGHTFS_ATTRIBUTE_DATA);
	AttributeDirectory *directoryAttribute = (AttributeDirectory *) FindAttribute(&directory, LIGHTFS_ATTRIBUTE_DIRECTORY);
...

	{
		...
		uint64_t depth = 0, blocks[LIGHTFS_INDEX_MAX_DEPTH] = { directoryAttribute->indexRootBlock };
		...
		if (blocks[0] == 0) {
			...
			memcpy(vertex->signature, LIGHTFS_INDEX_VERTEX_SIGNATURE, 4);
		...
		while (vertex->count == vertex->maxCount) {
			...
			memcpy(sibling->signature, LIGHTFS_INDEX_VERTEX_SIGNATURE, 4);
			...
			if (newRoot) {
				...
				memcpy(parent->signature, LIGHTFS_INDEX_VERTEX_SIGNATURE, 4);
...
bool MountVolume() {
	// Read the superblock.
	blockSize = LIGHTFS_BOOT_SUPER_BLOCK_SIZE;
	...
	if (superblock.mounted) {
		Log("LightFS: Volume not unmounted, exiting...\n");
		LightFSError();
	}
...
bool FindNode(const char *cName, DirectoryEntry *node, DirectoryEntryReference directoryReference) {
	...
	for (uintptr_t i = 0; i < directoryAttribute->childNodes; i++) {
		...
		AttributeFilename *filename = (AttributeFilename *) FindAttribute(node, LIGHTFS_ATTRIBUTE_FILENAME);
	...
	for (uintptr_t i = 0; i < arrlenu(node.children); i++) {
		if (node.children[i].isFile) {
			...
			if (!data) {
				...
				if (!AddNode(node.children[i].name, LIGHTFS_NODE_TYPE_FILE, &entry, &reference, parentDirectory, node.children[i].contentType)) {
					return -1;
				}
			...
			DirectoryEntryReference reference;
			if (!AddNode(node.children[i].name, LIGHTFS_NODE_TYPE_DIRECTORY, NULL, &reference, parentDirectory, node.children[i].contentType)) return -1;
...
bool Format(uint64_t driveSize, const char *volumeName, EsUniqueIdentifier osInstallation,
	...
	if (driveSize < LIGHTFS_DRIVE_MINIMUM_SIZE) {
		Log("Error: Cannot create a drive of %d bytes (too small).\n", (int) driveSize);
		LightFSError();
	}

	if (strlen(volumeName) > LIGHTFS_MAXIMUM_VOLUME_NAME_LENGTH) {
		Log("Error: Volume name '%s' is too long; must be <= %d bytes.\n", volumeName, (int) LIGHTFS_MAXIMUM_VOLUME_NAME_LENGTH);
		LightFSError();
	}
	...
	{
		memcpy(superblock.signature, LIGHTFS_SIGNATURE_STRING, 16);
		memcpy(superblock.volumeName, volumeName, strlen(volumeName));

		superblock.requiredReadVersion = LIGHTFS_DRIVER_VERSION;
		superblock.requiredWriteVersion = LIGHTFS_DRIVER_VERSION;
		...
		uint64_t blockGDT = LIGHTFS_BOOT_SUPER_BLOCK_SIZE * 2 / superblock.blockSize + 1;
		...
		uint64_t end = blockCoreNodes + ((LIGHTFS_CORE_NODE_COUNT + superblock.directoryEntriesPerBlock - 1) / superblock.directoryEntriesPerBlock);
		...
		DirectoryEntry coreNodes[LIGHTFS_CORE_NODE_COUNT] = {};

		superblock.root.block =              (blockCoreNodes * superblock.blockSize + sizeof(DirectoryEntry) * LIGHTFS_CORE_NODE_ROOT)   / superblock.blockSize;
		superblock.root.offsetIntoBlock =    (blockCoreNodes * superblock.blockSize + sizeof(DirectoryEntry) * LIGHTFS_CORE_NODE_ROOT)   % superblock.blockSize;
		superblock.kernel.block =            (blockCoreNodes * superblock.blockSize + sizeof(DirectoryEntry) * LIGHTFS_CORE_NODE_KERNEL) / superblock.blockSize;
		superblock.kernel.offsetIntoBlock =  (blockCoreNodes * superblock.blockSize + sizeof(DirectoryEntry) * LIGHTFS_CORE_NODE_KERNEL) % superblock.blockSize;

		{
			// Root directory.
			DirectoryEntry *entry = coreNodes + LIGHTFS_CORE_NODE_ROOT;
			memcpy(entry->signature, LIGHTFS_DIRECTORY_ENTRY_SIGNATURE, 8);
			GenerateUniqueIdentifier(&entry->identifier, false);
			entry->attributeOffset = LIGHTFS_ATTRIBUTE_OFFSET;
			entry->nodeType = LIGHTFS_NODE_TYPE_DIRECTORY;
			...
			data->type = LIGHTFS_ATTRIBUTE_DATA;
			data->size = sizeof(DirectoryEntry) - LIGHTFS_ATTRIBUTE_OFFSET - directory->size;
			data->indirection = LIGHTFS_INDIRECTION_L1;
			data->dataOffset = LIGHTFS_ATTRIBUTE_OFFSET;
			entry->checksum = CalculateCRC32(entry, sizeof(DirectoryEntry), 0);
		}
		...
		if (!WriteBytes(LIGHTFS_BOOT_SUPER_BLOCK_SIZE, LIGHTFS_BOOT_SUPER_BLOCK_SIZE, &superblock)) {
			return false;
		}

		{
			...
			for (uintptr_t i = 0; i < superblock.groupCount; i++) {
				memcpy(buffer[i].signature, LIGHTFS_GROUP_DESCRIPTOR_SIGNATURE, 4);
	...
	if (MountVolume()) {
		...
		NewDirectoryEntry(&entry, LIGHTFS_NODE_TYPE_FILE, unused, "Kernel", elf);
