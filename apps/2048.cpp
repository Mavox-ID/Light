...
#define TILE_BOUNDS(x, y) LT_RECT_4PD(mainArea.l + TILE_GAP * (x + 1) + TILE_SIZE * x, mainArea.t + TILE_GAP * (y + 1) + TILE_SIZE * y, TILE_SIZE, TILE_SIZE)
	...
	if (!MoveTiles(-1, 0, true) && !MoveTiles(1, 0, true) && !MoveTiles(0, -1, true) && !MoveTiles(0, 1, true)) {
		...
		if (highScore < score) {
			EsDialogShow(instance->window, INTERFACE_STRING(Game2048GameOver), INTERFACE_STRING(Game2048NewHighScore), 
					LT_ICON_DIALOG_INFORMATION, LT_DIALOG_ALERT_OK_BUTTON);
		} else {
...
void DrawTileText(EsPainter *painter, EsElement *element, EsRectangle bounds, float opacity, uint8_t number) {
	...
	EsTextStyle style = { .font = { .family = LT_FONT_SANS, .weight = LT_FONT_SEMIBOLD }, .size = (uint16_t) tileTextSizes[number] };

	if (number >= 12) {
		style.color = TILE_TEXT_GLOW | alpha;
		style.blur = 3;
		EsDrawTextSimple(painter, element, bounds, buffer, bytes, style, LT_TEXT_H_CENTER | LT_TEXT_V_CENTER);
	}
...
void DrawTile(EsPainter *painter, EsElement *element, uint8_t sourceNumber, uint8_t targetNumber, 
	...
	EsDrawRoundedRectangle(painter, bounds, fill, EsColorBlend(fill, 0x20000000, true), LT_RECT_4(0, 0, 0, 3 * scale), cornerRadii);
...
int GameAreaMessage(EsElement *element, EsMessage *message) {
	if (message->type == LT_MSG_PAINT) {
		...
		EsRectangle mainArea = EsRectangleFit(bounds, LT_RECT_1S(MAIN_AREA_SIZE()), false);
		EsDrawRoundedRectangle(painter, mainArea, MAIN_AREA_FILL, MAIN_AREA_BORDER, LT_RECT_1(scale * 1), cornerRadii);
		...
		for (uintptr_t j = 0; j < TILE_COUNT; j++) {
			for (uintptr_t i = 0; i < TILE_COUNT; i++) {
				if (grid[i][j] && animationComplete) {
					...
					EsDrawRoundedRectangle(painter, TILE_BOUNDS(i, j), CELL_FILL, 0, LT_RECT_1(0), cornerRadii);
				}
			}
		}
...
void NewGameCommand(EsInstance *, EsElement *, EsCommand *) {
	SaveConfiguration();
	EsElementStartTransition(gameArea, LT_TRANSITION_SLIDE_UP);
...
void ProcessApplicationMessage(EsMessage *message) {
	if (message->type == LT_MSG_INSTANCE_CREATE) {
		...
		EsWindowSetIcon(instance->window, LT_ICON_APPLICATIONS_OTHER);

		// Main horizontal stack.
		EsPanel *panel = EsPanelCreate(instance->window, LT_CELL_FILL | LT_PANEL_HORIZONTAL, LT_STYLE_PANEL_WINDOW_BACKGROUND);
		EsSpacerCreate(panel, LT_CELL_FILL);
		gameArea = EsCustomElementCreate(panel, LT_ELEMENT_FOCUSABLE);
		gameArea->messageUser = GameAreaMessage;
		EsSpacerCreate(panel, LT_FLAGS_DEFAULT, 0, 30, 0);
		EsPanel *info = EsPanelCreate(panel);
		EsSpacerCreate(panel, LT_CELL_FILL);

		// Info panel.
		info->messageUser = InfoPanelMessage;
		EsTextDisplayCreate(info, LT_CELL_H_FILL, LT_STYLE_TEXT_LABEL_SECONDARY, INTERFACE_STRING(Game2048Score));
		scoreDisplay = EsTextDisplayCreate(info, LT_CELL_H_FILL, LT_STYLE_TEXT_HEADING0);
		EsSpacerCreate(info, LT_FLAGS_DEFAULT, 0, 0, 10);
		highScoreDisplay = EsTextDisplayCreate(info, LT_CELL_H_FILL | LT_TEXT_DISPLAY_RICH_TEXT, LT_STYLE_TEXT_LABEL_SECONDARY);
		EsSpacerCreate(info, LT_CELL_FILL);
		EsTextDisplayCreate(info, LT_CELL_H_FILL | LT_TEXT_DISPLAY_RICH_TEXT, LT_STYLE_TEXT_PARAGRAPH_SECONDARY, INTERFACE_STRING(Game2048Instructions));
		EsSpacerCreate(info, LT_FLAGS_DEFAULT, 0, 0, 10);
		EsButton *newGame = EsButtonCreate(info, LT_CELL_H_LEFT | LT_BUTTON_NOT_FOCUSABLE, 0, INTERFACE_STRING(Game2048NewGame));
		newGame->accessKey = 'N';
