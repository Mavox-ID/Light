...
EsStyle styleFontList = {
	...
	.metrics = {
		.mask = LT_THEME_METRICS_GAP_MINOR | LT_THEME_METRICS_GAP_WRAP | LT_THEME_METRICS_INSETS,
		.insets = LT_RECT_1(25),
		.gapMinor = 15,
		.gapWrap = 15,
	},
...
EsStyle styleFontItem = {
	.inherit = LT_STYLE_PANEL_SHEET,

	.metrics = {
		.mask = LT_THEME_METRICS_PREFERRED_WIDTH | LT_THEME_METRICS_PREFERRED_HEIGHT,
		.preferredWidth = 300,
		.preferredHeight = 250,
	},
};
...
EsStyle styleFontPreviewPage = {
	.inherit = LT_STYLE_PANEL_DOCUMENT,
...
int FontPreviewMessage(EsElement *element, EsMessage *message) {
	if (message->type == LT_MSG_PAINT) {
		...
		properties.flags = LT_TEXT_PLAN_SINGLE_USE | LT_TEXT_PLAN_TRIM_SPACES | LT_TEXT_WRAP | LT_TEXT_ELLIPSIS | LT_TEXT_PLAN_NO_FONT_SUBSTITUTION;
		...
		if (element->instance->fontVariant / 10 == 1) runs[0].style.font.flags |= LT_FONT_ITALIC;
...
int FontListMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_LIST_VIEW_CREATE_ITEM) {
		EsPanel *panel = EsPanelCreate(message->createItem.item, LT_CELL_FILL, EsStyleIntern(&styleFontInformationPanel));
		EsFontInformation *font = &instance->fonts[message->createItem.index];

		EsPanel *row = EsPanelCreate(panel, LT_CELL_H_FILL | LT_PANEL_HORIZONTAL, EsStyleIntern(&styleFontInformationRow));
		// EsIconDisplayCreate(row, LT_FLAGS_DEFAULT, LT_STYLE_ICON_DISPLAY_SMALL, LT_ICON_FONT_X_GENERIC);
		EsTextDisplayCreate(row, LT_FLAGS_DEFAULT, EsStyleIntern(&styleFontName), font->name, font->nameBytes);
		EsSpacerCreate(row, LT_CELL_H_FILL, LT_STYLE_SEPARATOR_HORIZONTAL);

		EsElement *preview = EsCustomElementCreate(panel, LT_CELL_FILL, LT_STYLE_TEXT_PARAGRAPH);
		...
		row = EsPanelCreate(panel, LT_CELL_H_FILL | LT_PANEL_HORIZONTAL, EsStyleIntern(&styleFontInformationRow));
		...
		EsTextDisplayCreate(row, LT_CELL_H_FILL | LT_CELL_V_BOTTOM, LT_STYLE_TEXT_LABEL_SECONDARY, description, descriptionBytes);

#if 0
		EsButton *openButton = EsButtonCreate(row, LT_BUTTON_NOT_FOCUSABLE);
		EsButtonSetIcon(openButton, LT_ICON_GO_NEXT_SYMBOLIC);
#endif
	}
...
void VariantsPopupCreate(Instance *instance, EsElement *element, EsCommand *) {
	...
	EsListView *list = EsListViewCreate(panel, LT_LIST_VIEW_CHOICE_SELECT | LT_LIST_VIEW_FIXED_ITEMS, LT_STYLE_LIST_CHOICE_BORDERED);
...
LT_MACRO_SORT(LoadFontsFromDatabaseSort, EsFontInformation, result = EsStringCompare(_left->name, _left->nameBytes, _right->name, _right->nameBytes);, int);
...
void BackCommand(Instance *instance, EsElement *, EsCommand *) {
	EsPanelSwitchTo(instance->switcher, instance->fontList, LT_TRANSITION_NONE);
	EsWindowSwitchToolbar(instance->window, instance->fontListToolbar, LT_TRANSITION_SLIDE_DOWN);
}

int InstanceCallback(Instance *instance, EsMessage *message) {
	if (message->type == LT_MSG_INSTANCE_CLOSE) {
	...
	} else if (message->type == LT_MSG_INSTANCE_DESTROY) {
		...
		if (!message->instanceOpen.update) {
			...
			EsPanel *titleRow = EsPanelCreate(instance->fontPreview, LT_CELL_H_CENTER | LT_PANEL_HORIZONTAL, EsStyleIntern(&styleFontInformationRow));
			EsIconDisplayCreate(titleRow, LT_FLAGS_DEFAULT, LT_STYLE_ICON_DISPLAY, LT_ICON_FONT_X_GENERIC);
			EsTextDisplayCreate(titleRow, LT_FLAGS_DEFAULT, LT_STYLE_TEXT_HEADING0, message->instanceOpen.nameOrPath, message->instanceOpen.nameOrPathBytes);
			EsSpacerCreate(instance->fontPreview, LT_FLAGS_DEFAULT, 0, 0, 20);

			int sizes[] = { 12, 18, 24, 36, 48, 60, 72, 0 };

			for (uintptr_t i = 0; sizes[i]; i++) {
				EsPanel *row = EsPanelCreate(instance->fontPreview, LT_CELL_H_FILL | LT_PANEL_HORIZONTAL, EsStyleIntern(&styleFontInformationRow));
				char buffer[64];
				EsTextDisplayCreate(row, LT_FLAGS_DEFAULT, 0, buffer, EsStringFormat(buffer, sizeof(buffer), "%d", sizes[i]));
				EsTextDisplay *display = EsTextDisplayCreate(row, LT_TEXT_DISPLAY_NO_FONT_SUBSTITUTION);
			...
			EsPanelSwitchTo(instance->switcher, instance->fontPreview, LT_TRANSITION_NONE);
			EsWindowSwitchToolbar(instance->window, instance->fontPreviewToolbar, LT_TRANSITION_SLIDE_UP);
		}
...
void _start() {
	...
	while (true) {
		...
		if (message->type == LT_MSG_INSTANCE_CREATE) {
			...
			EsWindowSetIcon(instance->window, LT_ICON_APPLICATIONS_FONTS);
			EsPanel *rootPanel = EsPanelCreate(instance->window, LT_CELL_FILL, LT_STYLE_PANEL_WINDOW_DIVIDER);

			instance->switcher = EsPanelCreate(rootPanel, LT_CELL_FILL | LT_PANEL_SWITCHER);
			...
			uint64_t flags = LT_CELL_FILL | LT_LIST_VIEW_TILED | LT_LIST_VIEW_CENTER_TILES;
			...
			instance->fontPreview = EsPanelCreate(instance->switcher, LT_CELL_FILL | LT_PANEL_V_SCROLL_AUTO, EsStyleIntern(&styleFontPreviewPage));
			...
			EsTextDisplayCreate(section, LT_FLAGS_DEFAULT, 0, INTERFACE_STRING(FontBookTextSize));
			instance->fontSizeTextbox = EsTextboxCreate(section, LT_TEXTBOX_EDIT_BASED, LT_STYLE_TEXTBOX_BORDERED_SINGLE_COMPACT);
			...
			EsTextDisplayCreate(section, LT_FLAGS_DEFAULT, 0, INTERFACE_STRING(FontBookPreviewText));
			instance->previewTextTextbox = EsTextboxCreate(section, LT_FLAGS_DEFAULT, LT_STYLE_TEXTBOX_BORDERED_SINGLE);
			...
			EsButton *button = EsButtonCreate(toolbar, LT_BUTTON_DROPDOWN, {}, INTERFACE_STRING(FontBookVariants));
			button->accessKey = 'V';
			...
			button = EsButtonCreate(toolbar, LT_FLAGS_DEFAULT, 0, INTERFACE_STRING(FontBookNavigationBack));
			button->accessKey = 'B';
			EsButtonSetIcon(button, LT_ICON_GO_PREVIOUS_SYMBOLIC);
