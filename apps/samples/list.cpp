...
void _start() {
	...
	while (true) {
		...
		if (message->type == LT_MSG_INSTANCE_CREATE) {
			...
			// Create a wrapper panel at the root of the window with the style LT_STYLE_PANEL_WINDOW_DIVIDER.
			...
			EsListView *list = EsListViewCreate(
					...
					LT_CELL_FILL                   // Fill the wrapper panel, which in turn fills the window.
					| LT_LIST_VIEW_COLUMNS         // Display a column header.
					| LT_LIST_VIEW_FIXED_ITEMS     // Use fixed items mode. Otherwise we'd have to provide the item data via a callback.
					| LT_LIST_VIEW_SINGLE_SELECT); // Allow a single item to be selected at once (or no items).

			// Register the name column.
			EsListViewRegisterColumn(list, 
					COLUMN_NAME,                  // The column's ID.
					"Name", -1,                   // Its title string.
					LT_LIST_VIEW_COLUMN_HAS_MENU, // Allow the user to click on the column header to get the menu for sorting options.
			...
			EsListViewRegisterColumn(list, 
					...
					LT_LIST_VIEW_COLUMN_HAS_MENU        // Column header has a menu.
					| LT_TEXT_H_RIGHT                   // Align the text in the column to the right.
					| LT_DRAW_CONTENT_TABULAR           // Use the tabular digits style, so that digits line up between rows.
					| LT_LIST_VIEW_COLUMN_DATA_INTEGERS // We're storing integers in this column (the default is strings).
					| LT_LIST_VIEW_COLUMN_SORT_SIZE,    // The items in the column can be sorted by their size.
					100);                               // Initial width.
			...
			EsListViewRegisterColumn(list, 
					...
					LT_LIST_VIEW_COLUMN_HAS_MENU             // Column header has a menu.
					| LT_DRAW_CONTENT_RICH_TEXT              // Parse rich text markup in the strings. (See colorStrings above).
					| LT_LIST_VIEW_COLUMN_FORMAT_ENUM_STRING // To display an item, lookup an enum string from the array.
					| LT_LIST_VIEW_COLUMN_DATA_INTEGERS,     // The enum values are stored as integers.
					150);                                    // Initial widths.
