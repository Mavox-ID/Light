...
void ThemeInitialise() {
	...
	if (!theming.cursorData) {
		...
		theming.cursorData = EsMemoryCreateShareableRegion(LT_THEME_CURSORS_WIDTH * LT_THEME_CURSORS_HEIGHT * 4);
		void *destination = EsMemoryMapObject(theming.cursorData, 0, LT_THEME_CURSORS_WIDTH * LT_THEME_CURSORS_HEIGHT * 4, LT_MEMORY_MAP_OBJECT_READ_WRITE);
		LoadImage(cursorsBitmap, cursorsBitmapBytes, destination, LT_THEME_CURSORS_WIDTH, LT_THEME_CURSORS_HEIGHT, true);
		EsObjectUnmap(destination);
	}

	theming.cursors.width = LT_THEME_CURSORS_WIDTH;
	theming.cursors.height = LT_THEME_CURSORS_HEIGHT;
	theming.cursors.stride = LT_THEME_CURSORS_WIDTH * 4;
	theming.cursors.bits = EsMemoryMapObject(theming.cursorData, 0, LT_MEMORY_MAP_OBJECT_ALL, LT_MEMORY_MAP_OBJECT_READ_ONLY);
...
void ThemeStyleCopyInlineMetrics(UIStyle *style) {
	...
	style->font.flags = style->metrics->isItalic ? LT_FONT_ITALIC : LT_FLAGS_DEFAULT;
...
void _ThemeAnimationBuildAddProperties(ThemeAnimation *animation, UIStyle *style, uint16_t stateFlags) {
	...
	for (uintptr_t i = 0; i < style->style->layerCount; i++) {
		...
		for (uintptr_t i = 0; i < layer->overrideCount; i++) {
			...
			LT_MACRO_SEARCH(animation->properties.Length(), { 
...
void ThemeStylePrepare(UIStyle *style, EsStyle *esStyle, UIStyleKey key) {
	...
	if (customMetrics) {
#define LT_RECTANGLE_TO_RECTANGLE_8(x) { (int8_t) (x).l, (int8_t) (x).r, (int8_t) (x).t, (int8_t) (x).b }
		if (customMetrics->mask & LT_THEME_METRICS_INSETS) style->metrics->insets = LT_RECTANGLE_TO_RECTANGLE_8(customMetrics->insets);
		if (customMetrics->mask & LT_THEME_METRICS_CLIP_INSETS) style->metrics->clipInsets = LT_RECTANGLE_TO_RECTANGLE_8(customMetrics->clipInsets);
		if (customMetrics->mask & LT_THEME_METRICS_CLIP_ENABLED) style->metrics->clipEnabled = customMetrics->clipEnabled;
		if (customMetrics->mask & LT_THEME_METRICS_CURSOR) style->metrics->cursor = customMetrics->cursor;
		if (customMetrics->mask & LT_THEME_METRICS_PREFERRED_WIDTH) style->metrics->preferredWidth = customMetrics->preferredWidth;
		if (customMetrics->mask & LT_THEME_METRICS_PREFERRED_HEIGHT) style->metrics->preferredHeight = customMetrics->preferredHeight;
		if (customMetrics->mask & LT_THEME_METRICS_MINIMUM_WIDTH) style->metrics->minimumWidth = customMetrics->minimumWidth;
		if (customMetrics->mask & LT_THEME_METRICS_MINIMUM_HEIGHT) style->metrics->minimumHeight = customMetrics->minimumHeight;
		if (customMetrics->mask & LT_THEME_METRICS_MAXIMUM_WIDTH) style->metrics->maximumWidth = customMetrics->maximumWidth;
		if (customMetrics->mask & LT_THEME_METRICS_MAXIMUM_HEIGHT) style->metrics->maximumHeight = customMetrics->maximumHeight;
		if (customMetrics->mask & LT_THEME_METRICS_GAP_MAJOR) style->metrics->gapMajor = customMetrics->gapMajor;
		if (customMetrics->mask & LT_THEME_METRICS_GAP_MINOR) style->metrics->gapMinor = customMetrics->gapMinor;
		if (customMetrics->mask & LT_THEME_METRICS_GAP_WRAP) style->metrics->gapWrap = customMetrics->gapWrap;
		if (customMetrics->mask & LT_THEME_METRICS_TEXT_COLOR) style->metrics->textColor = customMetrics->textColor;
		if (customMetrics->mask & LT_THEME_METRICS_TEXT_FIGURES) style->metrics->textFigures = customMetrics->textFigures;
		if (customMetrics->mask & LT_THEME_METRICS_SELECTED_BACKGROUND) style->metrics->selectedBackground = customMetrics->selectedBackground;
		if (customMetrics->mask & LT_THEME_METRICS_SELECTED_TEXT) style->metrics->selectedText = customMetrics->selectedText;
		if (customMetrics->mask & LT_THEME_METRICS_ICON_COLOR) style->metrics->iconColor = customMetrics->iconColor;
		if (customMetrics->mask & LT_THEME_METRICS_TEXT_ALIGN) style->metrics->textAlign = customMetrics->textAlign;
		if (customMetrics->mask & LT_THEME_METRICS_TEXT_SIZE) style->metrics->textSize = customMetrics->textSize;
		if (customMetrics->mask & LT_THEME_METRICS_FONT_FAMILY) style->metrics->fontFamily = customMetrics->fontFamily;
		if (customMetrics->mask & LT_THEME_METRICS_FONT_WEIGHT) style->metrics->fontWeight = customMetrics->fontWeight;
		if (customMetrics->mask & LT_THEME_METRICS_ICON_SIZE) style->metrics->iconSize = customMetrics->iconSize;
		if (customMetrics->mask & LT_THEME_METRICS_IS_ITALIC) style->metrics->isItalic = customMetrics->isItalic;
		if (customMetrics->mask & LT_THEME_METRICS_LAYOUT_VERTICAL) style->metrics->layoutVertical = customMetrics->layoutVertical;
	}
	...
	if (style->appearanceIndex != -1) {
		...
		if ((appearance->backgroundColor & 0xFF000000) == 0xFF000000) {
			style->opaqueInsets = LT_RECT_1(0);
		} else {
			style->opaqueInsets = LT_RECT_1(0x7F);
		}
	}
...
void GetPreferredSizeFromStylePart(EsStyleID esStyle, int64_t *width, int64_t *height) {
	UIStyle *style = GetStyle(MakeStyleKey(esStyle, LT_FLAGS_DEFAULT), true);
...
void UIStyle::PaintText(EsPainter *painter, EsElement *element, EsRectangle rectangle, 
	...
	if (flags & (LT_DRAW_CONTENT_MARKER_DOWN_ARROW | LT_DRAW_CONTENT_MARKER_UP_ARROW)) {
		EsStyleID part = (flags & LT_DRAW_CONTENT_MARKER_DOWN_ARROW) ? LT_STYLE_MARKER_DOWN_ARROW : LT_STYLE_MARKER_UP_ARROW;
		UIStyle *style = GetStyle(MakeStyleKey(part, 0), true);
		textBounds.r -= style->preferredWidth + gapMinor;
		EsRectangle location = LT_RECT_4PD(bounds.r - style->preferredWidth - painter->offsetX, 
	...
	if (textBytes) {
		...
		if (flags & LT_TEXT_H_LEFT)   properties.flags = (properties.flags & ~(LT_TEXT_H_CENTER | LT_TEXT_H_RIGHT)) | LT_TEXT_H_LEFT;
		if (flags & LT_TEXT_H_CENTER) properties.flags = (properties.flags & ~(LT_TEXT_H_LEFT | LT_TEXT_H_RIGHT)) | LT_TEXT_H_CENTER;
		if (flags & LT_TEXT_H_RIGHT)  properties.flags = (properties.flags & ~(LT_TEXT_H_LEFT | LT_TEXT_H_CENTER)) | LT_TEXT_H_RIGHT;
		if (flags & LT_TEXT_V_TOP)    properties.flags = (properties.flags & ~(LT_TEXT_V_CENTER | LT_TEXT_V_BOTTOM)) | LT_TEXT_V_TOP;
		if (flags & LT_TEXT_V_CENTER) properties.flags = (properties.flags & ~(LT_TEXT_V_TOP | LT_TEXT_V_BOTTOM)) | LT_TEXT_V_CENTER;
		if (flags & LT_TEXT_V_BOTTOM) properties.flags = (properties.flags & ~(LT_TEXT_V_TOP | LT_TEXT_V_CENTER)) | LT_TEXT_V_BOTTOM;

		EsTextRun textRun[2] = {};
		textRun[1].offset = textBytes;
		GetTextStyle(&textRun[0].style);

		if (flags & LT_DRAW_CONTENT_TABULAR) {
			textRun[0].style.figures = LT_TEXT_FIGURE_TABULAR;
		}

		if (flags & LT_DRAW_CONTENT_RICH_TEXT) {
			char *string;
			EsTextRun *textRuns;
			size_t textRunCount;
