...
// 	Get text rendering on non-Light platforms.
...
#define LT_TEXT_H_LEFT 	 (1 << 0)
#define LT_TEXT_H_CENTER (1 << 1)
#define LT_TEXT_H_RIGHT  (1 << 2)
#define LT_TEXT_V_TOP 	 (1 << 3)
#define LT_TEXT_V_CENTER (1 << 4)
#define LT_TEXT_V_BOTTOM (1 << 5)
#define LT_TEXT_ELLIPSIS (1 << 6)
#define LT_TEXT_WRAP 	 (1 << 7)

#define LT_FONT_SANS 	   (0xFFFF)
#define LT_FONT_SERIF 	   (0xFFFE)
#define LT_FONT_MONOSPACED (0xFFFD)

#define LT_RECT_4(a, b, c, d) UI_RECT_4((int32_t) (a), (int32_t) (b), (int32_t) (c), (int32_t) (d))

#define LT_FUNCTION_OPTIMISE_O2 __attribute__((optimize("-O2")))
#define LT_FUNCTION_OPTIMISE_O3 __attribute__((optimize("-O3")))

#define LT_INFINITY INFINITY
...
char g_currentFolder[4096];
size_t g_currentFolderBytes;
#endif // OS_LIGHT globals
...
void InspectorPopulate() {
	...
	if (object && object->type != OBJ_INSTANCE) {
		...
		if (object->type == OBJ_STYLE) {
			...
			UIPanelCreate(0, UI_ELEMENT_PARENT_PUSH | UI_PANEL_HORIZONTAL);
			InspectorAddRadioSwitch(object, "Sans-serif", "fontFamily", LT_FONT_SANS);
			InspectorAddRadioSwitch(object, "Serif", "fontFamily", LT_FONT_SERIF);
			InspectorAddRadioSwitch(object, "Monospaced", "fontFamily", LT_FONT_MONOSPACED);
...
void CanvasDrawLayer(Object *object, UIRectangle bounds, UIPainter *painter, int depth = 0) {
	...
	if (object->type == OBJ_LAYER_BOX) {
		...
		textStyle.font.family = LT_FONT_SANS;
		...
		EsDrawTextSimple((_EsPainter *) &themePainter, ui.instance->window, bounds, "Sample", -1, textStyle, LT_TEXT_H_CENTER | LT_TEXT_V_CENTER); 
#endif
	} else if (object->type == OBJ_LAYER_PATH) {
		uint8_t buffer[4096];
		EsBuffer data = { .out = buffer, .bytes = sizeof(buffer) };
...
void CanvasDrawStyle(Object *object, UIRectangle bounds, UIPainter *painter, int depth = 0) {
	...
	if (object->type == OBJ_VAR_TEXT_STYLE) {
		...
		textStyle.font.flags = (GraphGetIntegerFromProperty(PropertyFindOrInherit(object, "isItalic"))) ? LT_FONT_ITALIC : LT_FLAGS_DEFAULT;
		...
		EsDrawTextSimple((_EsPainter *) &themePainter, ui.instance->window, bounds, "Sample", -1, textStyle, LT_TEXT_H_CENTER | LT_TEXT_V_CENTER); 
#if 0
		EsDrawStandardIcon((_EsPainter *) &themePainter, LT_ICON_GO_NEXT_SYMBOLIC, 
...
#ifdef OS_LIGHT
static struct { uint8_t *data; size_t pos, size, cap; } _gef;
static void _gef_write(const void *src, size_t n) {
	if (!n || !src) return;
	if (_gef.pos + n > _gef.cap) {
		size_t newCap = (_gef.cap + n) * 2 + 4096;
		uint8_t *nd = (uint8_t *)EsHeapAllocate(newCap, false);
		if (_gef.data) { EsMemoryCopy(nd, _gef.data, _gef.size); EsHeapFree(_gef.data); }
		_gef.data = nd; _gef.cap = newCap;
	}
	EsMemoryCopy(_gef.data + _gef.pos, src, n);
	_gef.pos += n;
	if (_gef.pos > _gef.size) _gef.size = _gef.pos;
}
#endif

void Export() {
	if (!g_exportFromGUI) DocumentLoad();
	...
	#undef assert
	#undef ftell
	#define fwrite(_d, _a, _b, _f)    _gef_write((_d), (_a) * (_b))
	#define fseek(_f, _pos, _whence)  (_gef.pos = (size_t)(_pos))
	#define ftell(_f)                 ((long)_gef.pos)
	#define fopen(_p, _m)             (_gef.pos = 0, _gef.size = 0, (void *)1)
	#define fclose(_f)                ((void)0)
	#define fprintf(...)              ((void)0)
	#define assert(_x)                EsAssert(_x)
	void *output = nullptr;
	(void)output;
#else
	FILE *output = nullptr;
	if (!g_exportFromGUI) output = fopen("styles.header", "wb");
#endif
	...
	for (uintptr_t i = 0; i < objects.Length(); i++) {
		...
		if (object->type == OBJ_STYLE) {
			bool isPublic = PropertyReadInt32(object, "isPublic");(void)isPublic;
			...
			fprintf(output, "%sdefine LT_STYLE_", !isPublic ? "private " : "");
	...
	if (!g_exportFromGUI) fclose(output);

	output = fopen("Theme.dat", "wb");
	...
	for (uintptr_t i = 0; i < objects.Length(); i++) {
		...
		if (metrics && textStyle) {
			...
			_metrics.textAlign = (wrapText ? LT_TEXT_WRAP : 0) | (ellipsis ? LT_TEXT_ELLIPSIS : 0)
				| (horizontalTextAlign == 1 ? LT_TEXT_H_LEFT : horizontalTextAlign == 3 ? LT_TEXT_H_RIGHT : LT_TEXT_H_CENTER)
				| (verticalTextAlign == 1 ? LT_TEXT_V_TOP : verticalTextAlign == 3 ? LT_TEXT_V_BOTTOM : LT_TEXT_V_CENTER);
...
#undef LAYER_READ_INT32
	...
	if (!g_exportFromGUI) DocumentFree();
	fclose(output);
	exportOffsets.Free();
...
void UpdateCompileButton() {
	...
	char themePath[4096];
	size_t themePathBytes = EsStringFormat(themePath, sizeof(themePath), "%s%s",
		g_currentFolderBytes, g_currentFolder, -1, "Theme.dat");
	bool exists = g_currentFolderBytes > 0 && EsPathExists(themePath, themePathBytes);
	SetBit(&compileButton->e.flags, UI_ELEMENT_DISABLED, !exists);
	UIElementRefresh(&compileButton->e);
#endif
}

void CompileThemeGUI(void *) {
#ifdef OS_LIGHT
	g_exportFromGUI = true;
	Export();
	g_exportFromGUI = false;
	char themePath[4096];
	size_t themePathBytes = EsStringFormat(themePath, sizeof(themePath), "%s%s",
		g_currentFolderBytes, g_currentFolder, -1, "Theme.dat");
	EsFileWriteAll(themePath, themePathBytes, _gef.data, _gef.size);
	EsHeapFree(_gef.data);
	EsMemoryZero(&_gef, sizeof(_gef));
	UpdateCompileButton();
#endif
}
...
void DocumentFileMenu(void *) {
	EsFileMenuCreate(ui.instance, ui.instance->window, LT_MENU_AT_CURSOR);
}
...
int _UIInstanceCallback(EsInstance *instance, EsMessage *message) {
	if (message->type == LT_MSG_INSTANCE_SAVE) {
		...
		// Extract folder from opened file path
		if (message->instanceOpen.nameOrPath && message->instanceOpen.nameOrPathBytes > 0) {
			const char *p = (const char *)message->instanceOpen.nameOrPath;
			size_t len = message->instanceOpen.nameOrPathBytes;
			size_t lastSlash = 0;
			for (size_t i = 0; i < len; i++) if (p[i] == '/') lastSlash = i + 1;
			if (lastSlash > 0 && lastSlash < sizeof(g_currentFolder) - 1) {
				EsMemoryCopy(g_currentFolder, p, lastSlash);
				g_currentFolder[lastSlash] = '\0';
				g_currentFolderBytes = lastSlash;
			}
		}
		DocumentLoad();
...
int main(int argc, char **argv) {
	...
	window = UIWindowCreate(0, UI_ELEMENT_PARENT_PUSH | UI_WINDOW_MAXIMIZE, "Light Designer", 0, 0);
...
#ifdef OS_LIGHT
	UIPanelCreate(0, UI_ELEMENT_PARENT_PUSH | UI_PANEL_HORIZONTAL | UI_PANEL_GRAY | UI_PANEL_MEDIUM_SPACING);
		compileButton = UIButtonCreate(0, 0, "Compile Theme", -1);
		compileButton->invoke = CompileThemeGUI;
		compileButton->e.flags |= UI_ELEMENT_DISABLED;
	UIParentPop();
#endif
...
#ifdef OS_LIGHT
	EsWindowSetIcon(ui.instance->window, LT_ICON_APPLICATIONS_INTERFACEDESIGN);
