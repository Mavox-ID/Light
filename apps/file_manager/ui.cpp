...
void InstanceRefreshViewType(Instance *instance) {
	EsCommandSetCheck(&instance->commandViewDetails,    instance->viewSettings.viewType == VIEW_DETAILS    ? LT_CHECK_CHECKED : LT_CHECK_UNCHECKED, false);
	EsCommandSetCheck(&instance->commandViewTiles,      instance->viewSettings.viewType == VIEW_TILES      ? LT_CHECK_CHECKED : LT_CHECK_UNCHECKED, false);
	EsCommandSetCheck(&instance->commandViewThumbnails, instance->viewSettings.viewType == VIEW_THUMBNAILS ? LT_CHECK_CHECKED : LT_CHECK_UNCHECKED, false);

	if (instance->viewSettings.viewType == VIEW_DETAILS) {
		EsListViewChangeStyles(instance->list, EsStyleIntern(&styleFolderView), LT_STYLE_LIST_ITEM, 0, 0, LT_LIST_VIEW_COLUMNS, LT_LIST_VIEW_TILED);
		EsListViewAddAllColumns(instance->list);
	} else if (instance->viewSettings.viewType == VIEW_TILES) {
		EsListViewChangeStyles(instance->list, EsStyleIntern(&styleFolderViewTiled), LT_STYLE_LIST_ITEM_TILE, 0, 0, LT_LIST_VIEW_TILED, LT_LIST_VIEW_COLUMNS);
	} else if (instance->viewSettings.viewType == VIEW_THUMBNAILS) {
		EsListViewChangeStyles(instance->list, EsStyleIntern(&styleFolderViewTiled), EsStyleIntern(&styleFolderItemThumbnail), 0, 0, LT_LIST_VIEW_TILED, LT_LIST_VIEW_COLUMNS);
	}
}

void InstanceRemoveItemSelectionCommands(Instance *instance) {
	EsCommandSetCallback(EsCommandByID(instance, LT_COMMAND_CUT), nullptr);
	EsCommandSetCallback(EsCommandByID(instance, LT_COMMAND_COPY), nullptr);
	EsCommandSetCallback(EsCommandByID(instance, LT_COMMAND_PASTE), nullptr);
}
...
void InstanceUpdateItemSelectionCountCommands(Instance *instance) {
	EsCommandSetEnabled(&instance->commandRename, instance->selectedItemCount == 1 && instance->folder->itemHandler->renameItem && !instance->folder->readOnly);
        EsCommandSetEnabled(&instance->commandDelete, instance->selectedItemCount >= 1 && instance->folder->itemHandler->removeItem && !instance->folder->readOnly);
...
#define COMMAND_SET(id, callback, enabled) \
	...
	if (EsElementIsFocused(instance->list)) {
		COMMAND_SET(LT_COMMAND_CUT, CommandCut, instance->selectedItemCount >= 1 && instance->folder->itemHandler->canCut && !instance->folder->readOnly);
		COMMAND_SET(LT_COMMAND_COPY, CommandCopy, instance->selectedItemCount >= 1 && instance->folder->itemHandler->canCopy);
		COMMAND_SET(LT_COMMAND_PASTE, CommandPaste, instance->folder->itemHandler->canPaste && EsClipboardHasData(LT_CLIPBOARD_PRIMARY) && !instance->folder->readOnly);
	}
}
...
LT_MACRO_SORT(InstanceSortListContents, ListEntry, {
...
void ThumbnailGenerateTask(Instance *, Task *task) {
	...
	EsFileInformation information = EsFileOpen(STRING(task->string), LT_FILE_READ | LT_NODE_FAIL_IF_NOT_FOUND);
	...
	EsRectangle targetRectangle = EsRectangleFit(LT_RECT_2S(thumbnailMaximumWidth, thumbnailMaximumHeight), LT_RECT_2S(originalWidth, originalHeight), false);
	uint32_t targetWidth = LT_RECT_WIDTH(targetRectangle), targetHeight = LT_RECT_HEIGHT(targetRectangle);
	uint32_t *targetBits;
...
int ListCallback(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_FOCUSED_START || message->type == LT_MSG_PRIMARY_CLIPBOARD_UPDATED) {
		InstanceUpdateItemSelectionCountCommands(instance);
		return 0;
		...
		InstanceRemoveItemSelectionCommands(instance);
		return 0;
	} else if (message->type == LT_MSG_LIST_VIEW_GET_CONTENT) {
	...
	} else if (message->type == LT_MSG_LIST_VIEW_GET_ITEM_DATA) {
	...
	} else if (message->type == LT_MSG_LIST_VIEW_GET_SUMMARY) {
		...
		message->getContent.icon = fileType->iconID;
		message->getContent.drawContentFlags = LT_DRAW_CONTENT_RICH_TEXT;
	} else if (message->type == LT_MSG_LIST_VIEW_SELECT_RANGE) {
		...
		StringDestroy(&instance->delayedFocusItem);
		InstanceUpdateStatusString(instance);
	} else if (message->type == LT_MSG_LIST_VIEW_SELECT) {
		...
		StringDestroy(&instance->delayedFocusItem);
		InstanceUpdateStatusString(instance);
	} else if (message->type == LT_MSG_LIST_VIEW_IS_SELECTED) {
		ListEntry *entry = &instance->listContents[message->selectItem.index];
		message->selectItem.isSelected = entry->selected;
	} else if (message->type == LT_MSG_LIST_VIEW_CHOOSE_ITEM) {
		...
		if (listEntry) {
			...
			if (entry->isFolder) {
				...
				if (EsKeyboardIsCtrlHeld() || message->chooseItem.source == LT_LIST_VIEW_CHOOSE_ITEM_MIDDLE_CLICK) {
					...
					request.flags = LT_APPLICATION_STARTUP_IN_SAME_CONTAINER | LT_APPLICATION_STARTUP_NO_DOCUMENT;
				...
				if (StringEquals(path, StringFromLiteral("0:/Light/Desktop.lpp")) 
						|| StringEquals(path, StringFromLiteral("0:/Light/Kernel.lpp"))) {
				...
				if (typeEntry) {
					...
					request.filePath = path.text;
					request.filePathBytes = path.bytes;
					request.flags = EsKeyboardIsCtrlHeld() || message->chooseItem.source == LT_LIST_VIEW_CHOOSE_ITEM_MIDDLE_CLICK 
						? LT_APPLICATION_STARTUP_IN_SAME_CONTAINER : LT_FLAGS_DEFAULT;
					EsApplicationStart(instance, &request);
					StringDestroy(&path);
				} else {
					EsDialogShow(instance->window, INTERFACE_STRING(FileManagerOpenFileError),
	...
	} else if (message->type == LT_MSG_LIST_VIEW_COLUMN_MENU) {
...
#define COLUMN_SIZE (2)
		...
		EsMenuAddItem(menu, instance->viewSettings.sortColumn == index ? LT_MENU_ITEM_CHECKED : 0, 
				ascending, -1, InstanceChangeSortColumn, index);
		EsMenuAddItem(menu, instance->viewSettings.sortColumn == (index | (1 << 8)) ? LT_MENU_ITEM_CHECKED : 0, 
				descending, -1, InstanceChangeSortColumn, index | (1 << 8));
		EsMenuShow(menu);
	} else if (message->type == LT_MSG_LIST_VIEW_GET_COLUMN_SORT) {
		if (message->getColumnSort.index == (instance->viewSettings.sortColumn & 0xFF)) {
			return (instance->viewSettings.sortColumn & (1 << 8)) ? LT_LIST_VIEW_COLUMN_SORT_DESCENDING : LT_LIST_VIEW_COLUMN_SORT_ASCENDING;
		}
	} else if (message->type == LT_MSG_LIST_VIEW_CREATE_ITEM) {
	...
	} else if (message->type == LT_MSG_LIST_VIEW_CONTEXT_MENU) {
		...
		if (information.isOpen) {
			...
			EsMenuAddItem(menu, LT_MENU_ITEM_HEADER, buffer, bytes);
			EsMenuAddSeparator(menu);
		}

		EsMenuAddCommand(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(CommonClipboardCut), EsCommandByID(instance, LT_COMMAND_CUT));
		EsMenuAddCommand(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(CommonClipboardCopy), EsCommandByID(instance, LT_COMMAND_COPY));
		EsMenuAddSeparator(menu);
		...
		EsMenuAddItem(menu, LT_MENU_ITEM_HEADER, INTERFACE_STRING(FileManagerListContextActions));
		EsMenuAddCommand(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(CommonClipboardPaste),            EsCommandByID(instance, LT_COMMAND_PASTE));
		EsMenuAddCommand(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(CommonSelectionSelectAll),        EsCommandByID(instance, LT_COMMAND_SELECT_ALL));
		EsMenuAddCommand(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(FileManagerNewFolderToolbarItem), &instance->commandNewFolder);
		EsMenuAddCommand(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(FileManagerRefresh),              &instance->commandRefresh);

		EsMenuAddSeparator(menu);

#define ADD_VIEW_TYPE_MENU_ITEM(_command, _string) \
		EsMenuAddCommand(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(_string), _command)
		EsMenuAddItem(menu, LT_MENU_ITEM_HEADER, INTERFACE_STRING(CommonListViewType));
...
#define ADD_SORT_COLUMN_MENU_ITEM(_column, _string) \
		EsMenuAddItem(menu, instance->viewSettings.sortColumn == (_column) ? LT_MENU_ITEM_CHECKED : LT_FLAGS_DEFAULT, \
				INTERFACE_STRING(_string), InstanceChangeSortColumn, _column)
		EsMenuAddItem(menu, LT_MENU_ITEM_HEADER, INTERFACE_STRING(CommonSortAscending));
		...
		EsMenuAddSeparator(menu);
		EsMenuAddItem(menu, LT_MENU_ITEM_HEADER, INTERFACE_STRING(CommonSortDescending));
...
int PlacesViewCallback(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_LIST_VIEW_GET_CONTENT) {
	...
	} else if (message->type == LT_MSG_LIST_VIEW_SELECT && message->selectItem.isSelected) {
		if (message->selectItem.group == PLACLT_VIEW_GROUP_DRIVES) {
		...
		} else if (message->selectItem.group == PLACLT_VIEW_GROUP_BOOKMARKS && message->selectItem.index) {
		...
		if (message->selectItem.group == PLACLT_VIEW_GROUP_BOOKMARKS && !message->selectItem.index) {
			...
			if (isCurrentFolderBookmarked) {
				EsMenuAddItem(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(FileManagerBookmarksRemoveHere), [] (EsMenu *menu, EsGeneric) {
					BookmarkRemove(menu->instance->path);
				});
			} else {
				EsMenuAddItem(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(FileManagerBookmarksAddHere), [] (EsMenu *menu, EsGeneric) {
...
int BreadcrumbBarMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_TEXTBOX_ACTIVATE_BREADCRUMB) {
		...
		StringDestroy(&section);
	} else if (message->type == LT_MSG_TEXTBOX_GET_BREADCRUMB) {
...
#define ADD_BUTTON_TO_TOOLBAR(_command, _label, _icon, _accessKey, _name) \
	{ \
		_name = EsButtonCreate(buttonGroup, LT_FLAGS_DEFAULT, 0, _label); \
...
#define ADD_BUTTON_TO_STATUS_BAR(_command, _label, _icon, _accessKey, _name) \
	{ \
		_name = EsButtonCreate(buttonGroup, LT_FLAGS_DEFAULT, LT_STYLE_PUSH_BUTTON_STATUS_BAR, _label); \
		EsButtonSetIcon(_name, _icon);  \
		EsCommandAddButton(&instance->_command, _name); \
		_name->accessKey = _accessKey; \
	}
...
void InstanceCreateUI(Instance *instance) {
	...
	EsWindowSetIcon(instance->window, LT_ICON_SYSTEM_FILE_MANAGER);
	InstanceRegisterCommands(instance);

	EsPanel *rootPanel = EsPanelCreate(instance->window, LT_CELL_FILL | LT_PANEL_VERTICAL);
	EsSplitter *splitter = EsSplitterCreate(rootPanel, LT_SPLITTER_HORIZONTAL | LT_CELL_FILL, LT_STYLE_PANEL_WINDOW_WITH_STATUS_BAR_CONTENT);

	// Places:

	instance->placesView = EsListViewCreate(splitter, LT_CELL_EXPAND | LT_CELL_COLLAPSABLE | LT_LIST_VIEW_SINGLE_SELECT | LT_CELL_V_FILL, 
			EsStyleIntern(&stylePlacesView), LT_STYLE_LIST_ITEM, LT_STYLE_LIST_ITEM, 0);
	instance->placesView->accessKey = 'P';
	instance->placesView->messageUser = PlacesViewCallback;
	EsListViewInsertGroup(instance->placesView, PLACLT_VIEW_GROUP_BOOKMARKS, LT_LIST_VIEW_GROUP_HAS_HEADER | LT_LIST_VIEW_GROUP_INDENT);
	EsListViewInsertGroup(instance->placesView, PLACLT_VIEW_GROUP_DRIVES, LT_LIST_VIEW_GROUP_HAS_HEADER | LT_LIST_VIEW_GROUP_INDENT);
	if (bookmarks.Length()) EsListViewInsert(instance->placesView, PLACLT_VIEW_GROUP_BOOKMARKS, 1, bookmarks.Length());
	EsListViewInsert(instance->placesView, PLACLT_VIEW_GROUP_DRIVES, 1, drives.Length());

	// Main list:

	instance->list = EsListViewCreate(splitter, LT_CELL_FILL | LT_LIST_VIEW_COLUMNS | LT_LIST_VIEW_MULTI_SELECT, EsStyleIntern(&styleFolderView));
	instance->list->accessKey = 'L';
	instance->list->messageUser = ListCallback;
	EsListViewRegisterColumn(instance->list, COLUMN_NAME, INTERFACE_STRING(FileManagerColumnName), LT_LIST_VIEW_COLUMN_HAS_MENU);
	EsListViewRegisterColumn(instance->list, COLUMN_TYPE, INTERFACE_STRING(FileManagerColumnType), LT_LIST_VIEW_COLUMN_HAS_MENU);
	EsListViewRegisterColumn(instance->list, COLUMN_SIZE, INTERFACE_STRING(FileManagerColumnSize), 
			LT_LIST_VIEW_COLUMN_HAS_MENU | LT_TEXT_H_RIGHT | LT_LIST_VIEW_COLUMN_DATA_INTEGERS | LT_LIST_VIEW_COLUMN_FORMAT_BYTES);
	EsListViewAddAllColumns(instance->list);
	...
	EsPanel *buttonGroup = EsPanelCreate(toolbar, LT_PANEL_HORIZONTAL | LT_ELEMENT_AUTO_GROUP);
	ADD_BUTTON_TO_TOOLBAR(commandGoBackwards, nullptr, LT_ICON_GO_PREVIOUS_SYMBOLIC, 'B', button);
	EsSpacerCreate(buttonGroup, LT_CELL_V_FILL, LT_STYLE_TOOLBAR_BUTTON_GROUP_SEPARATOR);
	ADD_BUTTON_TO_TOOLBAR(commandGoForwards, nullptr, LT_ICON_GO_NEXT_SYMBOLIC, 'F', button);
	EsSpacerCreate(buttonGroup, LT_CELL_V_FILL, LT_STYLE_TOOLBAR_BUTTON_GROUP_SEPARATOR);
	ADD_BUTTON_TO_TOOLBAR(commandGoParent, nullptr, LT_ICON_GO_UP_SYMBOLIC, 'U', button);
	EsSpacerCreate(toolbar, LT_FLAGS_DEFAULT, LT_STYLE_TOOLBAR_SPACER);
	instance->breadcrumbBar = EsTextboxCreate(toolbar, LT_CELL_H_FILL | LT_TEXTBOX_EDIT_BASED | LT_TEXTBOX_REJECT_EDIT_IF_LOST_FOCUS);
	...
	EsSpacerCreate(toolbar, LT_FLAGS_DEFAULT, LT_STYLE_TOOLBAR_SPACER);
	buttonGroup = EsPanelCreate(toolbar, LT_PANEL_HORIZONTAL);
	ADD_BUTTON_TO_TOOLBAR(commandNewFolder, interfaceString_FileManagerNewFolderToolbarItem, LT_ICON_FOLDER_NEW_SYMBOLIC, 'N', instance->newFolderButton);

	// Status bar:

	EsPanel *statusBar = EsPanelCreate(rootPanel, LT_CELL_H_FILL | LT_PANEL_HORIZONTAL, LT_STYLE_PANEL_STATUS_BAR);
	instance->status = EsTextDisplayCreate(statusBar, LT_CELL_H_FILL);
	buttonGroup = EsPanelCreate(statusBar, LT_PANEL_HORIZONTAL | LT_ELEMENT_AUTO_GROUP);
	ADD_BUTTON_TO_STATUS_BAR(commandViewDetails, nullptr, LT_ICON_VIEW_LIST_SYMBOLIC, 0, button);
	EsSpacerCreate(buttonGroup, LT_CELL_V_FILL, LT_STYLE_TOOLBAR_BUTTON_GROUP_SEPARATOR);
	ADD_BUTTON_TO_STATUS_BAR(commandViewTiles, nullptr, LT_ICON_VIEW_LIST_COMPACT_SYMBOLIC, 0, button);
	EsSpacerCreate(buttonGroup, LT_CELL_V_FILL, LT_STYLE_TOOLBAR_BUTTON_GROUP_SEPARATOR);
	ADD_BUTTON_TO_STATUS_BAR(commandViewThumbnails, nullptr, LT_ICON_VIEW_GRID_SYMBOLIC, 0, button);
...
void InstanceReportError(Instance *instance, int error, EsError code) {
	...
	if (code == LT_ERROR_ALREADY_EXISTS) {
		message = interfaceString_FileManagerItemAlreadyExistsError;
	} else if (code == LT_ERROR_FILE_DOLT_NOT_EXIST) {
		message = interfaceString_FileManagerItemDoesNotExistError;
	} else if (code == LT_ERROR_PERMISSION_NOT_GRANTED) {
