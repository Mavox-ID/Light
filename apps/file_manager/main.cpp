...
#define PLACLT_VIEW_GROUP_BOOKMARKS (0)
#define PLACLT_VIEW_GROUP_DRIVES (1)

#define MESSAGE_BLOCKING_TASK_COMPLETE ((EsMessageType) (LT_MSG_USER_START + 1))
#define MESSAGE_NON_BLOCKING_TASK_COMPLETE ((EsMessageType) (LT_MSG_USER_START + 2))
...
struct NamespaceHandler {
	...
	void (*removeItem)(Folder *folder, String name);
...
const EsStyle styleFolderView = {
	.inherit = LT_STYLE_LIST_VIEW,

	.metrics = {
		.mask = LT_THEME_METRICS_MINIMUM_WIDTH | LT_THEME_METRICS_PREFERRED_WIDTH,
		.preferredWidth = 200,
		.minimumWidth = 150,
	},
};
...
const EsStyle styleFolderViewTiled = {
	...
	.metrics = {
		.mask = LT_THEME_METRICS_MINIMUM_WIDTH | LT_THEME_METRICS_PREFERRED_WIDTH | LT_THEME_METRICS_GAP_WRAP | LT_THEME_METRICS_GAP_MINOR,
		.preferredWidth = 200,
		.minimumWidth = 150,
		.gapMinor = 5,
		.gapWrap = 5,
...
const EsStyle styleFolderItemThumbnail = {
	.inherit = LT_STYLE_LIST_ITEM_TILE,

	.metrics = {
		.mask = LT_THEME_METRICS_PREFERRED_WIDTH | LT_THEME_METRICS_PREFERRED_HEIGHT 
			| LT_THEME_METRICS_ICON_SIZE | LT_THEME_METRICS_LAYOUT_VERTICAL | LT_THEME_METRICS_INSETS | LT_THEME_METRICS_TEXT_ALIGN,
		.insets = LT_RECT_2(5, 10),
		.preferredWidth = 170,
		.preferredHeight = 150,
		.textAlign = LT_TEXT_H_CENTER | LT_TEXT_V_CENTER | LT_TEXT_ELLIPSIS,
		.iconSize = 80,
		.layoutVertical = true,
	},
};
...
const EsStyle stylePlacesView = {
	...
	.metrics = {
		.mask = LT_THEME_METRICS_MINIMUM_WIDTH | LT_THEME_METRICS_PREFERRED_WIDTH | LT_THEME_METRICS_INSETS
			| LT_THEME_METRICS_GAP_WRAP | LT_THEME_METRICS_GAP_MAJOR,
		.insets = LT_RECT_1(8),
		.preferredWidth = 200,
		.minimumWidth = 150,
		.gapMajor = 16,
...
void BlockingTaskQueue(Instance *instance, Task task) {
	...
	if (result == LT_ERROR_TIMEOUT_REACHED) {
		instance->blockingTaskReachedTimeout = true;
		instance->blockingDialog = EsDialogShow(instance->window, task.cDescription, -1, 
				INTERFACE_STRING(FileManagerOngoingTaskDescription), LT_ICON_TOOLS_TIMER_SYMBOLIC);
...
void DriveRefreshFolders(bool removed, const char *prefix, size_t prefixBytes) {
	...
	for (uintptr_t i = 0; i < loadedFolders.Length(); i++) {
		...
		if (folder->itemHandler->type == NAMESPACE_HANDLER_DRIVLT_PAGE || StringStartsWith(folder->path, StringFromLiteralWithSize(prefix, prefixBytes))) {
	...
	for (uintptr_t index = 0; index < drives.Length(); index++) {
		if (drives[index].deviceID == deviceID) {
			...
			for (uintptr_t i = 0; i < instances.Length(); i++) {
				...
				EsListViewRemove(instances[i]->placesView, PLACLT_VIEW_GROUP_DRIVES, index, 1);
			}
...
void DriveAdd(EsHandle deviceHandle, EsObjectID deviceID) {
	...
	char prefix[16];
	bool isBootFileSystem = EsDeviceControl(deviceHandle, LT_DEVICE_CONTROL_FS_IS_BOOT, 0, nullptr) == 1;
	size_t prefixBytes = EsStringFormat(prefix, sizeof(prefix), "%fd:", LT_STRING_FORMAT_SIMPLE, isBootFileSystem ? 0 : nextPrefix);
	...
	for (uintptr_t i = 0; i < instances.Length(); i++) {
		if (instances[i]->closed) continue;
		EsListViewInsert(instances[i]->placesView, PLACLT_VIEW_GROUP_DRIVES, drives.Length(), 1);
	}
...
void _start() {
	...
	for (uintptr_t i = 0; i < deviceCount; i++) if (devices[i].type == LT_DEVICE_FILE_SYSTEM) DriveAdd(devices[i].handle, devices[i].id);
	EsHeapFree(devices);
	...
	while (true) {
		...
		if (message->type == LT_MSG_INSTANCE_CREATE) {
			Instance *instance = EsInstanceCreate(message, INTERFACE_STRING(FileManagerTitle));
                        // EsCommandRegister(&instance->commandDelete, instance, INTERFACE_STRING(FileManagerDeleteAction), CommandDelete, stableCommandID++);
		...
		} else if (message->type == LT_MSG_DEVICE_DISCONNECTED && message->device.type == LT_DEVICE_FILE_SYSTEM) {
			DriveRemove(message->device.id);
		} else if (message->type == LT_MSG_FILE_MANAGER_FILE_MODIFIED) {
			...
			EsHeapFree(_path);
		} else if (message->type == LT_MSG_FILE_MANAGER_PATH_MOVED) {
			...
			EsHeapFree(data);
		} else if (message->type == LT_MSG_FILE_MANAGER_DOCUMENT_UPDATE) {
