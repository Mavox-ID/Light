...
#define NAMESPACE_HANDLER_DRIVLT_PAGE (2)
...
// Define a FolderFileUpdatedAtPath
...
bool FSDirHandlesPath(String path) {
	...
	if (LT_SUCCESS == EsPathQueryInformation(STRING(path), &information)) {
		return information.type == LT_NODE_DIRECTORY;
	} else {
		return false;
	}
}

void FSDirRemoveItem(Folder *folder, String name) {
    size_t pathBytes;
    char *path = EsStringAllocateAndFormat(&pathBytes, "%s%s", STRFMT(folder->path), STRFMT(name));
    EsPathDelete(path, pathBytes); 
    FolderFileUpdatedAtPath(StringFromLiteralWithSize(path, pathBytes), nullptr); // TODO
    EsHeapFree(path);
}
...
EsError FSDirCreateChildFolder(Folder *folder, String *name, bool findUniqueName) {
	...
	EsError error = EsPathCreate(path, pathBytes, LT_NODE_DIRECTORY, false);
    
        if (error == LT_SUCCESS) {
            FolderFileUpdatedAtPath(StringFromLiteralWithSize(path, pathBytes), nullptr);
        }
	EsHeapFree(path);
	return error;
}
...
EsError FSDirRenameItem(Folder *folder, String oldName, String newName) {
	...
	/* if (error == LT_SUCCESS) {
		FolderFileUpdatedAtPath(StringFromLiteralWithSize(oldPath, oldPathBytes), nullptr);
		FolderFileUpdatedAtPath(StringFromLiteralWithSize(newPath, newPathBytes), nullptr);
	}
	*/
...
EsError FSDirEnumerate(Folder *folder) {
	...
	if (!EsPathExists(STRING(folder->path), &type) || type != LT_NODE_DIRECTORY) {
	...
	if (EsMountPointGetVolumeInformation(STRING(folder->path), &volume)) {
		folder->readOnly = volume.flags & LT_VOLUME_READ_ONLY;
	}
...
void FSDirGetTotalSize(Folder *folder) {
	...
	if (LT_SUCCESS == EsPathQueryInformation(STRING(folder->path), &information)) {
...
uint32_t DrivesPageGetFileType(String path) {
	...
	if (EsMountPointGetVolumeInformation(STRING(string), &volume)) {
		if (volume.driveType == LT_DRIVE_TYPE_HDD             ) return KNOWN_FILE_TYPE_DRIVE_HDD;
		if (volume.driveType == LT_DRIVE_TYPE_SSD             ) return KNOWN_FILE_TYPE_DRIVE_SSD;
		if (volume.driveType == LT_DRIVE_TYPE_CDROM           ) return KNOWN_FILE_TYPE_DRIVE_CDROM;
		if (volume.driveType == LT_DRIVE_TYPE_USB_MASS_STORAGE) return KNOWN_FILE_TYPE_DRIVE_USB_MASS_STORAGE;
...
uint32_t NamespaceRootGetFileType(String path) {
	if (StringEquals(path, StringFromLiteralWithSize(INTERFACE_STRING(FileManagerDrivesPage)))) {
		return KNOWN_FILE_TYPE_DRIVLT_PAGE;
	} else {
		return KNOWN_FILE_TYPE_DIRECTORY;
	}
}
...
NamespaceHandler namespaceHandlers[] = {
	{
		.type = NAMESPACE_HANDLER_DRIVLT_PAGE,
...

	{
		...
		.rootContainerHandlerType = NAMESPACE_HANDLER_DRIVLT_PAGE,
		.canCut = true,
		.canCopy = true,
		.canPaste = true,
...
void BookmarkAdd(String path, bool saveConfiguration = true) {
	...
	for (uintptr_t i = 0; i < instances.Length(); i++) {
		...
		EsListViewInsert(instances[i]->placesView, PLACLT_VIEW_GROUP_BOOKMARKS, bookmarks.Length(), 1);
	}
}
...
void BookmarkRemove(String path) {
	for (uintptr_t i = 0; i < bookmarks.Length(); i++) {
		if (0 == EsStringCompareRaw(STRING(bookmarks[i]), STRING(path))) {
			...
			for (uintptr_t i = 0; i < instances.Length(); i++) {
				if (instances[i]->closed) continue;
				EsListViewRemove(instances[i]->placesView, PLACLT_VIEW_GROUP_BOOKMARKS, i + 1, 1);
			}
...
FolderEntry *FolderAddEntry(Folder *folder, const char *_name, size_t nameBytes, EsDirectoryChild *information, uint64_t id) {
	...
	entry->isFolder = information->type == LT_NODE_DIRECTORY;
	entry->sizeUnknown = entry->isFolder && information->directoryChildren == LT_DIRECTORY_CHILDREN_UNKNOWN;
...
void FolderFileUpdatedAtPath(String path, Instance *instance, bool setGuessedContentType) {
	path = PathRemoveTrailingSlash(path);
	String file = PathGetName(path);
	String folder = PathGetParent(path);
	EsDirectoryChild information = {};
	bool add = LT_SUCCESS == EsPathQueryInformation(STRING(path), &information);
	...
	if (setGuessedContentType && 0 == EsMemoryCompare(&information.contentType, &zeroIdentifier, sizeof(EsUniqueIdentifier))) {
		...
		if (EsMemoryCompare(&identifier, &zeroIdentifier, sizeof(EsUniqueIdentifier))) {
			EsFileInformation information = EsFileOpen(STRING(path), LT_FILE_WRITE_SHARED | LT_NODE_FAIL_IF_NOT_FOUND);

			if (information.error == LT_SUCCESS) {
				EsFileControl(information.handle, LT_FILE_CONTROL_SET_CONTENT_TYPE, &identifier, sizeof(identifier));
...
void FolderAttachInstance(Instance *instance, String path, bool recurse) {
	...
	// DO NOT UNCOMMING THIS STRING!!!!!!
	//IT'S WILL BE CRUSH FILE MANEGER!
}
