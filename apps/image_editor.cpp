...
void ImageCopyFromPaintTarget(Instance *instance, Image *image, EsRectangle modifiedBounds) {
	...
	modifiedBounds = EsRectangleIntersection(modifiedBounds, LT_RECT_4(0, width, 0, height));
...
int BrushSizeMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_TEXTBOX_NUMBER_DRAG_DELTA) {
...
EsRectangle DrawFill(Instance *instance, EsPoint point) {
	...
	EsRectangle modifiedBounds = LT_RECT_4(point.x, point.x, point.y, point.y);
...
EsRectangle DrawLine(Instance *instance, bool force = false) {
	...
	uint32_t color = 0xFF000000 | EsColorWellGetRGB(instance->colorWell);
	EsRectangle modifiedBounds = LT_RECT_4(instance->bitmapWidth, 0, instance->bitmapHeight, 0);
	...
	while (true) {
		...
		EsRectangle bounds = LT_RECT_4(x0 - brushSize / 2 - 1, x0 + brushSize / 2 + 1, 
				y0 - brushSize / 2 - 1, y0 + brushSize / 2 + 1);
		bounds = EsRectangleIntersection(bounds, LT_RECT_4(0, instance->bitmapWidth, 0, instance->bitmapHeight));
...
int CanvasMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_PAINT) {
		...
		EsRectangle area = LT_RECT_4(bounds.l, bounds.l + instance->bitmapWidth, bounds.t, bounds.t + instance->bitmapHeight);
		EsDrawPaintTarget(painter, instance->bitmap, area, LT_RECT_4(0, instance->bitmapWidth, 0, instance->bitmapHeight), 0xFF); 

		if (instance->commandRectangle.check == LT_CHECK_CHECKED && instance->dragged) {
	...
	} else if ((message->type == LT_MSG_MOUSE_LEFT_DRAG || message->type == LT_MSG_MOUSE_MOVED) 
			&& EsMouseIsLeftHeld() && instance->commandBrush.check == LT_CHECK_CHECKED) {
		instance->modifiedBounds = EsRectangleBounding(DrawLine(instance), instance->modifiedBounds);
	} else if (message->type == LT_MSG_MOUSE_LEFT_DRAG && instance->commandRectangle.check == LT_CHECK_CHECKED) {
	...
	} else if (message->type == LT_MSG_MOUSE_LEFT_DOWN && instance->commandBrush.check == LT_CHECK_CHECKED) {
		...
		instance->modifiedBounds = LT_RECT_4(instance->bitmapWidth, 0, instance->bitmapHeight, 0);
		DrawLine(instance, true);
	} else if (message->type == LT_MSG_MOUSE_LEFT_DOWN && instance->commandRectangle.check == LT_CHECK_CHECKED) {
		EsPoint point = EsMouseGetPosition(element); 
		instance->previousPointX = point.x, instance->previousPointY = point.y;
		instance->dragged = false;
	} else if (message->type == LT_MSG_MOUSE_LEFT_UP && instance->commandBrush.check == LT_CHECK_CHECKED) {
		ImageCopyFromPaintTarget(instance, &instance->image, instance->modifiedBounds);
	} else if (message->type == LT_MSG_MOUSE_LEFT_UP && instance->commandRectangle.check == LT_CHECK_CHECKED && instance->dragged) {
		instance->dragged = false;
		EsUndoPush(instance->undoManager, ImageUndoMessage, &instance->image, sizeof(Image));
		instance->image = ImageFork(instance, instance->image);
		EsPainter painter = {};
		painter.clip = LT_RECT_4(0, instance->bitmapWidth, 0, instance->bitmapHeight);
		...
		ImageCopyFromPaintTarget(instance, &instance->image, instance->modifiedBounds);
	} else if (message->type == LT_MSG_MOUSE_LEFT_UP && instance->commandFill.check == LT_CHECK_CHECKED) {
		EsUndoPush(instance->undoManager, ImageUndoMessage, &instance->image, sizeof(Image));
		instance->image = ImageFork(instance, instance->image);
		...
		message->cursorStyle = LT_CURSOR_CROSS_HAIR_PICK;
...
void CommandSelectTool(Instance *instance, EsElement *, EsCommand *command) {
	if (command->check == LT_CHECK_CHECKED) {
		return;
	}

	EsCommandSetCheck(&instance->commandBrush, LT_CHECK_UNCHECKED, false);
	EsCommandSetCheck(&instance->commandFill, LT_CHECK_UNCHECKED, false);
	EsCommandSetCheck(&instance->commandRectangle, LT_CHECK_UNCHECKED, false);
	EsCommandSetCheck(&instance->commandSelect, LT_CHECK_UNCHECKED, false);
	EsCommandSetCheck(&instance->commandText, LT_CHECK_UNCHECKED, false);
	EsCommandSetCheck(command, LT_CHECK_CHECKED, false);

	if (command == &instance->commandBrush) EsButtonSetIcon(instance->toolDropdown, LT_ICON_DRAW_FREEHAND);
	if (command == &instance->commandFill) EsButtonSetIcon(instance->toolDropdown, LT_ICON_COLOR_FILL);
	if (command == &instance->commandRectangle) EsButtonSetIcon(instance->toolDropdown, LT_ICON_DRAW_RECTANGLE);
	if (command == &instance->commandSelect) EsButtonSetIcon(instance->toolDropdown, LT_ICON_OBJECT_GROUP);
	if (command == &instance->commandText) EsButtonSetIcon(instance->toolDropdown, LT_ICON_DRAW_TEXT);
...
int BitmapSizeTextboxMessage(EsElement *element, EsMessage *message) {
	...
	if (message->type == LT_MSG_TEXTBOX_EDIT_END || message->type == LT_MSG_TEXTBOX_NUMBER_DRAG_END) {
		...
		if (textbox->userData.i) {
			...
			clearRegion = LT_RECT_4(0, instance->bitmapWidth, oldSize, newSize);
		} else {
			instance->bitmapWidth = newSize;
			clearRegion = LT_RECT_4(oldSize, newSize, 0, instance->bitmapHeight);
		}
...
void ImageTransform(EsMenu *menu, EsGeneric context) {
	...
	ImageCopyFromPaintTarget(instance, &instance->image, LT_RECT_4(0, instance->bitmapWidth, 0, instance->bitmapHeight));
	EsElementRepaint(instance->canvas);
}
...
void MenuImage(Instance *instance, EsElement *element, EsCommand *) {
	...
	EsMenuAddItem(menu, LT_MENU_ITEM_HEADER, INTERFACE_STRING(ImageEditorCanvasSize));
	EsPanel *table = EsPanelCreate(menu, LT_PANEL_HORIZONTAL | LT_PANEL_TABLE, EsStyleIntern(&styleImageMenuTable));
	...
	EsTextDisplayCreate(table, LT_CELL_H_RIGHT, LT_STYLE_TEXT_LABEL, INTERFACE_STRING(ImageEditorPropertyWidth));
	textbox = EsTextboxCreate(table, LT_TEXTBOX_EDIT_BASED, LT_STYLE_TEXTBOX_BORDERED_SINGLE_MEDIUM);
	...
	EsTextDisplayCreate(table, LT_CELL_H_RIGHT, LT_STYLE_TEXT_LABEL, INTERFACE_STRING(ImageEditorPropertyHeight));
	textbox = EsTextboxCreate(table, LT_TEXTBOX_EDIT_BASED, LT_STYLE_TEXTBOX_BORDERED_SINGLE_MEDIUM);
	EsTextboxInsert(textbox, buffer, bytes, false);
	...
	EsMenuAddItem(menu, LT_MENU_ITEM_HEADER, INTERFACE_STRING(ImageEditorImageTransformations));
	EsMenuAddItem(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(ImageEditorRotateLeft), ImageTransform, 1);
	EsMenuAddItem(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(ImageEditorRotateRight), ImageTransform, 2);
	EsMenuAddItem(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(ImageEditorFlipHorizontally), ImageTransform, 3);
	EsMenuAddItem(menu, LT_FLAGS_DEFAULT, INTERFACE_STRING(ImageEditorFlipVertically), ImageTransform, 4);
...
int InstanceCallback(Instance *instance, EsMessage *message) {
	...
	} else if (message->type == LT_MSG_INSTANCE_SAVE) {
		// TODO Error handling.
...
void InstanceCreate(EsMessage *message) {
	...
	EsCommandSetCheck(&instance->commandBrush, LT_CHECK_CHECKED, false);
	...
	button = EsButtonCreate(toolbar, LT_BUTTON_DROPDOWN, LT_STYLE_PUSH_BUTTON_TOOLBAR, INTERFACE_STRING(ImageEditorImage));
	EsButtonSetIcon(button, LT_ICON_IMAGE_X_GENERIC);
	button->accessKey = 'I';
	...
	EsCommandAddButton(EsCommandByID(instance, LT_COMMAND_UNDO), button);
	EsButtonSetIcon(button, LT_ICON_EDIT_UNDO_SYMBOLIC);
	button->accessKey = 'U';
	EsSpacerCreate(buttonGroup, LT_CELL_V_FILL, LT_STYLE_TOOLBAR_BUTTON_GROUP_SEPARATOR);
	button = EsButtonCreate(buttonGroup);
	EsCommandAddButton(EsCommandByID(instance, LT_COMMAND_REDO), button);
	EsButtonSetIcon(button, LT_ICON_EDIT_REDO_SYMBOLIC);
	button->accessKey = 'R';

	EsSpacerCreate(toolbar, LT_CELL_FILL);

	button = instance->toolDropdown = EsButtonCreate(toolbar, LT_BUTTON_DROPDOWN,
			LT_STYLE_PUSH_BUTTON_TOOLBAR_BIG, INTERFACE_STRING(ImageEditorPickTool));
	EsButtonSetIcon(button, LT_ICON_DRAW_FREEHAND);
	EsButtonOnCommand(button, MenuTools);
	button->accessKey = 'T';

	instance->toolPanel = EsPanelCreate(toolbar, LT_PANEL_HORIZONTAL | LT_ELEMENT_AUTO_GROUP);
	button = EsButtonCreate(instance->toolPanel, LT_FLAGS_DEFAULT, LT_STYLE_PUSH_BUTTON_TOOLBAR_BIG, INTERFACE_STRING(ImageEditorToolBrush));
	EsCommandAddButton(&instance->commandBrush, button);
	EsButtonSetIcon(button, LT_ICON_DRAW_FREEHAND);
	button->accessKey = 'B';
	EsSpacerCreate(instance->toolPanel, LT_CELL_V_FILL, LT_STYLE_TOOLBAR_BUTTON_GROUP_SEPARATOR);
	button = EsButtonCreate(instance->toolPanel, LT_FLAGS_DEFAULT, LT_STYLE_PUSH_BUTTON_TOOLBAR_BIG, INTERFACE_STRING(ImageEditorToolFill));
	EsCommandAddButton(&instance->commandFill, button);
	EsButtonSetIcon(button, LT_ICON_COLOR_FILL);
	button->accessKey = 'F';
	EsSpacerCreate(instance->toolPanel, LT_CELL_V_FILL, LT_STYLE_TOOLBAR_BUTTON_GROUP_SEPARATOR);
	button = EsButtonCreate(instance->toolPanel, LT_FLAGS_DEFAULT, LT_STYLE_PUSH_BUTTON_TOOLBAR_BIG, INTERFACE_STRING(ImageEditorToolRectangle));
	EsCommandAddButton(&instance->commandRectangle, button);
	EsButtonSetIcon(button, LT_ICON_DRAW_RECTANGLE);
	button->accessKey = 'E';
	EsSpacerCreate(instance->toolPanel, LT_CELL_V_FILL, LT_STYLE_TOOLBAR_BUTTON_GROUP_SEPARATOR);
	button = EsButtonCreate(instance->toolPanel, LT_FLAGS_DEFAULT, LT_STYLE_PUSH_BUTTON_TOOLBAR_BIG, INTERFACE_STRING(ImageEditorToolSelect));
	EsCommandAddButton(&instance->commandSelect, button);
	EsButtonSetIcon(button, LT_ICON_OBJECT_GROUP);
	button->accessKey = 'S';
	EsSpacerCreate(instance->toolPanel, LT_CELL_V_FILL, LT_STYLE_TOOLBAR_BUTTON_GROUP_SEPARATOR);
	button = EsButtonCreate(instance->toolPanel, LT_FLAGS_DEFAULT, LT_STYLE_PUSH_BUTTON_TOOLBAR_BIG, INTERFACE_STRING(ImageEditorToolText));
	EsCommandAddButton(&instance->commandText, button);
	EsButtonSetIcon(button, LT_ICON_DRAW_TEXT);
	button->accessKey = 'T';
	...
	EsTextDisplayCreate(section, LT_FLAGS_DEFAULT, 0, INTERFACE_STRING(ImageEditorPropertyColor));
	instance->colorWell = EsColorWellCreate(section, LT_FLAGS_DEFAULT, 0xFFFF0000);
	...
	EsTextDisplayCreate(section, LT_FLAGS_DEFAULT, 0, INTERFACE_STRING(ImageEditorPropertyBrushSize));
	instance->brushSize = EsTextboxCreate(section, LT_TEXTBOX_EDIT_BASED, LT_STYLE_TEXTBOX_BORDERED_SINGLE_COMPACT);
	...
	EsSpacerCreate(toolbar, LT_FLAGS_DEFAULT, 0, 1, 0);

	// Create the user interface.

	EsWindowSetIcon(instance->window, LT_ICON_MULTIMEDIA_PHOTO_MANAGER);

	EsCanvasPane *canvasPane = EsCanvasPaneCreate(instance->window, LT_CELL_FILL | LT_CANVAS_PANE_SHOW_SHADOW, LT_STYLE_PANEL_WINDOW_BACKGROUND);
	instance->canvas = EsCustomElementCreate(canvasPane, LT_CELL_CENTER | LT_ELEMENT_FOCUSABLE);
