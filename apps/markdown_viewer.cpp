...
#define MAKE_TEXT_STYLE(_textColor, _textSize, _fontFamily, _fontWeight, _isItalic) \
	{ \
		.metrics = { \
			.mask = LT_THEME_METRICS_TEXT_COLOR | LT_THEME_METRICS_TEXT_ALIGN | LT_THEME_METRICS_TEXT_SIZE \
				| LT_THEME_METRICS_FONT_FAMILY | LT_THEME_METRICS_FONT_WEIGHT  \
				| LT_THEME_METRICS_IS_ITALIC, \
			.textColor = _textColor, \
			.textAlign = LT_TEXT_H_LEFT | LT_TEXT_V_TOP | LT_TEXT_WRAP, \
...
EsStyle styleRoot = {
	.metrics = {
		.mask = LT_THEME_METRICS_INSETS | LT_THEME_METRICS_PREFERRED_WIDTH,
		.insets = LT_RECT_4(32, 32, 16, 32),
		.preferredWidth = 800,
	},
};
...
EsStyle styleQuote = {
	.metrics = {
		...
		.insets = LT_RECT_4(20, 16, 0, 0),
	},

	.appearance = {
		.enabled = true,
		.borderColor = COLOR_GRAY1,
		.borderSize = LT_RECT_4(4, 0, 0, 0),
	},
};
...
EsStyle styleHeadingUnderline = {
	.metrics = {
		.mask = LT_THEME_METRICS_PREFERRED_HEIGHT,
		.preferredHeight = 1,
	},
...
EsStyle styleCodeBlock = {
	.metrics = {
		.mask = LT_THEME_METRICS_INSETS | LT_THEME_METRICS_CLIP_ENABLED,
		.insets = LT_RECT_4(16, 16, 10, 10),
		.clipEnabled = true,
	},
...
EsStyle styleTD = {
	...
	.metrics = {
		.mask = LT_THEME_METRICS_INSETS | LT_THEME_METRICS_TEXT_SIZE | LT_THEME_METRICS_TEXT_COLOR,
		.insets = LT_RECT_4(8, 8, 8, 0),
...
EsStyle styleTH = {
	...
	.metrics = {
		.mask = LT_THEME_METRICS_INSETS | LT_THEME_METRICS_TEXT_SIZE | LT_THEME_METRICS_TEXT_COLOR | LT_THEME_METRICS_FONT_WEIGHT,
		.insets = LT_RECT_4(8, 8, 0, 4),
		.textColor = COLOR_TEXT1,
		.textSize = (int) (16 * 0.64 + 0.5),
		.fontWeight = 5,
	},
	...
	.appearance = {
		.enabled = true,
		.borderColor = COLOR_GRAY1,
		.borderSize = LT_RECT_4(0, 0, 0, 1),
	},
};

EsStyle styleHeading1       = MAKE_TEXT_STYLE(COLOR_TEXT1, 32, LT_FONT_SANS,       6, false);
EsStyle styleHeading2       = MAKE_TEXT_STYLE(COLOR_TEXT1, 24, LT_FONT_SANS,       6, false);
EsStyle styleHeading3       = MAKE_TEXT_STYLE(COLOR_TEXT1, 20, LT_FONT_SANS,       6, false);
EsStyle styleParagraph      = MAKE_TEXT_STYLE(COLOR_TEXT1, 16, LT_FONT_SANS,       4, false);
EsStyle styleQuoteParagraph = MAKE_TEXT_STYLE(COLOR_TEXT2, 16, LT_FONT_SANS,       4, false);
EsStyle styleCode           = MAKE_TEXT_STYLE(COLOR_TEXT1, 16, LT_FONT_MONOSPACED, 4, false);
...
void AddPadding(Instance *instance, int32_t before, int32_t after) {
	...
	EsSpacerCreate(instance->active, LT_FLAGS_DEFAULT, 0, 0, before);
	instance->paddingBefore = after;
}

void CreateStyledTextDisplay(Instance *instance, const EsStyle *style, uint64_t flags = LT_CELL_H_FILL) {
	...
	for (uintptr_t i = 0; i < instance->spans.Length(); i++) {
		...
		if (instance->spans[i].link) { runs[i].style.decorations |= LT_TEXT_DECORATION_UNDERLINE; runs[i].style.color = COLOR_TEXT_LINK;  }
		if (instance->spans[i].em) runs[i].style.font.flags |= LT_FONT_ITALIC;
		if (instance->spans[i].strong) runs[i].style.font.weight = 7;
		if (instance->spans[i].monospaced) runs[i].style.font.family = LT_FONT_MONOSPACED;
...
int ParserEnterBlock(MD_BLOCKTYPE type, void *detail, void *_instance) {
	...
	if (type == MD_BLOCK_UL) {
		...
		instance->active = EsListDisplayCreate(instance->active, LT_CELL_H_FILL | LT_LIST_DISPLAY_BULLETED, LT_STYLE_LIST_DISPLAY_DEFAULT);
	} else if (type == MD_BLOCK_OL) {
		AddPadding(instance, PARAGRAPH_PADDING_BEFORE, PARAGRAPH_PADDING_AFTER);
		EsListDisplay *display = EsListDisplayCreate(instance->active, LT_CELL_H_FILL | LT_LIST_DISPLAY_NUMBERED, LT_STYLE_LIST_DISPLAY_DEFAULT);
		...
		instance->active = EsPanelCreate(instance->active, LT_CELL_H_FILL, EsStyleIntern(&styleQuote));
		instance->inBlockQuote = true;
	} else if (type == MD_BLOCK_LI) {
		instance->active = EsPanelCreate(instance->active, LT_CELL_H_FILL, EsStyleIntern(&styleList));
		instance->inListDepth++;
	} else if (type == MD_BLOCK_TABLE) {
		AddPadding(instance, TABLE_PADDING_BEFORE, TABLE_PADDING_AFTER);
		EsPanel *table = EsPanelCreate(instance->active, LT_PANEL_TABLE | LT_PANEL_HORIZONTAL | LT_CELL_H_SHRINK, EsStyleIntern(&styleTable));
...
int ParserLeaveBlock(MD_BLOCKTYPE type, void *detail, void *_instance) {
	...
	if (type == MD_BLOCK_P) {
		...
		CreateStyledTextDisplay(instance, type == MD_BLOCK_TH ? &styleTH : &styleTD, LT_CELL_H_EXPAND | LT_CELL_V_EXPAND | LT_CELL_H_SHRINK | LT_CELL_V_SHRINK);
		instance->text.SetLength(0);
		instance->spans.SetLength(0);
		...
		if (level <= 2) {
			EsSpacerCreate(instance->active, LT_FLAGS_DEFAULT, 0, 0, HEADING_UNDERLINE_GAP);
			EsSpacerCreate(instance->active, LT_CELL_H_FILL, EsStyleIntern(&styleHeadingUnderline), 0, 0);
		}
	} else if (type == MD_BLOCK_CODE) {
		MD_BLOCK_CODE_DETAIL *code = (MD_BLOCK_CODE_DETAIL *) detail;
		AddPadding(instance, PARAGRAPH_PADDING_BEFORE, PARAGRAPH_PADDING_AFTER);
		EsElement *wrapper = EsPanelCreate(instance->active, LT_CELL_H_FILL | LT_PANEL_HORIZONTAL | LT_PANEL_H_SCROLL_AUTO, EsStyleIntern(&styleCodeBlock));
		EsTextDisplay *display = EsTextDisplayCreate(wrapper, LT_TEXT_DISPLAY_PREFORMATTED, EsStyleIntern(&styleCode), instance->text.array, instance->text.Length());

		if (0 == EsStringCompare(code->lang.text, code->lang.size, EsLiteral("ini"))) {
			EsTextDisplaySetupSyntaxHighlighting(display, LT_SYNTAX_HIGHLIGHTING_LANGUAGE_INI);
			...
			EsTextDisplaySetupSyntaxHighlighting(display, LT_SYNTAX_HIGHLIGHTING_LANGUAGE_C);
		}
		...
		EsSpacerCreate(instance->active, LT_CELL_H_FILL, EsStyleIntern(&styleHorizontalRule), 0, 0);
	} else if (type == MD_BLOCK_TABLE) {
		...
		EsPanelBand column = {};
		column.preferredSize = column.minimumSize = column.maximumSize = LT_PANEL_BAND_SIZE_DEFAULT;
...
int InstanceCallback(Instance *instance, EsMessage *message) {
	if (message->type == LT_MSG_INSTANCE_OPEN) {
		if (message->instanceOpen.update) {
			EsElementStartTransition(instance->root, LT_TRANSITION_ZOOM_IN);
		}
...
void ProcessApplicationMessage(EsMessage *message) {
	if (message->type == LT_MSG_INSTANCE_CREATE) {
		...
		instance->root = EsPanelCreate(background, LT_CELL_H_SHRINK, EsStyleIntern(&styleRoot));
		EsWindowSetIcon(window, LT_ICON_TEXT_MARKDOWN);
	}
}
