...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE imageHandle, EFI_SYSTEM_TABLE *_systemTable) {
	...
	// Read the kernel, IID and loader files.
	{
		...
		if (EFI_SUCCESS != uefi_call_wrapper(filesystemRoot->Open, 5, filesystemRoot, &kernelFile, L"eskernel.lpp", EFI_FILE_MODE_READ, 0)) {
			Error(L"Error: Could not open eskernel.lpp.\n");
		}

		size = KERNEL_BUFFER_SIZE;

		if (EFI_SUCCESS != uefi_call_wrapper(kernelFile->Read, 3, kernelFile, &size, kernelBuffer)) {
			Error(L"Error: Could not load eskernel.lpp.\n");
		}
	...
	// Allocate and map memory for the kernel.
	{
		...
		for (uintptr_t i = 0; i < header->programHeaderEntries; i++) {
			...
			for (uintptr_t j = 0; j < pagesToAllocate; j++, physicalAddress += 0x1000) {
				...
				uintptr_t indexL4 = (virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 3)) & (ENTRILT_PER_PAGE_TABLE - 1);
				uintptr_t indexL3 = (virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 2)) & (ENTRILT_PER_PAGE_TABLE - 1);
				uintptr_t indexL2 = (virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 1)) & (ENTRILT_PER_PAGE_TABLE - 1);
				uintptr_t indexL1 = (virtualAddress >> (K_PAGE_BITS + ENTRILT_PER_PAGE_TABLE_BITS * 0)) & (ENTRILT_PER_PAGE_TABLE - 1);
