...
bool AHCIController::Access(uintptr_t portIndex, uint64_t offsetBytes, size_t countBytes, int operation, 
	...
	while (!KDMABufferIsComplete(buffer)) {
		...
		prdt[0 + 4 * prdtEntryCount] = LT_PTR64_LS32(segment.physicalAddress);
		prdt[1 + 4 * prdtEntryCount] = LT_PTR64_MS32(segment.physicalAddress);
...
void AHCIController::Initialise() {
	...
	for (uintptr_t i = 0; i < MAX_PORTS; i++) {
		...
		WR_REGISTER_PCLB(i, LT_PTR64_LS32(physicalAddress)); 
		WR_REGISTER_PFB(i, LT_PTR64_LS32(physicalAddress + 0x400));
		if (dma64Supported) WR_REGISTER_PCLBU(i, LT_PTR64_MS32(physicalAddress));
		if (dma64Supported) WR_REGISTER_PFBU(i, LT_PTR64_MS32(physicalAddress + 0x400));
		...
		for (uintptr_t j = 0; j < commandSlotCount; j++) {
			...
			commandList[j * 8 + 2] = LT_PTR64_LS32(address);
			commandList[j * 8 + 3] = LT_PTR64_MS32(address);
		}
	...
	for (uintptr_t i = 0; i < MAX_PORTS; i++) {
		...
		prdt[0] = LT_PTR64_LS32(identifyDataPhysical);
		prdt[1] = LT_PTR64_MS32(identifyDataPhysical);
		prdt[2] = 0;
		prdt[3] = 0x200 - 1;
	...
	for (uintptr_t i = 0; i < MAX_PORTS; i++) {
		...
		device->information.driveType = ports[i].atapi ? LT_DRIVE_TYPE_CDROM : ports[i].ssd ? LT_DRIVE_TYPE_SSD : LT_DRIVE_TYPE_HDD;
