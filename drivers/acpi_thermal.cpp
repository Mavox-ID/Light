...
static void ACPIThermalRefreshTemperature(KAsyncTask *task) {
	...
	if (error == LT_SUCCESS) KernelLog(LOG_INFO, "ACPIThermal", "temperature", "Current temperature: %d K.\n", device->currentTemperature / 10);
...
static void ACPIThermalRefreshThresholds(KAsyncTask *task) {
	...
	if (error == LT_SUCCESS) KernelLog(LOG_INFO, "ACPIThermal", "threshold", "Critical temperature threshold: %d K.\n", device->criticalThreshold / 10);

	error = KACPIObjectEvaluateInteger(object, "_HOT", &device->hotThreshold);
	if (error == LT_SUCCESS) KernelLog(LOG_INFO, "ACPIThermal", "threshold", "Hot temperature threshold: %d K.\n", device->hotThreshold / 10);

	error = KACPIObjectEvaluateInteger(object, "_PSV", &device->passiveThreshold);
	if (error == LT_SUCCESS) KernelLog(LOG_INFO, "ACPIThermal", "threshold", "Passive temperature threshold: %d K.\n", device->passiveThreshold / 10);

	error = KACPIObjectEvaluateInteger(object, "_TZP", &device->passiveThreshold);
	if (error == LT_SUCCESS) KernelLog(LOG_INFO, "ACPIThermal", "threshold", "Recommended polling frequency: %d s.\n", device->pollingFrequency / 10);

	char name[5] = "_AC0";

	for (uintptr_t i = 0; i <= 9; i++, name[3]++) {
		EsError error = KACPIObjectEvaluateInteger(object, name, &device->activeThresholds[i]);
		if (error == LT_SUCCESS) KernelLog(LOG_INFO, "ACPIThermal", "threshold", "Active temperature threshold %d: %d K.\n", i, device->activeThresholds[i] / 10);
		else break;
	}
...
static void ACPIThermalDeviceAttach(KDevice *parent) {
	...
	if (error == LT_SUCCESS) KernelLog(LOG_INFO, "ACPIThermal", "notification handler", "Successfully installed notification handler.\n");
	...
	if (error == LT_SUCCESS) KernelLog(LOG_INFO, "ACPIThermal", "cooling policy", "Successfully set active cooling policy.\n");
