...
static bool HDAControllerSendCommand(HDAController *controller, uint32_t codec, uint32_t node, uint32_t data, uint32_t *response) {
	...
	WR_REGISTER_CORBWP(LT_ISOLATE_BITS(RD_REGISTER_CORBWP(), 15, 8) | corbWritePointer);
...
static void HDAControllerExploreFunctionGroup(HDAController *controller, uint32_t codec, uint32_t functionGroupNode) {
	...
	uint32_t firstChildNode = LT_EXTRACT_BITS(childNodeCount, 23, 16);
	childNodeCount = LT_EXTRACT_BITS(childNodeCount, 7, 0);
	type = LT_EXTRACT_BITS(type, 7, 0);
	...
	for (uintptr_t j = firstChildNode; j < firstChildNode + childNodeCount; j++) {
		...
		uint32_t widgetType = LT_EXTRACT_BITS(widgetCapabilities, 23, 20);
		...
		if (widgetCapabilities & (1 << 8)) {
			...
			if (HDAControllerSendCommand(controller, codec, widget->node, READ_PARAMETER_CONNECTION_LIST_LENGTH, &connectionListLength)
					...
					&& LT_EXTRACT_BITS(connectionListLength, 6, 0)) {
				uintptr_t index = 0;
	...
	for (uintptr_t i = 0; i < controller->children.Length(); i++) {
		...
		if (widget->type == HDA_WIDGET_PIN_COMPLEX) {
			...
			uint32_t portConnectivity = LT_EXTRACT_BITS(widget->pinConfiguration, 31, 30);
			uint32_t location = LT_EXTRACT_BITS(widget->pinConfiguration, 29, 24);
			uint32_t defaultDevice = LT_EXTRACT_BITS(widget->pinConfiguration, 23, 20);
			uint32_t connectionType = LT_EXTRACT_BITS(widget->pinConfiguration, 19, 16);
			uint32_t color = LT_EXTRACT_BITS(widget->pinConfiguration, 15, 12);
			...
			KernelLog(LOG_INFO, "HDA", "pin information", "Connectivity: %z; location: %z %z; default device: %z; connection type: %z; color: %z.\n", 
					...
					locationHighStrings[LT_EXTRACT_BITS(location, 5, 4)], locationLowStrings[LT_EXTRACT_BITS(location, 3, 0)],
			...
			KDeviceSendConnectedMessage(widget, LT_DEVICE_AUDIO);
		}
	}
}
...
static void HDAControllerAttach(KDevice *_parent) {
	...
	controller->outputStreamsSupported = LT_EXTRACT_BITS(globalCapabilities, 15, 12);
	controller->inputStreamsSupported = LT_EXTRACT_BITS(globalCapabilities, 11, 8);
	controller->bidirectionalStreamsSupported = LT_EXTRACT_BITS(globalCapabilities, 7, 3);
...
#define CHECK_TIMEOUT(message) \
	...
	WR_REGISTER_CORBSIZE(LT_ISOLATE_BITS(RD_REGISTER_CORBSIZE(), 7, 2) | (controller->corbEntries == 16 ? 1 : controller->corbEntries == 256 ? 2 : 0));
	WR_REGISTER_RIRBSIZE(LT_ISOLATE_BITS(RD_REGISTER_RIRBSIZE(), 7, 2) | (controller->rirbEntries == 16 ? 1 : controller->rirbEntries == 256 ? 2 : 0));
	...
	WR_REGISTER_RINTCNT(LT_ISOLATE_BITS(RD_REGISTER_RINTCNT(), 15, 8) | 1 /* interrupt after every response */);

	WR_REGISTER_CORBCTL(LT_ISOLATE_BITS(RD_REGISTER_CORBCTL(), 7, 2) | (1 << 1 /* run */) | (1 << 0 /* interrupt on error */));
	WR_REGISTER_RIRBCTL(LT_ISOLATE_BITS(RD_REGISTER_RIRBCTL(), 7, 3) | (1 << 1 /* run */) | (1 << 0 /* interrupt on response */));
