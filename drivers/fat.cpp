...
static EsError Load(KNode *_directory, KNode *_node, KNodeMetadata *, const void *entryData) {
	...
	if (!clusterBuffer) return LT_ERROR_INSUFFICIENT_RESOURCES;
	...
	if (!directory->rootDirectory) {
		EsError error = volume->Access((reference.cluster * superBlock->sectorsPerCluster + volume->sectorOffset) * SECTOR_SIZE, 
				superBlock->sectorsPerCluster * SECTOR_SIZE, K_ACCESS_READ, (uint8_t *) clusterBuffer, LT_FLAGS_DEFAULT);
...
#define READ_FAILURE(message, error) do { KernelLog(LOG_ERROR, "FAT", "read failure", "Read - " message); return error; } while (0)
	...
	while (count) {
		...
		EsError error = volume->Access((currentCluster * superBlock->sectorsPerCluster + volume->sectorOffset) * SECTOR_SIZE, 
				...
				(uint8_t *) clusterBuffer, LT_FLAGS_DEFAULT);
		if (error != LT_SUCCESS) READ_FAILURE("Could not read cluster.\n", error);
...
#define SCAN_FAILURE(message, error) do { KernelLog(LOG_ERROR, "FAT", "scan failure", "Scan - " message); return error; } while (0)
...

	{
		...
		while (i < nameLength) {
			if (j == 11) return LT_ERROR_FILE_DOLT_NOT_EXIST; // Name too long.
			uint8_t c = _name[i++];
	...
	while (currentCluster < volume->terminateCluster) {
		if (!directory->rootDirectory) {
			EsError error = volume->Access((currentCluster * superBlock->sectorsPerCluster + volume->sectorOffset) * SECTOR_SIZE, 
					superBlock->sectorsPerCluster * SECTOR_SIZE, K_ACCESS_READ, (uint8_t *) clusterBuffer, LT_FLAGS_DEFAULT);
			if (error != LT_SUCCESS) SCAN_FAILURE("Could not read cluster.\n", error);
		}
		...
		for (uintptr_t i = 0; i < superBlock->sectorsPerCluster * SECTOR_SIZE / sizeof(FATDirectoryEntry); i++, directoryPosition++) {
			...
			if (!entry->name[0]) return LT_ERROR_FILE_DOLT_NOT_EXIST;
...

			{
				...
				metadata.type = (entry->attributes & 0x10) ? LT_NODE_DIRECTORY : LT_NODE_FILE;

				if (metadata.type == LT_NODE_FILE) {
					metadata.totalSize = entry->fileSizeBytes;
				} else if (metadata.type == LT_NODE_DIRECTORY) {
...
#define ENUMERATE_FAILURE(message, error) do { KernelLog(LOG_ERROR, "FAT", "enumerate failure", "Enumerate - " message); return error; } while (0)
	...
	while (currentCluster < volume->terminateCluster) {
		if (!directory->rootDirectory) {
			EsError error = volume->Access((currentCluster * superBlock->sectorsPerCluster + volume->sectorOffset) * SECTOR_SIZE, 
					superBlock->sectorsPerCluster * SECTOR_SIZE, K_ACCESS_READ, (uint8_t *) clusterBuffer, LT_FLAGS_DEFAULT);
			if (error != LT_SUCCESS) ENUMERATE_FAILURE("Could not read cluster.\n", error);
		}
...
#define MOUNT_FAILURE(message) do { KernelLog(LOG_ERROR, "FAT", "mount failure", "Mount - " message); goto failure; } while (0)

	{
		EsError error;
		SuperBlockCommon *superBlock = &volume->superBlock;
		error = volume->Access(0, SECTOR_SIZE, K_ACCESS_READ, (uint8_t *) superBlock, LT_FLAGS_DEFAULT); 
		if (error != LT_SUCCESS) MOUNT_FAILURE("Could not read super block.\n");
		...
		error = volume->Access(superBlock->reservedSectors * SECTOR_SIZE, sectorsPerFAT * SECTOR_SIZE, K_ACCESS_READ, volume->fat, LT_FLAGS_DEFAULT);
		if (error != LT_SUCCESS) MOUNT_FAILURE("Could not read FAT.\n");
		...
		if (volume->type == TYPE_FAT32) {
			...
			error = volume->Access(rootDirectoryOffset * SECTOR_SIZE, rootDirectorySectors * SECTOR_SIZE, 
					K_ACCESS_READ, (uint8_t *) root->rootDirectory, LT_FLAGS_DEFAULT);
			if (error != LT_SUCCESS) MOUNT_FAILURE("Could not read root directory.\n");
