...
// TODO LIGHTFS_CHECK_XXX are used to report out of memory errors, which shouldn't report KERNEL_PROBLEM_DAMAGED_FILESYSTEM.

#define LIGHTFS_CHECK(x, y)                 if (!(x)) { KernelLog(LOG_ERROR, "LightFS", "damaged file system", y "\n"); return false; }
#define LIGHTFS_CHECK_ERROR(x, y)           if ((x) != LT_SUCCESS) { KernelLog(LOG_ERROR, "LightFS", "damaged file system", y "\n"); return x; }
#define LIGHTFS_CHECK_TO_ERROR(x, y, z)     if (!(x)) { KernelLog(LOG_ERROR, "LightFS", "damaged file system", y "\n"); return z; }
#define LIGHTFS_CHECK_VA(x, y, ...)         if (!(x)) { KernelLog(LOG_ERROR, "LightFS", "damaged file system", y "\n", __VA_ARGS__); return false; }
#define LIGHTFS_CHECK_RETURN(x, y)          if (!(x)) { KernelLog(LOG_ERROR, "LightFS", "damaged file system", y "\n"); return; }
#define LIGHTFS_CHECK_CORRUPT(x, y)         if (!(x)) { KernelLog(LOG_ERROR, "LightFS", "damaged file system", y "\n"); return LT_ERROR_CORRUPT_DATA; }
#define LIGHTFS_CHECK_FATAL(x, y)           if (!(x)) { KernelLog(LOG_ERROR, "LightFS", "damaged file system", "Mount - " y "\n"); return false; }
#define LIGHTFS_CHECK_READ_ONLY(x, y)       if (!(x)) { KernelLog(LOG_ERROR, "LightFS", "mount read only", "Mount - " y " Mounting as read only.\n"); volume->readOnly = true; }
#define LIGHTFS_CHECK_ERROR_READ_ONLY(x, y) if ((x) != LT_SUCCESS) { KernelLog(LOG_ERROR, "LightFS", "mount read only", "Mount - " y " Mounting as read only.\n"); volume->readOnly = true; }
...
static bool AccessBlock(Volume *volume, uint64_t index, uint64_t count, void *buffer, uint64_t flags, int driveAccess) {
	...
	LIGHTFS_CHECK(index < superblock->blockCount && count <= superblock->blockCount - index, "AccessBlock - Access past the end of the file system.");
	EsError error = volume->Access(index * superblock->blockSize, count * superblock->blockSize, driveAccess, buffer, flags, nullptr);
	LIGHTFS_CHECK_ERROR(error, "AccessBlock - Could not access blocks.");
...
static bool ValidateIndexVertex(Superblock *superblock, IndexVertex *vertex) {
	...
	LIGHTFS_CHECK(checksum == calculated, "ValidateIndexVertex - Invalid vertex checksum.");
	LIGHTFS_CHECK(0 == EsMemoryCompare(vertex->signature, LIGHTFS_INDEX_VERTEX_SIGNATURE, 4), "ValidateIndexVertex - Invalid vertex signature.");
	LIGHTFS_CHECK(vertex->offset + vertex->maxCount * sizeof(IndexKey) < superblock->blockSize, "ValidateIndexVertex - Keys do not fit in vertex.");
	LIGHTFS_CHECK(vertex->count <= vertex->maxCount, "ValidateIndexVertex - Too many keys in vertex.");
...
static EsError FindDirectoryEntryReferenceFromIndex(Volume *volume, uint8_t *buffer /* superblock->blockSize */, 
	...
	if (!rootBlock) return LT_ERROR_FILE_DOLT_NOT_EXIST; // No index - the directory is empty?
	...
	if (!AccessBlock(volume, rootBlock, 1, vertex, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ)) {
		return LT_ERROR_HARDWARE_FAILURE;
	}

	int depth = 0;

	while (true) {
		LIGHTFS_CHECK(depth++ < LIGHTFS_INDEX_MAX_DEPTH - 1, "FindDirectoryEntryReferenceFromIndex - Reached tree max depth.");

		if (!ValidateIndexVertex(superblock, vertex)) return LT_ERROR_CORRUPT_DATA;
		...
		for (int i = 0; i <= vertex->count; i++) {
			if (i == vertex->count || keys[i].value > nameHash) {
				if (keys[i].child) {
					...
					if (!AccessBlock(volume, keys[i].child, 1, vertex, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ)) return LT_ERROR_HARDWARE_FAILURE;
				...
				LIGHTFS_CHECK_CORRUPT(keys[i].data.block < superblock->blockCount 
...
static bool ValidateDirectoryEntry(Volume *volume, DirectoryEntry *entry) {
	...
	LIGHTFS_CHECK_VA(checksum == calculated, "ValidateDirectoryEntry - Invalid checksum (%x, calculated %x).", checksum, calculated);
	LIGHTFS_CHECK(0 == EsMemoryCompare(entry->signature, LIGHTFS_DIRECTORY_ENTRY_SIGNATURE, 8), "ValidateDirectoryEntry - Invalid signature.");
	LIGHTFS_CHECK(entry->attributeOffset < sizeof(DirectoryEntry) - sizeof(Attribute), "ValidateDirectoryEntry - Invalid attribute offset.");

	Attribute *attribute = (Attribute *) ((uint8_t *) entry + entry->attributeOffset);

	for (uintptr_t i = 0; i < entry->attributeCount; i++) {
		LIGHTFS_CHECK(attribute->size && attribute->type, "ValidateDirectoryEntry - Invalid attribute.");
		LIGHTFS_CHECK((uintptr_t) attribute <= (uintptr_t) entry + sizeof(DirectoryEntry), "ValidateDirectoryEntry - Too many attributes.");

		if (attribute->type == LIGHTFS_ATTRIBUTE_DATA) {
			AttributeData *data = (AttributeData *) attribute;

			if (data->indirection == LIGHTFS_INDIRECTION_DIRECT) {
				LIGHTFS_CHECK(data->dataOffset + data->count <= data->size, "ValidateDirectoryEntry - Data too long.");
				LIGHTFS_CHECK(data->count == entry->fileSize, "ValidateDirectoryEntry - Expected direct attribute to cover entire file.");
			} 
		} else if (attribute->type == LIGHTFS_ATTRIBUTE_DIRECTORY) {
			AttributeDirectory *directory = (AttributeDirectory *) attribute;
			LIGHTFS_CHECK(directory->indexRootBlock < volume->superblock.blockCount, "ValidateDirectoryEntry - Directory index root block outside volume.");
		} else if (attribute->type == LIGHTFS_ATTRIBUTE_FILENAME) {
			AttributeFilename *filename = (AttributeFilename *) attribute;
			LIGHTFS_CHECK(filename->length + 8 <= filename->size, "ValidateDirectoryEntry - Filename too long.");
		} else {
...
static Attribute *FindAttribute(DirectoryEntry *entry, uint16_t type) {
	...
	while (attribute->type != type) {
		if (!attribute->size) {
			KernelLog(LOG_ERROR, "LightFS", "damaged file system", "FindAttribute - Attribute size was 0.\n"); 
			return nullptr;
		}
...
static bool ReadWrite(FSNode *file, uint64_t offset, uint64_t count, uint8_t *buffer, bool needBlockBuffer, bool write, 
	...
	if (file->type == LT_NODE_DIRECTORY) {
	...
	LIGHTFS_CHECK(!needBlockBuffer || blockBuffer, "Read - Could not allocate block buffer.");
	...
	AttributeData *data = (AttributeData *) FindAttribute(entry, LIGHTFS_ATTRIBUTE_DATA);
	LIGHTFS_CHECK(data, "Read - Expected data attribute.");
	...
	if (data->indirection == LIGHTFS_INDIRECTION_DIRECT) {
	...
	} else if (data->indirection == LIGHTFS_INDIRECTION_L1) {
		...
		while (count) {
			...
			while (!extentStart) {
				if (extentIndex == data->count) {
					LIGHTFS_CHECK(false, "Read - Invalid extent.");
				}
			...
			if (offsetIntoCurrentBlock || count < superblock->blockSize) {
				if (!needBlockBuffer) {
					KernelPanic("LightFS::Read - Need a block buffer, but needBlockBuffer was false.\n");
				}
		...
		LIGHTFS_CHECK(data, "Read - Unrecognised indirection mode.");
		return false;
	}
...
static size_t Read(KNode *node, void *_buffer, EsFileOffset offset, EsFileOffset count) {
	...
	if (file->corrupt) return LT_ERROR_CORRUPT_DATA;
	return ReadWrite(file, offset, count, (uint8_t *) _buffer, true, false) ? count : LT_ERROR_UNKNOWN;
}

static size_t Write(KNode *node, const void *_buffer, EsFileOffset offset, EsFileOffset count) {
	FSNode *file = (FSNode *) node->driverNode;
	if (file->corrupt) return LT_ERROR_CORRUPT_DATA;
	return ReadWrite(file, offset, count, (uint8_t *) _buffer, true, true) ? count : LT_ERROR_UNKNOWN;
}

static void Sync(KNode *_directory, KNode *node) {
	(void) _directory;
	FSNode *file = (FSNode *) node->driverNode;
	if (!file) KernelPanic("LightFS::Sync - Node %x has null driver node.\n", node);
	...
	if (file->type == LT_NODE_DIRECTORY) {
		...
		AttributeDirectory *directoryAttribute = (AttributeDirectory *) FindAttribute(&file->entry, LIGHTFS_ATTRIBUTE_DIRECTORY);
		directoryAttribute->totalSize = FSNodeGetTotalSize(node);
	} else if (file->type == LT_NODE_FILE) {
	...
	EsDefer(EsHeapFree(blockBuffer, 0, K_FIXED));
	LIGHTFS_CHECK_RETURN(blockBuffer, "Sync - Could not allocate block buffer.");

	if (!AccessBlock(volume, file->reference.block, 1, blockBuffer, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ)) {
		KernelLog(LOG_ERROR, "LightFS", "drive access failure", "Sync - Could not read reference block.\n");
		return;
	}
	...
	if (!AccessBlock(volume, file->reference.block, 1, blockBuffer, FS_BLOCK_ACCESS_CACHED, K_ACCESS_WRITE)) {
		KernelLog(LOG_ERROR, "LightFS", "drive access failure", "Sync - Could not write reference block.\n");
		return;
	}
}
...
static EsError Enumerate(KNode *node) {
	...
	uint8_t *blockBuffer = (uint8_t *) EsHeapAllocate(superblock->blockSize, false, K_FIXED);
	if (!blockBuffer) return LT_ERROR_INSUFFICIENT_RESOURCES;
	EsDefer(EsHeapFree(blockBuffer, 0, K_FIXED));

	DirectoryEntryReference reference = {};
	AttributeDirectory *directory = (AttributeDirectory *) FindAttribute(entry, LIGHTFS_ATTRIBUTE_DIRECTORY);
	...
	for (uint64_t i = 0; i < blocksInDirectory; i++) {
		...
		for (uint64_t j = 0; j < entriesInThisBlock; j++, reference.offsetIntoBlock += sizeof(DirectoryEntry)) {
			...
			AttributeFilename *filename = (AttributeFilename *) FindAttribute(entry, LIGHTFS_ATTRIBUTE_FILENAME);
			if (!filename) continue;

			KNodeMetadata metadata = {};

			metadata.type = entry->nodeType == LIGHTFS_NODE_TYPE_DIRECTORY ? LT_NODE_DIRECTORY : LT_NODE_FILE;

			if (metadata.type == LT_NODE_DIRECTORY) {
				AttributeDirectory *directory = (AttributeDirectory *) FindAttribute(entry, LIGHTFS_ATTRIBUTE_DIRECTORY);
			...
			} else if (metadata.type == LT_NODE_FILE) {
...
static bool ValidateGroupDescriptor(GroupDescriptor *descriptor) {
	...
	LIGHTFS_CHECK(checksum == calculated, "ValidateGroupDescriptor - Invalid checksum.");
	LIGHTFS_CHECK(0 == EsMemoryCompare(descriptor->signature, LIGHTFS_GROUP_DESCRIPTOR_SIGNATURE, 4), "ValidateGroupDescriptor - Invalid signature.");
	return true;
}
...
static bool ValidateBlockBitmap(GroupDescriptor *descriptor, uint8_t *bitmap, Superblock *superblock) {
	...
	LIGHTFS_CHECK(calculated == descriptor->bitmapChecksum, "ValidateBlockBitmap - Invalid checksum.");
	...
	LIGHTFS_CHECK(blocksUsed == descriptor->blocksUsed, "ValidateBlockBitmap - Used block count mismatch.");
...
static bool AllocateExtent(Volume *volume, uint64_t nearby, uint64_t increaseBlocks, uint64_t *extentStart, uint64_t *extentCount, bool zero) {
	...
	if (zero) {
		...
		LIGHTFS_CHECK(zeroBuffer, "AllocateExtent - Could not allocate buffer for zeroing extent.");
	}
	...
	LIGHTFS_CHECK(ValidateGroupDescriptor(target), "AllocateExtent - Invalid group descriptor.");
	...
	LIGHTFS_CHECK(bitmap, "AllocateExtent - Could not allocate buffer for block bitmap.");

	{
		if (target->blockBitmap) {
			...
			LIGHTFS_CHECK(ValidateBlockBitmap(target, bitmap, superblock), "AllocateExtent - Invalid block bitmap.");
		} else {
	...
	*extentStart += (target - volume->groupDescriptorTable) * superblock->blocksPerGroup;
	...
	if (zero) {
		...
		for (uint64_t i = 0; i < *extentCount * superblock->blockSize; i += zeroBufferSize) {
			...
			if (!AccessBlock(volume, *extentStart + i / superblock->blockSize, count / superblock->blockSize, zeroBuffer, LT_FLAGS_DEFAULT, K_ACCESS_WRITE)) {
				return false;
			}
		}
	}
...
static bool FreeExtent(Volume *volume, uint64_t extentStart, uint64_t extentCount) {
	...
	LIGHTFS_CHECK((extentStart + extentCount - 1) / superblock->blocksPerGroup == blockGroup, "FreeExtent - Extent spans multiple block groups.");
	LIGHTFS_CHECK(extentCount < superblock->blocksUsed, "FreeExtent - Extent is larged than the number of used blocks.");
	LIGHTFS_CHECK(extentStart + extentCount < superblock->blockCount, "FreeExtent - Extent goes past end of the volume.");

	// Load the block bitmap.

	GroupDescriptor *target = volume->groupDescriptorTable + blockGroup;
	LIGHTFS_CHECK(ValidateGroupDescriptor(target), "FreeExtent - Invalid group descriptor.");
	uint8_t *bitmap = (uint8_t *) EsHeapAllocate(superblock->blocksPerGroupBlockBitmap * superblock->blockSize, false, K_FIXED);
	EsDefer(EsHeapFree(bitmap, 0, K_FIXED));
	LIGHTFS_CHECK(bitmap, "FreeExtent - Could not allocate buffer for block bitmap.");
	LIGHTFS_CHECK(target->blockBitmap, "FreeExtent - Group descriptor does not have block bitmap.");
	LIGHTFS_CHECK(target->blocksUsed >= extentCount, "FreeExtent - Group descriptor indicates fewer blocks are used than are given in this extent.");
	LIGHTFS_CHECK(AccessBlock(volume, target->blockBitmap, superblock->blocksPerGroupBlockBitmap, bitmap, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ), "FreeExtent - Could not read block bitmap.");
	LIGHTFS_CHECK(ValidateBlockBitmap(target, bitmap, superblock), "FreeExtent - Invalid block bitmap.");
	...
	for (uint64_t i = 0; i < extentCount; i++) {
		...
		LIGHTFS_CHECK(bit, "FreeExtent - Attempting to free a block that has not been allocated.");
...
static uint64_t ResizeInternal(FSNode *file, uint64_t newSize, EsError *error, uint64_t newDataAttributeSize = 0) {
	if (file->corrupt) return *error = LT_ERROR_CORRUPT_DATA, 0;
	...
	if (newDataAttributeSize && newSize != oldSize) {
		KernelPanic("LightFS::ResizeInternal - Attempting to change data attribute and node size at the same time.\n");
	}
	...
	*error = LT_ERROR_INSUFFICIENT_RESOURCES;
	LIGHTFS_CHECK(blockBuffer, "Resize - Could not allocate block buffer.");
	...
	if (newDataBufferSize >= newSize && entry->nodeType != LIGHTFS_NODE_TYPE_DIRECTORY) {
		if (data->indirection == LIGHTFS_INDIRECTION_DIRECT) {
		} else if (data->indirection == LIGHTFS_INDIRECTION_L1) {
			...
			if (entry->fileSize) {
				if (!ReadWrite(file, 0, superblock->blockSize, blockBuffer, false, false)) {
					return *error = LT_ERROR_HARDWARE_FAILURE, entry->fileSize;
				}
			}
			...
			for (uintptr_t i = 0; i < data->count; i++) {
				...
				if (!FreeExtent(volume, previousExtentStart, extentCount)) {
					file->corrupt = true;
					entry->fileSize = 0;
					*error = LT_ERROR_HARDWARE_FAILURE;
					return (entry->fileSize = 0);
				}
			}
			...
			*error = LT_ERROR_UNSUPPORTED;
			return entry->fileSize; // Unrecognised indirection.
		}

		data->indirection = LIGHTFS_INDIRECTION_DIRECT;
		data->count = newSize;
		...
		data->indirection = LIGHTFS_INDIRECTION_L1;
...
static IndexKey *InsertKeyIntoVertex(uint64_t newKey, IndexVertex *vertex) {
	...
	for (position = 0; position < vertex->count; position++) {
		if (newKey < LIGHTFS_VERTEX_KEY(vertex, position)->value) {
			break;
		}
	}
	...
	IndexKey *insertionPosition = LIGHTFS_VERTEX_KEY(vertex, position);
	EsMemoryMove(insertionPosition, LIGHTFS_VERTEX_KEY(vertex, vertex->count + 1), sizeof(IndexKey), true);
	vertex->count++;
...
static bool IndexModifyKey(Volume *volume, uint64_t newKey, DirectoryEntryReference reference, uint64_t rootBlock, uint8_t *buffer /* superblock->blockSize */) {
	...
	while (true) {
		LIGHTFS_CHECK(depth++ < LIGHTFS_INDEX_MAX_DEPTH - 1, "IndexModifyKey - Reached tree max depth.");
...
static bool IndexAddKey(Volume *volume, uint64_t newKey, DirectoryEntryReference reference, uint64_t *rootBlock) {
	...
	LIGHTFS_CHECK(blockBuffers, "IndexAddKey - Could not allocate block buffers.");
	...
	IndexVertex *vertex = (IndexVertex *) buffer;
	uint64_t depth = 0, blocks[LIGHTFS_INDEX_MAX_DEPTH] = { *rootBlock };
	...
	if (blocks[0] == 0) {
		...
		KWriterLockTake(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE);
		LIGHTFS_CHECK(AllocateExtent(volume, 0 /* TODO */, 1, &blocks[0], &_unused, false), "IndexAddKey - Could not allocate space.");
		KWriterLockReturn(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE);
		*rootBlock = blocks[0];
		vertex->maxCount = (superblock->blockSize - LIGHTFS_INDEX_KEY_OFFSET) / sizeof(IndexKey) - 1 /* +1 key */;
		vertex->offset = LIGHTFS_INDEX_KEY_OFFSET;
		EsMemoryCopy(vertex->signature, LIGHTFS_INDEX_VERTEX_SIGNATURE, 4);
		skipFirst = true;
	}

	{
		...
		if (!skipFirst) {
			LIGHTFS_CHECK(AccessBlock(volume, blocks[depth], 1, vertex, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ), "IndexAddKey - Could not read index.");
		...
		for (int i = 0; i < vertex->count; i++) {
			if (LIGHTFS_VERTEX_KEY(vertex, i)->value == newKey) {
				// The key is already in the tree.
				// TODO Hash collisions.
				LIGHTFS_CHECK(false, "IndexAddKey - Possible hash collision?");
			}
		}

		for (int i = 0; i <= vertex->count; i++) {
			IndexKey *key = LIGHTFS_VERTEX_KEY(vertex, i);

			if ((i == vertex->count || newKey < key->value) && key->child) {
				LIGHTFS_CHECK(depth < LIGHTFS_INDEX_MAX_DEPTH - 1, "IndexAddKey - Reached tree max depth.");
	...
	LIGHTFS_CHECK(vertex->count < vertex->maxCount, "IndexAddKey - Vertex is full; unbalanced tree.");
	...
	while (vertex->count == vertex->maxCount) {
...

		{
			KWriterLockTake(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE);
			EsDefer(KWriterLockReturn(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE));
			LIGHTFS_CHECK(AllocateExtent(volume, blocks[depth], 1, &siblingBlock, &_unused, false), "IndexAddKey - Could not allocate space.");
		}

		IndexVertex *sibling = (IndexVertex *) _buffer0;
		sibling->maxCount = (superblock->blockSize - LIGHTFS_INDEX_KEY_OFFSET) / sizeof(IndexKey) - 1 /* +1 key */;
		sibling->offset = LIGHTFS_INDEX_KEY_OFFSET;
		EsMemoryCopy(sibling->signature, LIGHTFS_INDEX_VERTEX_SIGNATURE, 4);
		...
		if (newRoot) {
...

			{
				KWriterLockTake(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE);
				EsDefer(KWriterLockReturn(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE));
				LIGHTFS_CHECK(AllocateExtent(volume, blocks[1], 1, &blocks[0], &_unused, false), "IndexAddKey - Could not allocate space.");
			}

			parent->maxCount = (superblock->blockSize - LIGHTFS_INDEX_KEY_OFFSET) / sizeof(IndexKey) - 1 /* +1 key */;
			parent->offset = LIGHTFS_INDEX_KEY_OFFSET;
			EsMemoryCopy(parent->signature, LIGHTFS_INDEX_VERTEX_SIGNATURE, 4);
			...
			*rootBlock = blocks[0];
			...
			LIGHTFS_CHECK(AccessBlock(volume, blocks[depth - 1], 1, parent, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ), "IndexAddKey - Could not read index.");
		}
		...
		LIGHTFS_CHECK(found == 1, "IndexAddKey - Could not find current vertex in its parent.");
		...
		vertex->checksum = 0; vertex->checksum = CalculateCRC32(vertex, superblock->blockSize, 0);
		LIGHTFS_CHECK(AccessBlock(volume, siblingBlock, 1, sibling, FS_BLOCK_ACCESS_CACHED, K_ACCESS_WRITE), "IndexAddKey - Could not update index.");
		LIGHTFS_CHECK(AccessBlock(volume, blocks[depth], 1, vertex, FS_BLOCK_ACCESS_CACHED, K_ACCESS_WRITE), "IndexAddKey - Could not update index.");
...
static bool IndexRemoveKey(Volume *volume, uint64_t removeKey, uint64_t *rootBlock) {
	...
	uint8_t *blockBuffers = (uint8_t *) EsHeapAllocate(superblock->blockSize * 3, true, K_FIXED);
	EsDefer(EsHeapFree(blockBuffers, 0, K_FIXED));
	LIGHTFS_CHECK(blockBuffers, "IndexRemoveKey - Could not allocate block buffers.");
	...
	IndexVertex *vertex = (IndexVertex *) buffer;
	uint64_t depth = 0, blocks[LIGHTFS_INDEX_MAX_DEPTH] = { *rootBlock };
	LIGHTFS_CHECK(blocks[0], "IndexRemoveKey - Index has no root.");
	int position = 0;

	{
		...
		LIGHTFS_CHECK(AccessBlock(volume, blocks[depth], 1, vertex, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ), "IndexRemoveKey - Could not read index.");
		if (!ValidateIndexVertex(superblock, vertex)) return false;
		...
		for (i = 0; i <= vertex->count; i++) {
			...
			if (i != vertex->count && key->value == removeKey) {
				...
				LIGHTFS_CHECK(depth < LIGHTFS_INDEX_MAX_DEPTH - 1, "IndexRemoveKey - Reached tree max depth.");
				blocks[++depth] = key->child;
		...
		LIGHTFS_CHECK(false, "IndexRemoveKey - The key was not in the tree.");
		done:;
		position = i;
	}

	if (LIGHTFS_VERTEX_KEY(vertex, position)->child) {
		...
		blocks[++depth] = LIGHTFS_VERTEX_KEY(vertex, position + 1)->child;

		while (true) {
			LIGHTFS_CHECK(AccessBlock(volume, blocks[depth], 1, search, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ), "IndexRemoveKey - Could not read index.");
			if (!ValidateIndexVertex(superblock, search)) return false;

			if (LIGHTFS_VERTEX_KEY(search, 0)->child) {
				LIGHTFS_CHECK(depth < LIGHTFS_INDEX_MAX_DEPTH - 1, "IndexRemoveKey - Reached tree max depth.");
				blocks[++depth] = LIGHTFS_VERTEX_KEY(vertex, 1)->child;
			} else break;
		}

		LIGHTFS_VERTEX_KEY(vertex, position)->value = LIGHTFS_VERTEX_KEY(search, 0)->value;
		LIGHTFS_VERTEX_KEY(vertex, position)->data  = LIGHTFS_VERTEX_KEY(search, 0)->data;

		vertex->checksum = 0; vertex->checksum = CalculateCRC32(vertex, superblock->blockSize, 0);
		LIGHTFS_CHECK(AccessBlock(volume, blocks[startDepth], 1, vertex, FS_BLOCK_ACCESS_CACHED, K_ACCESS_WRITE), "IndexRemoveKey - Could not write index.");
	...
	EsMemoryMove(LIGHTFS_VERTEX_KEY(vertex, position + 1), LIGHTFS_VERTEX_KEY(vertex, vertex->count + 1), LT_MEMORY_MOVE_BACKWARDS sizeof(IndexKey), true);
	vertex->count--;
	...
	if (vertex->count >= (vertex->maxCount - 1) / 2) {
		...
		vertex->checksum = 0; vertex->checksum = CalculateCRC32(vertex, superblock->blockSize, 0);
		LIGHTFS_CHECK(AccessBlock(volume, blocks[depth], 1, vertex, FS_BLOCK_ACCESS_CACHED, K_ACCESS_WRITE), "IndexRemoveKey - Could not write index.");
		return true;
	}
	...
	if (depth == 0) {
		if (!vertex->count && vertex->keys[0].child) {
			...
			KWriterLockTake(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE);
			EsDefer(KWriterLockReturn(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE));
			LIGHTFS_CHECK(FreeExtent(volume, blocks[0], 1), "IndexRemoveKey - Could not free the old root index block.");
	...
	LIGHTFS_CHECK(AccessBlock(volume, blocks[depth - 1], 1, parent, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ), "IndexRemoveKey - Could not read index.");
	if (!ValidateIndexVertex(superblock, parent)) return false;

	int positionInParent = -1;

	for (int i = 0; i <= parent->count; i++) {
		if (LIGHTFS_VERTEX_KEY(parent, i)->child == blocks[depth]) {
			positionInParent = i;
			break;
		}
	}

	LIGHTFS_CHECK(positionInParent != -1, "IndexRemoveKey - Could not find position in parent.");

	uint8_t *buffer3 = blockBuffers + 2 * superblock->blockSize;
	IndexVertex *sibling = (IndexVertex *) buffer3;

	if (positionInParent) {
		LIGHTFS_CHECK(AccessBlock(volume, LIGHTFS_VERTEX_KEY(parent, positionInParent - 1)->child, 1, sibling, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ), "IndexRemoveKey - Could not read index.");

		if (sibling->count > (sibling->maxCount - 1) / 2) {
			// Steal left.

			EsMemoryMove(LIGHTFS_VERTEX_KEY(vertex, 0), LIGHTFS_VERTEX_KEY(vertex, vertex->count + 1), sizeof(IndexKey), true);

			LIGHTFS_VERTEX_KEY(vertex, 0)->			value = LIGHTFS_VERTEX_KEY(parent, 	positionInParent - 1)->	value;
			LIGHTFS_VERTEX_KEY(vertex, 0)->			data = 	LIGHTFS_VERTEX_KEY(parent, 	positionInParent - 1)->	data;
			LIGHTFS_VERTEX_KEY(vertex, 0)->			child = LIGHTFS_VERTEX_KEY(sibling, 	sibling->count)->	child;
			LIGHTFS_VERTEX_KEY(parent, positionInParent - 1)->	value = LIGHTFS_VERTEX_KEY(sibling, 	sibling->count - 1)->	value;
			LIGHTFS_VERTEX_KEY(parent, positionInParent - 1)->	data = 	LIGHTFS_VERTEX_KEY(sibling, 	sibling->count - 1)->	data;
			...
			LIGHTFS_CHECK(AccessBlock(volume, LIGHTFS_VERTEX_KEY(parent, positionInParent - 1)->child, 1, sibling, FS_BLOCK_ACCESS_CACHED, K_ACCESS_WRITE), "IndexRemoveKey - Could not write index.");
			LIGHTFS_CHECK(AccessBlock(volume, blocks[depth], 1, vertex, FS_BLOCK_ACCESS_CACHED, K_ACCESS_WRITE), "IndexRemoveKey - Could not write index.");
			LIGHTFS_CHECK(AccessBlock(volume, blocks[depth - 1], 1, parent, FS_BLOCK_ACCESS_CACHED, K_ACCESS_WRITE), "IndexRemoveKey - Could not write index.");
	...
	if (positionInParent != parent->count) {
		...
		if (sibling->count > (sibling->maxCount - 1) / 2) {
			...
			LIGHTFS_VERTEX_KEY(vertex, vertex->count)->	value = LIGHTFS_VERTEX_KEY(parent, 	positionInParent)->	value;
			LIGHTFS_VERTEX_KEY(vertex, vertex->count)->	data = 	LIGHTFS_VERTEX_KEY(parent, 	positionInParent)->	data;
			LIGHTFS_VERTEX_KEY(vertex, vertex->count + 1)->	child = LIGHTFS_VERTEX_KEY(sibling, 	0)->			child;
			LIGHTFS_VERTEX_KEY(parent, positionInParent)->	value = LIGHTFS_VERTEX_KEY(sibling, 	0)->			value;
			LIGHTFS_VERTEX_KEY(parent, positionInParent)->	data = 	LIGHTFS_VERTEX_KEY(sibling, 	0)->			data;

			EsMemoryMove(LIGHTFS_VERTEX_KEY(sibling, 1), LIGHTFS_VERTEX_KEY(sibling, sibling->count + 1), LT_MEMORY_MOVE_BACKWARDS sizeof(IndexKey), true);
...

	{
		...
		if (!positionInParent) {
			...
			positionInParent = 1;
			blocks[depth] = LIGHTFS_VERTEX_KEY(parent, positionInParent)->child;
			LIGHTFS_CHECK(AccessBlock(volume, blocks[depth], 1, vertex, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ), "IndexRemoveKey - Could not read index.");
			if (!ValidateIndexVertex(superblock, vertex)) return false;
		} else {
			LIGHTFS_CHECK(AccessBlock(volume, LIGHTFS_VERTEX_KEY(parent, positionInParent - 1)->child, 1, sibling, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ), "IndexRemoveKey - Could not read index.");
		...
		LIGHTFS_CHECK(sibling->count + vertex->count + 1 < sibling->maxCount, "IndexRemoveKey - Merged node would be too large.");

		LIGHTFS_VERTEX_KEY(sibling, sibling->count)->	value = LIGHTFS_VERTEX_KEY(parent, positionInParent - 1)->value;
		LIGHTFS_VERTEX_KEY(sibling, sibling->count)->	data = 	LIGHTFS_VERTEX_KEY(parent, positionInParent - 1)->data;
		LIGHTFS_VERTEX_KEY(parent,  positionInParent)->	child = LIGHTFS_VERTEX_KEY(parent, positionInParent - 1)->child;

		EsMemoryCopy(LIGHTFS_VERTEX_KEY(sibling, sibling->count + 1), LIGHTFS_VERTEX_KEY(vertex, 0), (vertex->count + 1) * sizeof(IndexKey));
		EsMemoryMove(LIGHTFS_VERTEX_KEY(parent, positionInParent), LIGHTFS_VERTEX_KEY(parent, parent->count + 1), LT_MEMORY_MOVE_BACKWARDS sizeof(IndexKey), true);
...

		{
			KWriterLockTake(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE);
			EsDefer(KWriterLockReturn(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE));
			LIGHTFS_CHECK(FreeExtent(volume, blocks[depth], 1), "IndexRemoveKey - Could not free merged vertex.");
		}
...
static EsError RemoveDirectoryEntry(FSNode *file, uint8_t *blockBuffers /* superblock->blockSize * 2 */, FSNode *directory, KNode *_directory) {
	...
	Superblock *superblock = &volume->superblock;
	DirectoryEntry *entry = &file->entry;
	AttributeDirectory *directoryAttribute = (AttributeDirectory *) FindAttribute(&directory->entry, LIGHTFS_ATTRIBUTE_DIRECTORY);
	EsError error;
	...
	LIGHTFS_CHECK_TO_ERROR(AccessBlock(volume, file->reference.block, 1, blockBuffers, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ), 
			"Remove - Could not load the container block.", LT_ERROR_HARDWARE_FAILURE);
	LIGHTFS_CHECK_TO_ERROR(ReadWrite(directory, positionOfLastEntry & ~(superblock->blockSize - 1), superblock->blockSize, 
				blockBuffers + superblock->blockSize, false, false), "Remove - Could not load the last block.", LT_ERROR_HARDWARE_FAILURE);
	LIGHTFS_CHECK_TO_ERROR(0 == EsMemoryCompare(blockBuffers + file->reference.offsetIntoBlock, &file->entry, sizeof(DirectoryEntry)), 
			"Remove - Inconsistent file entry.", LT_ERROR_HARDWARE_FAILURE);
	...
	LIGHTFS_CHECK_TO_ERROR(AccessBlock(volume, file->reference.block, 1, blockBuffers, FS_BLOCK_ACCESS_CACHED, K_ACCESS_WRITE), 
			"Remove - Could not save the container block.", LT_ERROR_HARDWARE_FAILURE);
	...
	if (EsMemoryCompare(movedEntry->identifier.d, entry->identifier.d, sizeof(EsUniqueIdentifier))) {
		AttributeFilename *filename = (AttributeFilename *) FindAttribute(movedEntry, LIGHTFS_ATTRIBUTE_FILENAME);
		...
		if (filename) {
			...
			LIGHTFS_CHECK_TO_ERROR(IndexModifyKey(volume, key, file->reference, directoryAttribute->indexRootBlock, blockBuffers + superblock->blockSize), 
					"Remove - Could not update index (2).", LT_ERROR_HARDWARE_FAILURE);
		}
	}
	...
	if (!(directoryAttribute->childNodes % superblock->directoryEntriesPerBlock)) {
		...
		LIGHTFS_CHECK_TO_ERROR(newSize == ResizeInternal(directory, newSize, &error), "Remove - Could not resize directory.", LT_ERROR_HARDWARE_FAILURE);
	}
	...
	if (filename) {
		...
		LIGHTFS_CHECK_TO_ERROR(IndexRemoveKey(volume, removeKey, &directoryAttribute->indexRootBlock), "Remove - Could not update index.", LT_ERROR_HARDWARE_FAILURE);
	}
...
static EsError Remove(KNode *_directory, KNode *node) {
	...
	DirectoryEntry *entry = &file->entry;
	AttributeDirectory *directoryAttribute = (AttributeDirectory *) FindAttribute(&directory->entry, LIGHTFS_ATTRIBUTE_DIRECTORY);
	LIGHTFS_CHECK_TO_ERROR(directoryAttribute->childNodes, "Remove - Directory is empty.", LT_ERROR_CORRUPT_DATA);

	uint8_t *blockBuffers = (uint8_t *) EsHeapAllocate(superblock->blockSize * 2, false, K_FIXED);
	if (!blockBuffers) return LT_ERROR_INSUFFICIENT_RESOURCES;
	EsDefer(EsHeapFree(blockBuffers, 0, K_FIXED));

	// Step 1: If we're deleting a directory, deallocate its empty index.

	if (entry->nodeType == LIGHTFS_NODE_TYPE_DIRECTORY) {
		AttributeDirectory *attribute = (AttributeDirectory *) FindAttribute(entry, LIGHTFS_ATTRIBUTE_DIRECTORY);
		LIGHTFS_CHECK_TO_ERROR(!attribute->childNodes, "Remove - Directory was not empty.", LT_ERROR_CORRUPT_DATA);
		
		if (attribute->indexRootBlock) {
			IndexVertex *vertex = (IndexVertex *) blockBuffers;
			LIGHTFS_CHECK_TO_ERROR(AccessBlock(volume, attribute->indexRootBlock, 1, vertex, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ), 
					"Remove - Could not access index root.", LT_ERROR_HARDWARE_FAILURE);
			LIGHTFS_CHECK_TO_ERROR(!vertex->count, "Remove - Index was not empty (although it should be as the directory is empty).", LT_ERROR_CORRUPT_DATA);
			KWriterLockTake(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE);
			EsDefer(KWriterLockReturn(&volume->blockBitmapLock, K_LOCK_EXCLUSIVE));
			LIGHTFS_CHECK_TO_ERROR(FreeExtent(volume, attribute->indexRootBlock, 1), 
					"Remove - Could not free the root index block.", LT_ERROR_UNKNOWN);
		}
	}
	...
	if (ResizeInternal(file, 0, &error)) {
		LIGHTFS_CHECK_TO_ERROR(false, "Remove - Could not resize node.", error);
	}
...
static bool RenameInternal(FSNode *existingNode, DirectoryEntry *entry, const void *name, size_t nameLength) {
	...
	size_t newFilenameSize = ((nameLength + LIGHTFS_FILENAME_HEADER_SIZE - 1) & ~7) + 8; 
	...
	if (filename && data && (uint8_t *) filename - (uint8_t *) data == data->size) {
		...
		EsError error;
		LIGHTFS_CHECK(existingNode->entry.fileSize == ResizeInternal(existingNode, existingNode->entry.fileSize, &error, newDataSize), 
				"CreateInternal - Could not resize data attribute.");
		LIGHTFS_CHECK(error == LT_SUCCESS, "CreateInternal - Could not resize data attribute.");

		AttributeFilename *filename = (AttributeFilename *) ((uint8_t *) data + data->size);
		filename->type = LIGHTFS_ATTRIBUTE_FILENAME;
		filename->size = newFilenameSize;
		filename->length = nameLength;
		EsMemoryCopy(filename->filename, name, filename->length);
	} else {
		LIGHTFS_CHECK(false, "CreateInternal - Could not rename node.");
	}
...
static bool CreateInternal(const char *name, size_t nameLength, EsNodeType type, FSNode *parent, uint8_t *buffer /* superblock->blockSize */, 
	...
	if (type != LT_NODE_DIRECTORY && type != LT_NODE_FILE) {
		return false;
	}

	AttributeDirectory *directoryAttribute = (AttributeDirectory *) FindAttribute(&parent->entry, LIGHTFS_ATTRIBUTE_DIRECTORY);
	...
	if (!(directoryAttribute->childNodes % superblock->directoryEntriesPerBlock)) {
		...
		LIGHTFS_CHECK(newSize == ResizeInternal(parent, newSize, &error), "Create - Could not resize directory.");
	}

	DirectoryEntry *entry = nullptr;
	size_t newFilenameSize = ((nameLength + LIGHTFS_FILENAME_HEADER_SIZE - 1) & ~7) + 8; // Size of name + size of header, rounded up to the nearest 8 bytes.
	...
	if (!existingNode) {
		...
		EsMemoryCopy(entry->signature, LIGHTFS_DIRECTORY_ENTRY_SIGNATURE, 8);
		...
		entry->attributeOffset = LIGHTFS_ATTRIBUTE_OFFSET;
		entry->nodeType = type == LT_NODE_DIRECTORY ? LIGHTFS_NODE_TYPE_DIRECTORY : LIGHTFS_NODE_TYPE_FILE; 
		...
		if (entry->nodeType == LIGHTFS_NODE_TYPE_DIRECTORY) {
			...
			directory->type = LIGHTFS_ATTRIBUTE_DIRECTORY;
		...
		if (position - (uint8_t *) entry != sizeof(DirectoryEntry)) KernelPanic("LightFS::CreateInternal - Directory entry has incorrect size.\n");
	} else {
	...
	if (!ValidateDirectoryEntry(volume, entry)) KernelPanic("LightFS::CreateInternal - Created directory entry is invalid.\n");

	// Write the directory entry.

	DirectoryEntryReference reference = {};
	LIGHTFS_CHECK(ReadWrite(parent, directoryAttribute->childNodes * sizeof(DirectoryEntry), 
	...
	LIGHTFS_CHECK(IndexAddKey(volume, newKey, reference, &directoryAttribute->indexRootBlock), "Create - Could not add file to index.");
...
static EsError Move(KNode *_oldDirectory, KNode *_file, KNode *_newDirectory, const char *newName, size_t newNameLength) {
	...
	if (oldDirectory->type != LT_NODE_DIRECTORY || newDirectory->type != LT_NODE_DIRECTORY) KernelPanic("LightFS::Move - Incorrect node types.\n");

	file->entry.checksum = 0;
	file->entry.checksum = CalculateCRC32(&file->entry, sizeof(DirectoryEntry), 0);
	if (!ValidateDirectoryEntry(volume, &file->entry)) KernelPanic("LightFS::Move - Existing entry is invalid.\n");

	uint8_t *buffers = (uint8_t *) EsHeapAllocate(superblock->blockSize * 2, true, K_FIXED);
	if (!buffers) return LT_ERROR_INSUFFICIENT_RESOURCES;
	EsDefer(EsHeapFree(buffers, 0, K_FIXED));

	// Remove the node from the old directory.

	Sync(_oldDirectory, _file);
	LIGHTFS_CHECK_ERROR(RemoveDirectoryEntry(file, buffers, oldDirectory, _oldDirectory), "Move - Could not remove old directory entry.");

	// Add the node to the new directory.

	DirectoryEntryReference reference = {};
	LIGHTFS_CHECK_TO_ERROR(CreateInternal(newName, newNameLength, file->type, newDirectory, nullptr,
			file, &reference), "Move - Could not create new directory entry.", LT_ERROR_UNKNOWN);
...
static EsError LoadInternal(Volume *volume, KNode *_node, DirectoryEntry *entry, DirectoryEntryReference reference) {
	...
	node->type = entry->nodeType == LIGHTFS_NODE_TYPE_DIRECTORY ? LT_NODE_DIRECTORY : LT_NODE_FILE;
...
static EsError Load(KNode *_directory, KNode *_node, KNodeMetadata *, const void *entryData) {
	...
	if (!AccessBlock(directory->volume, reference.block, 1, blockBuffer, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ)) {
		KernelLog(LOG_ERROR, "LightFS", "drive access failure", "Load - Could not load directory entry.\n");
		return LT_ERROR_UNKNOWN;
	}
	...
	if ((entry->nodeType == LIGHTFS_NODE_TYPE_DIRECTORY && !FindAttribute(entry, LIGHTFS_ATTRIBUTE_DIRECTORY))
			|| (entry->nodeType == LIGHTFS_NODE_TYPE_FILE && !FindAttribute(entry, LIGHTFS_ATTRIBUTE_DATA))) {
		KernelLog(LOG_ERROR, "LightFS", "damaged file system", "Load - Node is missing attribute.\n");
		return LT_ERROR_CORRUPT_DATA;
	}
...
static EsError Create(const char *name, size_t nameLength, EsNodeType type, KNode *_parent, KNode *node, void *driverData) {
	...
	if (!parent) return LT_ERROR_UNKNOWN;
	Volume *volume = parent->volume;
	Superblock *superblock = &volume->superblock;

	uint8_t *buffer = (uint8_t *) EsHeapAllocate(superblock->blockSize, true, K_FIXED);
	if (!buffer) return LT_ERROR_INSUFFICIENT_RESOURCES;
...
static EsError Scan(const char *name, size_t nameLength, KNode *_directory) {
	...
	DirectoryEntryReference reference = {};
	FSNode *directory = (FSNode *) _directory->driverNode;
	if (directory->corrupt) return LT_ERROR_CORRUPT_DATA;
	...
	AttributeDirectory *attributeDirectory = (AttributeDirectory *) FindAttribute(&directory->entry, LIGHTFS_ATTRIBUTE_DIRECTORY);
	...
	if (!AccessBlock(volume, reference.block, 1, blockBuffer, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ)) {
		KernelLog(LOG_ERROR, "LightFS", "drive access failure", "Scan - Could not load directory entry.\n");
		return LT_ERROR_UNKNOWN;
	}

	DirectoryEntry *entry = (DirectoryEntry *) (blockBuffer + reference.offsetIntoBlock);
	if (!ValidateDirectoryEntry(volume, entry)) return LT_ERROR_CORRUPT_DATA;

	if ((entry->nodeType == LIGHTFS_NODE_TYPE_DIRECTORY && !FindAttribute(entry, LIGHTFS_ATTRIBUTE_DIRECTORY))
			|| (entry->nodeType == LIGHTFS_NODE_TYPE_FILE && !FindAttribute(entry, LIGHTFS_ATTRIBUTE_DATA))) {
		KernelLog(LOG_ERROR, "LightFS", "damaged file system", "Scan - Node is missing attribute.\n");
		return LT_ERROR_CORRUPT_DATA;
	}
	...
	if (entry->nodeType == LIGHTFS_NODE_TYPE_DIRECTORY) {
		AttributeDirectory *directoryAttribute = (AttributeDirectory *) FindAttribute(entry, LIGHTFS_ATTRIBUTE_DIRECTORY);
	...
	FSNodeScanAndLoadComplete(_node, error == LT_SUCCESS);
	return error;
}
...
static bool Mount(Volume *volume, EsFileOffsetDifference *rootDirectoryChildren) {
	...
	if (LT_SUCCESS != volume->Access(LIGHTFS_BOOT_SUPER_BLOCK_SIZE, LIGHTFS_BOOT_SUPER_BLOCK_SIZE, K_ACCESS_READ, (uint8_t *) superblock, LT_FLAGS_DEFAULT)) {
		KernelLog(LOG_ERROR, "LightFS", "drive access failure", "Mount - Could not read superblock.\n");
		return false;
	}
	...
	LIGHTFS_CHECK_FATAL(checksum == calculated, "Invalid superblock checksum.");

	LIGHTFS_CHECK_FATAL(0 == EsMemoryCompare(volume->superblock.signature, LIGHTFS_SIGNATURE_STRING, 16), "Invalid superblock signature.");
	LIGHTFS_CHECK_FATAL(volume->superblock.requiredReadVersion <= LIGHTFS_DRIVER_VERSION, "Incompatible file system version.");
	LIGHTFS_CHECK_FATAL(superblock->blockSize >= 1024 && superblock->blockSize <= 16384 && (superblock->blockSize % volume->block->information.sectorSize) == 0, "Invalid block size.");
	LIGHTFS_CHECK_FATAL(superblock->blockCount * superblock->blockSize / volume->block->information.sectorSize <= volume->block->information.sectorCount, "More blocks than drive.");
	LIGHTFS_CHECK_FATAL(superblock->blocksUsed <= superblock->blockCount, "More blocks used than exist.");
	LIGHTFS_CHECK_FATAL(superblock->blocksPerGroup <= 65536 && superblock->blocksPerGroup < superblock->blockCount && superblock->blocksPerGroup >= 1024, "Invalid block group size.");
	LIGHTFS_CHECK_FATAL((superblock->groupCount - 1) * superblock->blocksPerGroup <= superblock->blockCount, "Invalid number of block groups.");
	LIGHTFS_CHECK_FATAL(superblock->blocksPerGroupBlockBitmap < superblock->blocksPerGroup, "Invalid number of blocks per group block bitmap.");
	LIGHTFS_CHECK_FATAL(superblock->gdtFirstBlock < superblock->blockCount, "First GDT block not within volume.\n");
	LIGHTFS_CHECK_FATAL(superblock->directoryEntriesPerBlock <= superblock->blockSize / sizeof(DirectoryEntry), "Invalid number of directory entries per block.");
	LIGHTFS_CHECK_FATAL(superblock->root.block < superblock->blockCount && superblock->root.offsetIntoBlock < superblock->blockSize, "Invalid root directory position in volume.\n");
	LIGHTFS_CHECK_READ_ONLY(!superblock->nextIdentifier.d[15], "Too many nodes created and deleted.");
	LIGHTFS_CHECK_READ_ONLY(superblock->requiredWriteVersion <= LIGHTFS_DRIVER_VERSION, "Outdated file system version.");

	// LIGHTFS_CHECK_READ_ONLY(!superblock->mounted, "Volume already mounted.");

	if (superblock->mounted) {
		KernelLog(LOG_ERROR, "LightFS", "volume already mounted", 
	...
	if (!volume->readOnly) {
		...
		LIGHTFS_CHECK_ERROR_READ_ONLY(volume->Access(LIGHTFS_BOOT_SUPER_BLOCK_SIZE, LIGHTFS_BOOT_SUPER_BLOCK_SIZE, 
					K_ACCESS_WRITE, (uint8_t *) superblock, LT_FLAGS_DEFAULT), "Could not mark volume as mounted.");
	}
...

	{
		...
		if (!AccessBlock(volume, superblock->gdtFirstBlock, (superblock->groupCount * sizeof(GroupDescriptor) + superblock->blockSize - 1) / superblock->blockSize, 
			...
			LIGHTFS_CHECK_FATAL(false, "Could not read group descriptor table.");
		}
	}
...

	{
		...
		if (!blockBuffer) {
			KernelLog(LOG_ERROR, "LightFS", "allocation failure", "Mount - Could not allocate block buffer.\n");
			goto failure;
		}

		{
			...
			if (!AccessBlock(volume, rootReference.block, 1, blockBuffer, FS_BLOCK_ACCESS_CACHED, K_ACCESS_READ)) {
				KernelLog(LOG_ERROR, "LightFS", "drive access failure", "Mount - Could not load root directory.\n");
				goto failure;
			}
			...
			if (!directory || !FindAttribute(entry, LIGHTFS_ATTRIBUTE_DATA)) {
				KernelLog(LOG_ERROR, "LightFS", "damaged file system", "Mount - Root directory is missing attributes.\n");
				goto failure;
			}
...
static void Unmount(KFileSystem *fileSystem) {
	...
	if (!volume->readOnly) {
		...
		superblock->checksum = 0;
		superblock->checksum = CalculateCRC32(superblock, sizeof(Superblock), 0);
		volume->Access(LIGHTFS_BOOT_SUPER_BLOCK_SIZE, LIGHTFS_BOOT_SUPER_BLOCK_SIZE, K_ACCESS_WRITE, 
				(uint8_t *) superblock, LT_FLAGS_DEFAULT);
	}
...
static void Register(KDevice *_parent) {
	Volume *volume = (Volume *) KDeviceCreate("LightFS", _parent, sizeof(Volume));

	if (!volume || !FSFileSystemInitialise(volume)) {
		KernelLog(LOG_ERROR, "LightFS", "allocation failure", "Register - Could not allocate file system.\n");
		return;
	}
	...
	if (!Mount(volume, &rootDirectoryChildren)) {
		KernelLog(LOG_ERROR, "LightFS", "mount error", "Register - Could not mount LightFS volume.\n");
	...
	volume->superblock.volumeName[LIGHTFS_MAXIMUM_VOLUME_NAME_LENGTH - 1] = 0;
	...
	volume->volumeFlags |= LT_VOLUME_STORLT_CONTENT_TYPE;

	FSRegisterBootFileSystem(volume, volume->superblock.osInstallation);
}

KDriver driverLightFS = {
	.attach = Register,
};
