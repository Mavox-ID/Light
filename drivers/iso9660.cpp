...
#define MOUNT_FAILURE(message) do { KernelLog(LOG_ERROR, "ISO9660", "mount failure", "Mount - " message); return false; } while (0)
	...
	while (true) {
		if (LT_SUCCESS != volume->Access(32768 + SECTOR_SIZE * descriptorIndex, SECTOR_SIZE, K_ACCESS_READ, &volume->primaryDescriptor, LT_FLAGS_DEFAULT)) {
...

	{
		...
		if (0 != EsMemoryCompare("Light::", volume->primaryDescriptor.applicationSpecific, 9)) {
			goto notBoot;
		}
		...
		DirectoryRecord record = {};
		ScanInternal(EsLiteral("LIGHT.DAT;1"), volume->rootDirectory, &record);
		...
		if (LT_SUCCESS != volume->Access(record.extentStart.x * SECTOR_SIZE, SECTOR_SIZE, K_ACCESS_READ, firstSector, LT_FLAGS_DEFAULT)) {
			goto notBoot;
		}	
		...
		FSPartitionDeviceCreate(volume->block, record.extentStart.x, record.extentSize.x, LT_FLAGS_DEFAULT, EsLiteral("CD-ROM boot partition"));
	}
...
#define READ_FAILURE(message, error) do { KernelLog(LOG_ERROR, "ISO9660", "read failure", "Read - " message); return error; } while (0)
	...
	while (count) {
		if (offset || count < SECTOR_SIZE) {
			EsError error = volume->Access(lba * SECTOR_SIZE, SECTOR_SIZE, K_ACCESS_READ, sectorBuffer, LT_FLAGS_DEFAULT);
			if (error != LT_SUCCESS) READ_FAILURE("Could not read file sector.\n", error);
			...
			EsError error = volume->Access(lba * SECTOR_SIZE, sectorsToRead * SECTOR_SIZE, K_ACCESS_READ, outputBuffer, LT_FLAGS_DEFAULT);
			if (error != LT_SUCCESS) READ_FAILURE("Could not read file sectors.\n", error);
...
#define ENUMERATE_FAILURE(message, error) do { KernelLog(LOG_ERROR, "ISO9660", "enumerate failure", "Enumerate - " message); return error; } while (0)
	...
	while (remainingBytes) {
		EsError accessResult = volume->Access(currentSector * SECTOR_SIZE, SECTOR_SIZE, K_ACCESS_READ, (uint8_t *) sectorBuffer, LT_FLAGS_DEFAULT);

		if (accessResult != LT_SUCCESS) {
		...
		while (positionInSector < SECTOR_SIZE && positionInSector < remainingBytes) {
...

			{
				...
				metadata.type = (record->flags & (1 << 1)) ? LT_NODE_DIRECTORY : LT_NODE_FILE;
...
static EsError Load(KNode *_directory, KNode *_node, KNodeMetadata *, const void *entryData) {
	...
	EsError error = volume->Access(reference.sector * SECTOR_SIZE, SECTOR_SIZE, K_ACCESS_READ, (uint8_t *) sectorBuffer, LT_FLAGS_DEFAULT);
