...
#define FIXUP_FAILURE(message) do { KernelLog(LOG_ERROR, "NTFS", "fixup failure", "ApplyFixup - " message); return LT_SUCCESS; } while (0)
...
#define VALIDATE_FILE_FAILURE(message) do { KernelLog(LOG_ERROR, "NTFS", "validate file record failure", "ValidateFileRecord - " message); return LT_SUCCESS; } while (0)
...
#define READ_FILE_CLUSTERS_FAILURE(message) do { KernelLog(LOG_ERROR, "NTFS", "read file clusters failure", "ReadFileClusters - " message); return LT_SUCCESS; } while (0)
	...
	while (((uint8_t *) dataRun - (uint8_t *) dataHeader) < dataHeader->length && dataRun->lengthFieldBytes) {
		...
		if (foundStart) {
			...
			volume->fileSystem->Access(firstClusterToRead * volume->clusterBytes, 
					...
					buffer, LT_FLAGS_DEFAULT, &dispatchGroup);
...
static bool ReadFileRecord(KNodeMetadata *metadata, FSNode *node, uint64_t index, EsNodeType nodeType, void *clusterBuffer) {
#define READ_FILE_RECORD_FAILURE(message) do { KernelLog(LOG_ERROR, "NTFS", "read file record failure", "ReadFileRecord - " message); return LT_SUCCESS; } while (0)
	...
	if (!ValidateFileRecord(volume, node, nodeType == LT_NODE_DIRECTORY)) {
		return LT_SUCCESS;
	}
	...
	if (nodeType == LT_NODE_FILE) {
		...
		if (dataHeader->nonResident) {
			metadata->totalSize = ((NonResidentAttributeHeader *) dataHeader)->attributeSize;
		} else {
			metadata->totalSize = ((ResidentAttributeHeader *) dataHeader)->attributeLength;
		}

		metadata->type = LT_NODE_FILE;
	} else if (nodeType == LT_NODE_DIRECTORY) {
		...
		if (!indexAllocationHeader || indexAllocationHeader->flags || !indexAllocationHeader->nonResident) {
			metadata->directoryChildren = MFT_FILE_SIZE / 16;
		} else {
			// TODO This is a terrible upper estimate! Find a better one?
			metadata->directoryChildren = indexAllocationHeader->attributeSize / 16;
		}

		metadata->type = LT_NODE_DIRECTORY;
	}
...
#define MOUNT_FAILURE(message) do { KernelLog(LOG_ERROR, "NTFS", "mount failure", "Mount - " message); return LT_SUCCESS; } while (0)

	// Load the boot sector.

	{
		void *buffer = EsHeapAllocate(volume->fileSystem->block->information.sectorSize, false, K_FIXED);

		if (!buffer) {
			MOUNT_FAILURE("Could not allocate buffer.\n");
		}

		EsDefer(EsHeapFree(buffer, volume->fileSystem->block->information.sectorSize, K_FIXED));

		if (!volume->fileSystem->Access(0, volume->fileSystem->block->information.sectorSize, K_ACCESS_READ, buffer, LT_FLAGS_DEFAULT)) {
		...
		volume->clusterCount = volume->fileSystem->block->information.sectorCount / volume->bootSector.sectorsPerCluster;

		if (volume->bootSector.bytesPerSector != volume->fileSystem->block->information.sectorSize) MOUNT_FAILURE("Invalid bytes per sector.\n");
...

	{
		if (!volume->fileSystem->Access(volume->bootSector.mftStart * volume->clusterBytes, 
					volume->clusterBytes, K_ACCESS_READ, buffer, LT_FLAGS_DEFAULT)) {
...

	{
		...
		if (!ReadFileRecord(nullptr, &volume->volumeNode, 3, LT_NODE_FILE, buffer)) {
			return LT_SUCCESS;
		}
	}
	...
	{
		volume->root = volume->fileSystem->rootDirectory;
		...
		KNodeMetadata rootMetadata = {};
		if (!ReadFileRecord(&rootMetadata, root, 5, LT_NODE_DIRECTORY, buffer)) {
			return LT_SUCCESS;
		}
		
		volume->fileSystem->rootDirectoryInitialChildren = rootMetadata.directoryChildren;
	}
...

	{
		...
		if (!ReadFileRecord(nullptr, upCase, 10, LT_NODE_FILE, buffer)) {
			return LT_SUCCESS;
		}
...
#define ENUMERATE_FAILURE(message) do { KernelLog(LOG_ERROR, "NTFS", "enumerate failure", "Enumerate - " message); return LT_ERROR_UNKNOWN; } while (0)
...
static EsError EnumerateIndexEntry(IndexEntryHeader *entry, uint8_t *bufferLimit, KNode *directory) {
	...
	if ((fileNameAttribute->namespaceType == 1 || fileNameAttribute->namespaceType == 3) && entry->fileRecordNumber >= 24) {
		...
		metadata.type = (fileNameAttribute->flags & 0x10000000) ? LT_NODE_DIRECTORY : LT_NODE_FILE;

		if (metadata.type == LT_NODE_FILE) {
			metadata.totalSize = fileNameAttribute->realSize;
		} else {
			metadata.directoryChildren = LT_DIRECTORY_CHILDREN_UNKNOWN;
		}
		...
		char childName[LT_MAX_DIRECTORY_CHILD_NAME_LENGTH];
		...
		while (inputCharactersRemaining) {
			...
			if (namePosition + encodedBytes <= LT_MAX_DIRECTORY_CHILD_NAME_LENGTH) {
		...
		uint64_t reference = entry->fileRecordNumber;
		EsError error = FSDirectoryEntryFound(directory, &metadata, 
				&reference, childName, childNameBytes, false);
	...
	while (((uint8_t *) dataRun - (uint8_t *) indexAllocationHeader) < indexAllocationHeader->length && dataRun->lengthFieldBytes) {
		...
		while (positionInRun != length) {
			...
			volume->fileSystem->Access((clusterNumber + positionInRun) * volume->clusterBytes, 
					clustersToRead * volume->clusterBytes, K_ACCESS_READ, 
					buffer + clustersInBuffer * volume->clusterBytes, LT_FLAGS_DEFAULT);
...
static EsError Scan(const char *_name, size_t _nameBytes, KNode *_directory) {
#define SCAN_FAILURE(message) do { KernelLog(LOG_ERROR, "NTFS", "scan failure", "Scan - " message); return LT_ERROR_UNKNOWN; } while (0)
	...
	while (_nameBytes) {
		...
		if (value <= 0xFFFF) {
			if (searchNameCharacters >= 255) return LT_ERROR_UNKNOWN;
			searchName[searchNameCharacters++] = value;
		} else {
			if (searchNameCharacters >= 254) return LT_ERROR_UNKNOWN;
			value -= 0x10000;
	...
	while (entriesPosition + sizeof(IndexEntryHeader) <= entriesEnd) {
		...
		if (entry->flags & (1 << 1)) {
			...
			if (fileNameHeader->namespaceType == 1 || fileNameHeader->namespaceType == 3) {
				...
				if (match) {
					KNodeMetadata metadata = {};
					metadata.type = (fileNameHeader->flags & 0x10000000) ? LT_NODE_DIRECTORY : LT_NODE_FILE;

					if (metadata.type == LT_NODE_FILE) {
						metadata.totalSize = fileNameHeader->realSize;
					} else {
						metadata.directoryChildren = LT_DIRECTORY_CHILDREN_UNKNOWN;
					}

					uint64_t reference = entry->fileRecordNumber;
					return FSDirectoryEntryFound(_directory, &metadata, &reference, _name2, _nameBytes2, false);
				}
			}
		}
			...
			while (((uint8_t *) dataRun - (uint8_t *) indexAllocationHeader) < indexAllocationHeader->length && dataRun->lengthFieldBytes) {
				...
				if (foundStart) {
					...
					if (!volume->fileSystem->Access(clusterIndex * volume->clusterBytes, 
								...
								buffer + positionInBuffer * volume->clusterBytes, LT_FLAGS_DEFAULT)) {
...
#define READ_FAILURE(message) do { KernelLog(LOG_ERROR, "NTFS", "read failure", "Read - " message); return LT_ERROR_UNKNOWN; } while (0)
	...
	while (((uint8_t *) dataRun - (uint8_t *) dataHeader) < dataHeader->length && dataRun->lengthFieldBytes) {
		...
		while (runOffset < length && clusterPosition <= endCluster) {
			...
			if (clusterPosition == startCluster) {
				volume->fileSystem->Access(firstClusterToRead * volume->clusterBytes, 
						clustersToRead * volume->clusterBytes, K_ACCESS_READ, 
						clusterBuffer, LT_FLAGS_DEFAULT, &dispatchGroup);
				buffer += volume->clusterBytes - startOffset;
			} else if (clusterPosition == endCluster) {
				volume->fileSystem->Access(firstClusterToRead * volume->clusterBytes, 
						clustersToRead * volume->clusterBytes, K_ACCESS_READ, 
						clusterBuffer2, LT_FLAGS_DEFAULT, &dispatchGroup);
			} else {
				volume->fileSystem->Access(firstClusterToRead * volume->clusterBytes, 
						clustersToRead * volume->clusterBytes, K_ACCESS_READ, 
						buffer, LT_FLAGS_DEFAULT, &dispatchGroup);
...
static EsError Load(KNode *directory, KNode *node, KNodeMetadata *metadata, const void *entryData) {
	FSNode *dirNode = (FSNode *) directory->driverNode;
	Volume *volume = dirNode->volume;
	uint64_t reference = *(const uint64_t *) entryData;

	node->driverNode = EsHeapAllocate(sizeof(FSNode), true, K_FIXED);
	if (!node->driverNode) return LT_ERROR_UNKNOWN;

	FSNode *fsNode = (FSNode *) node->driverNode;
	fsNode->volume = volume;

	uint8_t *buffer = (uint8_t *) EsHeapAllocate(volume->clusterBytes, false, K_FIXED);
	if (!buffer) {
	...
	if (!ReadFileRecord(metadata, fsNode, reference, metadata->type, buffer)) {
		EsHeapFree(node->driverNode, sizeof(FSNode), K_FIXED);
		return LT_ERROR_UNKNOWN;
	}
...
static void DeviceAttach(KDevice *parent) {
	...
	volume->fileSystem->directoryEntryDataBytes = sizeof(uint64_t);
