...
bool ATAController::Access(uintptr_t drive, uint64_t offset, size_t countBytes, int operation, uint8_t *_buffer) {
	...
	{
		while (true) {
			KEventWait(&semaphore.available, LT_WAIT_NO_TIMEOUT);
			KMutexAcquire(&blockedPacketsMutex);
...
void ATAController::Initialise() {
	...
	for (uintptr_t bus = 0; bus < ATA_BUSES; bus++) {
		...
		if (dmaDrivesOnBus) {
			...
			if (!MMPhysicalAllocateAndMap(131072, 131072, 32, true, LT_FLAGS_DEFAULT, &dataVirtual, &dataPhysical)) {
	...
	for (uintptr_t i = 0; i < ATA_DRIVES; i++) {
		if (sectorCount[i]) {
			...
			device->information.driveType = isATAPI[i] ? LT_DRIVE_TYPE_CDROM : LT_DRIVE_TYPE_HDD;
