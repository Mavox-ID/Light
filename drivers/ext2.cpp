...
#define MOUNT_FAILURE(message) do { KernelLog(LOG_ERROR, "Ext2", "mount failure", "Mount - " message); return false; } while (0)
	...
	{
		if (LT_SUCCESS != volume->Access(1024, volume->block->information.sectorSize, K_ACCESS_READ, sectorBuffer, LT_FLAGS_DEFAULT)) {
...

	{
		...
		if (LT_SUCCESS != volume->Access(firstBlockContainingBlockGroupDescriptorTable * volume->blockBytes, 
				blockGroupDescriptorTableLengthInBlocks * volume->blockBytes, 
				K_ACCESS_READ, volume->blockGroupDescriptorTable, LT_FLAGS_DEFAULT)) {
...

	{
		...
		if (LT_SUCCESS != volume->Access(blockGroupDescriptor->inodeTable * volume->blockBytes 
					+ sectorInInodeTable * volume->block->information.sectorSize, 
				volume->block->information.sectorSize, 
				K_ACCESS_READ, sectorBuffer, LT_FLAGS_DEFAULT)) {
...
#define GET_DATA_BLOCK_ACCESS_FAILURE() do { KernelLog(LOG_ERROR, "Ext2", "block access failure", "GetDataBlock - Could not read block.\n"); return 0; } while (0)
	...
	if (blockIndex < blockPointersPerBlock) {
		...
		CHECK_BLOCK_INDEX();
		if (LT_SUCCESS != volume->Access(offset, volume->blockBytes, K_ACCESS_READ, blockBuffer, LT_FLAGS_DEFAULT)) GET_DATA_BLOCK_ACCESS_FAILURE();
...
#define ENUMERATE_FAILURE(message, error) do { KernelLog(LOG_ERROR, "Ext2", "enumerate failure", "Enumerate - " message); return error; } while (0)
	...
	for (uintptr_t i = 0; i < blocksInDirectory; i++) {
		...
		EsError error = volume->Access((uint64_t) block * volume->blockBytes, volume->blockBytes, K_ACCESS_READ, blockBuffer, LT_FLAGS_DEFAULT);
		if (error != LT_SUCCESS) ENUMERATE_FAILURE("Could not read block.\n", error);
		...
		while (positionInBlock + sizeof(DirectoryEntry) < volume->blockBytes) {
			...
			metadata.type = entry->type == DIRENT_TYPE_DIRECTORY ? LT_NODE_DIRECTORY : entry->type == DIRENT_TYPE_REGULAR ? LT_NODE_FILE : LT_NODE_INVALID;

			if (metadata.type == LT_NODE_DIRECTORY) {
				metadata.directoryChildren = LT_DIRECTORY_CHILDREN_UNKNOWN;
			}

			if (metadata.type != LT_NODE_INVALID
					&& !(nameBytes == 1 && name[0] == '.')
...
#define SCAN_FAILURE(message, error) do { KernelLog(LOG_ERROR, "Ext2", "scan failure", "Scan - " message); return error; } while (0)
	...
	if (nameBytes == 2 && name[0] == '.' && name[1] == '.') return LT_ERROR_FILE_DOLT_NOT_EXIST;
	if (nameBytes == 1 && name[0] == '.') return LT_ERROR_FILE_DOLT_NOT_EXIST;
	...
	for (uintptr_t i = 0; i < blocksInDirectory; i++) {
		...
		EsError error = volume->Access((uint64_t) block * volume->blockBytes, volume->blockBytes, K_ACCESS_READ, blockBuffer, LT_FLAGS_DEFAULT);
		if (error != LT_SUCCESS) SCAN_FAILURE("Could not read block.\n", error);
...
static EsError Load(KNode *_directory, KNode *node, KNodeMetadata *metadata, const void *entryData) {
	...
	EsError error = volume->Access(blockGroupDescriptor->inodeTable * volume->blockBytes 
			+ sectorInInodeTable * volume->block->information.sectorSize, 
			volume->block->information.sectorSize, 
			K_ACCESS_READ, blockBuffer, LT_FLAGS_DEFAULT);
...
struct ReadDispatchGroup : KWorkGroup {
	...
	void QueueExtent() {
		...
		volume->Access(extentIndex * volume->blockBytes, 
				volume->blockBytes * extentCount, K_ACCESS_READ, extentBuffer, LT_FLAGS_DEFAULT, this);
	}
...
#define READ_FAILURE(message, error) do { KernelLog(LOG_ERROR, "Ext2", "read failure", "Read - " message); return error; } while (0)
	...
	while (currentBlock <= lastBlock) {
		...
		if (readEntireBlock) {
			...
			EsError error = volume->Access((uint64_t) block * volume->blockBytes, volume->blockBytes, K_ACCESS_READ, blockBuffer, LT_FLAGS_DEFAULT);
			if (error != LT_SUCCESS) READ_FAILURE("Could not read blocks from drive.\n", error);
