...
void NVMeController::Initialise() {
	...
	adminSubmissionQueue = (uint8_t *) MMMapPhysical(MMGetKernelSpace(), adminSubmissionQueuePhysicalAddress, adminSubmissionQueueBytes, LT_FLAGS_DEFAULT);
	adminCompletionQueue = (uint8_t *) MMMapPhysical(MMGetKernelSpace(), adminCompletionQueuePhysicalAddress, adminCompletionQueueBytes, LT_FLAGS_DEFAULT);
	...
	uint8_t *identifyData = (uint8_t *) MMMapPhysical(MMGetKernelSpace(), identifyDataPhysicalAddress, 4096 * 2, LT_FLAGS_DEFAULT);
	...
	
	{
		...
		ioCompletionQueue = (uint8_t *) MMMapPhysical(MMGetKernelSpace(), physicalAddress, bytes, LT_FLAGS_DEFAULT);
	...
	
	{
		...
		ioSubmissionQueue = (uint8_t *) MMMapPhysical(MMGetKernelSpace(), physicalAddress, bytes, LT_FLAGS_DEFAULT);
...

	{
		...
		prpListVirtual = (uint64_t *) MMMapPhysical(MMGetKernelSpace(), prpListPages[0], K_PAGE_SIZE, LT_FLAGS_DEFAULT);
	...
	while (true) {
		...
		for (uintptr_t i = 0; i < 1024; i++) {
			...
			device->information.driveType = LT_DRIVE_TYPE_SSD;
