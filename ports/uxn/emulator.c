...
file_talk(Device *d, Uint8 b0, Uint8 w)
{
	...
	if(w && (read || b0 == 0xf)) {
		...
		EsFileInformation file = EsFileOpen(path, pathBytes, read ? (LT_FILE_READ | LT_NODE_FAIL_IF_NOT_FOUND) : (LT_FILE_WRITE));
...
EsRectangle GetImageBounds(EsRectangle bounds /* canvas bounds */) {
	if (!imageWidth || !imageHeight) return LT_RECT_1(0);
	int zoomX = LT_RECT_WIDTH(bounds) / imageWidth, zoomY = LT_RECT_HEIGHT(bounds) / imageHeight;
	int zoom = zoomX < 1 || zoomY < 1 ? 1 : zoomX > zoomY ? zoomY : zoomX;
	return EsRectangleCenter(bounds, LT_RECT_2S(imageWidth * zoom, imageHeight * zoom));
}
...
void MouseEvent(bool pressed, bool released, bool right) {
	...
	int zoom = LT_RECT_WIDTH(imageBounds) / imageWidth;
...
int CanvasMessage(EsElement *element, EsMessage *message) {
	if (message->type == LT_MSG_PAINT) {
		...
		EsDrawBitmapScaled(message->painter, imageBounds, LT_RECT_2S(imageWidth, imageHeight), imageBits, imageWidth * 4, LT_DRAW_BITMAP_OPAQUE);
	...
	} else if (message->type == LT_MSG_MOUSE_MOVED || message->type == LT_MSG_MOUSE_LEFT_DRAG || message->type == LT_MSG_MOUSE_RIGHT_DRAG) {
		...
		MouseEvent(false, true, false);
	} else if (message->type == LT_MSG_GET_CURSOR && inImageBounds) {
...
void _start() {
	while (true) {
		...
		if (message->type == LT_MSG_INSTANCE_CREATE) {
			...
			canvas = EsCustomElementCreate(instance->window, LT_CELL_FILL, LT_STYLE_PANEL_WINDOW_DIVIDER);
