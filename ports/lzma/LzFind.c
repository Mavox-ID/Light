...
#define GET_MATCHLT_HEADER2(minLen, ret_op) \
  unsigned lenLimit; UInt32 hv; Byte *cur; UInt32 curMatch; \
  lenLimit = (unsigned)p->lenLimit; { if (lenLimit < minLen) { MatchFinder_MovePos(p); ret_op; }} \
  cur = p->buffer;

#define GET_MATCHLT_HEADER(minLen) GET_MATCHLT_HEADER2(minLen, return distances)
#define SKIP_HEADER(minLen)   do { GET_MATCHLT_HEADER2(minLen, continue)
...
#define GET_MATCHLT_FOOTER_BASE(_maxLen_, func) \
  distances = func(MF_PARAMS(p), \
  distances, (UInt32)_maxLen_); MOVE_POS_RET;

#define GET_MATCHLT_FOOTER_BT(_maxLen_) \
  GET_MATCHLT_FOOTER_BASE(_maxLen_, GetMatchesSpec1)

#define GET_MATCHLT_FOOTER_HC(_maxLen_) \
  GET_MATCHLT_FOOTER_BASE(_maxLen_, Hc_GetMatchesSpec)
...
{
  GET_MATCHLT_HEADER(2)
  HASH2_CALC;
  curMatch = p->hash[hv];
  p->hash[hv] = p->pos;
  GET_MATCHLT_FOOTER_BT(1)
}

UInt32* Bt3Zip_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances)
{
  GET_MATCHLT_HEADER(3)
  HASH_ZIP_CALC;
  curMatch = p->hash[hv];
  p->hash[hv] = p->pos;
  GET_MATCHLT_FOOTER_BT(2)
}
