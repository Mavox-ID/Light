...
#define LZMA_NUM_PB_STATLT_MAX (1 << LZMA_PB_MAX)
...
typedef struct
{
  CLzmaProb low[LZMA_NUM_PB_STATLT_MAX << (kLenNumLowBits + 1)];
...
{
  unsigned tableSize;
  UInt32 prices[LZMA_NUM_PB_STATLT_MAX][kLenNumSymbolsTotal];
  // UInt32 prices1[LZMA_NUM_PB_STATLT_MAX][kLenNumLowSymbols * 2];
...
typedef struct
{
  ...
  CLzmaProb isMatch[kNumStates][LZMA_NUM_PB_STATLT_MAX];
  CLzmaProb isRep0Long[kNumStates][LZMA_NUM_PB_STATLT_MAX];
...
{
  unsigned i;
  for (i = 0; i < (LZMA_NUM_PB_STATLT_MAX << (kLenNumLowBits + 1)); i++)
...
static SRes CheckErrors(CLzmaEnc *p)
{
  ...
  {
    p->result = MY_HRLT_ERROR__INTERNAL_ERROR;
...
static void LzmaEnc_Init(CLzmaEnc *p)
{
  ...
  {
    unsigned j;
    for (j = 0; j < LZMA_NUM_PB_STATLT_MAX; j++)
    {
